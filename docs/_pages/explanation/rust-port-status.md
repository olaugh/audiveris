...
| 11 | `REDUCTION` | **In progress** | Production terminal STEMS SIGs execute Java's complete `reduceFoundations()` outer fixed point and the enabled stem head-end refinement immediately after it. The native path retains exact anchors, direction, median/skew reliable-line selection, vertical-ribbon bounds, atomic traces, stable tombstones, multi-staff ownership, ordered `allRemoved`, and Java's outer-local set shadowing. | Beam-group consistency; free-stem length measurement and glyph cleanup; stage publication and corpus parity. |
