...
The first twenty semantic `STEMS` boundaries are production-shaped and graded (the eighteenth through twentieth under the fast-evidence policy documented in `rust/PORTING.md`).
...
| 5 | `HEADERS` | **Native and published** | `recognize_native_headers` composes clef, key, and time columns in Java order from live GRID state alone. All nine pages and 65 staves match for starts/stops and selected evidence, including 34 keys, 17 times, and all 30 downstream erase rectangles. Schema 1 publishes selected inters, lifecycle/classifier evidence, staff ranges, and system-owned erases. Header SIG grades are now carried per inter rather than only per key: each key alter retains its own pitched grade with `intrinsicRatio` applied, and a key's grade is the mean of its members, so six of Chula system 1's ten header grades match Java bit for bit and the other four agree to ten decimals. | Close the last four header grades (staff 1's two alters, their key, one time), then widen the corpus. |
