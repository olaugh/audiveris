...
| 11 | `REDUCTION` | **In progress** | Production terminal STEMS SIGs execute Java's complete `reduceFoundations()` outer fixed point, enabled stem head-end refinement, beam-group consistency, and sheet-wide free-stem length measurement. The native path retains exact anchors, direction, median/skew line selection, deskewed beam ordering, concrete-stem filtering, stable tombstones, and the upper-middle pixel/interline median. | Glyph-index cleanup; stage publication and corpus parity. |
