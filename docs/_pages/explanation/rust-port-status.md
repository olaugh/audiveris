...
| 11 | `REDUCTION` | **In progress** | Dependency-light lifecycle/contracts plus a production Java-order foundations prefix over terminal STEMS SIGs: lossless overlap scheduling, chord analysis, contextual weak purges, complete head validation, hook and standard-beam validation, shared-ledger repair, and iterative ledger-support pruning. Exact staff/ledger splines, multi-staff ownership, stable tombstones, and extensive ensemble/member cleanup survive the stage boundary. | `checkStems`, consistency fixed-point and late-consistency epochs; endpoint refinement; beam-group checks; stage publication and corpus parity. |
