...
sections in Java order and orientation.
...
| 9 | `HEADS` | **Components graded** | Prolog, spot dispatch contract, classifier mutation order, ownership, cleanup, and quorum scale. BEAMS carries the exact threshold-170 vertical `HEAD_SPOTS` table across all eight graded pages, and LEDGERS carries exact final glyph rasters rather than median approximations. | Compose persistent staff lines plus these ledger/spot and accepted-seed rasters, then port the visual classifier internals. |
...
1. Finish and grade native `HEADS` prolog composition from GRID, BEAMS,
   LEDGERS, and STEM_SEEDS.
