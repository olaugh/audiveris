...
`Both` and RIGHT is `TopOnly`; Java returns false with `undefs=[LEFT]`, zero
closure writes, zero unlinked additions, and no graph, registry, or linker
...
`undefs=[LEFT]`, zero closure writes, zero unlinked additions, and no graph,
