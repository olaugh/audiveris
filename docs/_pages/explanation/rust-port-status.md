...
| 9 | `HEADS` | **Components graded** | Prolog, spot dispatch contract, classifier mutation order, ownership, cleanup, and quorum scale. `recognize_native_heads_prolog` composes real GRID, BEAMS, LEDGERS, and STEM_SEEDS state into BINARY erasure, Chamfer-3 distance values, 2,790 transient components, and 3,097 per-system spot references; all eight pages now match Java exactly through the `NoteHeadsBuilder` boundary. BEAMS' BlackHeadSizer side effect is production-wired and exact for all 2,739 sizing inputs, 936 singles, 5 stacks, 470 core samples, eight font scales, and 55 staff point sizes. All five real point-size catalogs and four active shapes are now checked-in native data; the exhaustive gate replays all 32 page-local templates, 192 anchors, and 27,207 keyed pixels exactly. The validated evaluator preserves Java's anchor rounding, weights, skips, and accumulation. | Connect per-staff catalog selection, scanning, and interpretation. Grade one-line/drum sizing suppression separately. |
...
1. Connect each measured staff point size to its checked-in native catalog.
2. Port and grade HEADS scanner geometry around the native evaluator.
