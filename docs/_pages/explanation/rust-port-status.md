...
| 11 | `REDUCTION` | **In progress** | Dependency-light lifecycle/contracts plus the first production semantic primitive: terminal native STEMS SIGs can be contextualized and their existing exclusions reduced with Java's strongest-branch selection, insertion-order tie behavior, target-on-equal deletion, dying-ensemble cleanup, tombstones, and repeated contextualization. | Overlap discovery; foundation consistency epochs for heads, stems, beams/hooks, ledgers, and chords; endpoint refinement; beam-group checks; stage publication and corpus parity. |
