...
| 12 | `CUE_BEAMS` | **Native lifecycle composed; connected sidecar pending** | Exact default `smallHeads=false` prolog skip and schema-1 ordinary/stream output remain. With the qualified switch enabled, production now composes aggregate discovery, morphology, SmallBeam creation, grouping, stem lookup/checks, and terminal BeamStem SIG mutation. Ordinary recognition and supplemental hook recovery are independently controlled. | Freeze a non-empty real-score differential and publish connected cue metadata for StageAligner. |
