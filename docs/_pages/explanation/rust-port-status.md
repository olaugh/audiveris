...
and 174 retained candidates, including conflict provenance and IoU bits. The streaming range kernel is
now exact as well: all 6,759 spot slices, 921,558 x visits, 3,119,882 shape
attempts, and 34,101 raw candidates match four independent per-staff Java
hashes. End-to-end aggregation, seed-conflict, and glyph composition are the
active implementation boundary. The following full HEADS epilog is frozen too:
...
rows. The 3.82 MB compact fixture is deterministic across fresh JVMs (SHA-256
...
1. Compose the graded range scan through native aggregation, seed-head conflict filtering, and glyph retrieval.
