...
through Java's default-disabled `CUE_BEAMS` no-op, including the final reduced
...
| 12 | `CUE_BEAMS` | **Default path published** | Exact default `smallHeads=false` prolog skip, typed active-path refusal, schema-1 ordinary/stream output, plus dependency-light active lifecycle/contracts. The active path now arrives with exact small-head HEADS templates and retained small stemmed heads. | Active cue aggregation, morphology, and linking. |
