...
| 11 | `REDUCTION` | **In progress** | Production terminal STEMS SIGs now execute Java's complete `reduceFoundations()` outer fixed point: lossless overlap scheduling, chord analysis, contextual weak purges, all six foundation consistency checks, exclusion and late-consistency epochs, exact outer-local set shadowing, ordered `allRemoved`, and the terminal empty-local-sets epoch. Exact staff/ledger splines, multi-staff ownership, stable tombstones, and ensemble/member cleanup survive the stage boundary. | Stem endpoint refinement; beam-group checks; stage publication and corpus parity. |
