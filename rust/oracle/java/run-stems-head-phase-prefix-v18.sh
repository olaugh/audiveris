...
tmp_base=/private/tmp/stems-head-phase-prefix-v18
...
if ! cmp -s "$semantic_one" "$semantic_two"; then
    echo "two fresh post-STUMPS v18 semantic passes are not byte-identical" >&2
...
if [ "$(grep -c '^stemsheadphasecontinue ' "$rows")" -ne 1 ] || \
    ...
    echo "v18 bounded order-18 continuation contract differs" >&2
    exit 1
fi
...
v17_runner_sha=$(shasum -a 256 "$script_dir/run-stems-head-phase-prefix-v17.sh" | awk '{print $1}')
if [ "$v17_runner_sha" != "84c176b45ec8adb7af8e0ab1014acabfe8c57c2e6b3cbbe5e8bbd0e971823196" ]; then
    echo "v18 base v17 runner drifted" >&2
    exit 1
fi
v17_fixture="$repo_root/rust/oracle/stems-head-phase-prefix-chula-system1-v17.txt"
v17_fixture_sha=$(shasum -a 256 "$v17_fixture" | awk '{print $1}')
if [ "$v17_fixture_sha" != "8e4909edc2196f2baff6f517693f9a9af50405cf85fc88bcf3e771711bae2b4b" ]; then
    echo "v18 base v17 fixture drifted" >&2
    exit 1
fi
...
out="$repo_root/rust/oracle/stems-head-phase-prefix-chula-system1-v18.txt"
{
    echo '# Java Audiveris 5.11 (Temurin JDK 25.0.3+9 LTS) post-STUMPS head phase v18.'
    echo '# schema: stems-head-phase-prefix-v18'
    echo '# Bounded order-18 derivative: orders 1-17 execute linkSides without persistent snapshots; order 18 emits C-link envelope/result plus continuation.'
    echo '# This minimized scope is intentional for deterministic replay under the full-snapshot heap limit.'
    cat "$rows"
    printf '%s\n' \
        "stemsheadphaseprefix summary schema stems-head-phase-prefix-v18 page chula.png#1 system 1 rows $row_count baseProbeSourceSha256 $base_probe_sha baseV17RunnerSourceSha256 $v17_runner_sha baseV17FixtureSha256 $v17_fixture_sha fragmentSourceSha256 $fragment_sha probeSourceSha256 $probe_sha runnerSourceSha256 $runner_sha emittedBodySha256 $body_sha semanticPassSha256 $semantic_sha completeStumpsFixtureSha256 $stumps_sha freshRuns 2 freshRunsByteIdentical true nativeScope BoundedSnapshotMinimizedOrder18WithCLinkEnvelope javaEvidence ReturnedBeforeNineteenthHead"
} > "$out"
echo "wrote $out"
