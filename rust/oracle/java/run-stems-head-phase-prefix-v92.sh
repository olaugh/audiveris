...
# Read-only snapshot-minimized order92 continuation from v91; prior orders mutate without snapshots.
set -eu
...
tmp_base=/private/tmp/stems-head-phase-prefix-v92
...
awk -v fragment="$fragment18" '
{
    ...
    if (capture_after && index($0, "heads, allLinkers);") != 0) {
        print
        print "            if (headOrder == 7 || headOrder == 22 || headOrder == 27 || headOrder == 34 || headOrder == 36 || headOrder == 37 || headOrder == 38 || headOrder == 39 || headOrder == 40 || headOrder == 41 || headOrder == 42 || headOrder == 43 || headOrder == 44 || headOrder == 45 || headOrder == 46 || headOrder == 47 || headOrder == 48 || headOrder == 49 || headOrder == 50 || headOrder == 51 || headOrder == 52 || headOrder == 53 || headOrder == 54 || headOrder == 55 || headOrder == 56 || headOrder == 57 || headOrder == 58 || headOrder == 59 || headOrder == 60 || headOrder == 61 || headOrder == 62 || headOrder == 63 || headOrder == 64 || headOrder == 65 || headOrder == 66 || headOrder == 67 || headOrder == 68 || headOrder == 69 || headOrder == 70 || headOrder == 71 || headOrder == 72 || headOrder == 73 || headOrder == 74 || headOrder == 75 || headOrder == 76 || headOrder == 77 || headOrder == 78 || headOrder == 79 || headOrder == 80 || headOrder == 81 || headOrder == 82 || headOrder == 83 || headOrder == 84 || headOrder == 85 || headOrder == 86 || headOrder == 87 || headOrder == 88 || headOrder == 89 || headOrder == 90 || headOrder == 91 || headOrder == 92) emitHeadCLinkResult(before, after, headOrder);"
        capture_after = 0
        in_linker = 0
        next
    }
...
if [ "$(grep -c '^stemsheadphasecontinue ' "$rows")" -ne 1 ] || \
        ! grep -q 'headOrder 92 headX 54 headSig 78 headInterId 1445 .*decisions \[LEFT:SkipAlreadyLinked,RIGHT:SkipClosed\].*incident \[stem2362:headSideLEFT:heads\[x51:sig82:id1453:sideLEFT,x54:sig78:id1445:sideLEFT,x55:sig79:id1447:sideLEFT\].*closureWrites \[x51:sig82:LEFT:true->true,x51:sig82:RIGHT:true->true,x55:sig79:LEFT:true->true,x55:sig79:RIGHT:true->true\].*closedValueChanges 0 .*nextHeadOrder 93 nextHeadX 37 nextHeadSig 44 nextHeadInterId 1373 ' "$rows" || \
        ! grep -q '^stemsheadclinkfrontier .*headOrder 92 headX 54 headSig 78 .*cAlias h:54:LEFT:BOTTOM .*lastIndex 1 maxIndex 1 relations 2 .*glyphs 1 .*candidateIdBefore 334 existingGlyph glyph:334 existingActive true existingStem 2362 ' "$rows" || \
        ! grep -q '^stemsheadclinkresult headOrder 92 allocatorBefore 2385 allocatorAfter 2385 .*addedVertices - addedEdges - addedSystemStems - linkerChanges - ' "$rows"; then
    echo "v92 zero-write existing-stem reconciliation contract differs" >&2
    exit 1
fi
: <<'DISABLED_V27_ACTIVE'
...
v91_runner_sha=$(shasum -a 256 "$script_dir/run-stems-head-phase-prefix-v91.sh" | awk '{print $1}')
if [ "$v91_runner_sha" != "cb63feba6a88cdc44daab868b32d6f869cc5da14848dd7ad3a978a547b0fa2c8" ]; then
    echo "v92 base v91 runner drifted" >&2
    exit 1
fi
v91_fixture="$repo_root/rust/oracle/stems-head-phase-prefix-chula-system1-v91.txt"
v91_fixture_sha=$(shasum -a 256 "$v91_fixture" | awk '{print $1}')
if [ "$v91_fixture_sha" != "f4b8bbe832c33ffd5bf439c34f34eb60b966cbb82a7859368c1e3e789b5ad121" ]; then
    echo "v92 base v91 fixture drifted" >&2
    exit 1
fi
...
out="/private/tmp/stems-head-phase-prefix-chula-system1-v92-audit.txt"
{
    echo '# Java Audiveris 5.11 (Temurin JDK 25.0.3+9 LTS) post-STUMPS head phase v92.'
    echo '# schema: stems-head-phase-prefix-v92'
    echo '# Snapshot-minimized order-92 derivative: orders 1-91 mutate without snapshots; order 92 emits the zero-write existing-stem reconciliation.'
    echo '# This bounded scope is intentional for deterministic replay under the full-snapshot heap limit.'
    cat "$rows"
    printf '%s\n' \
        "stemsheadphaseprefix summary schema stems-head-phase-prefix-v92 page chula.png#1 system 1 rows $row_count baseProbeSourceSha256 $base_probe_sha baseV91RunnerSourceSha256 $v91_runner_sha baseV91FixtureSha256 $v91_fixture_sha fragmentSourceSha256 $fragment_sha probeSourceSha256 $probe_sha runnerSourceSha256 $runner_sha emittedBodySha256 $body_sha semanticPassSha256 $semantic_sha completeStumpsFixtureSha256 $stumps_sha freshRuns 2 freshRunsByteIdentical true nativeScope BoundedSnapshotMinimizedOrder92ExistingStemReconciliation javaEvidence ReturnedBeforeNinetyThirdHead"
} > "$out"
echo "wrote $out"
