...
# Read-only snapshot-minimized order59 continuation from v58; prior orders mutate without snapshots.
set -eu
...
tmp_base=/private/tmp/stems-head-phase-prefix-v59
...
awk -v fragment="$fragment18" '
{
    ...
    if (capture_after && index($0, "heads, allLinkers);") != 0) {
        print
        print "            if (headOrder == 7 || headOrder == 22 || headOrder == 27 || headOrder == 34 || headOrder == 36 || headOrder == 37 || headOrder == 38 || headOrder == 39 || headOrder == 40 || headOrder == 41 || headOrder == 42 || headOrder == 43 || headOrder == 44 || headOrder == 45 || headOrder == 46 || headOrder == 47 || headOrder == 48 || headOrder == 49 || headOrder == 50 || headOrder == 51 || headOrder == 52 || headOrder == 53 || headOrder == 54 || headOrder == 55 || headOrder == 56 || headOrder == 57 || headOrder == 58 || headOrder == 59) emitHeadCLinkResult(before, after, headOrder);"
        capture_after = 0
        in_linker = 0
        next
    }
...
if [ "$(grep -c '^stemsheadphasecontinue ' "$rows")" -ne 1 ] || \
        ! grep -q 'headOrder 59 headX 100 headSig 42 headInterId 1369 .*decisions \[LEFT:SkipAlreadyLinked,RIGHT:SkipClosed\].*incident \[stem2343:headSideLEFT:heads\[x100:sig42:id1369:sideLEFT,x101:sig43:id1371:sideLEFT\].*closureWrites \[x101:sig43:LEFT:false->true,x101:sig43:RIGHT:false->true\].*closedValueChanges 2 .*nextHeadOrder 60 nextHeadX 71 nextHeadSig 49 nextHeadInterId 1387 ' "$rows" || \
        ! grep -q '^stemsheadclinkfrontier .*headOrder 59 headX 100 headSig 42 .*cAlias h:100:LEFT:BOTTOM .*lastIndex 4 maxIndex 4 relations 4 .*glyphs 2 .*candidateIdBefore 0 existingGlyph glyph:333 existingActive true existingStem 2343 ' "$rows" || \
        ! grep -q '^stemsheadclinkresult headOrder 59 allocatorBefore 2385 allocatorAfter 2385 .*addedVertices - addedEdges - addedSystemStems - .*linkerChanges \[h:101:LEFT:BOTTOM:true:false->true:true' "$rows"; then
    echo "v59 existing-stem reconciliation contract differs" >&2
    exit 1
fi
: <<'DISABLED_V27_ACTIVE'
...
v58_runner_sha=$(shasum -a 256 "$script_dir/run-stems-head-phase-prefix-v58.sh" | awk '{print $1}')
if [ "$v58_runner_sha" != "9964348b54b3500efda3f1e98b1fcf4e54d9e518de4d416b170a2b1fbe8ea757" ]; then
    echo "v59 base v58 runner drifted" >&2
    exit 1
fi
v58_fixture="$repo_root/rust/oracle/stems-head-phase-prefix-chula-system1-v58.txt"
v58_fixture_sha=$(shasum -a 256 "$v58_fixture" | awk '{print $1}')
if [ "$v58_fixture_sha" != "a262c7a657a028a7c2e273283176749bc364717837735a391540cb2783a2ed06" ]; then
    echo "v59 base v58 fixture drifted" >&2
    exit 1
fi
...
out="/private/tmp/stems-head-phase-prefix-chula-system1-v59-audit.txt"
{
    echo '# Java Audiveris 5.11 (Temurin JDK 25.0.3+9 LTS) post-STUMPS head phase v59.'
    echo '# schema: stems-head-phase-prefix-v59'
    echo '# Snapshot-minimized order-59 derivative: orders 1-58 mutate without snapshots; order 59 emits existing-stem reconciliation.'
    echo '# This bounded scope is intentional for deterministic replay under the full-snapshot heap limit.'
    cat "$rows"
    printf '%s\n' \
        "stemsheadphaseprefix summary schema stems-head-phase-prefix-v59 page chula.png#1 system 1 rows $row_count baseProbeSourceSha256 $base_probe_sha baseV58RunnerSourceSha256 $v58_runner_sha baseV58FixtureSha256 $v58_fixture_sha fragmentSourceSha256 $fragment_sha probeSourceSha256 $probe_sha runnerSourceSha256 $runner_sha emittedBodySha256 $body_sha semanticPassSha256 $semantic_sha completeStumpsFixtureSha256 $stumps_sha freshRuns 2 freshRunsByteIdentical true nativeScope BoundedSnapshotMinimizedOrder59ExistingStemReconciliation javaEvidence ReturnedBeforeSixtiethHead"
} > "$out"
echo "wrote $out"
