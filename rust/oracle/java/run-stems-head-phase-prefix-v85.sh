...
# Read-only snapshot-minimized order85 continuation from v84; prior orders mutate without snapshots.
set -eu
...
tmp_base=/private/tmp/stems-head-phase-prefix-v85
...
awk -v fragment="$fragment18" '
{
    ...
    if (capture_after && index($0, "heads, allLinkers);") != 0) {
        print
        print "            if (headOrder == 7 || headOrder == 22 || headOrder == 27 || headOrder == 34 || headOrder == 36 || headOrder == 37 || headOrder == 38 || headOrder == 39 || headOrder == 40 || headOrder == 41 || headOrder == 42 || headOrder == 43 || headOrder == 44 || headOrder == 45 || headOrder == 46 || headOrder == 47 || headOrder == 48 || headOrder == 49 || headOrder == 50 || headOrder == 51 || headOrder == 52 || headOrder == 53 || headOrder == 54 || headOrder == 55 || headOrder == 56 || headOrder == 57 || headOrder == 58 || headOrder == 59 || headOrder == 60 || headOrder == 61 || headOrder == 62 || headOrder == 63 || headOrder == 64 || headOrder == 65 || headOrder == 66 || headOrder == 67 || headOrder == 68 || headOrder == 69 || headOrder == 70 || headOrder == 71 || headOrder == 72 || headOrder == 73 || headOrder == 74 || headOrder == 75 || headOrder == 76 || headOrder == 77 || headOrder == 78 || headOrder == 79 || headOrder == 80 || headOrder == 81 || headOrder == 82 || headOrder == 83 || headOrder == 84 || headOrder == 85) emitHeadCLinkResult(before, after, headOrder);"
        capture_after = 0
        in_linker = 0
        next
    }
...
if [ "$(grep -c '^stemsheadphasecontinue ' "$rows")" -ne 1 ] || \
        ! grep -q 'headOrder 85 headX 79 headSig 40 headInterId 1365 .*decisions \[LEFT:SkipAlreadyLinked,RIGHT:SkipClosed\].*incident \[stem2371:headSideLEFT:heads\[x79:sig40:id1365:sideLEFT,x80:sig32:id1349:sideLEFT,x81:sig33:id1351:sideLEFT\].*closureWrites \[x80:sig32:LEFT:true->true,x80:sig32:RIGHT:true->true,x81:sig33:LEFT:true->true,x81:sig33:RIGHT:true->true\].*closedValueChanges 0 .*nextHeadOrder 86 nextHeadX 51 nextHeadSig 82 nextHeadInterId 1453 ' "$rows" || \
        ! grep -q '^stemsheadclinkfrontier .*headOrder 85 headX 79 headSig 40 .*cAlias h:79:LEFT:BOTTOM .*lastIndex 2 maxIndex 2 relations 3 .*glyphs 1 .*candidateIdBefore 306 existingGlyph glyph:306 existingActive true existingStem 2371 ' "$rows" || \
        ! grep -q '^stemsheadclinkresult headOrder 85 allocatorBefore 2385 allocatorAfter 2385 .*addedVertices - addedEdges - addedSystemStems - linkerChanges - ' "$rows"; then
    echo "v85 zero-write existing-stem reconciliation contract differs" >&2
    exit 1
fi
: <<'DISABLED_V27_ACTIVE'
...
v84_runner_sha=$(shasum -a 256 "$script_dir/run-stems-head-phase-prefix-v84.sh" | awk '{print $1}')
if [ "$v84_runner_sha" != "87552c47731d7c9692cfc3f1cbcfd8a5dde655c598d3b83069257b09dd8c286f" ]; then
    echo "v85 base v84 runner drifted" >&2
    exit 1
fi
v84_fixture="$repo_root/rust/oracle/stems-head-phase-prefix-chula-system1-v84.txt"
v84_fixture_sha=$(shasum -a 256 "$v84_fixture" | awk '{print $1}')
if [ "$v84_fixture_sha" != "a5be0414d7c9035cfabd8d023a4b50b8e0ff5d89d14f5f3fbe56d33c5abf18c9" ]; then
    echo "v85 base v84 fixture drifted" >&2
    exit 1
fi
...
out="/private/tmp/stems-head-phase-prefix-chula-system1-v85-audit.txt"
{
    echo '# Java Audiveris 5.11 (Temurin JDK 25.0.3+9 LTS) post-STUMPS head phase v85.'
    echo '# schema: stems-head-phase-prefix-v85'
    echo '# Snapshot-minimized order-85 derivative: orders 1-84 mutate without snapshots; order 85 emits the zero-write existing-stem reconciliation.'
    echo '# This bounded scope is intentional for deterministic replay under the full-snapshot heap limit.'
    cat "$rows"
    printf '%s\n' \
        "stemsheadphaseprefix summary schema stems-head-phase-prefix-v85 page chula.png#1 system 1 rows $row_count baseProbeSourceSha256 $base_probe_sha baseV84RunnerSourceSha256 $v84_runner_sha baseV84FixtureSha256 $v84_fixture_sha fragmentSourceSha256 $fragment_sha probeSourceSha256 $probe_sha runnerSourceSha256 $runner_sha emittedBodySha256 $body_sha semanticPassSha256 $semantic_sha completeStumpsFixtureSha256 $stumps_sha freshRuns 2 freshRunsByteIdentical true nativeScope BoundedSnapshotMinimizedOrder85ExistingStemReconciliation javaEvidence ReturnedBeforeEightySixthHead"
} > "$out"
echo "wrote $out"
