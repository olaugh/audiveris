...
probe_source="$repo_root/rust/oracle/java/StemsBeamVLinkSiblingLinksProbe.java"
runner_source="$repo_root/rust/oracle/java/run-stems-beam-vlink-sibling-links.sh"
...
case "$page_key" in
    ...
    *) echo "unknown beam VLink sibling-links page key: $page_key" >&2; exit 2 ;;
esac
...
for required in "$probe_cp_file" "$scheduler_fixture" "$expand_fixture" \
        ...
        "$b_linker_flag_probe" "$b_linker_flag_runner"; do
    if [ ! -f "$required" ]; then
...
b16_active_source_bundle_sha256_before=$(b16_active_source_rows | sha256_text)

lock_dir=/private/tmp/stems-beam-vlink-sibling-links.lock
if ! mkdir "$lock_dir"; then
    echo "another sibling-links oracle runner owns $lock_dir" >&2
    exit 1
fi
probe_classes=
pass_one=
...
trap 'exit 130' INT
trap 'exit 143' TERM
probe_classes=$(mktemp -d /private/tmp/stems-beam-vlink-sibling-links-classes.XXXXXX)
pass_one=$(mktemp /private/tmp/stems-beam-vlink-sibling-links-pass1.XXXXXX)
pass_two=$(mktemp /private/tmp/stems-beam-vlink-sibling-links-pass2.XXXXXX)
    ...
    while [ "$system_id" -le "$expected_systems" ]; do
        (
            ...
            env -u JAVA_TOOL_OPTIONS -u _JAVA_OPTIONS -u JDK_JAVA_OPTIONS \
                ...
                org.audiveris.omr.rustport.StemsBeamVLinkSiblingLinksProbe \
                --system "$system_id" \
...
if ! cmp -s "$pass_one" "$pass_two"; then
    echo "two fresh $page_key sibling-links passes are not byte-identical" >&2
    exit 1
fi
if [ "$(sha256_file "$probe_source")" != "$probe_source_sha256_before" ] || \
        ...
        [ "$(b16_active_source_rows | sha256_text)" \
            != "$b16_active_source_bundle_sha256_before" ] || \
    ...
    echo "sibling-links input changed during the two-pass run" >&2
    exit 1
fi

schema_count=$(awk '$0 == "# schema: stems-beam-vlink-sibling-links-v1" { count++ } END { print count + 0 }' "$pass_one")
stale_count=$(awk '/^stemsbeamvlink(blinkerflag|baseapply)/ { count++ } END { print count + 0 }' "$pass_one")
if [ "$schema_count" -ne 1 ] || [ "$stale_count" -ne 0 ]; then
    echo "sibling-links schema header/output family drift" >&2
    exit 1
fi
...
census=$(awk -v systems="$expected_systems" -v pageRef="$page_file#1" '
    ...
    function sibling_key(k) { return k SUBSEP f["siblingOrdinal"] }
    function known(label) {
        return label ~ /^stemsbeamvlinksiblinglinks(predecessor|baseline|groupmember|sibling|sourceoutgoing|pairrelation|geometry|edge|stemincident|beamincident|callback|linkerlookup|linkerflag|siblingresult|result|deltaguard|summary)$/
    }
    function sibling_rank(label) {
        if (label == "stemsbeamvlinksiblinglinkssibling") return 1
        if (label == "stemsbeamvlinksiblinglinkssourceoutgoing") return 2
        if (label == "stemsbeamvlinksiblinglinkspairrelation") return 3
        if (label == "stemsbeamvlinksiblinglinksgeometry") return 4
        if (label == "stemsbeamvlinksiblinglinksedge") return 5
        if (label == "stemsbeamvlinksiblinglinksstemincident") return 6
        if (label == "stemsbeamvlinksiblinglinksbeamincident") return 7
        if (label == "stemsbeamvlinksiblinglinkscallback") return 8
        if (label == "stemsbeamvlinksiblinglinkslinkerlookup") return 9
        if (label == "stemsbeamvlinksiblinglinkslinkerflag") return 10
        if (label == "stemsbeamvlinksiblinglinkssiblingresult") return 11
        return 0
    }
    /^stemsbeamvlinksiblinglinkspage / {
        page++
        if ($2 != pageRef || $3 != "systems" || $4 != systems) bad++
        next
    }
    /^stemsbeamvlinksiblinglinkssyntheticcase / {
        fields(3)
        k = key()
        if ($2 != pageRef || f["system"] != 1 ||
                (f["scope"] != "synthetic" && f["scope"] != "envelope") ||
                f["case"] == "-" || f["join"] != "IsolatedBoundary15Replay" ||
                f["construction"] != "RealBookSheetSystemSIG" || syntheticSeen[k]++) bad++
        ...
        if (f["branch"] != "SameGlyph" && f["branch"] != "ExistingBeamStem" &&
                f["branch"] != "ShorterWrongSide" && f["branch"] != "Linked") bad++
        if (f["branch"] == "SameGlyph") sameGlyph++
        else if (f["branch"] == "ExistingBeamStem") existingBeamStem++
        else if (f["branch"] == "ShorterWrongSide") shorterWrongSide++
        else linked++
        siblings++
        if (f["freshRelationIdentity"] != "-") edgeTotal++
        flagTotal += f["writeCount"] + 0
        chordTotal += f["chordStemMatches"] + 0
        next
    }
    /^stemsbeamvlinksiblinglinkssyntheticevent / {
        fields(3)
        k = key()
        if (!(k in syntheticSeen) || f["eventOrdinal"] != syntheticEvents[k] ||
                (f["kind"] != "SigEdgeInserted" &&
                 f["kind"] != "RelationCallbackCompleted" &&
                 f["kind"] != "BLinkerLinkedAssigned" && f["kind"] != "Throw")) bad++
        syntheticEvents[k]++
        next
    }
    /^stemsbeamvlinksiblinglinkssyntheticguard / {
        fields(3)
        k = key()
        if (!(k in syntheticSeen) || syntheticGuard[k]++ ||
                ...
                f["headRelationLoopRead"] != "false") bad++
        if (syntheticScope[k] == "synthetic") synthetic++
        else envelope++
        transactions++
        syntheticEventTotal += syntheticEvents[k]
        next
    }
    /^stemsbeamvlinksiblinglinks/ {
        if (!known($1)) { bad++; next }
        fields(3)
        k = key()
        label = $1
        ...
        if ($2 != pageRef || f["system"] !~ /^[1-9][0-9]*$/ ||
                f["plan"] !~ /^[0-9]+$/) bad++
        if (f["scope"] != "real" && f["scope"] != "synthetic" &&
                f["scope"] != "envelope") bad++
        if (f["scope"] == "real" && (f["case"] != "-" || f["system"] > systems)) bad++
        if (f["scope"] != "real" && f["system"] != 1) bad++

        if (label == "stemsbeamvlinksiblinglinkspredecessor") {
            if (phase[k] != 0 || f["predecessorTerminal"] != "ReadyBeforeSiblingBeamLinks" ||
                    f["targetBLinked"] != "true") bad++
            if (f["scope"] == "real" && f["join"] != "FullBoundary15Replay") bad++
            phase[k] = 1
        } else if (label == "stemsbeamvlinksiblinglinksbaseline") {
            if (phase[k] != 1 || f["soleSigListener"] != "true" ||
                    f["cachedMedianSameIdentity"] != "true" || f["baseRemoved"] != "true" ||
                    f["chordStemMatches"] != 0) bad++
            phase[k] = 2
            expectedGroup[k] = f["groupOutgoingScanned"] + 0
            expectedSiblings[k] = f["siblings"] + 0
            chordTotal += f["chordStemMatches"] + 0
        } else if (label == "stemsbeamvlinksiblinglinksgroupmember") {
            if ((phase[k] != 2 && phase[k] != 3) ||
                    f["groupOutgoingOrdinal"] != groupByKey[k]) bad++
            phase[k] = 3
            groupByKey[k]++
            groupRows++
        } else if (label == "stemsbeamvlinksiblinglinkssibling") {
            if (phase[k] != 2 && phase[k] != 3 && phase[k] != 14) bad++
            if (f["siblingOrdinal"] != siblingByKey[k]) bad++
            sk = sibling_key(k)
            siblingRank[sk] = 1
            branch[sk] = f["branch"]
            expectedSource[sk] = f["sourceOutgoingScanned"] + 0
            expectedPair[sk] = f["pairRows"] + 0
            if (branch[sk] != "SameGlyph" && branch[sk] != "ExistingBeamStem" &&
                    branch[sk] != "ShorterWrongSide" && branch[sk] != "Linked") bad++
            if (branch[sk] == "SameGlyph") sameGlyph++
            else if (branch[sk] == "ExistingBeamStem") existingBeamStem++
            else if (branch[sk] == "ShorterWrongSide") shorterWrongSide++
            else linked++
            siblingByKey[k]++
            siblings++
            phase[k] = 4
        } else if (sibling_rank(label) != 0) {
            sk = sibling_key(k)
            rank = sibling_rank(label)
            if (phase[k] < 4 || phase[k] >= 14 ||
                    f["siblingOrdinal"] != siblingByKey[k] - 1 ||
                    rank < siblingRank[sk]) bad++
            siblingRank[sk] = rank
            if (label == "stemsbeamvlinksiblinglinkssourceoutgoing") {
                if (f["sourceOutgoingOrdinal"] != sourceRows[sk]) bad++
                sourceRows[sk]++
            } else if (label == "stemsbeamvlinksiblinglinkspairrelation") {
                if (f["pairOrdinal"] != pairRows[sk]) bad++
                pairRows[sk]++
            } else if (label == "stemsbeamvlinksiblinglinksgeometry") {
                geometryRows[sk]++
                if (f["branch"] != branch[sk]) bad++
            } else if (label == "stemsbeamvlinksiblinglinksedge") {
                edgeRows[sk]++
                if (f["eventOrdinal"] != nextEvent[k]) bad++
                nextEvent[k]++
                edgeTotal++
            } else if (label == "stemsbeamvlinksiblinglinksstemincident") {
                if (f["incidentOrdinal"] != stemRows[sk]) bad++
                stemRows[sk]++
            } else if (label == "stemsbeamvlinksiblinglinksbeamincident") {
                if (f["incidentOrdinal"] != beamRows[sk]) bad++
                beamRows[sk]++
            } else if (label == "stemsbeamvlinksiblinglinkscallback") {
                callbackRows[sk]++
                if (f["eventOrdinal"] != nextEvent[k] || f["chordStemMatches"] != 0) bad++
                nextEvent[k]++
                chordTotal += f["chordStemMatches"] + 0
            } else if (label == "stemsbeamvlinksiblinglinkslinkerlookup") {
                if (f["itemOrdinal"] != lookupRows[sk]) bad++
                lookupRows[sk]++
            } else if (label == "stemsbeamvlinksiblinglinkslinkerflag") {
                flagRows[sk]++
                if (f["eventOrdinal"] != "-") {
                    if (f["eventOrdinal"] != nextEvent[k]) bad++
                    nextEvent[k]++
            ...
            } else if (label == "stemsbeamvlinksiblinglinkssiblingresult") {
                if (sourceRows[sk] != expectedSource[sk] || pairRows[sk] != expectedPair[sk] ||
                        f["branch"] != branch[sk] || f["terminal"] != "Continue" ||
                        f["linkerLookupRows"] != lookupRows[sk]) bad++
                if (branch[sk] == "SameGlyph" || branch[sk] == "ExistingBeamStem") {
                    if (geometryRows[sk] != 0 || f["edgeCommitted"] != "false") bad++
                } else if (geometryRows[sk] != 1) bad++
                if (f["edgeCommitted"] == "true") {
                    if (edgeRows[sk] != 1 || callbackRows[sk] != 1 || flagRows[sk] != 1) bad++
                } else if (edgeRows[sk] != 0 || callbackRows[sk] != 0 || flagRows[sk] != 0) bad++
                phase[k] = 14
            }
        } else if (label == "stemsbeamvlinksiblinglinksresult") {
            if ((phase[k] != 2 && phase[k] != 3 && phase[k] != 14) ||
                    f["terminal"] != "ReadyBeforeHeadRelationLoop" ||
                    f["headRelationLoopRead"] != "false" ||
                    f["siblings"] != siblingByKey[k] || f["eventCount"] != nextEvent[k]) bad++
            phase[k] = 15
        } else if (label == "stemsbeamvlinksiblinglinksdeltaguard") {
            if (phase[k] != 15 || f["zeroChordStem"] != "true" ||
                    f["headRelationLoopRead"] != "false" ||
                    f["stopBeforeHeadRelationLoop"] != "true") bad++
            phase[k] = 16
        } else if (label == "stemsbeamvlinksiblinglinkssummary") {
            if (phase[k] != 16 || f["terminal"] != "ReadyBeforeHeadRelationLoop" ||
                    f["groupRows"] != groupByKey[k] || f["siblings"] != siblingByKey[k] ||
                    f["events"] != nextEvent[k]) bad++
            phase[k] = 17
            transactions++
            coreEventTotal += f["events"] + 0
            if (f["scope"] == "real") { real++; realSystem[f["system"]]++ }
            else if (f["scope"] == "synthetic") synthetic++
            else envelope++
        }
        next
    }
    ...
    { if (NF != 0) bad++ }
    END {
        expectedSyntheticOrder = "SameGlyph,ExistingBeamStem,ShorterWrongSide,LinkedBeam," \
                "LinkedSmallBeam,LinkedHook,LinkedNoBLinker,LinkedIdempotentBCell," \
                "ThrowBeforeInsertion,ThrowDuringCallback"
        if (page != 1 || real != systems || synthetic != 8 || envelope != 2 ||
                transactions != real + synthetic + envelope ||
                syntheticOrder != expectedSyntheticOrder) bad++
        for (i = 1; i <= systems; i++) if (realSystem[i] != 1) bad++
        for (k in seenKey) {
            if (phase[k] != 17 || groupByKey[k] != expectedGroup[k] ||
                    siblingByKey[k] != expectedSiblings[k] ||
                    family[k SUBSEP "stemsbeamvlinksiblinglinkspredecessor"] != 1 ||
                    family[k SUBSEP "stemsbeamvlinksiblinglinksbaseline"] != 1 ||
                    family[k SUBSEP "stemsbeamvlinksiblinglinksresult"] != 1 ||
                    family[k SUBSEP "stemsbeamvlinksiblinglinksdeltaguard"] != 1 ||
                    family[k SUBSEP "stemsbeamvlinksiblinglinkssummary"] != 1) bad++
        }
        printf "%d:%d:%d:%d:%d:%d:%d:%d:%d:%d:%d:%d:%d:%d:%d\n",
                bad, real, synthetic, envelope, transactions, groupRows, siblings,
                sameGlyph, existingBeamStem, shorterWrongSide, linked,
                edgeTotal, flagTotal, coreEventTotal + syntheticEventTotal, chordTotal
    }
' "$pass_one")
IFS=: read -r census_bad real_transactions synthetic_cases envelope_cases transaction_rows \
    group_rows sibling_candidates same_glyph existing_beam_stem shorter_wrong_side linked_siblings \
    sibling_edges_added linker_writes event_count chord_stem_matches <<EOF
$census
EOF
if [ "$census_bad" -ne 0 ]; then
    echo "invalid $page_key sibling-links census" >&2
    exit 1
fi
...
row_counts=""
for family in page predecessor baseline groupmember sibling sourceoutgoing pairrelation geometry \
        edge stemincident beamincident callback linkerlookup linkerflag siblingresult result \
        deltaguard summary syntheticcase syntheticevent syntheticguard; do
    label=stemsbeamvlinksiblinglinks$family
...
cat "$pass_one"
printf 'stemsbeamvlinksiblinglinkspagesummary %s systems %s realTransactions %s supportedSyntheticCases %s envelopeCases %s totalTransactions %s groupRows %s siblingCandidates %s sameGlyph %s existingBeamStem %s shorterWrongSide %s linked %s edgesAdded %s linkerWrites %s events %s chordStemMatches %s stopBeforeHeadRelationLoop true\n' \
    "$page_file#1" "$expected_systems" "$real_transactions" "$synthetic_cases" \
    "$envelope_cases" "$transaction_rows" "$group_rows" "$sibling_candidates" \
    "$same_glyph" "$existing_beam_stem" "$shorter_wrong_side" "$linked_siblings" \
    "$sibling_edges_added" "$linker_writes" "$event_count" "$chord_stem_matches"
runtime_processes=$((2 * expected_systems))
total_processes=$((runtime_processes + 1))
printf 'stemsbeamvlinksiblinglinkscorpussummary schema stems-beam-vlink-sibling-links-v1 mode %s pages 1 pageRefs %s rowCounts %s pageInputSha256 %s probeSourceSha256 %s runnerSourceSha256 %s effectiveClasspathSha256 %s jdkReleaseSha256 %s javaExecutableSha256 %s javaJpegLibrarySha256 %s javaModulesSha256 %s javaVmLibrarySha256 %s javaAwtLibrarySha256 %s javaAwtLwawtLibrarySha256 %s javaArchitecture %s javaRuntimeVersion %s javaVmVariant %s javaImageType %s beamLinkerClassSha256 %s bLinkerClassSha256 %s vLinkerClassSha256 %s stemLinkerClassSha256 %s%s jgraphtCoreVersion 1.5.2 jgraphtCoreJarSha256 %s schedulerFixtureSha256 %s expandFixtureSha256 %s createStemFixtureSha256 %s reuseCheckFixtureSha256 %s baseApplyFixtureSha256 %s baseApplyManifestSha256 %s bLinkerFlagFixtureSha256 %s bLinkerFlagManifestSha256 %s predecessorReplay FullBoundary15TypedReplayAndExactJavaRowJoin querySerialization UTF8ColonTokensLF-LazyNotReadLiteral emittedBodySha256 %s emittedBodyLines %s emittedBodyBytes %s freshRunsPerPage 2 freshRunsByteIdentical true rawPassSha256 %s freshJvmPerSystem true compilerJavaProcesses 1 runtimeJavaProcessesPerPass %s runtimeJavaProcesses %s totalJavaProcesses %s maximumConcurrentJavaProcesses 1 concurrencyScope Boundary16RunnerLockedInvocation compilerJavaProcessReaped true runtimeJavaProcessesReaped true foregroundJavaProcessesOnly true backgroundJavaProcessesStarted 0 realTransactions %s supportedSyntheticCases %s envelopeCases %s totalTransactions %s siblingCandidates %s sameGlyph %s existingBeamStem %s shorterWrongSide %s linked %s edgesAdded %s linkerWrites %s chordStemMatches %s system1SyntheticBlock true addEdgeReturnedFalseEvidence IndependentModelOnlyNoStockJavaFixture envelopeEvidenceScope JavaOnlyNotProductionEquivalent stopBeforeHeadRelationLoop true\n' \
    ...
    "$real_transactions" "$synthetic_cases" "$envelope_cases" "$transaction_rows" \
    "$sibling_candidates" "$same_glyph" "$existing_beam_stem" "$shorter_wrong_side" \
    "$linked_siblings" "$sibling_edges_added" "$linker_writes" "$chord_stem_matches"
