...
# Cucaracha system 1 queue 6 / Java Inter 1083.
function emit_after_reuse() {
    print "                    if (append && head.getId() == 1083) {"
    print "                        System.out.printf("
    print "                                \"stemsheadphase2cucarachas1q6reuse headInterId %d lastIndex %d selectedStem %s terminal ReturnedFromReuseStem%n\","
    print "                                head.getId(), lastIndex, rustStem(stem));"
    print "                    }"
}

function emit_reuse_match() {
    print "                            if (head.getId() == 1083) {"
    print "                                final StemInter rustReuse = (StemInter) sig.getOppositeInter(h, r);"
    print "                                System.out.printf("
    print "                                        \"stemsheadphase2cucarachas1q6reusematch headInterId %d sourceHeadId %d sourceCorner %s sourceSide %s relationGrade %s stem %s terminal SelectedReuseStem%n\","
...
function emit_committed_result() {
    print "                if (append && head.getId() == 1083) {"
    print "                    System.out.printf("
    print "                            \"stemsheadphase2cucarachas1q6commit headInterId %d linkedStem %s relationRows %s vertices %d edges %d allocator %d terminal ReturnedHeadCLinkTransaction%n\","
...
{
    gsub("head.getId[(][)] == 1777", "head.getId() == 1083")
    gsub("hSide == HorizontalSide.RIGHT", "hSide == HorizontalSide.LEFT")
    gsub("stemsheadphase2x14", "stemsheadphase2cucarachas1q6")
