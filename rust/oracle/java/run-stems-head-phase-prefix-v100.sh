...
# Read-only snapshot-minimized order100 continuation from v99; prior orders mutate without snapshots.
set -eu
...
tmp_base=/private/tmp/stems-head-phase-prefix-v100
...
awk -v fragment="$fragment18" '
{
    ...
    if (capture_after && index($0, "heads, allLinkers);") != 0) {
        print
        print "            if (headOrder == 7 || headOrder == 22 || headOrder == 27 || headOrder == 34 || headOrder == 36 || headOrder == 37 || headOrder == 38 || headOrder == 39 || headOrder == 40 || headOrder == 41 || headOrder == 42 || headOrder == 43 || headOrder == 44 || headOrder == 45 || headOrder == 46 || headOrder == 47 || headOrder == 48 || headOrder == 49 || headOrder == 50 || headOrder == 51 || headOrder == 52 || headOrder == 53 || headOrder == 54 || headOrder == 55 || headOrder == 56 || headOrder == 57 || headOrder == 58 || headOrder == 59 || headOrder == 60 || headOrder == 61 || headOrder == 62 || headOrder == 63 || headOrder == 64 || headOrder == 65 || headOrder == 66 || headOrder == 67 || headOrder == 68 || headOrder == 69 || headOrder == 70 || headOrder == 71 || headOrder == 72 || headOrder == 73 || headOrder == 74 || headOrder == 75 || headOrder == 76 || headOrder == 77 || headOrder == 78 || headOrder == 79 || headOrder == 80 || headOrder == 81 || headOrder == 82 || headOrder == 83 || headOrder == 84 || headOrder == 85 || headOrder == 86 || headOrder == 87 || headOrder == 88 || headOrder == 89 || headOrder == 90 || headOrder == 91 || headOrder == 92 || headOrder == 93 || headOrder == 94 || headOrder == 95 || headOrder == 96 || headOrder == 97 || headOrder == 98 || headOrder == 99 || headOrder == 100) emitHeadCLinkResult(before, after, headOrder);"
        capture_after = 0
        in_linker = 0
        next
    }
...
if [ "$(grep -c '^stemsheadphasecontinue ' "$rows")" -ne 1 ] || \
        ! grep -q 'headOrder 100 headX 86 headSig 85 headInterId 1459 .*decisions \[LEFT:SkipAlreadyLinked,RIGHT:SkipClosed\].*incident \[stem2366:headSideLEFT:heads\[x84:sig86:id1461:sideLEFT,x85:sig87:id1463:sideLEFT,x86:sig85:id1459:sideLEFT\].*closureWrites \[x84:sig86:LEFT:true->true,x84:sig86:RIGHT:true->true,x85:sig87:LEFT:true->true,x85:sig87:RIGHT:true->true\].*closedValueChanges 0 .*nextHeadOrder 101 nextHeadX 5 nextHeadSig 88 nextHeadInterId 1465 ' "$rows" || \
        ! grep -q '^stemsheadclinkfrontier .*headOrder 100 headX 86 headSig 85 .*cAlias h:86:LEFT:BOTTOM .*lastIndex 1 maxIndex 1 relations 2 .*glyphs 1 .*candidateIdBefore 320 existingGlyph glyph:320 existingActive true existingStem 2366 ' "$rows" || \
        ! grep -q '^stemsheadclinkresult headOrder 100 allocatorBefore 2385 allocatorAfter 2385 .*addedVertices - addedEdges - addedSystemStems - linkerChanges - ' "$rows"; then
    echo "v100 zero-write existing-stem reconciliation contract differs" >&2
    exit 1
fi
: <<'DISABLED_V27_ACTIVE'
...
v99_runner_sha=$(shasum -a 256 "$script_dir/run-stems-head-phase-prefix-v99.sh" | awk '{print $1}')
if [ "$v99_runner_sha" != "6f0f591dafa316dcdd6df14c6894004cb6d42f164c4db844bea6e7dc5128bd7d" ]; then
    echo "v100 base v99 runner drifted" >&2
    exit 1
fi
v99_fixture="$repo_root/rust/oracle/stems-head-phase-prefix-chula-system1-v99.txt"
v99_fixture_sha=$(shasum -a 256 "$v99_fixture" | awk '{print $1}')
if [ "$v99_fixture_sha" != "402e519f37113d96336718499d864a9dd29bbc85a5f86cab764a6b557034717b" ]; then
    echo "v100 base v99 fixture drifted" >&2
    exit 1
fi
...
out="/private/tmp/stems-head-phase-prefix-chula-system1-v100-audit.txt"
{
    echo '# Java Audiveris 5.11 (Temurin JDK 25.0.3+9 LTS) post-STUMPS head phase v100.'
    echo '# schema: stems-head-phase-prefix-v100'
    echo '# Snapshot-minimized order-100 derivative: orders 1-99 mutate without snapshots; order 100 emits the zero-write existing-stem reconciliation.'
    echo '# This bounded scope is intentional for deterministic replay under the full-snapshot heap limit.'
    cat "$rows"
    printf '%s\n' \
        "stemsheadphaseprefix summary schema stems-head-phase-prefix-v100 page chula.png#1 system 1 rows $row_count baseProbeSourceSha256 $base_probe_sha baseV99RunnerSourceSha256 $v99_runner_sha baseV99FixtureSha256 $v99_fixture_sha fragmentSourceSha256 $fragment_sha probeSourceSha256 $probe_sha runnerSourceSha256 $runner_sha emittedBodySha256 $body_sha semanticPassSha256 $semantic_sha completeStumpsFixtureSha256 $stumps_sha freshRuns 2 freshRunsByteIdentical true nativeScope BoundedSnapshotMinimizedOrder100ExistingStemReconciliation javaEvidence ReturnedBeforeHundredFirstHead"
} > "$out"
echo "wrote $out"
