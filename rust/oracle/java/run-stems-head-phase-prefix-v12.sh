...
# Freeze a bounded order-12 continuation replay.  Orders below 12 are
# executed without persistent snapshots; only the order-12 row is emitted.
set -eu
...
tmp_base=/private/tmp/stems-head-phase-prefix-v12
...
if ! cmp -s "$semantic_one" "$semantic_two"; then
    echo "two fresh post-STUMPS v12 semantic passes are not byte-identical" >&2
...
if [ "$(grep -c '^stemsheadphasecontinue ' "$rows")" -ne 1 ] || \
        ! grep -q 'headOrder 12 headX 55 headSig 79 headInterId 1447 .*nextHeadOrder 13 nextHeadX 53 nextHeadSig 3 nextHeadInterId 1291 ' "$rows" || \
        ! grep -q 'headOrder 12 .*closureWrites \[x51:sig82:LEFT:false->true,x51:sig82:RIGHT:false->true,x54:sig78:LEFT:false->true,x54:sig78:RIGHT:false->true\] closedValueChanges 4 unlinkedCount 0 ' "$rows" || \
        ! grep -q 'headOrder 12 .*sigVerticesBefore 680 sigVerticesAfter 680 sigEdgesBefore 691 sigEdgesAfter 691 systemStemsBefore 41 systemStemsAfter 41 ' "$rows"; then
    echo "v12 bounded order-12 continuation contract differs" >&2
    exit 1
fi
...
v11_runner_sha=$(shasum -a 256 "$script_dir/run-stems-head-phase-prefix-v11.sh" | awk '{print $1}')
if [ "$v11_runner_sha" != "f05ea06f61193785a84440b457b4e79b10e7d88e765b81bce51d6f996beb1702" ]; then
    echo "v12 base v11 runner drifted" >&2
    exit 1
fi
v11_fixture="$repo_root/rust/oracle/stems-head-phase-prefix-chula-system1-v11.txt"
v11_fixture_sha=$(shasum -a 256 "$v11_fixture" | awk '{print $1}')
if [ "$v11_fixture_sha" != "cad1527e556481a073ead938094de9edce09954e366bf5608ebc57a30ef946a3" ]; then
    echo "v12 base v11 fixture drifted" >&2
    exit 1
fi
...
out="$repo_root/rust/oracle/stems-head-phase-prefix-chula-system1-v12.txt"
{
    echo '# Java Audiveris 5.11 (Temurin JDK 25.0.3+9 LTS) post-STUMPS head phase v12.'
    echo '# schema: stems-head-phase-prefix-v12'
    echo '# Bounded order-12 derivative: orders 1-11 execute linkSides without persistent snapshots; only order 12 is emitted.'
    echo '# This minimized scope is intentional for deterministic replay under the full-snapshot heap limit.'
    cat "$rows"
    printf '%s\n' \
        "stemsheadphaseprefix summary schema stems-head-phase-prefix-v12 page chula.png#1 system 1 rows $row_count baseProbeSourceSha256 $base_probe_sha baseV11RunnerSourceSha256 $v11_runner_sha baseV11FixtureSha256 $v11_fixture_sha probeSourceSha256 $probe_sha runnerSourceSha256 $runner_sha emittedBodySha256 $body_sha semanticPassSha256 $semantic_sha completeStumpsFixtureSha256 $stumps_sha freshRuns 2 freshRunsByteIdentical true nativeScope BoundedSnapshotMinimizedOrder12 javaEvidence ReturnedBeforeThirteenthHead"
} > "$out"
echo "wrote $out"
