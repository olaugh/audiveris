...
# Freeze a snapshot-minimized order31 continuation from v30; prior orders mutate without snapshots.
set -eu
...
tmp_base=/private/tmp/stems-head-phase-prefix-v31
...
if [ "$(grep -c '^stemsheadphasecontinue ' "$rows")" -ne 1 ] || \
        ! grep -q 'headOrder 31 headX 16 headSig 81 headInterId 1451 .*decisions \[LEFT:SkipAlreadyLinked,RIGHT:top=false:bottom=false:branch=Neither\].*incident \[stem2360:headSideLEFT:heads\[x15:sig80:id1449:sideLEFT,x16:sig81:id1451:sideLEFT\]\].*closureWrites \[x15:sig80:LEFT:false->true,x15:sig80:RIGHT:false->true\].*closedValueChanges 2 .*nextHeadOrder 32 nextHeadX 34 nextHeadSig 77 nextHeadInterId 1443 ' "$rows"; then
    echo "v31 minimized closure contract differs" >&2
    exit 1
fi
: <<'DISABLED_V27_ACTIVE'
...
v30_runner_sha=$(shasum -a 256 "$script_dir/run-stems-head-phase-prefix-v30.sh" | awk '{print $1}')
if [ "$v30_runner_sha" != "d8f55efad82e15eb8b45c52ac8f99031c00ea0dd7143bc30c7c607fc103e71cf" ]; then
    echo "v31 base v30 runner drifted" >&2
    exit 1
fi
v30_fixture="$repo_root/rust/oracle/stems-head-phase-prefix-chula-system1-v30.txt"
v30_fixture_sha=$(shasum -a 256 "$v30_fixture" | awk '{print $1}')
if [ "$v30_fixture_sha" != "c4bde8384b872a03d7f9d7ecd87fdea60dc93a5b418ca831c8dbe5d8c3aa729d" ]; then
    echo "v31 base v30 fixture drifted" >&2
    exit 1
fi
...
out="$repo_root/rust/oracle/stems-head-phase-prefix-chula-system1-v31.txt"
{
    echo '# Java Audiveris 5.11 (Temurin JDK 25.0.3+9 LTS) post-STUMPS head phase v31.'
    echo '# schema: stems-head-phase-prefix-v31'
    echo '# Snapshot-minimized order-31 derivative: orders 1-30 mutate without snapshots; order 31 emits the prelinked closure row.'
    echo '# This minimized scope is intentional for deterministic replay under the full-snapshot heap limit.'
    cat "$rows"
    printf '%s\n' \
        "stemsheadphaseprefix summary schema stems-head-phase-prefix-v31 page chula.png#1 system 1 rows $row_count baseProbeSourceSha256 $base_probe_sha baseV30RunnerSourceSha256 $v30_runner_sha baseV30FixtureSha256 $v30_fixture_sha fragmentSourceSha256 $fragment_sha probeSourceSha256 $probe_sha runnerSourceSha256 $runner_sha emittedBodySha256 $body_sha semanticPassSha256 $semantic_sha completeStumpsFixtureSha256 $stumps_sha freshRuns 2 freshRunsByteIdentical true nativeScope BoundedSnapshotMinimizedOrder31PrelinkedClosure javaEvidence ReturnedBeforeThirtySecondHead"
} > "$out"
echo "wrote $out"
