...
# Deterministic Allegretto system-3 HEADS queue-116 prelinked-closure replay.
set -eu
...
tmp_dir=$(mktemp -d /private/tmp/stems-head-phase-allegretto-system3-order116.XXXXXX)
...
awk -v fragment="$fragment" '
{
    ...
    if (in_continuation && index($0, "final HeadLinker linker = head.getLinker();") != 0) {
        print
        print "            if (headOrder < 116 && headOrder != 53) {"
...
if ! grep -q '^stemsheadclinkfrontier allegretto.png#1 system 3 headOrder 53 headX 107 headSig 80 headInterId 1836 cAlias h:107:RIGHT:TOP .*lastIndex 3 maxIndex 3 relations 4 .*existingStem 2398 .*terminal ReadyForHeadCreateStem$' "$rows" || \
        ...
        ! grep -q '^stemsheadphasecontinue allegretto.png#1 system 3 headOrder 116 headX 66 headSig 33 headInterId 1743 ' "$rows"; then
    echo "Allegretto system-3 queue-116 audit contract differs" >&2
    exit 1
fi
...
base_runner="$script_dir/run-stems-head-phase-prefix-allegretto-system3-order115.sh"
base_fixture="$repo_root/rust/oracle/stems-head-phase-prefix-allegretto-system3-order115.txt"
...
if [ "$base_probe_sha" != "d5d46115fb4358918648d35e24cd043753b62ce709f767f8958d34ba25c9c4cf" ] || \
        ...
        [ "$base_runner_sha" != "b3c426db85a5c5402c7e8d5741e249c15905e0f2d8f4888d491ee9783982afa4" ] || \
        [ "$base_fixture_sha" != "01bda66e6eecf7d46bdd21f3d2d4d8ec977deff9bc51f01b4a3291092680fca2" ]; then
    echo "Allegretto system-3 queue-116 provenance drifted" >&2
    exit 1
fi
row_count=$(wc -l < "$rows" | tr -d ' ')
out="$repo_root/rust/oracle/stems-head-phase-prefix-allegretto-system3-order116.txt"
{
    echo '# Java Audiveris 5.11 (Temurin JDK 25.0.3+9 LTS) Allegretto system-3 post-STUMPS HEADS queue 116.'
    echo '# Snapshot-minimized G1 replay: orders 0-115 mutate without snapshots except the authenticated queue-53 multi-side transaction; queue 116 emits the target prelinked closure.'
    cat "$rows"
    printf '%s\n' \
        "stemsheadphaseprefix summary schema stems-head-phase-prefix-allegretto-system3-order116 page allegretto.png#1 system 3 rows $row_count baseProbeSourceSha256 $base_probe_sha fragmentSourceSha256 $fragment_sha glyphIndexSourceSha256 $glyph_index_sha retainedGlyphOverlaySha256 $overlay_sha allegrettoSystem3InitSha256 $init_sha baseSystem3Order115RunnerSha256 $base_runner_sha baseSystem3Order115FixtureSha256 $base_fixture_sha probeSourceSha256 $probe_sha runnerSourceSha256 $runner_sha emittedBodySha256 $body_sha semanticPassSha256 $semantic_sha freshRuns 2 freshRunsByteIdentical true nativeScope BoundedSnapshotMinimizedG1RetainedGlyphAllegrettoSystem3Order116PrelinkedClosure javaEvidence ReturnedBeforeOneHundredEighteenthHead"
} > "$out"
echo "wrote $out"
