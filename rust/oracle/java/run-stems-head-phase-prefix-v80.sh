...
# Read-only snapshot-minimized order80 continuation from v79; prior orders mutate without snapshots.
set -eu
...
tmp_base=/private/tmp/stems-head-phase-prefix-v80
...
awk -v fragment="$fragment18" '
{
    ...
    if (capture_after && index($0, "heads, allLinkers);") != 0) {
        print
        print "            if (headOrder == 7 || headOrder == 22 || headOrder == 27 || headOrder == 34 || headOrder == 36 || headOrder == 37 || headOrder == 38 || headOrder == 39 || headOrder == 40 || headOrder == 41 || headOrder == 42 || headOrder == 43 || headOrder == 44 || headOrder == 45 || headOrder == 46 || headOrder == 47 || headOrder == 48 || headOrder == 49 || headOrder == 50 || headOrder == 51 || headOrder == 52 || headOrder == 53 || headOrder == 54 || headOrder == 55 || headOrder == 56 || headOrder == 57 || headOrder == 58 || headOrder == 59 || headOrder == 60 || headOrder == 61 || headOrder == 62 || headOrder == 63 || headOrder == 64 || headOrder == 65 || headOrder == 66 || headOrder == 67 || headOrder == 68 || headOrder == 69 || headOrder == 70 || headOrder == 71 || headOrder == 72 || headOrder == 73 || headOrder == 74 || headOrder == 75 || headOrder == 76 || headOrder == 77 || headOrder == 78 || headOrder == 79 || headOrder == 80) emitHeadCLinkResult(before, after, headOrder);"
        capture_after = 0
        in_linker = 0
        next
    }
...
if [ "$(grep -c '^stemsheadphasecontinue ' "$rows")" -ne 1 ] || \
        ! grep -q 'headOrder 80 headX 29 headSig 66 headInterId 1421 .*decisions \[LEFT:SkipAlreadyLinked,RIGHT:SkipClosed\].*incident \[stem2357:headSideLEFT:heads\[x29:sig66:id1421:sideLEFT,x30:sig67:id1423:sideLEFT\].*closureWrites \[x30:sig67:LEFT:false->true,x30:sig67:RIGHT:false->true\].*closedValueChanges 2 .*nextHeadOrder 81 nextHeadX 98 nextHeadSig 60 nextHeadInterId 1409 ' "$rows" || \
        ! grep -q '^stemsheadclinkfrontier .*headOrder 80 headX 29 headSig 66 .*cAlias h:29:LEFT:BOTTOM .*lastIndex 1 maxIndex 1 relations 1 .*glyphs 2 .*candidateIdBefore 0 existingGlyph glyph:313 existingActive true existingStem 2357 ' "$rows" || \
        ! grep -q '^stemsheadclinkresult headOrder 80 allocatorBefore 2385 allocatorAfter 2385 .*addedVertices - addedEdges - addedSystemStems - .*linkerChanges \[h:30:LEFT:BOTTOM:true:false->true:true' "$rows"; then
    echo "v80 existing-stem reconciliation contract differs" >&2
    exit 1
fi
: <<'DISABLED_V27_ACTIVE'
...
v79_runner_sha=$(shasum -a 256 "$script_dir/run-stems-head-phase-prefix-v79.sh" | awk '{print $1}')
if [ "$v79_runner_sha" != "c76ece1975ddf35e64f0018355190346559e0bcd5329b5ef868bc3ac44a7ad32" ]; then
    echo "v80 base v79 runner drifted" >&2
    exit 1
fi
v79_fixture="$repo_root/rust/oracle/stems-head-phase-prefix-chula-system1-v79.txt"
v79_fixture_sha=$(shasum -a 256 "$v79_fixture" | awk '{print $1}')
if [ "$v79_fixture_sha" != "071cfe1bbed8ba76c8d29d3e93bc9dcb1df5eeb223df042ff17d0c4e3a8fbb8a" ]; then
    echo "v80 base v79 fixture drifted" >&2
    exit 1
fi
...
out="/private/tmp/stems-head-phase-prefix-chula-system1-v80-audit.txt"
{
    echo '# Java Audiveris 5.11 (Temurin JDK 25.0.3+9 LTS) post-STUMPS head phase v80.'
    echo '# schema: stems-head-phase-prefix-v80'
    echo '# Snapshot-minimized order-80 derivative: orders 1-79 mutate without snapshots; order 80 emits the existing-stem reconciliation.'
    echo '# This bounded scope is intentional for deterministic replay under the full-snapshot heap limit.'
    cat "$rows"
    printf '%s\n' \
        "stemsheadphaseprefix summary schema stems-head-phase-prefix-v80 page chula.png#1 system 1 rows $row_count baseProbeSourceSha256 $base_probe_sha baseV79RunnerSourceSha256 $v79_runner_sha baseV79FixtureSha256 $v79_fixture_sha fragmentSourceSha256 $fragment_sha probeSourceSha256 $probe_sha runnerSourceSha256 $runner_sha emittedBodySha256 $body_sha semanticPassSha256 $semantic_sha completeStumpsFixtureSha256 $stumps_sha freshRuns 2 freshRunsByteIdentical true nativeScope BoundedSnapshotMinimizedOrder80ExistingStemReconciliation javaEvidence ReturnedBeforeEightyFirstHead"
} > "$out"
echo "wrote $out"
