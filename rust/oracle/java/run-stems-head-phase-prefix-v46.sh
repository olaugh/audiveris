...
# Read-only snapshot-minimized order46 continuation from v45; prior orders mutate without snapshots.
set -eu
...
tmp_base=/private/tmp/stems-head-phase-prefix-v46
...
awk -v fragment="$fragment18" '
{
    ...
    if (index($0, "final boolean returned = linker.linkSides(") != 0) {
        print "            if (headOrder == 7 || headOrder == 22 || headOrder == 27 || headOrder == 34 || headOrder == 36 || headOrder == 37 || headOrder == 38 || headOrder == 39 || headOrder == 40 || headOrder == 41 || headOrder == 42 || headOrder == 43 || headOrder == 44 || headOrder == 45 || headOrder == 46) emitHeadCLinkEnvelope(head, linker, before, headOrder);"
        print
        in_linker = 1
        next
    }
    ...
    if (capture_after && index($0, "heads, allLinkers);") != 0) {
        print
        print "            if (headOrder == 7 || headOrder == 22 || headOrder == 27 || headOrder == 34 || headOrder == 36 || headOrder == 37 || headOrder == 38 || headOrder == 39 || headOrder == 40 || headOrder == 41 || headOrder == 42 || headOrder == 43 || headOrder == 44 || headOrder == 45 || headOrder == 46) emitHeadCLinkResult(before, after, headOrder);"
        capture_after = 0
        in_linker = 0
        next
    }
...
if [ "$(grep -c '^stemsheadphasecontinue ' "$rows")" -ne 1 ] || \
        ! grep -q 'headOrder 46 headX 40 headSig 27 headInterId 1339 .*decisions \[LEFT:SkipAlreadyLinked,RIGHT:SkipClosed\].*incident \[stem2350:headSideLEFT:heads\[x39:sig37:id1359:sideLEFT,x40:sig27:id1339:sideLEFT,x43:sig48:id1385:sideLEFT\].*closureWrites \[x39:sig37:LEFT:true->true,x39:sig37:RIGHT:true->true,x43:sig48:LEFT:false->true,x43:sig48:RIGHT:false->true\].*closedValueChanges 2 .*nextHeadOrder 47 nextHeadX 89 nextHeadSig 22 nextHeadInterId 1329 ' "$rows" || \
        ! grep -q '^stemsheadclinkfrontier .*headOrder 46 headX 40 headSig 27 .*cAlias h:40:LEFT:BOTTOM .*lastIndex 2 maxIndex 2 relations 2 .*glyphs 2 .*candidateIdBefore 0 existingGlyph glyph:326 existingActive true existingStem 2350 ' "$rows" || \
        ! grep -q '^stemsheadclinkresult headOrder 46 allocatorBefore 2385 allocatorAfter 2385 .*addedVertices - addedEdges - addedSystemStems - .*linkerChanges \[h:43:LEFT:BOTTOM:true:false->true:true' "$rows"; then
    echo "v46 existing-stem reconciliation contract differs" >&2
    exit 1
fi
: <<'DISABLED_V27_ACTIVE'
...
v45_runner_sha=$(shasum -a 256 "$script_dir/run-stems-head-phase-prefix-v45.sh" | awk '{print $1}')
if [ "$v45_runner_sha" != "ee0b1141ee872ac784c60e43062c7f8ae98e26730ef596d3fb9c110c520de728" ]; then
    echo "v46 base v45 runner drifted" >&2
    exit 1
fi
v45_fixture="$repo_root/rust/oracle/stems-head-phase-prefix-chula-system1-v45.txt"
v45_fixture_sha=$(shasum -a 256 "$v45_fixture" | awk '{print $1}')
if [ "$v45_fixture_sha" != "f70a5aeee405899ee2e9bf3be6957ffa657c6f0bcd5bc5d84ab0fc0288b19073" ]; then
    echo "v46 base v45 fixture drifted" >&2
    exit 1
fi
...
out="/private/tmp/stems-head-phase-prefix-chula-system1-v46-audit.txt"
{
    echo '# Java Audiveris 5.11 (Temurin JDK 25.0.3+9 LTS) post-STUMPS head phase v46.'
    echo '# schema: stems-head-phase-prefix-v46'
    echo '# Snapshot-minimized order-46 derivative: orders 1-45 mutate without snapshots; order 46 emits existing-stem reconciliation.'
    echo '# This bounded scope is intentional for deterministic replay under the full-snapshot heap limit.'
    cat "$rows"
    printf '%s\n' \
        "stemsheadphaseprefix summary schema stems-head-phase-prefix-v46 page chula.png#1 system 1 rows $row_count baseProbeSourceSha256 $base_probe_sha baseV45RunnerSourceSha256 $v45_runner_sha baseV45FixtureSha256 $v45_fixture_sha fragmentSourceSha256 $fragment_sha probeSourceSha256 $probe_sha runnerSourceSha256 $runner_sha emittedBodySha256 $body_sha semanticPassSha256 $semantic_sha completeStumpsFixtureSha256 $stumps_sha freshRuns 2 freshRunsByteIdentical true nativeScope BoundedSnapshotMinimizedOrder46ExistingStemReconciliation javaEvidence ReturnedBeforeFortySeventhHead"
} > "$out"
echo "wrote $out"
