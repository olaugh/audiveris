...
# Read-only snapshot-minimized order78 continuation from v77; prior orders mutate without snapshots.
set -eu
...
tmp_base=/private/tmp/stems-head-phase-prefix-v78
...
awk -v fragment="$fragment18" '
{
    ...
    if (capture_after && index($0, "heads, allLinkers);") != 0) {
        print
        print "            if (headOrder == 7 || headOrder == 22 || headOrder == 27 || headOrder == 34 || headOrder == 36 || headOrder == 37 || headOrder == 38 || headOrder == 39 || headOrder == 40 || headOrder == 41 || headOrder == 42 || headOrder == 43 || headOrder == 44 || headOrder == 45 || headOrder == 46 || headOrder == 47 || headOrder == 48 || headOrder == 49 || headOrder == 50 || headOrder == 51 || headOrder == 52 || headOrder == 53 || headOrder == 54 || headOrder == 55 || headOrder == 56 || headOrder == 57 || headOrder == 58 || headOrder == 59 || headOrder == 60 || headOrder == 61 || headOrder == 62 || headOrder == 63 || headOrder == 64 || headOrder == 65 || headOrder == 66 || headOrder == 67 || headOrder == 68 || headOrder == 69 || headOrder == 70 || headOrder == 71 || headOrder == 72 || headOrder == 73 || headOrder == 74 || headOrder == 75 || headOrder == 76 || headOrder == 77 || headOrder == 78) emitHeadCLinkResult(before, after, headOrder);"
        capture_after = 0
        in_linker = 0
        next
    }
...
if [ "$(grep -c '^stemsheadphasecontinue ' "$rows")" -ne 1 ] || \
        ! grep -q 'headOrder 78 headX 82 headSig 20 headInterId 1325 .*decisions \[LEFT:SkipAlreadyLinked,RIGHT:SkipClosed\].*incident \[stem2358:headSideLEFT:heads\[x82:sig20:id1325:sideLEFT,x83:sig21:id1327:sideLEFT\].*closureWrites \[x83:sig21:LEFT:false->true,x83:sig21:RIGHT:false->true\].*closedValueChanges 2 .*nextHeadOrder 79 nextHeadX 17 nextHeadSig 10 nextHeadInterId 1305 ' "$rows" || \
        ! grep -q '^stemsheadclinkfrontier .*headOrder 78 headX 82 headSig 20 .*cAlias h:82:LEFT:BOTTOM .*lastIndex 2 maxIndex 2 relations 2 .*glyphs 2 .*candidateIdBefore 0 existingGlyph glyph:301 existingActive true existingStem 2358 ' "$rows" || \
        ! grep -q '^stemsheadclinkresult headOrder 78 allocatorBefore 2385 allocatorAfter 2385 .*addedVertices - addedEdges - addedSystemStems - .*linkerChanges \[h:83:LEFT:BOTTOM:true:false->true:true' "$rows"; then
    echo "v78 existing-stem reconciliation contract differs" >&2
    exit 1
fi
: <<'DISABLED_V27_ACTIVE'
...
v77_runner_sha=$(shasum -a 256 "$script_dir/run-stems-head-phase-prefix-v77.sh" | awk '{print $1}')
if [ "$v77_runner_sha" != "5f4d3bed7d720e1025a85cacb6af2b8340cf154ae1f957c214b9cef392362095" ]; then
    echo "v78 base v77 runner drifted" >&2
    exit 1
fi
v77_fixture="$repo_root/rust/oracle/stems-head-phase-prefix-chula-system1-v77.txt"
v77_fixture_sha=$(shasum -a 256 "$v77_fixture" | awk '{print $1}')
if [ "$v77_fixture_sha" != "e1ab516af406562ef9465b5f63ef584ff3b021611255d6615f31dcc0ceb586e5" ]; then
    echo "v78 base v77 fixture drifted" >&2
    exit 1
fi
...
out="/private/tmp/stems-head-phase-prefix-chula-system1-v78-audit.txt"
{
    echo '# Java Audiveris 5.11 (Temurin JDK 25.0.3+9 LTS) post-STUMPS head phase v78.'
    echo '# schema: stems-head-phase-prefix-v78'
    echo '# Snapshot-minimized order-78 derivative: orders 1-77 mutate without snapshots; order 78 emits the existing-stem reconciliation.'
    echo '# This bounded scope is intentional for deterministic replay under the full-snapshot heap limit.'
    cat "$rows"
    printf '%s\n' \
        "stemsheadphaseprefix summary schema stems-head-phase-prefix-v78 page chula.png#1 system 1 rows $row_count baseProbeSourceSha256 $base_probe_sha baseV77RunnerSourceSha256 $v77_runner_sha baseV77FixtureSha256 $v77_fixture_sha fragmentSourceSha256 $fragment_sha probeSourceSha256 $probe_sha runnerSourceSha256 $runner_sha emittedBodySha256 $body_sha semanticPassSha256 $semantic_sha completeStumpsFixtureSha256 $stumps_sha freshRuns 2 freshRunsByteIdentical true nativeScope BoundedSnapshotMinimizedOrder78ExistingStemReconciliation javaEvidence ReturnedBeforeSeventyNinthHead"
} > "$out"
echo "wrote $out"
