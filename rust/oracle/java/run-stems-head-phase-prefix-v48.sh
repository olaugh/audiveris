...
# Read-only snapshot-minimized order48 continuation from v47; prior orders mutate without snapshots.
set -eu
...
tmp_base=/private/tmp/stems-head-phase-prefix-v48
...
awk -v fragment="$fragment18" '
{
    ...
    if (index($0, "final boolean returned = linker.linkSides(") != 0) {
        print "            if (headOrder == 7 || headOrder == 22 || headOrder == 27 || headOrder == 34 || headOrder == 36 || headOrder == 37 || headOrder == 38 || headOrder == 39 || headOrder == 40 || headOrder == 41 || headOrder == 42 || headOrder == 43 || headOrder == 44 || headOrder == 45 || headOrder == 46 || headOrder == 47 || headOrder == 48) emitHeadCLinkEnvelope(head, linker, before, headOrder);"
        print
        in_linker = 1
        next
    }
    ...
    if (capture_after && index($0, "heads, allLinkers);") != 0) {
        print
        print "            if (headOrder == 7 || headOrder == 22 || headOrder == 27 || headOrder == 34 || headOrder == 36 || headOrder == 37 || headOrder == 38 || headOrder == 39 || headOrder == 40 || headOrder == 41 || headOrder == 42 || headOrder == 43 || headOrder == 44 || headOrder == 45 || headOrder == 46 || headOrder == 47 || headOrder == 48) emitHeadCLinkResult(before, after, headOrder);"
        capture_after = 0
        in_linker = 0
        next
    }
...
if [ "$(grep -c '^stemsheadphasecontinue ' "$rows")" -ne 1 ] || \
        ! grep -q 'headOrder 48 headX 52 headSig 2 headInterId 1289 .*decisions \[LEFT:SkipAlreadyLinked,RIGHT:SkipClosed\].*incident \[stem2344:headSideLEFT:heads\[x52:sig2:id1289:sideLEFT,x53:sig3:id1291:sideLEFT\].*closureWrites \[x53:sig3:LEFT:false->true,x53:sig3:RIGHT:false->true\].*closedValueChanges 2 .*nextHeadOrder 49 nextHeadX 35 nextHeadSig 68 nextHeadInterId 1425 ' "$rows" || \
        ! grep -q '^stemsheadclinkfrontier .*headOrder 48 headX 52 headSig 2 .*cAlias h:52:LEFT:BOTTOM .*lastIndex 3 maxIndex 3 relations 4 .*glyphs 1 .*candidateIdBefore 296 existingGlyph glyph:296 existingActive true existingStem 2344 ' "$rows" || \
        ! grep -q '^stemsheadclinkresult headOrder 48 allocatorBefore 2385 allocatorAfter 2385 .*addedVertices - addedEdges - addedSystemStems - .*linkerChanges \[h:53:LEFT:BOTTOM:true:false->true:true' "$rows"; then
    echo "v48 existing-stem reconciliation contract differs" >&2
    exit 1
fi
: <<'DISABLED_V27_ACTIVE'
...
v47_runner_sha=$(shasum -a 256 "$script_dir/run-stems-head-phase-prefix-v47.sh" | awk '{print $1}')
if [ "$v47_runner_sha" != "7a9605cf09f1d78f899423a816c0c6adc2b121786f56c69c271b41da5527f6ab" ]; then
    echo "v48 base v47 runner drifted" >&2
    exit 1
fi
v47_fixture="$repo_root/rust/oracle/stems-head-phase-prefix-chula-system1-v47.txt"
v47_fixture_sha=$(shasum -a 256 "$v47_fixture" | awk '{print $1}')
if [ "$v47_fixture_sha" != "5a7989434b78dbd6ea72f113cd9f66078ae8e9c3acabb8980ecdb7577120de39" ]; then
    echo "v48 base v47 fixture drifted" >&2
    exit 1
fi
...
out="/private/tmp/stems-head-phase-prefix-chula-system1-v48-audit.txt"
{
    echo '# Java Audiveris 5.11 (Temurin JDK 25.0.3+9 LTS) post-STUMPS head phase v48.'
    echo '# schema: stems-head-phase-prefix-v48'
    echo '# Snapshot-minimized order-48 derivative: orders 1-47 mutate without snapshots; order 48 emits existing-stem reconciliation.'
    echo '# This bounded scope is intentional for deterministic replay under the full-snapshot heap limit.'
    cat "$rows"
    printf '%s\n' \
        "stemsheadphaseprefix summary schema stems-head-phase-prefix-v48 page chula.png#1 system 1 rows $row_count baseProbeSourceSha256 $base_probe_sha baseV47RunnerSourceSha256 $v47_runner_sha baseV47FixtureSha256 $v47_fixture_sha fragmentSourceSha256 $fragment_sha probeSourceSha256 $probe_sha runnerSourceSha256 $runner_sha emittedBodySha256 $body_sha semanticPassSha256 $semantic_sha completeStumpsFixtureSha256 $stumps_sha freshRuns 2 freshRunsByteIdentical true nativeScope BoundedSnapshotMinimizedOrder48ExistingStemReconciliation javaEvidence ReturnedBeforeFortyNinthHead"
} > "$out"
echo "wrote $out"
