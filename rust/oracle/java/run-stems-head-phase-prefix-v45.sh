...
# Read-only snapshot-minimized order45 continuation from v44; prior orders mutate without snapshots.
set -eu
...
tmp_base=/private/tmp/stems-head-phase-prefix-v45
...
awk -v fragment="$fragment18" '
{
    ...
    if (index($0, "final boolean returned = linker.linkSides(") != 0) {
        print "            if (headOrder == 7 || headOrder == 22 || headOrder == 27 || headOrder == 34 || headOrder == 36 || headOrder == 37 || headOrder == 38 || headOrder == 39 || headOrder == 40 || headOrder == 41 || headOrder == 42 || headOrder == 43 || headOrder == 44 || headOrder == 45) emitHeadCLinkEnvelope(head, linker, before, headOrder);"
        print
        in_linker = 1
        next
    }
    ...
    if (capture_after && index($0, "heads, allLinkers);") != 0) {
        print
        print "            if (headOrder == 7 || headOrder == 22 || headOrder == 27 || headOrder == 34 || headOrder == 36 || headOrder == 37 || headOrder == 38 || headOrder == 39 || headOrder == 40 || headOrder == 41 || headOrder == 42 || headOrder == 43 || headOrder == 44 || headOrder == 45) emitHeadCLinkResult(before, after, headOrder);"
        capture_after = 0
        in_linker = 0
        next
    }
...
if [ "$(grep -c '^stemsheadphasecontinue ' "$rows")" -ne 1 ] || \
        ! grep -q 'headOrder 45 headX 57 headSig 5 headInterId 1295 .*decisions \[LEFT:SkipAlreadyLinked,RIGHT:top=false:bottom=false:branch=Neither\].*incident \[stem2374:headSideLEFT:heads\[x56:sig15:id1315:sideLEFT,x57:sig5:id1295:sideLEFT\].*closureWrites \[x56:sig15:LEFT:false->true,x56:sig15:RIGHT:false->true\].*closedValueChanges 2 .*nextHeadOrder 46 nextHeadX 40 nextHeadSig 27 nextHeadInterId 1339 ' "$rows" || \
        ! grep -q '^stemsheadclinkfrontier .*headOrder 45 headX 57 headSig 5 .*cAlias h:57:LEFT:BOTTOM .*lastIndex 2 maxIndex 2 relations 2 .*glyphs 2 .*candidateIdBefore 0 existingGlyph glyph:303 existingActive true existingStem 2374 ' "$rows" || \
        ! grep -q '^stemsheadclinkresult headOrder 45 allocatorBefore 2385 allocatorAfter 2385 .*addedVertices - addedEdges - addedSystemStems - .*linkerChanges \[h:56:LEFT:BOTTOM:true:false->true:true' "$rows"; then
    echo "v45 existing-stem reconciliation contract differs" >&2
    exit 1
fi
: <<'DISABLED_V27_ACTIVE'
...
v44_runner_sha=$(shasum -a 256 "$script_dir/run-stems-head-phase-prefix-v44.sh" | awk '{print $1}')
if [ "$v44_runner_sha" != "ee0fb2771acf9693f47814e4abe1de1d7e6434a748178fad32ea823d5e3797d7" ]; then
    echo "v45 base v44 runner drifted" >&2
    exit 1
fi
v44_fixture="$repo_root/rust/oracle/stems-head-phase-prefix-chula-system1-v44.txt"
v44_fixture_sha=$(shasum -a 256 "$v44_fixture" | awk '{print $1}')
if [ "$v44_fixture_sha" != "1d5c98477377e64e95a659fa04ed8d8331e02d5e87962811b790ff80f0315515" ]; then
    echo "v45 base v44 fixture drifted" >&2
    exit 1
fi
...
out="/private/tmp/stems-head-phase-prefix-chula-system1-v45-audit.txt"
{
    echo '# Java Audiveris 5.11 (Temurin JDK 25.0.3+9 LTS) post-STUMPS head phase v45.'
    echo '# schema: stems-head-phase-prefix-v45'
    echo '# Snapshot-minimized order-45 derivative: orders 1-44 mutate without snapshots; order 45 emits existing-stem reconciliation.'
    echo '# This bounded scope is intentional for deterministic replay under the full-snapshot heap limit.'
    cat "$rows"
    printf '%s\n' \
        "stemsheadphaseprefix summary schema stems-head-phase-prefix-v45 page chula.png#1 system 1 rows $row_count baseProbeSourceSha256 $base_probe_sha baseV44RunnerSourceSha256 $v44_runner_sha baseV44FixtureSha256 $v44_fixture_sha fragmentSourceSha256 $fragment_sha probeSourceSha256 $probe_sha runnerSourceSha256 $runner_sha emittedBodySha256 $body_sha semanticPassSha256 $semantic_sha completeStumpsFixtureSha256 $stumps_sha freshRuns 2 freshRunsByteIdentical true nativeScope BoundedSnapshotMinimizedOrder45ExistingStemReconciliation javaEvidence ReturnedBeforeFortySixthHead"
} > "$out"
echo "wrote $out"
