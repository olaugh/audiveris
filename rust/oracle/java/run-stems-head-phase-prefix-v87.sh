...
# Read-only snapshot-minimized order87 continuation from v86; prior orders mutate without snapshots.
set -eu
...
tmp_base=/private/tmp/stems-head-phase-prefix-v87
...
awk -v fragment="$fragment18" '
{
    ...
    if (capture_after && index($0, "heads, allLinkers);") != 0) {
        print
        print "            if (headOrder == 7 || headOrder == 22 || headOrder == 27 || headOrder == 34 || headOrder == 36 || headOrder == 37 || headOrder == 38 || headOrder == 39 || headOrder == 40 || headOrder == 41 || headOrder == 42 || headOrder == 43 || headOrder == 44 || headOrder == 45 || headOrder == 46 || headOrder == 47 || headOrder == 48 || headOrder == 49 || headOrder == 50 || headOrder == 51 || headOrder == 52 || headOrder == 53 || headOrder == 54 || headOrder == 55 || headOrder == 56 || headOrder == 57 || headOrder == 58 || headOrder == 59 || headOrder == 60 || headOrder == 61 || headOrder == 62 || headOrder == 63 || headOrder == 64 || headOrder == 65 || headOrder == 66 || headOrder == 67 || headOrder == 68 || headOrder == 69 || headOrder == 70 || headOrder == 71 || headOrder == 72 || headOrder == 73 || headOrder == 74 || headOrder == 75 || headOrder == 76 || headOrder == 77 || headOrder == 78 || headOrder == 79 || headOrder == 80 || headOrder == 81 || headOrder == 82 || headOrder == 83 || headOrder == 84 || headOrder == 85 || headOrder == 86 || headOrder == 87) emitHeadCLinkResult(before, after, headOrder);"
        capture_after = 0
        in_linker = 0
        next
    }
...
if [ "$(grep -c '^stemsheadphasecontinue ' "$rows")" -ne 1 ] || \
        ! grep -q 'headOrder 87 headX 45 headSig 56 headInterId 1401 .*decisions \[LEFT:SkipAlreadyLinked,RIGHT:SkipClosed\].*incident \[stem2377:headSideLEFT:heads\[x44:sig70:id1429:sideLEFT,x45:sig56:id1401:sideLEFT,x46:sig57:id1403:sideLEFT\].*closureWrites \[x44:sig70:LEFT:true->true,x44:sig70:RIGHT:true->true,x46:sig57:LEFT:false->true,x46:sig57:RIGHT:false->true\].*closedValueChanges 2 .*nextHeadOrder 88 nextHeadX 72 nextHeadSig 101 nextHeadInterId 1491 ' "$rows" || \
        ! grep -q '^stemsheadclinkfrontier .*headOrder 87 headX 45 headSig 56 .*cAlias h:45:LEFT:BOTTOM .*lastIndex 1 maxIndex 1 relations 2 .*glyphs 1 .*candidateIdBefore 302 existingGlyph glyph:302 existingActive true existingStem 2377 ' "$rows" || \
        ! grep -q '^stemsheadclinkresult headOrder 87 allocatorBefore 2385 allocatorAfter 2385 .*addedVertices - addedEdges - addedSystemStems - .*linkerChanges \[h:46:LEFT:BOTTOM:true:false->true:true' "$rows"; then
    echo "v87 three-head existing-stem reconciliation contract differs" >&2
    exit 1
fi
: <<'DISABLED_V27_ACTIVE'
...
v86_runner_sha=$(shasum -a 256 "$script_dir/run-stems-head-phase-prefix-v86.sh" | awk '{print $1}')
if [ "$v86_runner_sha" != "724e45d8a40b4eb17284004048d9c5c49c9242c15ff70e72c55837dbd05e46fa" ]; then
    echo "v87 base v86 runner drifted" >&2
    exit 1
fi
v86_fixture="$repo_root/rust/oracle/stems-head-phase-prefix-chula-system1-v86.txt"
v86_fixture_sha=$(shasum -a 256 "$v86_fixture" | awk '{print $1}')
if [ "$v86_fixture_sha" != "9d2a075961321850ffb365466b5d0d9fed9f447a31bf7dc9f83e3571b0b69097" ]; then
    echo "v87 base v86 fixture drifted" >&2
    exit 1
fi
...
out="/private/tmp/stems-head-phase-prefix-chula-system1-v87-audit.txt"
{
    echo '# Java Audiveris 5.11 (Temurin JDK 25.0.3+9 LTS) post-STUMPS head phase v87.'
    echo '# schema: stems-head-phase-prefix-v87'
    echo '# Snapshot-minimized order-87 derivative: orders 1-86 mutate without snapshots; order 87 emits the three-head existing-stem reconciliation.'
    echo '# This bounded scope is intentional for deterministic replay under the full-snapshot heap limit.'
    cat "$rows"
    printf '%s\n' \
        "stemsheadphaseprefix summary schema stems-head-phase-prefix-v87 page chula.png#1 system 1 rows $row_count baseProbeSourceSha256 $base_probe_sha baseV86RunnerSourceSha256 $v86_runner_sha baseV86FixtureSha256 $v86_fixture_sha fragmentSourceSha256 $fragment_sha probeSourceSha256 $probe_sha runnerSourceSha256 $runner_sha emittedBodySha256 $body_sha semanticPassSha256 $semantic_sha completeStumpsFixtureSha256 $stumps_sha freshRuns 2 freshRunsByteIdentical true nativeScope BoundedSnapshotMinimizedOrder87ExistingStemReconciliation javaEvidence ReturnedBeforeEightyEighthHead"
} > "$out"
echo "wrote $out"
