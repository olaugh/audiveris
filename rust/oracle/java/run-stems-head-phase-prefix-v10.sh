...
# Freeze the v9 head prefix plus the next order-10 prelinked continuation.
set -eu
...
tmp_base=/private/tmp/stems-head-phase-prefix-v10
...
if ! cmp -s "$semantic_one" "$semantic_two"; then
    echo "two fresh post-STUMPS v10 semantic passes are not byte-identical" >&2
...
if [ "$(wc -l < "$rows" | tr -d ' ')" -ne 18 ] || \
        [ "$(grep -c '^stemsheadphasecontinue ' "$rows")" -ne 10 ] || \
...
v9_runner_sha=$(shasum -a 256 "$script_dir/run-stems-head-phase-prefix-v9.sh" | awk '{print $1}')
if [ "$v9_runner_sha" != "368724efe73e194aff024d68204d758d089d81511e9bbfaa4dfb9ef9516f4c48" ]; then
    echo "v10 base v9 runner drifted" >&2
    exit 1
fi
v9_fixture="$repo_root/rust/oracle/stems-head-phase-prefix-chula-system1-v9.txt"
v9_fixture_sha=$(shasum -a 256 "$v9_fixture" | awk '{print $1}')
if [ "$v9_fixture_sha" != "b0d3c67f9b76a56a528d8a962f3f2bc54710616f2e86650ac8500e750534ff2c" ]; then
    echo "v10 base v9 fixture drifted" >&2
    exit 1
fi
...
out="$repo_root/rust/oracle/stems-head-phase-prefix-chula-system1-v10.txt"
{
    echo '# Java Audiveris 5.11 (Temurin JDK 25.0.3+9 LTS) post-STUMPS head phase v10.'
    echo '# schema: stems-head-phase-prefix-v10'
    echo '# v9 head continuation plus the next real order-10 phase-1 linkSides call.'
    echo '# Expected rows stay unread until the native transaction returns.'
    cat "$rows"
    printf '%s\n' \
        "stemsheadphaseprefix summary schema stems-head-phase-prefix-v10 page chula.png#1 system 1 rows 18 baseProbeSourceSha256 $base_probe_sha baseV9RunnerSourceSha256 $v9_runner_sha baseV9FixtureSha256 $v9_fixture_sha probeSourceSha256 $probe_sha runnerSourceSha256 $runner_sha emittedBodySha256 $body_sha semanticPassSha256 $semantic_sha completeStumpsFixtureSha256 $stumps_sha freshRuns 2 freshRunsByteIdentical true nativeScope ReturnedOrder10HeadContinuation javaEvidence ReturnedBeforeTwelfthHead"
} > "$out"
echo "wrote $out"
