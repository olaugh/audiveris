...
# Read-only snapshot-minimized order53 continuation from v52; prior orders mutate without snapshots.
set -eu
...
tmp_base=/private/tmp/stems-head-phase-prefix-v53
...
awk -v fragment="$fragment18" '
{
    ...
    if (index($0, "final boolean returned = linker.linkSides(") != 0) {
        print "            if (headOrder == 7 || headOrder == 22 || headOrder == 27 || headOrder == 34 || headOrder == 36 || headOrder == 37 || headOrder == 38 || headOrder == 39 || headOrder == 40 || headOrder == 41 || headOrder == 42 || headOrder == 43 || headOrder == 44 || headOrder == 45 || headOrder == 46 || headOrder == 47 || headOrder == 48 || headOrder == 49 || headOrder == 50 || headOrder == 51 || headOrder == 52 || headOrder == 53) emitHeadCLinkEnvelope(head, linker, before, headOrder);"
        print
        in_linker = 1
        next
    }
    ...
    if (capture_after && index($0, "heads, allLinkers);") != 0) {
        print
        print "            if (headOrder == 7 || headOrder == 22 || headOrder == 27 || headOrder == 34 || headOrder == 36 || headOrder == 37 || headOrder == 38 || headOrder == 39 || headOrder == 40 || headOrder == 41 || headOrder == 42 || headOrder == 43 || headOrder == 44 || headOrder == 45 || headOrder == 46 || headOrder == 47 || headOrder == 48 || headOrder == 49 || headOrder == 50 || headOrder == 51 || headOrder == 52 || headOrder == 53) emitHeadCLinkResult(before, after, headOrder);"
        capture_after = 0
        in_linker = 0
        next
    }
...
if [ "$(grep -c '^stemsheadphasecontinue ' "$rows")" -ne 1 ] || \
        ! grep -q 'headOrder 53 headX 84 headSig 86 headInterId 1461 .*decisions \[LEFT:SkipAlreadyLinked,RIGHT:SkipClosed\].*incident \[stem2366:headSideLEFT:heads\[x84:sig86:id1461:sideLEFT,x85:sig87:id1463:sideLEFT,x86:sig85:id1459:sideLEFT\].*closureWrites \[x85:sig87:LEFT:false->true,x85:sig87:RIGHT:false->true,x86:sig85:LEFT:true->true,x86:sig85:RIGHT:true->true\].*closedValueChanges 2 .*nextHeadOrder 54 nextHeadX 11 nextHeadSig 62 nextHeadInterId 1413 ' "$rows" || \
        ! grep -q '^stemsheadclinkfrontier .*headOrder 53 headX 84 headSig 86 .*cAlias h:84:LEFT:BOTTOM .*lastIndex 1 maxIndex 1 relations 2 .*glyphs 1 .*candidateIdBefore 320 existingGlyph glyph:320 existingActive true existingStem 2366 ' "$rows" || \
        ! grep -q '^stemsheadclinkresult headOrder 53 allocatorBefore 2385 allocatorAfter 2385 .*addedVertices - addedEdges - addedSystemStems - .*linkerChanges \[h:85:LEFT:BOTTOM:true:false->true:true' "$rows"; then
    echo "v53 existing-stem reconciliation contract differs" >&2
    exit 1
fi
: <<'DISABLED_V27_ACTIVE'
...
v52_runner_sha=$(shasum -a 256 "$script_dir/run-stems-head-phase-prefix-v52.sh" | awk '{print $1}')
if [ "$v52_runner_sha" != "c314a1da865f91ce57128468a77dff85b7dd20719427c9119ca29057331728a6" ]; then
    echo "v53 base v52 runner drifted" >&2
    exit 1
fi
v52_fixture="$repo_root/rust/oracle/stems-head-phase-prefix-chula-system1-v52.txt"
v52_fixture_sha=$(shasum -a 256 "$v52_fixture" | awk '{print $1}')
if [ "$v52_fixture_sha" != "ff3bbe3fdf9ba0e6140b8105ab46e4c65972d4f4f013d39f90eade2270b64224" ]; then
    echo "v53 base v52 fixture drifted" >&2
    exit 1
fi
...
out="/private/tmp/stems-head-phase-prefix-chula-system1-v53-audit.txt"
{
    echo '# Java Audiveris 5.11 (Temurin JDK 25.0.3+9 LTS) post-STUMPS head phase v53.'
    echo '# schema: stems-head-phase-prefix-v53'
    echo '# Snapshot-minimized order-53 derivative: orders 1-52 mutate without snapshots; order 53 emits existing-stem reconciliation.'
    echo '# This bounded scope is intentional for deterministic replay under the full-snapshot heap limit.'
    cat "$rows"
    printf '%s\n' \
        "stemsheadphaseprefix summary schema stems-head-phase-prefix-v53 page chula.png#1 system 1 rows $row_count baseProbeSourceSha256 $base_probe_sha baseV52RunnerSourceSha256 $v52_runner_sha baseV52FixtureSha256 $v52_fixture_sha fragmentSourceSha256 $fragment_sha probeSourceSha256 $probe_sha runnerSourceSha256 $runner_sha emittedBodySha256 $body_sha semanticPassSha256 $semantic_sha completeStumpsFixtureSha256 $stumps_sha freshRuns 2 freshRunsByteIdentical true nativeScope BoundedSnapshotMinimizedOrder53ExistingStemReconciliation javaEvidence ReturnedBeforeFiftyFourthHead"
} > "$out"
echo "wrote $out"
