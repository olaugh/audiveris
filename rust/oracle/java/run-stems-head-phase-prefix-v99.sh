...
# Read-only snapshot-minimized order99 continuation from v98; prior orders mutate without snapshots.
set -eu
...
tmp_base=/private/tmp/stems-head-phase-prefix-v99
...
awk -v fragment="$fragment18" '
{
    ...
    if (capture_after && index($0, "heads, allLinkers);") != 0) {
        print
        print "            if (headOrder == 7 || headOrder == 22 || headOrder == 27 || headOrder == 34 || headOrder == 36 || headOrder == 37 || headOrder == 38 || headOrder == 39 || headOrder == 40 || headOrder == 41 || headOrder == 42 || headOrder == 43 || headOrder == 44 || headOrder == 45 || headOrder == 46 || headOrder == 47 || headOrder == 48 || headOrder == 49 || headOrder == 50 || headOrder == 51 || headOrder == 52 || headOrder == 53 || headOrder == 54 || headOrder == 55 || headOrder == 56 || headOrder == 57 || headOrder == 58 || headOrder == 59 || headOrder == 60 || headOrder == 61 || headOrder == 62 || headOrder == 63 || headOrder == 64 || headOrder == 65 || headOrder == 66 || headOrder == 67 || headOrder == 68 || headOrder == 69 || headOrder == 70 || headOrder == 71 || headOrder == 72 || headOrder == 73 || headOrder == 74 || headOrder == 75 || headOrder == 76 || headOrder == 77 || headOrder == 78 || headOrder == 79 || headOrder == 80 || headOrder == 81 || headOrder == 82 || headOrder == 83 || headOrder == 84 || headOrder == 85 || headOrder == 86 || headOrder == 87 || headOrder == 88 || headOrder == 89 || headOrder == 90 || headOrder == 91 || headOrder == 92 || headOrder == 93 || headOrder == 94 || headOrder == 95 || headOrder == 96 || headOrder == 97 || headOrder == 98 || headOrder == 99) emitHeadCLinkResult(before, after, headOrder);"
        capture_after = 0
        in_linker = 0
        next
    }
...
if [ "$(grep -c '^stemsheadphasecontinue ' "$rows")" -ne 1 ] || \
        ! grep -q 'headOrder 99 headX 56 headSig 15 headInterId 1315 .*decisions \[LEFT:SkipAlreadyLinked,RIGHT:SkipClosed\].*incident \[stem2374:headSideLEFT:heads\[x56:sig15:id1315:sideLEFT,x57:sig5:id1295:sideLEFT\].*closureWrites \[x57:sig5:LEFT:false->true,x57:sig5:RIGHT:false->true\].*closedValueChanges 2 .*nextHeadOrder 100 nextHeadX 86 nextHeadSig 85 nextHeadInterId 1459 ' "$rows" || \
        ! grep -q '^stemsheadclinkfrontier .*headOrder 99 headX 56 headSig 15 .*cAlias h:56:LEFT:BOTTOM .*lastIndex 2 maxIndex 2 relations 2 .*glyphs 2 .*candidateIdBefore 0 existingGlyph glyph:303 existingActive true existingStem 2374 ' "$rows" || \
        ! grep -q '^stemsheadclinkresult headOrder 99 allocatorBefore 2385 allocatorAfter 2385 .*addedVertices - addedEdges - addedSystemStems - .*linkerChanges \[h:57:LEFT:BOTTOM:true:false->true:true' "$rows"; then
    echo "v99 existing-stem reconciliation contract differs" >&2
    exit 1
fi
: <<'DISABLED_V27_ACTIVE'
...
v98_runner_sha=$(shasum -a 256 "$script_dir/run-stems-head-phase-prefix-v98.sh" | awk '{print $1}')
if [ "$v98_runner_sha" != "129ba2436311cf43fe3d8feb6f53fd7e1d19775e8ccde8971af55c2006fba3f2" ]; then
    echo "v99 base v98 runner drifted" >&2
    exit 1
fi
v98_fixture="$repo_root/rust/oracle/stems-head-phase-prefix-chula-system1-v98.txt"
v98_fixture_sha=$(shasum -a 256 "$v98_fixture" | awk '{print $1}')
if [ "$v98_fixture_sha" != "9f8070083b564d5c480ecb90026efbd95b76452acbf14c492e5da7c9438b336e" ]; then
    echo "v99 base v98 fixture drifted" >&2
    exit 1
fi
...
out="/private/tmp/stems-head-phase-prefix-chula-system1-v99-audit.txt"
{
    echo '# Java Audiveris 5.11 (Temurin JDK 25.0.3+9 LTS) post-STUMPS head phase v99.'
    echo '# schema: stems-head-phase-prefix-v99'
    echo '# Snapshot-minimized order-99 derivative: orders 1-98 mutate without snapshots; order 99 emits the existing-stem reconciliation.'
    echo '# This bounded scope is intentional for deterministic replay under the full-snapshot heap limit.'
    cat "$rows"
    printf '%s\n' \
        "stemsheadphaseprefix summary schema stems-head-phase-prefix-v99 page chula.png#1 system 1 rows $row_count baseProbeSourceSha256 $base_probe_sha baseV98RunnerSourceSha256 $v98_runner_sha baseV98FixtureSha256 $v98_fixture_sha fragmentSourceSha256 $fragment_sha probeSourceSha256 $probe_sha runnerSourceSha256 $runner_sha emittedBodySha256 $body_sha semanticPassSha256 $semantic_sha completeStumpsFixtureSha256 $stumps_sha freshRuns 2 freshRunsByteIdentical true nativeScope BoundedSnapshotMinimizedOrder99ExistingStemReconciliation javaEvidence ReturnedBeforeHundredthHead"
} > "$out"
echo "wrote $out"
