...
# Read-only snapshot-minimized order96 continuation from v95; prior orders mutate without snapshots.
set -eu
...
tmp_base=/private/tmp/stems-head-phase-prefix-v96
...
awk -v fragment="$fragment18" '
{
    ...
    if (capture_after && index($0, "heads, allLinkers);") != 0) {
        print
        print "            if (headOrder == 7 || headOrder == 22 || headOrder == 27 || headOrder == 34 || headOrder == 36 || headOrder == 37 || headOrder == 38 || headOrder == 39 || headOrder == 40 || headOrder == 41 || headOrder == 42 || headOrder == 43 || headOrder == 44 || headOrder == 45 || headOrder == 46 || headOrder == 47 || headOrder == 48 || headOrder == 49 || headOrder == 50 || headOrder == 51 || headOrder == 52 || headOrder == 53 || headOrder == 54 || headOrder == 55 || headOrder == 56 || headOrder == 57 || headOrder == 58 || headOrder == 59 || headOrder == 60 || headOrder == 61 || headOrder == 62 || headOrder == 63 || headOrder == 64 || headOrder == 65 || headOrder == 66 || headOrder == 67 || headOrder == 68 || headOrder == 69 || headOrder == 70 || headOrder == 71 || headOrder == 72 || headOrder == 73 || headOrder == 74 || headOrder == 75 || headOrder == 76 || headOrder == 77 || headOrder == 78 || headOrder == 79 || headOrder == 80 || headOrder == 81 || headOrder == 82 || headOrder == 83 || headOrder == 84 || headOrder == 85 || headOrder == 86 || headOrder == 87 || headOrder == 88 || headOrder == 89 || headOrder == 90 || headOrder == 91 || headOrder == 92 || headOrder == 93 || headOrder == 94 || headOrder == 95 || headOrder == 96) emitHeadCLinkResult(before, after, headOrder);"
        capture_after = 0
        in_linker = 0
        next
    }
...
if [ "$(grep -c '^stemsheadphasecontinue ' "$rows")" -ne 1 ] || \
        ! grep -q 'headOrder 96 headX 60 headSig 30 headInterId 1345 .*decisions \[LEFT:SkipAlreadyLinked,RIGHT:SkipClosed\].*incident \[stem2345:headSideLEFT:heads\[x60:sig30:id1345:sideLEFT,x61:sig31:id1347:sideLEFT\].*closureWrites \[x61:sig31:LEFT:false->true,x61:sig31:RIGHT:false->true\].*closedValueChanges 2 .*nextHeadOrder 97 nextHeadX 44 nextHeadSig 70 nextHeadInterId 1429 ' "$rows" || \
        ! grep -q '^stemsheadclinkfrontier .*headOrder 96 headX 60 headSig 30 .*cAlias h:60:LEFT:BOTTOM .*lastIndex 1 maxIndex 1 relations 1 .*glyphs 2 .*candidateIdBefore 0 existingGlyph glyph:335 existingActive true existingStem 2345 ' "$rows" || \
        ! grep -q '^stemsheadclinkresult headOrder 96 allocatorBefore 2385 allocatorAfter 2385 .*addedVertices - addedEdges - addedSystemStems - .*linkerChanges \[h:61:LEFT:BOTTOM:true:false->true:true' "$rows"; then
    echo "v96 existing-stem reconciliation contract differs" >&2
    exit 1
fi
: <<'DISABLED_V27_ACTIVE'
...
v95_runner_sha=$(shasum -a 256 "$script_dir/run-stems-head-phase-prefix-v95.sh" | awk '{print $1}')
if [ "$v95_runner_sha" != "25939a63c83a8a31fa981e808c10dbdcd4fb695337aa17aa52b96205a2821cd7" ]; then
    echo "v96 base v95 runner drifted" >&2
    exit 1
fi
v95_fixture="$repo_root/rust/oracle/stems-head-phase-prefix-chula-system1-v95.txt"
v95_fixture_sha=$(shasum -a 256 "$v95_fixture" | awk '{print $1}')
if [ "$v95_fixture_sha" != "004659ece99b047c1b433226fd9a64cfcfd69e2caf301f3f1c5dc4edf38a50b8" ]; then
    echo "v96 base v95 fixture drifted" >&2
    exit 1
fi
...
out="/private/tmp/stems-head-phase-prefix-chula-system1-v96-audit.txt"
{
    echo '# Java Audiveris 5.11 (Temurin JDK 25.0.3+9 LTS) post-STUMPS head phase v96.'
    echo '# schema: stems-head-phase-prefix-v96'
    echo '# Snapshot-minimized order-96 derivative: orders 1-95 mutate without snapshots; order 96 emits the existing-stem reconciliation.'
    echo '# This bounded scope is intentional for deterministic replay under the full-snapshot heap limit.'
    cat "$rows"
    printf '%s\n' \
        "stemsheadphaseprefix summary schema stems-head-phase-prefix-v96 page chula.png#1 system 1 rows $row_count baseProbeSourceSha256 $base_probe_sha baseV95RunnerSourceSha256 $v95_runner_sha baseV95FixtureSha256 $v95_fixture_sha fragmentSourceSha256 $fragment_sha probeSourceSha256 $probe_sha runnerSourceSha256 $runner_sha emittedBodySha256 $body_sha semanticPassSha256 $semantic_sha completeStumpsFixtureSha256 $stumps_sha freshRuns 2 freshRunsByteIdentical true nativeScope BoundedSnapshotMinimizedOrder96ExistingStemReconciliation javaEvidence ReturnedBeforeNinetySeventhHead"
} > "$out"
echo "wrote $out"
