...
# Deterministic Allegretto system-3 HEADS queue-53 multi-side and queue-115 no-link replay.
set -eu
...
tmp_dir=$(mktemp -d /private/tmp/stems-head-phase-allegretto-system3-order115.XXXXXX)
...
awk -v fragment="$fragment" '
{
    ...
    if (in_continuation && index($0, "final HeadLinker linker = head.getLinker();") != 0) {
        print
        print "            if (headOrder < 115 && headOrder != 53) {"
...
if ! grep -q '^stemsheadclinkfrontier allegretto.png#1 system 3 headOrder 53 headX 107 headSig 80 headInterId 1836 cAlias h:107:RIGHT:TOP .*lastIndex 3 maxIndex 3 relations 4 .*existingStem 2398 .*terminal ReadyForHeadCreateStem$' "$rows" || \
        ...
        ! grep -q '^stemsheaddebug115 hSide RIGHT vSide TOP length 98 myIndex 0 first h:108:RIGHT:TOP firstX 108 firstLinked true firstClosed true$' "$rows" || \
        ! grep -q '^stemsheadphasecontinue allegretto.png#1 system 3 headOrder 115 headX 113 headSig 75 headInterId 1826 .*decisions \[LEFT:top=false:bottom=false:branch=Neither,RIGHT:top=false:bottom=false:branch=Neither\] incident - returned false sidesAfter \[LEFT:false:true,RIGHT:false:true\] undefs \[\] closureWrites - closedValueChanges 0 unlinkedCount 0 sigVerticesBefore 649 sigVerticesAfter 649 sigEdgesBefore 593 sigEdgesAfter 593 systemStemsBefore 52 systemStemsAfter 52 .*nextHeadOrder 116 nextHeadX 66 nextHeadSig 33 nextHeadInterId 1743 .*terminal ReturnedBeforeNextHead$' "$rows"; then
    echo "Allegretto system-3 queue-53/115 contract differs" >&2
    exit 1
fi
...
base_runner="$script_dir/run-stems-head-phase-prefix-allegretto-system3-order61.sh"
base_fixture="$repo_root/rust/oracle/stems-head-phase-prefix-allegretto-system3-order61.txt"
...
out="$repo_root/rust/oracle/stems-head-phase-prefix-allegretto-system3-order115.txt"
{
    echo '# Java Audiveris 5.11 (Temurin JDK 25.0.3+9 LTS) Allegretto system-3 post-STUMPS HEADS queues 53 and 115.'
    echo '# Snapshot-minimized G1 replay: orders 0-114 mutate without snapshots except the measured queue-53 multi-side transaction; queue 115 emits the target C-link live state and no-link return.'
    cat "$rows"
    printf '%s\n' \
        "stemsheadphaseprefix summary schema stems-head-phase-prefix-allegretto-system3-order115 page allegretto.png#1 system 3 rows $row_count baseProbeSourceSha256 $base_probe_sha fragmentSourceSha256 $fragment_sha glyphIndexSourceSha256 $glyph_index_sha retainedGlyphOverlaySha256 $overlay_sha allegrettoSystem3InitSha256 $init_sha baseSystem3Order61RunnerSha256 $base_runner_sha baseSystem3Order61FixtureSha256 $base_fixture_sha probeSourceSha256 $probe_sha runnerSourceSha256 $runner_sha emittedBodySha256 $body_sha semanticPassSha256 $semantic_sha freshRuns 2 freshRunsByteIdentical true nativeScope BoundedSnapshotMinimizedG1RetainedGlyphAllegrettoSystem3Order53MultiSideAndOrder115NoLink javaEvidence ReturnedBeforeOneHundredSeventeenthHead"
} > "$out"
echo "wrote $out"
