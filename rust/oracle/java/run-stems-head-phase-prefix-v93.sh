...
# Read-only snapshot-minimized order93 continuation from v92; prior orders mutate without snapshots.
set -eu
...
tmp_base=/private/tmp/stems-head-phase-prefix-v93
...
awk -v fragment="$fragment18" '
{
    ...
    if (capture_after && index($0, "heads, allLinkers);") != 0) {
        print
        print "            if (headOrder == 7 || headOrder == 22 || headOrder == 27 || headOrder == 34 || headOrder == 36 || headOrder == 37 || headOrder == 38 || headOrder == 39 || headOrder == 40 || headOrder == 41 || headOrder == 42 || headOrder == 43 || headOrder == 44 || headOrder == 45 || headOrder == 46 || headOrder == 47 || headOrder == 48 || headOrder == 49 || headOrder == 50 || headOrder == 51 || headOrder == 52 || headOrder == 53 || headOrder == 54 || headOrder == 55 || headOrder == 56 || headOrder == 57 || headOrder == 58 || headOrder == 59 || headOrder == 60 || headOrder == 61 || headOrder == 62 || headOrder == 63 || headOrder == 64 || headOrder == 65 || headOrder == 66 || headOrder == 67 || headOrder == 68 || headOrder == 69 || headOrder == 70 || headOrder == 71 || headOrder == 72 || headOrder == 73 || headOrder == 74 || headOrder == 75 || headOrder == 76 || headOrder == 77 || headOrder == 78 || headOrder == 79 || headOrder == 80 || headOrder == 81 || headOrder == 82 || headOrder == 83 || headOrder == 84 || headOrder == 85 || headOrder == 86 || headOrder == 87 || headOrder == 88 || headOrder == 89 || headOrder == 90 || headOrder == 91 || headOrder == 92 || headOrder == 93) emitHeadCLinkResult(before, after, headOrder);"
        capture_after = 0
        in_linker = 0
        next
    }
...
if [ "$(grep -c '^stemsheadphasecontinue ' "$rows")" -ne 1 ] || \
        ! grep -q 'headOrder 93 headX 37 headSig 44 headInterId 1373 .*decisions \[LEFT:top=false:bottom=false:branch=Neither,RIGHT:top=true:bottom=false:branch=TopOnly\].*incident - returned true .*sidesAfter \[LEFT:false:false,RIGHT:true:false\] undefs \[\].*closureWrites - closedValueChanges 0 .*sigEdgesBefore 705 sigEdgesAfter 706 .*nextHeadOrder 94 nextHeadX 96 nextHeadSig 41 nextHeadInterId 1367 ' "$rows" || \
        ! grep -q '^stemsheadclinkfrontier .*headOrder 93 headX 37 headSig 44 .*cAlias h:37:LEFT:BOTTOM .*lastIndex -1 maxIndex 2 relations 1 .*glyphs 2 .*candidateIdBefore 0 existingGlyph - existingActive false existingStem - ' "$rows" || \
        ! grep -q '^stemsheadclinkresult headOrder 93 allocatorBefore 2385 allocatorAfter 2385 .*addedVertices - addedEdges \[system1:sourceId1373:targetId2379:org.audiveris.omr.sig.relation.HeadStemRelation:manual=false:grade=0x1.4fedcec6ab042p-1/3fe4fedcec6ab042.*headSide=RIGHT.*addedSystemStems - linkerChanges \[h:37:RIGHT:BOTTOM:false:false->true:false' "$rows"; then
    echo "v93 right-side C-link contract differs" >&2
    exit 1
fi
: <<'DISABLED_V27_ACTIVE'
...
v92_runner_sha=$(shasum -a 256 "$script_dir/run-stems-head-phase-prefix-v92.sh" | awk '{print $1}')
if [ "$v92_runner_sha" != "4096c4bad41a694d2c5f23a8405274c2fb83a973b7323a2cecd72790e582960c" ]; then
    echo "v93 base v92 runner drifted" >&2
    exit 1
fi
v92_fixture="$repo_root/rust/oracle/stems-head-phase-prefix-chula-system1-v92.txt"
v92_fixture_sha=$(shasum -a 256 "$v92_fixture" | awk '{print $1}')
if [ "$v92_fixture_sha" != "9ca2993d5ef00cda74cab28dd1258ee85317e6f52279b01a8841fe7d44ed5555" ]; then
    echo "v93 base v92 fixture drifted" >&2
    exit 1
fi
...
out="/private/tmp/stems-head-phase-prefix-chula-system1-v93-audit.txt"
{
    echo '# Java Audiveris 5.11 (Temurin JDK 25.0.3+9 LTS) post-STUMPS head phase v93.'
    echo '# schema: stems-head-phase-prefix-v93'
    echo '# Snapshot-minimized order-93 derivative: orders 1-92 mutate without snapshots; order 93 emits the RIGHT-side existing-stem C-link.'
    echo '# This bounded scope is intentional for deterministic replay under the full-snapshot heap limit.'
    cat "$rows"
    printf '%s\n' \
        "stemsheadphaseprefix summary schema stems-head-phase-prefix-v93 page chula.png#1 system 1 rows $row_count baseProbeSourceSha256 $base_probe_sha baseV92RunnerSourceSha256 $v92_runner_sha baseV92FixtureSha256 $v92_fixture_sha fragmentSourceSha256 $fragment_sha probeSourceSha256 $probe_sha runnerSourceSha256 $runner_sha emittedBodySha256 $body_sha semanticPassSha256 $semantic_sha completeStumpsFixtureSha256 $stumps_sha freshRuns 2 freshRunsByteIdentical true nativeScope BoundedSnapshotMinimizedOrder93RightSideReuseCLink javaEvidence ReturnedBeforeNinetyFourthHead"
} > "$out"
echo "wrote $out"
