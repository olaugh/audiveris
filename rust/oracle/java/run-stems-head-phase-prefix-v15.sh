...
# Freeze a bounded order-15 continuation replay. Orders below 15 are
# executed without persistent snapshots; only the order-15 row is emitted.
set -eu
...
tmp_base=/private/tmp/stems-head-phase-prefix-v15
...
if ! cmp -s "$semantic_one" "$semantic_two"; then
    echo "two fresh post-STUMPS v15 semantic passes are not byte-identical" >&2
...
if [ "$(grep -c '^stemsheadphasecontinue ' "$rows")" -ne 1 ] || \
        ! grep -q 'headOrder 15 headX 67 headSig 59 headInterId 1407 ' "$rows"; then
    echo "v15 bounded order-15 continuation contract differs" >&2
    exit 1
fi
...
v14_runner_sha=$(shasum -a 256 "$script_dir/run-stems-head-phase-prefix-v14.sh" | awk '{print $1}')
if [ "$v14_runner_sha" != "6b5e339f8b91db08d4e03edf7ed3b69ea8ab713b98ce95c62a95440a0652ccb9" ]; then
    echo "v15 base v14 runner drifted" >&2
    exit 1
fi
v14_fixture="$repo_root/rust/oracle/stems-head-phase-prefix-chula-system1-v14.txt"
v14_fixture_sha=$(shasum -a 256 "$v14_fixture" | awk '{print $1}')
if [ "$v14_fixture_sha" != "f60e5dff377e5e51038ec061b1ebeec5a5868f4cd51af6b9618377bfa3a12e6a" ]; then
    echo "v15 base v14 fixture drifted" >&2
    exit 1
fi
...
out="$repo_root/rust/oracle/stems-head-phase-prefix-chula-system1-v15.txt"
{
    echo '# Java Audiveris 5.11 (Temurin JDK 25.0.3+9 LTS) post-STUMPS head phase v15.'
    echo '# schema: stems-head-phase-prefix-v15'
    echo '# Bounded order-15 derivative: orders 1-14 execute linkSides without persistent snapshots; only order 15 is emitted.'
    echo '# This minimized scope is intentional for deterministic replay under the full-snapshot heap limit.'
    cat "$rows"
    printf '%s\n' \
        "stemsheadphaseprefix summary schema stems-head-phase-prefix-v15 page chula.png#1 system 1 rows $row_count baseProbeSourceSha256 $base_probe_sha baseV14RunnerSourceSha256 $v14_runner_sha baseV14FixtureSha256 $v14_fixture_sha probeSourceSha256 $probe_sha runnerSourceSha256 $runner_sha emittedBodySha256 $body_sha semanticPassSha256 $semantic_sha completeStumpsFixtureSha256 $stumps_sha freshRuns 2 freshRunsByteIdentical true nativeScope BoundedSnapshotMinimizedOrder15 javaEvidence ReturnedBeforeSixteenthHead"
} > "$out"
echo "wrote $out"
