...
# Read-only snapshot-minimized order77 continuation from v76; prior orders mutate without snapshots.
set -eu
...
tmp_base=/private/tmp/stems-head-phase-prefix-v77
...
awk -v fragment="$fragment18" '
{
    ...
    if (capture_after && index($0, "heads, allLinkers);") != 0) {
        print
        print "            if (headOrder == 7 || headOrder == 22 || headOrder == 27 || headOrder == 34 || headOrder == 36 || headOrder == 37 || headOrder == 38 || headOrder == 39 || headOrder == 40 || headOrder == 41 || headOrder == 42 || headOrder == 43 || headOrder == 44 || headOrder == 45 || headOrder == 46 || headOrder == 47 || headOrder == 48 || headOrder == 49 || headOrder == 50 || headOrder == 51 || headOrder == 52 || headOrder == 53 || headOrder == 54 || headOrder == 55 || headOrder == 56 || headOrder == 57 || headOrder == 58 || headOrder == 59 || headOrder == 60 || headOrder == 61 || headOrder == 62 || headOrder == 63 || headOrder == 64 || headOrder == 65 || headOrder == 66 || headOrder == 67 || headOrder == 68 || headOrder == 69 || headOrder == 70 || headOrder == 71 || headOrder == 72 || headOrder == 73 || headOrder == 74 || headOrder == 75 || headOrder == 76 || headOrder == 77) emitHeadCLinkResult(before, after, headOrder);"
        capture_after = 0
        in_linker = 0
        next
    }
...
if [ "$(grep -c '^stemsheadphasecontinue ' "$rows")" -ne 1 ] || \
        ! grep -q 'headOrder 77 headX 64 headSig 94 headInterId 1477 .*decisions \[LEFT:SkipAlreadyLinked,RIGHT:SkipClosed\].*incident \[stem2346:headSideLEFT:heads\[x64:sig94:id1477:sideLEFT,x65:sig95:id1479:sideLEFT\].*closureWrites \[x65:sig95:LEFT:false->true,x65:sig95:RIGHT:false->true\].*closedValueChanges 2 .*nextHeadOrder 78 nextHeadX 82 nextHeadSig 20 nextHeadInterId 1325 ' "$rows" || \
        ! grep -q '^stemsheadclinkfrontier .*headOrder 77 headX 64 headSig 94 .*cAlias h:64:LEFT:BOTTOM .*lastIndex 2 maxIndex 2 relations 3 .*glyphs 1 .*candidateIdBefore 291 existingGlyph glyph:291 existingActive true existingStem 2346 ' "$rows" || \
        ! grep -q '^stemsheadclinkresult headOrder 77 allocatorBefore 2385 allocatorAfter 2385 .*addedVertices - addedEdges - addedSystemStems - .*linkerChanges \[h:65:LEFT:BOTTOM:true:false->true:true' "$rows"; then
    echo "v77 existing-stem reconciliation contract differs" >&2
    exit 1
fi
: <<'DISABLED_V27_ACTIVE'
...
v76_runner_sha=$(shasum -a 256 "$script_dir/run-stems-head-phase-prefix-v76.sh" | awk '{print $1}')
if [ "$v76_runner_sha" != "3a899ca2e8cdcfd0b2eba4fb003f42bd46cbfc121eb31c5bd0ab428640f86ee3" ]; then
    echo "v77 base v76 runner drifted" >&2
    exit 1
fi
v76_fixture="$repo_root/rust/oracle/stems-head-phase-prefix-chula-system1-v76.txt"
v76_fixture_sha=$(shasum -a 256 "$v76_fixture" | awk '{print $1}')
if [ "$v76_fixture_sha" != "55f04e5c204f82b3e74d275e93cc93603ffc7b0a6f9e0e78bd7bcb6113544594" ]; then
    echo "v77 base v76 fixture drifted" >&2
    exit 1
fi
...
out="/private/tmp/stems-head-phase-prefix-chula-system1-v77-audit.txt"
{
    echo '# Java Audiveris 5.11 (Temurin JDK 25.0.3+9 LTS) post-STUMPS head phase v77.'
    echo '# schema: stems-head-phase-prefix-v77'
    echo '# Snapshot-minimized order-77 derivative: orders 1-76 mutate without snapshots; order 77 emits the existing-stem reconciliation.'
    echo '# This bounded scope is intentional for deterministic replay under the full-snapshot heap limit.'
    cat "$rows"
    printf '%s\n' \
        "stemsheadphaseprefix summary schema stems-head-phase-prefix-v77 page chula.png#1 system 1 rows $row_count baseProbeSourceSha256 $base_probe_sha baseV76RunnerSourceSha256 $v76_runner_sha baseV76FixtureSha256 $v76_fixture_sha fragmentSourceSha256 $fragment_sha probeSourceSha256 $probe_sha runnerSourceSha256 $runner_sha emittedBodySha256 $body_sha semanticPassSha256 $semantic_sha completeStumpsFixtureSha256 $stumps_sha freshRuns 2 freshRunsByteIdentical true nativeScope BoundedSnapshotMinimizedOrder77ExistingStemReconciliation javaEvidence ReturnedBeforeSeventyEighthHead"
} > "$out"
echo "wrote $out"
