...
# Freeze Java Bach system-2 phase-two queue 5's rejected top append retry.
set -eu
...
tmp_dir=$(mktemp -d /private/tmp/stems-head-phase-two-bach-s2-q5.XXXXXX)
...
grep -E '^(stemsheadphase2baseline .* system 2 |stemsheadphase2retry .* system 2 queueIndex 5 )' \
    "$tmp_dir/pass1" > "$tmp_dir/rows1"
grep -E '^(stemsheadphase2baseline .* system 2 |stemsheadphase2retry .* system 2 queueIndex 5 )' \
    "$tmp_dir/pass2" > "$tmp_dir/rows2"
if ! cmp -s "$tmp_dir/rows1" "$tmp_dir/rows2"; then
    echo "fresh Bach system-2 phase-two queue-5 Java passes are not byte-identical" >&2
...
if [ "$(wc -l < "$tmp_dir/rows1" | tr -d ' ')" -ne 2 ] || \
        ...
        ! grep -q '^stemsheadphase2retry page BachInvention5.jpg#1 system 2 queueIndex 5 headX 162 headSig 168 headInterId 3947 grade 3fcc27b9ce0db120 append true sidesBefore \[LEFT:false:true,RIGHT:false:true\] decisions \[LEFT:top=true:bottom=false:branch=TopOnly,RIGHT:top=false:bottom=false:branch=Neither\] returned false sidesAfter \[LEFT:false:true,RIGHT:false:true\] undefs \[\] sideChanges \[\] sigVerticesBefore 394 sigVerticesAfter 394 sigEdgesBefore 600 sigEdgesAfter 600 systemStemsBefore 77 systemStemsAfter 77 allocatorBefore 6815 allocatorAfter 6815$' "$tmp_dir/rows1"; then
    echo "Bach system-2 phase-two queue-5 Java contract differs" >&2
    cat "$tmp_dir/rows1" >&2
    exit 1
fi

base_runner="$script_dir/run-stems-head-phase-two-bach-system2-order4.sh"
base_fixture="$repo_root/rust/oracle/stems-head-phase-two-bach-system2-order4.txt"
base_runner_sha=$(shasum -a 256 "$base_runner" | awk '{print $1}')
base_fixture_sha=$(shasum -a 256 "$base_fixture" | awk '{print $1}')
if [ "$base_runner_sha" != "e03f0c788dd21a61d5629fbb4fc1c1afaf7d8439f567949afb26c09890361823" ] || \
        [ "$base_fixture_sha" != "6ba6a704229a926dd21fbae4e396d4917d48ee2b8149736178aa52aeaa833753" ]; then
    echo "Bach system-2 phase-two queue-4 predecessor drifted" >&2
    exit 1
fi
...
out="$repo_root/rust/oracle/stems-head-phase-two-bach-system2-order5.txt"
{
    echo '# Java Audiveris 5.11 (Temurin JDK 25.0.3+9 LTS) Bach system-2 phase-two queue 5.'
    echo '# schema: stems-head-phase-two-bach-system2-order5-v1'
    cat "$tmp_dir/rows1"
    printf '%s\n' \
        "stemsheadphase2bachs2q5summary schema stems-head-phase-two-bach-system2-order5-v1 page BachInvention5.jpg#1 system 2 rows 2 inputSha256 $input_sha stemsRetrieverSourceSha256 $stems_source_sha probeSourceSha256 $probe_sha initSourceSha256 $init_sha runnerSourceSha256 $runner_sha baseBoundary246RunnerSha256 $base_runner_sha baseBoundary246FixtureSha256 $base_fixture_sha emittedBodySha256 $body_sha semanticPassSha256 $body_sha freshRuns 2 freshRunsByteIdentical true nativeScope BachSystem2PhaseTwoOrder5RejectedTopAppend javaEvidence ReturnedAfterSystem2RetryIndex5"
} > "$out"
echo "wrote $out"
