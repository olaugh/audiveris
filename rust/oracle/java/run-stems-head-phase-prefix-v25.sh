...
# Freeze a bounded order-25 continuation replay from v24, including the
# prelinked existing-stem closure at x59/SIG74. Orders below 18 execute without snapshots.
set -eu
...
tmp_base=/private/tmp/stems-head-phase-prefix-v25
...
fragment18="$script_dir/stems-head-phase-v25-fragment.java"
...
awk -v fragment="$fragment18" '
{
    ...
    if (index($0, "final boolean returned = linker.linkSides(") != 0) {
        print "            if (headOrder == 7 || headOrder == 22) emitHeadCLinkEnvelope(head, linker, before);"
        print
        in_linker = 1
        next
    }
    ...
    if (capture_after && index($0, "heads, allLinkers);") != 0) {
        print
        print "            if (headOrder == 7 || headOrder == 22) emitHeadCLinkResult(before, after);"
        capture_after = 0
        in_linker = 0
        next
    }
...
if ! cmp -s "$semantic_one" "$semantic_two"; then
    echo "two fresh post-STUMPS v25 semantic passes are not byte-identical" >&2
...
if [ "$(grep -c '^stemsheadphasecontinue ' "$rows")" -ne 8 ] || \
        ! grep -q 'headOrder 24 headX 93 headSig 25 headInterId 1335 .*decisions \[LEFT:SkipAlreadyLinked,RIGHT:top=false:bottom=false:branch=Neither\].*closureWrites \[x92:sig24:LEFT:false->true,x92:sig24:RIGHT:false->true\].*closedValueChanges 2 .*nextHeadOrder 25 nextHeadX 59 nextHeadSig 74 nextHeadInterId 1437 ' "$rows" || \
        ! grep -q 'headOrder 25 headX 59 headSig 74 headInterId 1437 .*decisions \[LEFT:SkipAlreadyLinked,RIGHT:top=false:bottom=false:branch=Neither\].*incident \[stem2363:headSideLEFT:heads\[x58:sig73:id1435:sideLEFT,x59:sig74:id1437:sideLEFT\]\].*closureWrites \[x58:sig73:LEFT:false->true,x58:sig73:RIGHT:false->true\].*closedValueChanges 2 .*nextHeadOrder 26 nextHeadX 61 nextHeadSig 31 nextHeadInterId 1347 ' "$rows"; then
    echo "v25 bounded prelinked-closure contract differs" >&2
    exit 1
fi
if false; then
    :
...
v24_runner_sha=$(shasum -a 256 "$script_dir/run-stems-head-phase-prefix-v24.sh" | awk '{print $1}')
if [ "$v24_runner_sha" != "2d2a7b2b58f674bdf3db3716a6e66eac1b9d56694df7c79d7ec91ff7cb629293" ]; then
    echo "v25 base v24 runner drifted" >&2
    exit 1
fi
v24_fixture="$repo_root/rust/oracle/stems-head-phase-prefix-chula-system1-v24.txt"
v24_fixture_sha=$(shasum -a 256 "$v24_fixture" | awk '{print $1}')
if [ "$v24_fixture_sha" != "56684be47b32b49e3d6f3c1440a9f3062a6bdcdec28fa0554cc6f2be80242b6c" ]; then
    echo "v25 base v24 fixture drifted" >&2
    exit 1
fi
...
out="$repo_root/rust/oracle/stems-head-phase-prefix-chula-system1-v25.txt"
{
    echo '# Java Audiveris 5.11 (Temurin JDK 25.0.3+9 LTS) post-STUMPS head phase v25.'
    echo '# schema: stems-head-phase-prefix-v25'
    echo '# Bounded order-25 derivative: order 25 emits the prelinked closure continuation after the v24 predecessor.'
    echo '# This minimized scope is intentional for deterministic replay under the full-snapshot heap limit.'
    cat "$rows"
    printf '%s\n' \
        "stemsheadphaseprefix summary schema stems-head-phase-prefix-v25 page chula.png#1 system 1 rows $row_count baseProbeSourceSha256 $base_probe_sha baseV24RunnerSourceSha256 $v24_runner_sha baseV24FixtureSha256 $v24_fixture_sha fragmentSourceSha256 $fragment_sha probeSourceSha256 $probe_sha runnerSourceSha256 $runner_sha emittedBodySha256 $body_sha semanticPassSha256 $semantic_sha completeStumpsFixtureSha256 $stumps_sha freshRuns 2 freshRunsByteIdentical true nativeScope BoundedSnapshotMinimizedOrder25PrelinkedClosure javaEvidence ReturnedBeforeTwentySixthHead"
} > "$out"
echo "wrote $out"
