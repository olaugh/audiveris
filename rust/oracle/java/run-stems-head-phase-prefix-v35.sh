...
# Freeze a snapshot-minimized order35 continuation from v34; prior orders mutate without snapshots.
set -eu
...
tmp_base=/private/tmp/stems-head-phase-prefix-v35
...
if [ "$(grep -c '^stemsheadphasecontinue ' "$rows")" -ne 1 ] || \
        ! grep -q 'headOrder 35 headX 50 headSig 72 headInterId 1433 .*decisions \[LEFT:SkipAlreadyLinked,RIGHT:top=false:bottom=false:branch=Neither\].*incident \[stem2353:headSideLEFT:heads\[x49:sig71:id1431:sideLEFT,x50:sig72:id1433:sideLEFT\]\].*closureWrites \[x49:sig71:LEFT:false->true,x49:sig71:RIGHT:false->true\].*closedValueChanges 2 .*nextHeadOrder 36 nextHeadX 23 nextHeadSig 14 nextHeadInterId 1313 ' "$rows"; then
    echo "v35 minimized closure contract differs" >&2
    exit 1
fi
: <<'DISABLED_V27_ACTIVE'
...
v34_runner_sha=$(shasum -a 256 "$script_dir/run-stems-head-phase-prefix-v34.sh" | awk '{print $1}')
if [ "$v34_runner_sha" != "60b4cc5a9e0a9fe5c6d4a8bb1b03bfadf065259c07bc124c6587b3d7a9c3a93f" ]; then
    echo "v35 base v34 runner drifted" >&2
    exit 1
fi
v34_fixture="$repo_root/rust/oracle/stems-head-phase-prefix-chula-system1-v34.txt"
v34_fixture_sha=$(shasum -a 256 "$v34_fixture" | awk '{print $1}')
if [ "$v34_fixture_sha" != "b67514520fa848fd9758d0bdc740d2be4600c723ac341b57fced42f4657103a8" ]; then
    echo "v35 base v34 fixture drifted" >&2
    exit 1
fi
...
out="$repo_root/rust/oracle/stems-head-phase-prefix-chula-system1-v35.txt"
{
    echo '# Java Audiveris 5.11 (Temurin JDK 25.0.3+9 LTS) post-STUMPS head phase v35.'
    echo '# schema: stems-head-phase-prefix-v35'
    echo '# Snapshot-minimized order-35 derivative: orders 1-34 mutate without snapshots; order 35 emits the prelinked closure row.'
    echo '# This minimized scope is intentional for deterministic replay under the full-snapshot heap limit.'
    cat "$rows"
    printf '%s\n' \
        "stemsheadphaseprefix summary schema stems-head-phase-prefix-v35 page chula.png#1 system 1 rows $row_count baseProbeSourceSha256 $base_probe_sha baseV34RunnerSourceSha256 $v34_runner_sha baseV34FixtureSha256 $v34_fixture_sha fragmentSourceSha256 $fragment_sha probeSourceSha256 $probe_sha runnerSourceSha256 $runner_sha emittedBodySha256 $body_sha semanticPassSha256 $semantic_sha completeStumpsFixtureSha256 $stumps_sha freshRuns 2 freshRunsByteIdentical true nativeScope BoundedSnapshotMinimizedOrder35PrelinkedClosure javaEvidence ReturnedBeforeThirtySixthHead"
} > "$out"
echo "wrote $out"
