...
# Read-only snapshot-minimized order75 continuation from v74; prior orders mutate without snapshots.
set -eu
...
tmp_base=/private/tmp/stems-head-phase-prefix-v75
...
awk -v fragment="$fragment18" '
{
    ...
    if (capture_after && index($0, "heads, allLinkers);") != 0) {
        print
        print "            if (headOrder == 7 || headOrder == 22 || headOrder == 27 || headOrder == 34 || headOrder == 36 || headOrder == 37 || headOrder == 38 || headOrder == 39 || headOrder == 40 || headOrder == 41 || headOrder == 42 || headOrder == 43 || headOrder == 44 || headOrder == 45 || headOrder == 46 || headOrder == 47 || headOrder == 48 || headOrder == 49 || headOrder == 50 || headOrder == 51 || headOrder == 52 || headOrder == 53 || headOrder == 54 || headOrder == 55 || headOrder == 56 || headOrder == 57 || headOrder == 58 || headOrder == 59 || headOrder == 60 || headOrder == 61 || headOrder == 62 || headOrder == 63 || headOrder == 64 || headOrder == 65 || headOrder == 66 || headOrder == 67 || headOrder == 68 || headOrder == 69 || headOrder == 70 || headOrder == 71 || headOrder == 72 || headOrder == 73 || headOrder == 74 || headOrder == 75) emitHeadCLinkResult(before, after, headOrder);"
        capture_after = 0
        in_linker = 0
        next
    }
...
if [ "$(grep -c '^stemsheadphasecontinue ' "$rows")" -ne 1 ] || \
        ! grep -q 'headOrder 75 headX 31 headSig 47 headInterId 1381 .*decisions \[LEFT:top=true:bottom=true:branch=Both,RIGHT:top=true:bottom=false:branch=TopOnly\].*incident - returned false .*undefs \[LEFT\].*closedValueChanges 0 .*sigEdgesBefore 705 sigEdgesAfter 705 .*nextHeadOrder 76 nextHeadX 66 nextHeadSig 58 nextHeadInterId 1405 ' "$rows" || \
        ! grep -q '^stemsheadclinkfrontier .*headOrder 75 headX 31 headSig 47 .*cAlias h:31:LEFT:BOTTOM .*lastIndex -1 maxIndex 1 relations 1 .*glyphs 2 .*candidateIdBefore 0 existingGlyph glyph:314 existingActive true existingStem 2383 ' "$rows" || \
        ! grep -q '^stemsheadclinkresult headOrder 75 allocatorBefore 2385 allocatorAfter 2385 .*addedVertices - addedEdges - addedSystemStems - linkerChanges - .*terminal ReturnedHeadCLinkTransaction' "$rows"; then
    echo "v75 open-frontier contract differs" >&2
    exit 1
fi
: <<'DISABLED_V27_ACTIVE'
...
v74_runner_sha=$(shasum -a 256 "$script_dir/run-stems-head-phase-prefix-v74.sh" | awk '{print $1}')
if [ "$v74_runner_sha" != "179a2752d18180ef1c4798223b8a79a3a5398e5b6a45fe7cc711085c5caa178f" ]; then
    echo "v75 base v74 runner drifted" >&2
    exit 1
fi
v74_fixture="$repo_root/rust/oracle/stems-head-phase-prefix-chula-system1-v74.txt"
v74_fixture_sha=$(shasum -a 256 "$v74_fixture" | awk '{print $1}')
if [ "$v74_fixture_sha" != "fdaf7d51fb50ef62235c2fac95525a1e9d2added8222c9144586e9a7ef313e1c" ]; then
    echo "v75 base v74 fixture drifted" >&2
    exit 1
fi
...
out="/private/tmp/stems-head-phase-prefix-chula-system1-v75-audit.txt"
{
    echo '# Java Audiveris 5.11 (Temurin JDK 25.0.3+9 LTS) post-STUMPS head phase v75.'
    echo '# schema: stems-head-phase-prefix-v75'
    echo '# Snapshot-minimized order-75 derivative: orders 1-74 mutate without snapshots; order 75 emits the returned-false LEFT undef.'
    echo '# This bounded scope is intentional for deterministic replay under the full-snapshot heap limit.'
    cat "$rows"
    printf '%s\n' \
        "stemsheadphaseprefix summary schema stems-head-phase-prefix-v75 page chula.png#1 system 1 rows $row_count baseProbeSourceSha256 $base_probe_sha baseV74RunnerSourceSha256 $v74_runner_sha baseV74FixtureSha256 $v74_fixture_sha fragmentSourceSha256 $fragment_sha probeSourceSha256 $probe_sha runnerSourceSha256 $runner_sha emittedBodySha256 $body_sha semanticPassSha256 $semantic_sha completeStumpsFixtureSha256 $stumps_sha freshRuns 2 freshRunsByteIdentical true nativeScope BoundedSnapshotMinimizedOrder75OpenFrontier javaEvidence ReturnedBeforeSeventySixthHead"
} > "$out"
echo "wrote $out"
