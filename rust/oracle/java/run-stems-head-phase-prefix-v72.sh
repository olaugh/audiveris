...
# Read-only snapshot-minimized order72 continuation from v71; prior orders mutate without snapshots.
set -eu
...
tmp_base=/private/tmp/stems-head-phase-prefix-v72
...
awk -v fragment="$fragment18" '
{
    ...
    if (capture_after && index($0, "heads, allLinkers);") != 0) {
        print
        print "            if (headOrder == 7 || headOrder == 22 || headOrder == 27 || headOrder == 34 || headOrder == 36 || headOrder == 37 || headOrder == 38 || headOrder == 39 || headOrder == 40 || headOrder == 41 || headOrder == 42 || headOrder == 43 || headOrder == 44 || headOrder == 45 || headOrder == 46 || headOrder == 47 || headOrder == 48 || headOrder == 49 || headOrder == 50 || headOrder == 51 || headOrder == 52 || headOrder == 53 || headOrder == 54 || headOrder == 55 || headOrder == 56 || headOrder == 57 || headOrder == 58 || headOrder == 59 || headOrder == 60 || headOrder == 61 || headOrder == 62 || headOrder == 63 || headOrder == 64 || headOrder == 65 || headOrder == 66 || headOrder == 67 || headOrder == 68 || headOrder == 69 || headOrder == 70 || headOrder == 71 || headOrder == 72) emitHeadCLinkResult(before, after, headOrder);"
        capture_after = 0
        in_linker = 0
        next
    }
...
if [ "$(grep -c '^stemsheadphasecontinue ' "$rows")" -ne 1 ] || \
        ! grep -q 'headOrder 72 headX 26 headSig 13 headInterId 1311 .*decisions \[LEFT:top=false:bottom=true:branch=BottomOnly,RIGHT:top=false:bottom=false:branch=Neither\].*incident - returned true .*undefs \[\].*closureWrites - closedValueChanges 0 .*sigEdgesBefore 702 sigEdgesAfter 703 .*nextHeadOrder 73 nextHeadX 75 nextHeadSig 96 nextHeadInterId 1481 ' "$rows" || \
        ! grep -q '^stemsheadclinkfrontier .*headOrder 72 headX 26 headSig 13 .*cAlias h:26:LEFT:BOTTOM .*lastIndex 0 maxIndex 0 relations 1 .*glyphs 1 .*candidateIdBefore 324 existingGlyph glyph:324 existingActive true existingStem 2385 ' "$rows" || \
        ! grep -q '^stemsheadclinkresult headOrder 72 allocatorBefore 2385 allocatorAfter 2385 .*addedVertices - addedEdges \[system1:sourceId1311:targetId2385:org.audiveris.omr.sig.relation.HeadStemRelation.*addedSystemStems - linkerChanges \[h:23:LEFT:BOTTOM:true:false->true:true' "$rows"; then
    echo "v72 single-head reuse C-link contract differs" >&2
    exit 1
fi
: <<'DISABLED_V27_ACTIVE'
...
v71_runner_sha=$(shasum -a 256 "$script_dir/run-stems-head-phase-prefix-v71.sh" | awk '{print $1}')
if [ "$v71_runner_sha" != "ed5e460555016bf5c578a398322d6f1d3f256f29cedc3a4e858dc4b0e27a539b" ]; then
    echo "v72 base v71 runner drifted" >&2
    exit 1
fi
v71_fixture="$repo_root/rust/oracle/stems-head-phase-prefix-chula-system1-v71.txt"
v71_fixture_sha=$(shasum -a 256 "$v71_fixture" | awk '{print $1}')
if [ "$v71_fixture_sha" != "0b6e69a6b50ea4aae7af2da5f4899a9947471e749d60d20b821e60599aac6ea9" ]; then
    echo "v72 base v71 fixture drifted" >&2
    exit 1
fi
...
out="/private/tmp/stems-head-phase-prefix-chula-system1-v72-audit.txt"
{
    echo '# Java Audiveris 5.11 (Temurin JDK 25.0.3+9 LTS) post-STUMPS head phase v72.'
    echo '# schema: stems-head-phase-prefix-v72'
    echo '# Snapshot-minimized order-72 derivative: orders 1-71 mutate without snapshots; order 72 emits the single-head existing-stem C-link.'
    echo '# This bounded scope is intentional for deterministic replay under the full-snapshot heap limit.'
    cat "$rows"
    printf '%s\n' \
        "stemsheadphaseprefix summary schema stems-head-phase-prefix-v72 page chula.png#1 system 1 rows $row_count baseProbeSourceSha256 $base_probe_sha baseV71RunnerSourceSha256 $v71_runner_sha baseV71FixtureSha256 $v71_fixture_sha fragmentSourceSha256 $fragment_sha probeSourceSha256 $probe_sha runnerSourceSha256 $runner_sha emittedBodySha256 $body_sha semanticPassSha256 $semantic_sha completeStumpsFixtureSha256 $stumps_sha freshRuns 2 freshRunsByteIdentical true nativeScope BoundedSnapshotMinimizedOrder72SingleHeadReuseCLink javaEvidence ReturnedBeforeSeventyThirdHead"
} > "$out"
echo "wrote $out"
