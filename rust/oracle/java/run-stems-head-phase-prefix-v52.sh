...
# Read-only snapshot-minimized order52 continuation from v51; prior orders mutate without snapshots.
set -eu
...
tmp_base=/private/tmp/stems-head-phase-prefix-v52
...
awk -v fragment="$fragment18" '
{
    ...
    if (index($0, "final boolean returned = linker.linkSides(") != 0) {
        print "            if (headOrder == 7 || headOrder == 22 || headOrder == 27 || headOrder == 34 || headOrder == 36 || headOrder == 37 || headOrder == 38 || headOrder == 39 || headOrder == 40 || headOrder == 41 || headOrder == 42 || headOrder == 43 || headOrder == 44 || headOrder == 45 || headOrder == 46 || headOrder == 47 || headOrder == 48 || headOrder == 49 || headOrder == 50 || headOrder == 51 || headOrder == 52) emitHeadCLinkEnvelope(head, linker, before, headOrder);"
        print
        in_linker = 1
        next
    }
    ...
    if (capture_after && index($0, "heads, allLinkers);") != 0) {
        print
        print "            if (headOrder == 7 || headOrder == 22 || headOrder == 27 || headOrder == 34 || headOrder == 36 || headOrder == 37 || headOrder == 38 || headOrder == 39 || headOrder == 40 || headOrder == 41 || headOrder == 42 || headOrder == 43 || headOrder == 44 || headOrder == 45 || headOrder == 46 || headOrder == 47 || headOrder == 48 || headOrder == 49 || headOrder == 50 || headOrder == 51 || headOrder == 52) emitHeadCLinkResult(before, after, headOrder);"
        capture_after = 0
        in_linker = 0
        next
    }
...
if [ "$(grep -c '^stemsheadphasecontinue ' "$rows")" -ne 1 ] || \
        ! grep -q 'headOrder 52 headX 15 headSig 80 headInterId 1449 .*decisions \[LEFT:SkipAlreadyLinked,RIGHT:SkipClosed\].*incident \[stem2360:headSideLEFT:heads\[x15:sig80:id1449:sideLEFT,x16:sig81:id1451:sideLEFT\].*closureWrites \[x16:sig81:LEFT:false->true,x16:sig81:RIGHT:false->true\].*closedValueChanges 2 .*nextHeadOrder 53 nextHeadX 84 nextHeadSig 86 nextHeadInterId 1461 ' "$rows" || \
        ! grep -q '^stemsheadclinkfrontier .*headOrder 52 headX 15 headSig 80 .*cAlias h:15:LEFT:BOTTOM .*lastIndex 2 maxIndex 2 relations 2 .*glyphs 2 .*candidateIdBefore 0 existingGlyph glyph:329 existingActive true existingStem 2360 ' "$rows" || \
        ! grep -q '^stemsheadclinkresult headOrder 52 allocatorBefore 2385 allocatorAfter 2385 .*addedVertices - addedEdges - addedSystemStems - .*linkerChanges \[h:16:LEFT:BOTTOM:true:false->true:true' "$rows"; then
    echo "v52 existing-stem reconciliation contract differs" >&2
    exit 1
fi
: <<'DISABLED_V27_ACTIVE'
...
v51_runner_sha=$(shasum -a 256 "$script_dir/run-stems-head-phase-prefix-v51.sh" | awk '{print $1}')
if [ "$v51_runner_sha" != "55f522ebc3bff281f113715e86137e5feac2d74f513907b8361903cdf3b1a828" ]; then
    echo "v52 base v51 runner drifted" >&2
    exit 1
fi
v51_fixture="$repo_root/rust/oracle/stems-head-phase-prefix-chula-system1-v51.txt"
v51_fixture_sha=$(shasum -a 256 "$v51_fixture" | awk '{print $1}')
if [ "$v51_fixture_sha" != "778c06a6697f96a439146e06d85d9d182d1f7659ef19a15fd9204aef3455546e" ]; then
    echo "v52 base v51 fixture drifted" >&2
    exit 1
fi
...
out="/private/tmp/stems-head-phase-prefix-chula-system1-v52-audit.txt"
{
    echo '# Java Audiveris 5.11 (Temurin JDK 25.0.3+9 LTS) post-STUMPS head phase v52.'
    echo '# schema: stems-head-phase-prefix-v52'
    echo '# Snapshot-minimized order-52 derivative: orders 1-51 mutate without snapshots; order 52 emits existing-stem reconciliation.'
    echo '# This bounded scope is intentional for deterministic replay under the full-snapshot heap limit.'
    cat "$rows"
    printf '%s\n' \
        "stemsheadphaseprefix summary schema stems-head-phase-prefix-v52 page chula.png#1 system 1 rows $row_count baseProbeSourceSha256 $base_probe_sha baseV51RunnerSourceSha256 $v51_runner_sha baseV51FixtureSha256 $v51_fixture_sha fragmentSourceSha256 $fragment_sha probeSourceSha256 $probe_sha runnerSourceSha256 $runner_sha emittedBodySha256 $body_sha semanticPassSha256 $semantic_sha completeStumpsFixtureSha256 $stumps_sha freshRuns 2 freshRunsByteIdentical true nativeScope BoundedSnapshotMinimizedOrder52ExistingStemReconciliation javaEvidence ReturnedBeforeFiftyThirdHead"
} > "$out"
echo "wrote $out"
