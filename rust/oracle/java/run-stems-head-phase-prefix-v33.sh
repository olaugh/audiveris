...
# Freeze a snapshot-minimized order33 continuation from v32; prior orders mutate without snapshots.
set -eu
...
tmp_base=/private/tmp/stems-head-phase-prefix-v33
...
if [ "$(grep -c '^stemsheadphasecontinue ' "$rows")" -ne 1 ] || \
        ! grep -q 'headOrder 33 headX 88 headSig 84 headInterId 1457 .*decisions \[LEFT:SkipAlreadyLinked,RIGHT:top=false:bottom=false:branch=Neither\].*incident \[stem2367:headSideLEFT:heads\[x87:sig83:id1455:sideLEFT,x88:sig84:id1457:sideLEFT\]\].*closureWrites \[x87:sig83:LEFT:false->true,x87:sig83:RIGHT:false->true\].*closedValueChanges 2 .*nextHeadOrder 34 nextHeadX 2 nextHeadSig 36 nextHeadInterId 1357 ' "$rows"; then
    echo "v33 minimized closure contract differs" >&2
    exit 1
fi
: <<'DISABLED_V27_ACTIVE'
...
v32_runner_sha=$(shasum -a 256 "$script_dir/run-stems-head-phase-prefix-v32.sh" | awk '{print $1}')
if [ "$v32_runner_sha" != "fecd661b0c9b9e03f17c9eba3482a86b7f2ae381e49ac93bbbcbfea4756c3cd8" ]; then
    echo "v33 base v32 runner drifted" >&2
    exit 1
fi
v32_fixture="$repo_root/rust/oracle/stems-head-phase-prefix-chula-system1-v32.txt"
v32_fixture_sha=$(shasum -a 256 "$v32_fixture" | awk '{print $1}')
if [ "$v32_fixture_sha" != "cceda3e1b00ccf9e4ca5f701c71a0a4da4e764488e192bf056ea645f11ad72c4" ]; then
    echo "v33 base v32 fixture drifted" >&2
    exit 1
fi
...
out="$repo_root/rust/oracle/stems-head-phase-prefix-chula-system1-v33.txt"
{
    echo '# Java Audiveris 5.11 (Temurin JDK 25.0.3+9 LTS) post-STUMPS head phase v33.'
    echo '# schema: stems-head-phase-prefix-v33'
    echo '# Snapshot-minimized order-33 derivative: orders 1-32 mutate without snapshots; order 33 emits the prelinked closure row.'
    echo '# This minimized scope is intentional for deterministic replay under the full-snapshot heap limit.'
    cat "$rows"
    printf '%s\n' \
        "stemsheadphaseprefix summary schema stems-head-phase-prefix-v33 page chula.png#1 system 1 rows $row_count baseProbeSourceSha256 $base_probe_sha baseV32RunnerSourceSha256 $v32_runner_sha baseV32FixtureSha256 $v32_fixture_sha fragmentSourceSha256 $fragment_sha probeSourceSha256 $probe_sha runnerSourceSha256 $runner_sha emittedBodySha256 $body_sha semanticPassSha256 $semantic_sha completeStumpsFixtureSha256 $stumps_sha freshRuns 2 freshRunsByteIdentical true nativeScope BoundedSnapshotMinimizedOrder33PrelinkedClosure javaEvidence ReturnedBeforeThirtyFourthHead"
} > "$out"
echo "wrote $out"
