...
# Read-only snapshot-minimized order66 continuation from v65; prior orders mutate without snapshots.
set -eu
...
tmp_base=/private/tmp/stems-head-phase-prefix-v66
...
if [ "$(grep -c '^stemsheadphasecontinue ' "$rows")" -ne 1 ] || \
        ! grep -q 'headOrder 66 headX 13 headSig 0 headInterId 1285 .*decisions \[LEFT:SkipAlreadyLinked,RIGHT:SkipClosed\].*incident \[stem2340:headSideLEFT:heads\[x13:sig0:id1285:sideLEFT,x14:sig1:id1287:sideLEFT\].*closureWrites \[x14:sig1:LEFT:false->true,x14:sig1:RIGHT:false->true\].*closedValueChanges 2 .*nextHeadOrder 67 nextHeadX 73 nextHeadSig 18 nextHeadInterId 1321 ' "$rows" || \
        ! grep -q '^stemsheadclinkfrontier .*headOrder 66 headX 13 headSig 0 .*cAlias h:13:LEFT:BOTTOM .*lastIndex 4 maxIndex 4 relations 4 .*glyphs 2 .*candidateIdBefore 0 existingGlyph glyph:294 existingActive true existingStem 2340 ' "$rows" || \
        ! grep -q '^stemsheadclinkresult headOrder 66 allocatorBefore 2385 allocatorAfter 2385 .*addedVertices - addedEdges - addedSystemStems - .*linkerChanges \[h:14:LEFT:BOTTOM:true:false->true:true' "$rows"; then
    echo "v66 existing-stem reconciliation contract differs" >&2
    exit 1
fi
: <<'DISABLED_V27_ACTIVE'
...
v65_runner_sha=$(shasum -a 256 "$script_dir/run-stems-head-phase-prefix-v65.sh" | awk '{print $1}')
if [ "$v65_runner_sha" != "427bf9b0703059e8e31df582cf8c11b512c328717a4ef60bd6fcad5775855f3a" ]; then
    echo "v66 base v65 runner drifted" >&2
    exit 1
fi
v65_fixture="$repo_root/rust/oracle/stems-head-phase-prefix-chula-system1-v65.txt"
v65_fixture_sha=$(shasum -a 256 "$v65_fixture" | awk '{print $1}')
if [ "$v65_fixture_sha" != "fff9e008f3caf1bce93f124bd649a33cc08572cc55371324f600691a6f866db5" ]; then
    echo "v66 base v65 fixture drifted" >&2
    exit 1
fi
...
out="/private/tmp/stems-head-phase-prefix-chula-system1-v66-audit.txt"
{
    echo '# Java Audiveris 5.11 (Temurin JDK 25.0.3+9 LTS) post-STUMPS head phase v66.'
    echo '# schema: stems-head-phase-prefix-v66'
    echo '# Snapshot-minimized order-66 derivative: orders 1-65 mutate without snapshots; order 66 emits existing-stem reconciliation.'
    echo '# This bounded scope is intentional for deterministic replay under the full-snapshot heap limit.'
    cat "$rows"
    printf '%s\n' \
        "stemsheadphaseprefix summary schema stems-head-phase-prefix-v66 page chula.png#1 system 1 rows $row_count baseProbeSourceSha256 $base_probe_sha baseV65RunnerSourceSha256 $v65_runner_sha baseV65FixtureSha256 $v65_fixture_sha fragmentSourceSha256 $fragment_sha probeSourceSha256 $probe_sha runnerSourceSha256 $runner_sha emittedBodySha256 $body_sha semanticPassSha256 $semantic_sha completeStumpsFixtureSha256 $stumps_sha freshRuns 2 freshRunsByteIdentical true nativeScope BoundedSnapshotMinimizedOrder66ExistingStemReconciliation javaEvidence ReturnedBeforeSixtySeventhHead"
} > "$out"
echo "wrote $out"
