...
# Freeze a snapshot-minimized order36 both-open C-link continuation from v35; prior orders mutate without snapshots.
set -eu
...
tmp_base=/private/tmp/stems-head-phase-prefix-v36
...
awk -v fragment="$fragment18" '
{
    ...
    if (index($0, "final boolean returned = linker.linkSides(") != 0) {
        print "            if (headOrder == 7 || headOrder == 22 || headOrder == 27 || headOrder == 34 || headOrder == 36) emitHeadCLinkEnvelope(head, linker, before, headOrder);"
        print
        in_linker = 1
        next
    }
    ...
    if (capture_after && index($0, "heads, allLinkers);") != 0) {
        print
        print "            if (headOrder == 7 || headOrder == 22 || headOrder == 27 || headOrder == 34 || headOrder == 36) emitHeadCLinkResult(before, after, headOrder);"
        capture_after = 0
        in_linker = 0
        next
    }
...
if [ "$(grep -c '^stemsheadphasecontinue ' "$rows")" -ne 1 ] || \
        ! grep -q 'headOrder 36 headX 23 headSig 14 headInterId 1313 .*decisions \[LEFT:top=false:bottom=true:branch=BottomOnly,RIGHT:top=false:bottom=false:branch=Neither\].*sigVerticesBefore 684 sigVerticesAfter 685 sigEdgesBefore 695 sigEdgesAfter 696 .*nextHeadOrder 37 nextHeadX 14 nextHeadSig 1 nextHeadInterId 1287 ' "$rows" || \
        ! grep -q '^stemsheadclinkfrontier .*headOrder 36 headX 23 headSig 14 .*cAlias h:23:LEFT:BOTTOM .*lastIndex 0 maxIndex 0 relations 1 .*glyphs 1 .*candidateIdBefore 324 existingGlyph glyph:324 existingActive true existingStem - ' "$rows" || \
        ! grep -q '^stemsheadclinkresult headOrder 36 allocatorBefore 2384 allocatorAfter 2385 .*addedVertices \[id2385:.*addedEdges \[system1:sourceId1313:targetId2385:.*addedSystemStems \[g:868:374:3:86:.*:stemId2385\]' "$rows"; then
    echo "v36 both-open C-link contract differs" >&2
    exit 1
fi
: <<'DISABLED_V27_ACTIVE'
...
v35_runner_sha=$(shasum -a 256 "$script_dir/run-stems-head-phase-prefix-v35.sh" | awk '{print $1}')
if [ "$v35_runner_sha" != "74aec11451cb5933938b3bc82876ddfdb9e4bdab295e472644698c68d2cbc5ea" ]; then
    echo "v36 base v35 runner drifted" >&2
    exit 1
fi
v35_fixture="$repo_root/rust/oracle/stems-head-phase-prefix-chula-system1-v35.txt"
v35_fixture_sha=$(shasum -a 256 "$v35_fixture" | awk '{print $1}')
if [ "$v35_fixture_sha" != "2721b843514ce7a695fdacc797addd21597bd604b39168fe63533ecfc01bd55b" ]; then
    echo "v36 base v35 fixture drifted" >&2
    exit 1
fi
...
out="$repo_root/rust/oracle/stems-head-phase-prefix-chula-system1-v36.txt"
{
    echo '# Java Audiveris 5.11 (Temurin JDK 25.0.3+9 LTS) post-STUMPS head phase v36.'
    echo '# schema: stems-head-phase-prefix-v36'
    echo '# Snapshot-minimized order-36 derivative: orders 1-35 mutate without snapshots; order 36 emits the both-open C-link row.'
    echo '# This minimized scope is intentional for deterministic replay under the full-snapshot heap limit.'
    cat "$rows"
    printf '%s\n' \
        "stemsheadphaseprefix summary schema stems-head-phase-prefix-v36 page chula.png#1 system 1 rows $row_count baseProbeSourceSha256 $base_probe_sha baseV35RunnerSourceSha256 $v35_runner_sha baseV35FixtureSha256 $v35_fixture_sha fragmentSourceSha256 $fragment_sha probeSourceSha256 $probe_sha runnerSourceSha256 $runner_sha emittedBodySha256 $body_sha semanticPassSha256 $semantic_sha completeStumpsFixtureSha256 $stumps_sha freshRuns 2 freshRunsByteIdentical true nativeScope BoundedSnapshotMinimizedOrder36CLink javaEvidence ReturnedBeforeThirtySeventhHead"
} > "$out"
echo "wrote $out"
