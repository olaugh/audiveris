...
# Read-only snapshot-minimized order94 continuation from v93; prior orders mutate without snapshots.
set -eu
...
tmp_base=/private/tmp/stems-head-phase-prefix-v94
...
awk -v fragment="$fragment18" '
{
    ...
    if (capture_after && index($0, "heads, allLinkers);") != 0) {
        print
        print "            if (headOrder == 7 || headOrder == 22 || headOrder == 27 || headOrder == 34 || headOrder == 36 || headOrder == 37 || headOrder == 38 || headOrder == 39 || headOrder == 40 || headOrder == 41 || headOrder == 42 || headOrder == 43 || headOrder == 44 || headOrder == 45 || headOrder == 46 || headOrder == 47 || headOrder == 48 || headOrder == 49 || headOrder == 50 || headOrder == 51 || headOrder == 52 || headOrder == 53 || headOrder == 54 || headOrder == 55 || headOrder == 56 || headOrder == 57 || headOrder == 58 || headOrder == 59 || headOrder == 60 || headOrder == 61 || headOrder == 62 || headOrder == 63 || headOrder == 64 || headOrder == 65 || headOrder == 66 || headOrder == 67 || headOrder == 68 || headOrder == 69 || headOrder == 70 || headOrder == 71 || headOrder == 72 || headOrder == 73 || headOrder == 74 || headOrder == 75 || headOrder == 76 || headOrder == 77 || headOrder == 78 || headOrder == 79 || headOrder == 80 || headOrder == 81 || headOrder == 82 || headOrder == 83 || headOrder == 84 || headOrder == 85 || headOrder == 86 || headOrder == 87 || headOrder == 88 || headOrder == 89 || headOrder == 90 || headOrder == 91 || headOrder == 92 || headOrder == 93 || headOrder == 94) emitHeadCLinkResult(before, after, headOrder);"
        capture_after = 0
        in_linker = 0
        next
    }
...
if [ "$(grep -c '^stemsheadphasecontinue ' "$rows")" -ne 1 ] || \
        ! grep -q 'headOrder 94 headX 96 headSig 41 headInterId 1367 .*decisions \[LEFT:SkipAlreadyLinked,RIGHT:SkipClosed\].*incident \[stem2373:headSideLEFT:heads\[x96:sig41:id1367:sideLEFT,x97:sig34:id1353:sideLEFT\].*closureWrites \[x97:sig34:LEFT:false->true,x97:sig34:RIGHT:false->true\].*closedValueChanges 2 .*nextHeadOrder 95 nextHeadX 7 nextHeadSig 52 nextHeadInterId 1393 ' "$rows" || \
        ! grep -q '^stemsheadclinkfrontier .*headOrder 94 headX 96 headSig 41 .*cAlias h:96:LEFT:BOTTOM .*lastIndex 1 maxIndex 1 relations 2 .*glyphs 1 .*candidateIdBefore 321 existingGlyph glyph:321 existingActive true existingStem 2373 ' "$rows" || \
        ! grep -q '^stemsheadclinkresult headOrder 94 allocatorBefore 2385 allocatorAfter 2385 .*addedVertices - addedEdges - addedSystemStems - .*linkerChanges \[h:97:LEFT:BOTTOM:true:false->true:true' "$rows"; then
    echo "v94 existing-stem reconciliation contract differs" >&2
    exit 1
fi
: <<'DISABLED_V27_ACTIVE'
...
v93_runner_sha=$(shasum -a 256 "$script_dir/run-stems-head-phase-prefix-v93.sh" | awk '{print $1}')
if [ "$v93_runner_sha" != "11fc40b137e0adeea392789778c7340698bcca1176275dd6ba0eb430819fccad" ]; then
    echo "v94 base v93 runner drifted" >&2
    exit 1
fi
v93_fixture="$repo_root/rust/oracle/stems-head-phase-prefix-chula-system1-v93.txt"
v93_fixture_sha=$(shasum -a 256 "$v93_fixture" | awk '{print $1}')
if [ "$v93_fixture_sha" != "d42d1b84db222735c24771c2479e9f5dbdeca30f3bbbaf3bd33a3b016d2761e4" ]; then
    echo "v94 base v93 fixture drifted" >&2
    exit 1
fi
...
out="/private/tmp/stems-head-phase-prefix-chula-system1-v94-audit.txt"
{
    echo '# Java Audiveris 5.11 (Temurin JDK 25.0.3+9 LTS) post-STUMPS head phase v94.'
    echo '# schema: stems-head-phase-prefix-v94'
    echo '# Snapshot-minimized order-94 derivative: orders 1-93 mutate without snapshots; order 94 emits the existing-stem reconciliation.'
    echo '# This bounded scope is intentional for deterministic replay under the full-snapshot heap limit.'
    cat "$rows"
    printf '%s\n' \
        "stemsheadphaseprefix summary schema stems-head-phase-prefix-v94 page chula.png#1 system 1 rows $row_count baseProbeSourceSha256 $base_probe_sha baseV93RunnerSourceSha256 $v93_runner_sha baseV93FixtureSha256 $v93_fixture_sha fragmentSourceSha256 $fragment_sha probeSourceSha256 $probe_sha runnerSourceSha256 $runner_sha emittedBodySha256 $body_sha semanticPassSha256 $semantic_sha completeStumpsFixtureSha256 $stumps_sha freshRuns 2 freshRunsByteIdentical true nativeScope BoundedSnapshotMinimizedOrder94ExistingStemReconciliation javaEvidence ReturnedBeforeNinetyFifthHead"
} > "$out"
echo "wrote $out"
