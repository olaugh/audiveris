...
probe_classes=$(mktemp -d /private/tmp/black-head-sizer-classes.XXXXXX)
...
"$JAVA_HOME/bin/javac" \
    -cp "$probe_cp" \
    -d "$probe_classes" \
    "$repo_root/rust/oracle/java/BlackHeadSizerProbe.java"
...
run_probe ()
{
    ...
    (
        ...
        env -u JAVA_TOOL_OPTIONS "$JAVA_HOME/bin/java" \
            ...
            org.audiveris.omr.rustport.BlackHeadSizerProbe "$target"
    )
}

env -u JAVA_TOOL_OPTIONS "$JAVA_HOME/bin/java" \
    -cp "$probe_classes:$probe_cp" \
    org.audiveris.omr.rustport.BlackHeadSizerProbe --header
