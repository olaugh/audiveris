...
# Read-only full phase-2 append-retry sweep from v102; prior orders mutate without snapshots.
set -eu
...
tmp_base=/private/tmp/stems-head-phase-prefix-v103
...
v102_runner_sha=$(shasum -a 256 "$script_dir/run-stems-head-phase-prefix-v102.sh" | awk '{print $1}')
if [ "$v102_runner_sha" != "47c6b327c89b67d67b2de1d5d0ae27e3aea9ea7652549a1ecc7a8b9956425b81" ]; then
    echo "v103 base v102 runner drifted" >&2
    exit 1
fi
v102_fixture="$repo_root/rust/oracle/stems-head-phase-prefix-chula-system1-v102.txt"
v102_fixture_sha=$(shasum -a 256 "$v102_fixture" | awk '{print $1}')
if [ "$v102_fixture_sha" != "f3ba43f3b9808b9e8303b180399f94e21af32490e416243ae9807c772debfa27" ]; then
    echo "v103 base v102 fixture drifted" >&2
    exit 1
fi
...
out="/private/tmp/stems-head-phase-prefix-chula-system1-v103-audit.txt"
{
    echo '# Java Audiveris 5.11 (Temurin JDK 25.0.3+9 LTS) post-STUMPS head phase v103.'
    echo '# schema: stems-head-phase-prefix-v103'
    ...
    cat "$rows"
    printf '%s\n' \
        "stemsheadphaseprefix summary schema stems-head-phase-prefix-v103 page chula.png#1 system 1 rows $row_count baseProbeSourceSha256 $base_probe_sha baseV102RunnerSourceSha256 $v102_runner_sha baseV102FixtureSha256 $v102_fixture_sha fragmentSourceSha256 $fragment_sha probeSourceSha256 $probe_sha runnerSourceSha256 $runner_sha emittedBodySha256 $body_sha semanticPassSha256 $semantic_sha completeStumpsFixtureSha256 $stumps_sha freshRuns 2 freshRunsByteIdentical true nativeScope BoundedSnapshotMinimizedPhaseTwoFullAppendRetrySweep javaEvidence ReturnedAfterFinalAppendRetry"
} > "$out"
echo "wrote $out"
