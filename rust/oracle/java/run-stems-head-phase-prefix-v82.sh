...
# Read-only snapshot-minimized order82 continuation from v81; prior orders mutate without snapshots.
set -eu
...
tmp_base=/private/tmp/stems-head-phase-prefix-v82
...
awk -v fragment="$fragment18" '
{
    ...
    if (capture_after && index($0, "heads, allLinkers);") != 0) {
        print
        print "            if (headOrder == 7 || headOrder == 22 || headOrder == 27 || headOrder == 34 || headOrder == 36 || headOrder == 37 || headOrder == 38 || headOrder == 39 || headOrder == 40 || headOrder == 41 || headOrder == 42 || headOrder == 43 || headOrder == 44 || headOrder == 45 || headOrder == 46 || headOrder == 47 || headOrder == 48 || headOrder == 49 || headOrder == 50 || headOrder == 51 || headOrder == 52 || headOrder == 53 || headOrder == 54 || headOrder == 55 || headOrder == 56 || headOrder == 57 || headOrder == 58 || headOrder == 59 || headOrder == 60 || headOrder == 61 || headOrder == 62 || headOrder == 63 || headOrder == 64 || headOrder == 65 || headOrder == 66 || headOrder == 67 || headOrder == 68 || headOrder == 69 || headOrder == 70 || headOrder == 71 || headOrder == 72 || headOrder == 73 || headOrder == 74 || headOrder == 75 || headOrder == 76 || headOrder == 77 || headOrder == 78 || headOrder == 79 || headOrder == 80 || headOrder == 81 || headOrder == 82) emitHeadCLinkResult(before, after, headOrder);"
        capture_after = 0
        in_linker = 0
        next
    }
...
if [ "$(grep -c '^stemsheadphasecontinue ' "$rows")" -ne 1 ] || \
        ! grep -q 'headOrder 82 headX 80 headSig 32 headInterId 1349 .*decisions \[LEFT:SkipAlreadyLinked,RIGHT:SkipClosed\].*incident \[stem2371:headSideLEFT:heads\[x79:sig40:id1365:sideLEFT,x80:sig32:id1349:sideLEFT,x81:sig33:id1351:sideLEFT\].*closureWrites \[x79:sig40:LEFT:true->true,x79:sig40:RIGHT:true->true,x81:sig33:LEFT:false->true,x81:sig33:RIGHT:false->true\].*closedValueChanges 2 .*nextHeadOrder 83 nextHeadX 24 nextHeadSig 90 nextHeadInterId 1469 ' "$rows" || \
        ! grep -q '^stemsheadclinkfrontier .*headOrder 82 headX 80 headSig 32 .*cAlias h:80:LEFT:BOTTOM .*lastIndex 0 maxIndex 0 relations 1 .*glyphs 1 .*candidateIdBefore 306 existingGlyph glyph:306 existingActive true existingStem 2371 ' "$rows" || \
        ! grep -q '^stemsheadclinkresult headOrder 82 allocatorBefore 2385 allocatorAfter 2385 .*addedVertices - addedEdges - addedSystemStems - .*linkerChanges \[h:81:LEFT:BOTTOM:true:false->true:true' "$rows"; then
    echo "v82 three-head existing-stem reconciliation contract differs" >&2
    exit 1
fi
: <<'DISABLED_V27_ACTIVE'
...
v81_runner_sha=$(shasum -a 256 "$script_dir/run-stems-head-phase-prefix-v81.sh" | awk '{print $1}')
if [ "$v81_runner_sha" != "6bc0373119f11d790c9010aa66e201e571f95b0a2738c4607669b0e723fb8000" ]; then
    echo "v82 base v81 runner drifted" >&2
    exit 1
fi
v81_fixture="$repo_root/rust/oracle/stems-head-phase-prefix-chula-system1-v81.txt"
v81_fixture_sha=$(shasum -a 256 "$v81_fixture" | awk '{print $1}')
if [ "$v81_fixture_sha" != "5b4f89b4cb52fd0075e176a4c6dfe1c78571bc1403b9602cc716c02d63c7488f" ]; then
    echo "v82 base v81 fixture drifted" >&2
    exit 1
fi
...
out="/private/tmp/stems-head-phase-prefix-chula-system1-v82-audit.txt"
{
    echo '# Java Audiveris 5.11 (Temurin JDK 25.0.3+9 LTS) post-STUMPS head phase v82.'
    echo '# schema: stems-head-phase-prefix-v82'
    echo '# Snapshot-minimized order-82 derivative: orders 1-81 mutate without snapshots; order 82 emits the three-head existing-stem reconciliation.'
    echo '# This bounded scope is intentional for deterministic replay under the full-snapshot heap limit.'
    cat "$rows"
    printf '%s\n' \
        "stemsheadphaseprefix summary schema stems-head-phase-prefix-v82 page chula.png#1 system 1 rows $row_count baseProbeSourceSha256 $base_probe_sha baseV81RunnerSourceSha256 $v81_runner_sha baseV81FixtureSha256 $v81_fixture_sha fragmentSourceSha256 $fragment_sha probeSourceSha256 $probe_sha runnerSourceSha256 $runner_sha emittedBodySha256 $body_sha semanticPassSha256 $semantic_sha completeStumpsFixtureSha256 $stumps_sha freshRuns 2 freshRunsByteIdentical true nativeScope BoundedSnapshotMinimizedOrder82ExistingStemReconciliation javaEvidence ReturnedBeforeEightyThirdHead"
} > "$out"
echo "wrote $out"
