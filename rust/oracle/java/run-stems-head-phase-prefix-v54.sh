...
# Read-only snapshot-minimized order54 continuation from v53; prior orders mutate without snapshots.
set -eu
...
tmp_base=/private/tmp/stems-head-phase-prefix-v54
...
awk -v fragment="$fragment18" '
{
    ...
    if (index($0, "final boolean returned = linker.linkSides(") != 0) {
        print "            if (headOrder == 7 || headOrder == 22 || headOrder == 27 || headOrder == 34 || headOrder == 36 || headOrder == 37 || headOrder == 38 || headOrder == 39 || headOrder == 40 || headOrder == 41 || headOrder == 42 || headOrder == 43 || headOrder == 44 || headOrder == 45 || headOrder == 46 || headOrder == 47 || headOrder == 48 || headOrder == 49 || headOrder == 50 || headOrder == 51 || headOrder == 52 || headOrder == 53 || headOrder == 54) emitHeadCLinkEnvelope(head, linker, before, headOrder);"
        print
        in_linker = 1
        next
    }
    ...
    if (capture_after && index($0, "heads, allLinkers);") != 0) {
        print
        print "            if (headOrder == 7 || headOrder == 22 || headOrder == 27 || headOrder == 34 || headOrder == 36 || headOrder == 37 || headOrder == 38 || headOrder == 39 || headOrder == 40 || headOrder == 41 || headOrder == 42 || headOrder == 43 || headOrder == 44 || headOrder == 45 || headOrder == 46 || headOrder == 47 || headOrder == 48 || headOrder == 49 || headOrder == 50 || headOrder == 51 || headOrder == 52 || headOrder == 53 || headOrder == 54) emitHeadCLinkResult(before, after, headOrder);"
        capture_after = 0
        in_linker = 0
        next
    }
...
if [ "$(grep -c '^stemsheadphasecontinue ' "$rows")" -ne 1 ] || \
        ! grep -q 'headOrder 54 headX 11 headSig 62 headInterId 1413 .*decisions \[LEFT:SkipAlreadyLinked,RIGHT:SkipClosed\].*incident \[stem2349:headSideLEFT:heads\[x11:sig62:id1413:sideLEFT,x12:sig63:id1415:sideLEFT\].*closureWrites \[x12:sig63:LEFT:false->true,x12:sig63:RIGHT:false->true\].*closedValueChanges 2 .*nextHeadOrder 55 nextHeadX 68 nextHeadSig 75 nextHeadInterId 1439 ' "$rows" || \
        ! grep -q '^stemsheadclinkfrontier .*headOrder 54 headX 11 headSig 62 .*cAlias h:11:LEFT:BOTTOM .*lastIndex 4 maxIndex 4 relations 4 .*glyphs 2 .*candidateIdBefore 0 existingGlyph glyph:312 existingActive true existingStem 2349 ' "$rows" || \
        ! grep -q '^stemsheadclinkresult headOrder 54 allocatorBefore 2385 allocatorAfter 2385 .*addedVertices - addedEdges - addedSystemStems - .*linkerChanges \[h:12:LEFT:BOTTOM:true:false->true:true' "$rows"; then
    echo "v54 existing-stem reconciliation contract differs" >&2
    exit 1
fi
: <<'DISABLED_V27_ACTIVE'
...
v53_runner_sha=$(shasum -a 256 "$script_dir/run-stems-head-phase-prefix-v53.sh" | awk '{print $1}')
if [ "$v53_runner_sha" != "cdf67c2e57e5afcbb7e7030d0cf80a2ce50302778032154b5ced066f234a2611" ]; then
    echo "v54 base v53 runner drifted" >&2
    exit 1
fi
v53_fixture="$repo_root/rust/oracle/stems-head-phase-prefix-chula-system1-v53.txt"
v53_fixture_sha=$(shasum -a 256 "$v53_fixture" | awk '{print $1}')
if [ "$v53_fixture_sha" != "58671e0a19695e626a633a8963683d310083e3bb18ee9d419a69d1db0267be76" ]; then
    echo "v54 base v53 fixture drifted" >&2
    exit 1
fi
...
out="/private/tmp/stems-head-phase-prefix-chula-system1-v54-audit.txt"
{
    echo '# Java Audiveris 5.11 (Temurin JDK 25.0.3+9 LTS) post-STUMPS head phase v54.'
    echo '# schema: stems-head-phase-prefix-v54'
    echo '# Snapshot-minimized order-54 derivative: orders 1-53 mutate without snapshots; order 54 emits existing-stem reconciliation.'
    echo '# This bounded scope is intentional for deterministic replay under the full-snapshot heap limit.'
    cat "$rows"
    printf '%s\n' \
        "stemsheadphaseprefix summary schema stems-head-phase-prefix-v54 page chula.png#1 system 1 rows $row_count baseProbeSourceSha256 $base_probe_sha baseV53RunnerSourceSha256 $v53_runner_sha baseV53FixtureSha256 $v53_fixture_sha fragmentSourceSha256 $fragment_sha probeSourceSha256 $probe_sha runnerSourceSha256 $runner_sha emittedBodySha256 $body_sha semanticPassSha256 $semantic_sha completeStumpsFixtureSha256 $stumps_sha freshRuns 2 freshRunsByteIdentical true nativeScope BoundedSnapshotMinimizedOrder54ExistingStemReconciliation javaEvidence ReturnedBeforeFiftyFifthHead"
} > "$out"
echo "wrote $out"
