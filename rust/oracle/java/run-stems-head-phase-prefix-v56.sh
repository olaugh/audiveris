...
# Read-only snapshot-minimized order56 continuation from v55; prior orders mutate without snapshots.
set -eu
...
tmp_base=/private/tmp/stems-head-phase-prefix-v56
...
awk -v fragment="$fragment18" '
{
    ...
    if (index($0, "final boolean returned = linker.linkSides(") != 0) {
        print "            if (headOrder == 7 || headOrder == 22 || headOrder == 27 || headOrder == 34 || headOrder == 36 || headOrder == 37 || headOrder == 38 || headOrder == 39 || headOrder == 40 || headOrder == 41 || headOrder == 42 || headOrder == 43 || headOrder == 44 || headOrder == 45 || headOrder == 46 || headOrder == 47 || headOrder == 48 || headOrder == 49 || headOrder == 50 || headOrder == 51 || headOrder == 52 || headOrder == 53 || headOrder == 54 || headOrder == 55 || headOrder == 56) emitHeadCLinkEnvelope(head, linker, before, headOrder);"
        print
        in_linker = 1
        next
    }
    ...
    if (capture_after && index($0, "heads, allLinkers);") != 0) {
        print
        print "            if (headOrder == 7 || headOrder == 22 || headOrder == 27 || headOrder == 34 || headOrder == 36 || headOrder == 37 || headOrder == 38 || headOrder == 39 || headOrder == 40 || headOrder == 41 || headOrder == 42 || headOrder == 43 || headOrder == 44 || headOrder == 45 || headOrder == 46 || headOrder == 47 || headOrder == 48 || headOrder == 49 || headOrder == 50 || headOrder == 51 || headOrder == 52 || headOrder == 53 || headOrder == 54 || headOrder == 55 || headOrder == 56) emitHeadCLinkResult(before, after, headOrder);"
        capture_after = 0
        in_linker = 0
        next
    }
...
if [ "$(grep -c '^stemsheadphasecontinue ' "$rows")" -ne 1 ] || \
        ! grep -q 'headOrder 56 headX 21 headSig 11 headInterId 1307 .*decisions \[LEFT:SkipAlreadyLinked,RIGHT:SkipClosed\].*incident \[stem2341:headSideLEFT:heads\[x21:sig11:id1307:sideLEFT,x22:sig12:id1309:sideLEFT\].*closureWrites \[x22:sig12:LEFT:false->true,x22:sig12:RIGHT:false->true\].*closedValueChanges 2 .*nextHeadOrder 57 nextHeadX 62 nextHeadSig 16 nextHeadInterId 1317 ' "$rows" || \
        ! grep -q '^stemsheadclinkfrontier .*headOrder 56 headX 21 headSig 11 .*cAlias h:21:LEFT:BOTTOM .*lastIndex 3 maxIndex 3 relations 4 .*glyphs 1 .*candidateIdBefore 323 existingGlyph glyph:323 existingActive true existingStem 2341 ' "$rows" || \
        ! grep -q '^stemsheadclinkresult headOrder 56 allocatorBefore 2385 allocatorAfter 2385 .*addedVertices - addedEdges - addedSystemStems - .*linkerChanges \[h:22:LEFT:BOTTOM:true:false->true:true' "$rows"; then
    echo "v56 existing-stem reconciliation contract differs" >&2
    exit 1
fi
: <<'DISABLED_V27_ACTIVE'
...
v55_runner_sha=$(shasum -a 256 "$script_dir/run-stems-head-phase-prefix-v55.sh" | awk '{print $1}')
if [ "$v55_runner_sha" != "8dfffd31e6db65348598433cc0e19683d80be55705e9d843bd88e564deb5ca67" ]; then
    echo "v56 base v55 runner drifted" >&2
    exit 1
fi
v55_fixture="$repo_root/rust/oracle/stems-head-phase-prefix-chula-system1-v55.txt"
v55_fixture_sha=$(shasum -a 256 "$v55_fixture" | awk '{print $1}')
if [ "$v55_fixture_sha" != "8ed542577f68000705cd7d166c8d848e48c138ae9f6e8fad0de10a499e46c0ff" ]; then
    echo "v56 base v55 fixture drifted" >&2
    exit 1
fi
...
out="/private/tmp/stems-head-phase-prefix-chula-system1-v56-audit.txt"
{
    echo '# Java Audiveris 5.11 (Temurin JDK 25.0.3+9 LTS) post-STUMPS head phase v56.'
    echo '# schema: stems-head-phase-prefix-v56'
    echo '# Snapshot-minimized order-56 derivative: orders 1-55 mutate without snapshots; order 56 emits existing-stem reconciliation.'
    echo '# This bounded scope is intentional for deterministic replay under the full-snapshot heap limit.'
    cat "$rows"
    printf '%s\n' \
        "stemsheadphaseprefix summary schema stems-head-phase-prefix-v56 page chula.png#1 system 1 rows $row_count baseProbeSourceSha256 $base_probe_sha baseV55RunnerSourceSha256 $v55_runner_sha baseV55FixtureSha256 $v55_fixture_sha fragmentSourceSha256 $fragment_sha probeSourceSha256 $probe_sha runnerSourceSha256 $runner_sha emittedBodySha256 $body_sha semanticPassSha256 $semantic_sha completeStumpsFixtureSha256 $stumps_sha freshRuns 2 freshRunsByteIdentical true nativeScope BoundedSnapshotMinimizedOrder56ExistingStemReconciliation javaEvidence ReturnedBeforeFiftySeventhHead"
} > "$out"
echo "wrote $out"
