...
# Freeze a snapshot-minimized order44 existing-stem reconciliation from v43; prior orders mutate without snapshots.
set -eu
...
tmp_base=/private/tmp/stems-head-phase-prefix-v44
...
awk -v fragment="$fragment18" '
{
    ...
    if (index($0, "final boolean returned = linker.linkSides(") != 0) {
        print "            if (headOrder == 7 || headOrder == 22 || headOrder == 27 || headOrder == 34 || headOrder == 36 || headOrder == 37 || headOrder == 38 || headOrder == 39 || headOrder == 40 || headOrder == 41 || headOrder == 42 || headOrder == 43 || headOrder == 44) emitHeadCLinkEnvelope(head, linker, before, headOrder);"
        print
        in_linker = 1
        next
    }
    ...
    if (capture_after && index($0, "heads, allLinkers);") != 0) {
        print
        print "            if (headOrder == 7 || headOrder == 22 || headOrder == 27 || headOrder == 34 || headOrder == 36 || headOrder == 37 || headOrder == 38 || headOrder == 39 || headOrder == 40 || headOrder == 41 || headOrder == 42 || headOrder == 43 || headOrder == 44) emitHeadCLinkResult(before, after, headOrder);"
        capture_after = 0
        in_linker = 0
        next
    }
...
if [ "$(grep -c '^stemsheadphasecontinue ' "$rows")" -ne 1 ] || \
        ! grep -q 'headOrder 44 headX 83 headSig 21 headInterId 1327 .*decisions \[LEFT:SkipAlreadyLinked,RIGHT:top=false:bottom=false:branch=Neither\].*incident \[stem2358:headSideLEFT:heads\[x82:sig20:id1325:sideLEFT,x83:sig21:id1327:sideLEFT\].*closureWrites \[x82:sig20:LEFT:false->true,x82:sig20:RIGHT:false->true\].*closedValueChanges 2 .*nextHeadOrder 45 nextHeadX 57 nextHeadSig 5 nextHeadInterId 1295 ' "$rows" || \
        ! grep -q '^stemsheadclinkfrontier .*headOrder 44 headX 83 headSig 21 .*cAlias h:83:LEFT:BOTTOM .*lastIndex 2 maxIndex 2 relations 2 .*glyphs 2 .*candidateIdBefore 0 existingGlyph glyph:301 existingActive true existingStem 2358 ' "$rows" || \
        ! grep -q '^stemsheadclinkresult headOrder 44 allocatorBefore 2385 allocatorAfter 2385 .*addedVertices - addedEdges - addedSystemStems - .*linkerChanges \[h:82:LEFT:BOTTOM:true:false->true:true' "$rows"; then
    echo "v44 existing-stem reconciliation contract differs" >&2
    exit 1
fi
: <<'DISABLED_V27_ACTIVE'
...
v43_runner_sha=$(shasum -a 256 "$script_dir/run-stems-head-phase-prefix-v43.sh" | awk '{print $1}')
if [ "$v43_runner_sha" != "421c8cd9b3a9208b509ce511077c6656faeac230212deff7ab797f6ffec73d75" ]; then
    echo "v44 base v43 runner drifted" >&2
    exit 1
fi
v43_fixture="$repo_root/rust/oracle/stems-head-phase-prefix-chula-system1-v43.txt"
v43_fixture_sha=$(shasum -a 256 "$v43_fixture" | awk '{print $1}')
if [ "$v43_fixture_sha" != "dc5f7ce12d292a13cc149e7df0249703323df92de9054daf5eff52783b32919d" ]; then
    echo "v44 base v43 fixture drifted" >&2
    exit 1
fi
...
out="/private/tmp/stems-head-phase-prefix-chula-system1-v44-audit.txt"
{
    echo '# Java Audiveris 5.11 (Temurin JDK 25.0.3+9 LTS) post-STUMPS head phase v44.'
    echo '# schema: stems-head-phase-prefix-v44'
    echo '# Snapshot-minimized order-44 derivative: orders 1-43 mutate without snapshots; order 44 emits existing-stem reconciliation.'
    echo '# This bounded scope is intentional for deterministic replay under the full-snapshot heap limit.'
    cat "$rows"
    printf '%s\n' \
        "stemsheadphaseprefix summary schema stems-head-phase-prefix-v44 page chula.png#1 system 1 rows $row_count baseProbeSourceSha256 $base_probe_sha baseV43RunnerSourceSha256 $v43_runner_sha baseV43FixtureSha256 $v43_fixture_sha fragmentSourceSha256 $fragment_sha probeSourceSha256 $probe_sha runnerSourceSha256 $runner_sha emittedBodySha256 $body_sha semanticPassSha256 $semantic_sha completeStumpsFixtureSha256 $stumps_sha freshRuns 2 freshRunsByteIdentical true nativeScope BoundedSnapshotMinimizedOrder44ExistingStemReconciliation javaEvidence ReturnedBeforeFortyFifthHead"
} > "$out"
echo "wrote $out"
