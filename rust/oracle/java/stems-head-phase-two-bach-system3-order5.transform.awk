...
# Retarget the authenticated x14 C-link instrumentation to Bach system 3
# queue 5 / Java Inter 4152.
function emit_after_reuse() {
    print "                    if (append && head.getId() == 4152) {"
    print "                        System.out.printf("
    print "                                \"stemsheadphase2bachs3q5reuse headInterId %d lastIndex %d selectedStem %s terminal ReturnedFromReuseStem%n\","
    print "                                head.getId(), lastIndex, rustStem(stem));"
    print "                    }"
}

function emit_reuse_match() {
    print "                            if (head.getId() == 4152) {"
    print "                                final StemInter rustReuse = (StemInter) sig.getOppositeInter(h, r);"
    print "                                System.out.printf("
    print "                                        \"stemsheadphase2bachs3q5reusematch headInterId %d sourceHeadId %d sourceCorner %s sourceSide %s relationGrade %s stem %s terminal SelectedReuseStem%n\","
...
function emit_committed_result() {
    print "                if (append && head.getId() == 4152) {"
    print "                    System.out.printf("
    print "                            \"stemsheadphase2bachs3q5commit headInterId %d linkedStem %s relationRows %s vertices %d edges %d allocator %d terminal ReturnedHeadCLinkTransaction%n\","
...
{
    gsub("head.getId[(][)] == 1777", "head.getId() == 4152")
    gsub("stemsheadphase2x14", "stemsheadphase2bachs3q5")
