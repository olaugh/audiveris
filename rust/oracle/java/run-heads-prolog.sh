...
probe_classes=$(mktemp -d /private/tmp/heads-prolog-classes.XXXXXX)
...
"$JAVA_HOME/bin/javac" \
    -cp "$probe_cp" \
    -d "$probe_classes" \
    "$repo_root/rust/oracle/java/HeadsPrologProbe.java"
...
run_probe ()
{
    ...
    (
        ...
        env -u JAVA_TOOL_OPTIONS "$JAVA_HOME/bin/java" \
            ...
            org.audiveris.omr.rustport.HeadsPrologProbe "$target"
    )
}

env -u JAVA_TOOL_OPTIONS "$JAVA_HOME/bin/java" \
    -cp "$probe_classes:$probe_cp" \
    org.audiveris.omr.rustport.HeadsPrologProbe --header
