...
# Read-only snapshot-minimized order73 continuation from v72; prior orders mutate without snapshots.
set -eu
...
tmp_base=/private/tmp/stems-head-phase-prefix-v73
...
awk -v fragment="$fragment18" '
{
    ...
    if (capture_after && index($0, "heads, allLinkers);") != 0) {
        print
        print "            if (headOrder == 7 || headOrder == 22 || headOrder == 27 || headOrder == 34 || headOrder == 36 || headOrder == 37 || headOrder == 38 || headOrder == 39 || headOrder == 40 || headOrder == 41 || headOrder == 42 || headOrder == 43 || headOrder == 44 || headOrder == 45 || headOrder == 46 || headOrder == 47 || headOrder == 48 || headOrder == 49 || headOrder == 50 || headOrder == 51 || headOrder == 52 || headOrder == 53 || headOrder == 54 || headOrder == 55 || headOrder == 56 || headOrder == 57 || headOrder == 58 || headOrder == 59 || headOrder == 60 || headOrder == 61 || headOrder == 62 || headOrder == 63 || headOrder == 64 || headOrder == 65 || headOrder == 66 || headOrder == 67 || headOrder == 68 || headOrder == 69 || headOrder == 70 || headOrder == 71 || headOrder == 72 || headOrder == 73) emitHeadCLinkResult(before, after, headOrder);"
        capture_after = 0
        in_linker = 0
        next
    }
...
if [ "$(grep -c '^stemsheadphasecontinue ' "$rows")" -ne 1 ] || \
        ! grep -q 'headOrder 73 headX 75 headSig 96 headInterId 1481 .*decisions \[LEFT:top=false:bottom=true:branch=BottomOnly,RIGHT:top=false:bottom=false:branch=Neither\].*incident - returned true .*undefs \[\].*closureWrites - closedValueChanges 0 .*sigEdgesBefore 703 sigEdgesAfter 705 .*nextHeadOrder 74 nextHeadX 49 nextHeadSig 71 nextHeadInterId 1431 ' "$rows" || \
        ! grep -q '^stemsheadclinkfrontier .*headOrder 73 headX 75 headSig 96 .*cAlias h:75:LEFT:BOTTOM .*lastIndex 1 maxIndex 1 relations 2 .*glyphs 1 .*candidateIdBefore 319 existingGlyph glyph:319 existingActive true existingStem 2380 ' "$rows" || \
        ! grep -q '^stemsheadclinkresult headOrder 73 allocatorBefore 2385 allocatorAfter 2385 .*addedVertices - addedEdges \[system1:sourceId1481:targetId2380:org.audiveris.omr.sig.relation.HeadStemRelation.*addedSystemStems - linkerChanges \[h:72:LEFT:BOTTOM:false:false->true:true' "$rows"; then
    echo "v73 multi-head reuse C-link contract differs" >&2
    exit 1
fi
: <<'DISABLED_V27_ACTIVE'
...
v72_runner_sha=$(shasum -a 256 "$script_dir/run-stems-head-phase-prefix-v72.sh" | awk '{print $1}')
if [ "$v72_runner_sha" != "7d38e49da6221953158fde53a699b9436cb01fcf757f0dbce991a1672457eba7" ]; then
    echo "v73 base v72 runner drifted" >&2
    exit 1
fi
v72_fixture="$repo_root/rust/oracle/stems-head-phase-prefix-chula-system1-v72.txt"
v72_fixture_sha=$(shasum -a 256 "$v72_fixture" | awk '{print $1}')
if [ "$v72_fixture_sha" != "e7f02765e22b98dabd98a76f908b344a97fdec3d51b91886e0c795834a4085b6" ]; then
    echo "v73 base v72 fixture drifted" >&2
    exit 1
fi
...
out="/private/tmp/stems-head-phase-prefix-chula-system1-v73-audit.txt"
{
    echo '# Java Audiveris 5.11 (Temurin JDK 25.0.3+9 LTS) post-STUMPS head phase v73.'
    echo '# schema: stems-head-phase-prefix-v73'
    echo '# Snapshot-minimized order-73 derivative: orders 1-72 mutate without snapshots; order 73 emits the multi-head existing-stem C-link.'
    echo '# This bounded scope is intentional for deterministic replay under the full-snapshot heap limit.'
    cat "$rows"
    printf '%s\n' \
        "stemsheadphaseprefix summary schema stems-head-phase-prefix-v73 page chula.png#1 system 1 rows $row_count baseProbeSourceSha256 $base_probe_sha baseV72RunnerSourceSha256 $v72_runner_sha baseV72FixtureSha256 $v72_fixture_sha fragmentSourceSha256 $fragment_sha probeSourceSha256 $probe_sha runnerSourceSha256 $runner_sha emittedBodySha256 $body_sha semanticPassSha256 $semantic_sha completeStumpsFixtureSha256 $stumps_sha freshRuns 2 freshRunsByteIdentical true nativeScope BoundedSnapshotMinimizedOrder73MultiHeadReuseCLink javaEvidence ReturnedBeforeSeventyFourthHead"
} > "$out"
echo "wrote $out"
