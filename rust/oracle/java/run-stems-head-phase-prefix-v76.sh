...
# Read-only snapshot-minimized order76 continuation from v75; prior orders mutate without snapshots.
set -eu
...
tmp_base=/private/tmp/stems-head-phase-prefix-v76
...
awk -v fragment="$fragment18" '
{
    ...
    if (capture_after && index($0, "heads, allLinkers);") != 0) {
        print
        print "            if (headOrder == 7 || headOrder == 22 || headOrder == 27 || headOrder == 34 || headOrder == 36 || headOrder == 37 || headOrder == 38 || headOrder == 39 || headOrder == 40 || headOrder == 41 || headOrder == 42 || headOrder == 43 || headOrder == 44 || headOrder == 45 || headOrder == 46 || headOrder == 47 || headOrder == 48 || headOrder == 49 || headOrder == 50 || headOrder == 51 || headOrder == 52 || headOrder == 53 || headOrder == 54 || headOrder == 55 || headOrder == 56 || headOrder == 57 || headOrder == 58 || headOrder == 59 || headOrder == 60 || headOrder == 61 || headOrder == 62 || headOrder == 63 || headOrder == 64 || headOrder == 65 || headOrder == 66 || headOrder == 67 || headOrder == 68 || headOrder == 69 || headOrder == 70 || headOrder == 71 || headOrder == 72 || headOrder == 73 || headOrder == 74 || headOrder == 75 || headOrder == 76) emitHeadCLinkResult(before, after, headOrder);"
        capture_after = 0
        in_linker = 0
        next
    }
...
if [ "$(grep -c '^stemsheadphasecontinue ' "$rows")" -ne 1 ] || \
        ! grep -q 'headOrder 76 headX 66 headSig 58 headInterId 1405 .*decisions \[LEFT:SkipAlreadyLinked,RIGHT:SkipClosed\].*incident \[stem2375:headSideLEFT:heads\[x66:sig58:id1405:sideLEFT,x67:sig59:id1407:sideLEFT\].*closureWrites \[x67:sig59:LEFT:false->true,x67:sig59:RIGHT:false->true\].*closedValueChanges 2 .*nextHeadOrder 77 nextHeadX 64 nextHeadSig 94 nextHeadInterId 1477 ' "$rows" || \
        ! grep -q '^stemsheadclinkfrontier .*headOrder 76 headX 66 headSig 58 .*cAlias h:66:LEFT:BOTTOM .*lastIndex 2 maxIndex 2 relations 2 .*glyphs 2 .*candidateIdBefore 0 existingGlyph glyph:308 existingActive true existingStem 2375 ' "$rows" || \
        ! grep -q '^stemsheadclinkresult headOrder 76 allocatorBefore 2385 allocatorAfter 2385 .*addedVertices - addedEdges - addedSystemStems - .*linkerChanges \[h:67:LEFT:BOTTOM:true:false->true:true' "$rows"; then
    echo "v76 existing-stem reconciliation contract differs" >&2
    exit 1
fi
: <<'DISABLED_V27_ACTIVE'
...
v75_runner_sha=$(shasum -a 256 "$script_dir/run-stems-head-phase-prefix-v75.sh" | awk '{print $1}')
if [ "$v75_runner_sha" != "6535910b7379860aa4766e955fb5d4814d8f44e9841a020546409008195e3bb4" ]; then
    echo "v76 base v75 runner drifted" >&2
    exit 1
fi
v75_fixture="$repo_root/rust/oracle/stems-head-phase-prefix-chula-system1-v75.txt"
v75_fixture_sha=$(shasum -a 256 "$v75_fixture" | awk '{print $1}')
if [ "$v75_fixture_sha" != "890b3f32fadbef6085917d7f6c160e439bd31ac13d2e36ced82eaed5ccaf1c4d" ]; then
    echo "v76 base v75 fixture drifted" >&2
    exit 1
fi
...
out="/private/tmp/stems-head-phase-prefix-chula-system1-v76-audit.txt"
{
    echo '# Java Audiveris 5.11 (Temurin JDK 25.0.3+9 LTS) post-STUMPS head phase v76.'
    echo '# schema: stems-head-phase-prefix-v76'
    echo '# Snapshot-minimized order-76 derivative: orders 1-75 mutate without snapshots; order 76 emits the existing-stem reconciliation.'
    echo '# This bounded scope is intentional for deterministic replay under the full-snapshot heap limit.'
    cat "$rows"
    printf '%s\n' \
        "stemsheadphaseprefix summary schema stems-head-phase-prefix-v76 page chula.png#1 system 1 rows $row_count baseProbeSourceSha256 $base_probe_sha baseV75RunnerSourceSha256 $v75_runner_sha baseV75FixtureSha256 $v75_fixture_sha fragmentSourceSha256 $fragment_sha probeSourceSha256 $probe_sha runnerSourceSha256 $runner_sha emittedBodySha256 $body_sha semanticPassSha256 $semantic_sha completeStumpsFixtureSha256 $stumps_sha freshRuns 2 freshRunsByteIdentical true nativeScope BoundedSnapshotMinimizedOrder76ExistingStemReconciliation javaEvidence ReturnedBeforeSeventySeventhHead"
} > "$out"
echo "wrote $out"
