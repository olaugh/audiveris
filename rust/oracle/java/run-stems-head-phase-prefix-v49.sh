...
# Read-only snapshot-minimized order49 continuation from v48; prior orders mutate without snapshots.
set -eu
...
tmp_base=/private/tmp/stems-head-phase-prefix-v49
...
awk -v fragment="$fragment18" '
{
    ...
    if (index($0, "final boolean returned = linker.linkSides(") != 0) {
        print "            if (headOrder == 7 || headOrder == 22 || headOrder == 27 || headOrder == 34 || headOrder == 36 || headOrder == 37 || headOrder == 38 || headOrder == 39 || headOrder == 40 || headOrder == 41 || headOrder == 42 || headOrder == 43 || headOrder == 44 || headOrder == 45 || headOrder == 46 || headOrder == 47 || headOrder == 48 || headOrder == 49) emitHeadCLinkEnvelope(head, linker, before, headOrder);"
        print
        in_linker = 1
        next
    }
    ...
    if (capture_after && index($0, "heads, allLinkers);") != 0) {
        print
        print "            if (headOrder == 7 || headOrder == 22 || headOrder == 27 || headOrder == 34 || headOrder == 36 || headOrder == 37 || headOrder == 38 || headOrder == 39 || headOrder == 40 || headOrder == 41 || headOrder == 42 || headOrder == 43 || headOrder == 44 || headOrder == 45 || headOrder == 46 || headOrder == 47 || headOrder == 48 || headOrder == 49) emitHeadCLinkResult(before, after, headOrder);"
        capture_after = 0
        in_linker = 0
        next
    }
...
if [ "$(grep -c '^stemsheadphasecontinue ' "$rows")" -ne 1 ] || \
        ! grep -q 'headOrder 49 headX 35 headSig 68 headInterId 1425 .*decisions \[LEFT:SkipAlreadyLinked,RIGHT:SkipClosed\].*incident \[stem2369:headSideLEFT:heads\[x35:sig68:id1425:sideLEFT,x36:sig69:id1427:sideLEFT\].*closureWrites \[x36:sig69:LEFT:false->true,x36:sig69:RIGHT:false->true\].*closedValueChanges 2 .*nextHeadOrder 50 nextHeadX 32 nextHeadSig 50 nextHeadInterId 1389 ' "$rows" || \
        ! grep -q '^stemsheadclinkfrontier .*headOrder 49 headX 35 headSig 68 .*cAlias h:35:LEFT:BOTTOM .*lastIndex 0 maxIndex 0 relations 1 .*glyphs 1 .*candidateIdBefore 316 existingGlyph glyph:316 existingActive true existingStem 2369 ' "$rows" || \
        ! grep -q '^stemsheadclinkresult headOrder 49 allocatorBefore 2385 allocatorAfter 2385 .*addedVertices - addedEdges - addedSystemStems - .*linkerChanges \[h:36:LEFT:BOTTOM:true:false->true:true' "$rows"; then
    echo "v49 existing-stem reconciliation contract differs" >&2
    exit 1
fi
: <<'DISABLED_V27_ACTIVE'
...
v48_runner_sha=$(shasum -a 256 "$script_dir/run-stems-head-phase-prefix-v48.sh" | awk '{print $1}')
if [ "$v48_runner_sha" != "925536d8d119102e5a74a3690b2286bde856bd476151243806d68a049aa40fdb" ]; then
    echo "v49 base v48 runner drifted" >&2
    exit 1
fi
v48_fixture="$repo_root/rust/oracle/stems-head-phase-prefix-chula-system1-v48.txt"
v48_fixture_sha=$(shasum -a 256 "$v48_fixture" | awk '{print $1}')
if [ "$v48_fixture_sha" != "acc3436794b0ea828dbd689adfd072b6844125007131ee4207d9d4402c90cd5d" ]; then
    echo "v49 base v48 fixture drifted" >&2
    exit 1
fi
...
out="/private/tmp/stems-head-phase-prefix-chula-system1-v49-audit.txt"
{
    echo '# Java Audiveris 5.11 (Temurin JDK 25.0.3+9 LTS) post-STUMPS head phase v49.'
    echo '# schema: stems-head-phase-prefix-v49'
    echo '# Snapshot-minimized order-49 derivative: orders 1-48 mutate without snapshots; order 49 emits existing-stem reconciliation.'
    echo '# This bounded scope is intentional for deterministic replay under the full-snapshot heap limit.'
    cat "$rows"
    printf '%s\n' \
        "stemsheadphaseprefix summary schema stems-head-phase-prefix-v49 page chula.png#1 system 1 rows $row_count baseProbeSourceSha256 $base_probe_sha baseV48RunnerSourceSha256 $v48_runner_sha baseV48FixtureSha256 $v48_fixture_sha fragmentSourceSha256 $fragment_sha probeSourceSha256 $probe_sha runnerSourceSha256 $runner_sha emittedBodySha256 $body_sha semanticPassSha256 $semantic_sha completeStumpsFixtureSha256 $stumps_sha freshRuns 2 freshRunsByteIdentical true nativeScope BoundedSnapshotMinimizedOrder49ExistingStemReconciliation javaEvidence ReturnedBeforeFiftiethHead"
} > "$out"
echo "wrote $out"
