...
# Read-only snapshot-minimized order51 continuation from v50; prior orders mutate without snapshots.
set -eu
...
tmp_base=/private/tmp/stems-head-phase-prefix-v51
...
awk -v fragment="$fragment18" '
{
    ...
    if (index($0, "final boolean returned = linker.linkSides(") != 0) {
        print "            if (headOrder == 7 || headOrder == 22 || headOrder == 27 || headOrder == 34 || headOrder == 36 || headOrder == 37 || headOrder == 38 || headOrder == 39 || headOrder == 40 || headOrder == 41 || headOrder == 42 || headOrder == 43 || headOrder == 44 || headOrder == 45 || headOrder == 46 || headOrder == 47 || headOrder == 48 || headOrder == 49 || headOrder == 50 || headOrder == 51) emitHeadCLinkEnvelope(head, linker, before, headOrder);"
        print
        in_linker = 1
        next
    }
    ...
    if (capture_after && index($0, "heads, allLinkers);") != 0) {
        print
        print "            if (headOrder == 7 || headOrder == 22 || headOrder == 27 || headOrder == 34 || headOrder == 36 || headOrder == 37 || headOrder == 38 || headOrder == 39 || headOrder == 40 || headOrder == 41 || headOrder == 42 || headOrder == 43 || headOrder == 44 || headOrder == 45 || headOrder == 46 || headOrder == 47 || headOrder == 48 || headOrder == 49 || headOrder == 50 || headOrder == 51) emitHeadCLinkResult(before, after, headOrder);"
        capture_after = 0
        in_linker = 0
        next
    }
...
if [ "$(grep -c '^stemsheadphasecontinue ' "$rows")" -ne 1 ] || \
        ! grep -q 'headOrder 51 headX 19 headSig 64 headInterId 1417 .*decisions \[LEFT:SkipAlreadyLinked,RIGHT:SkipClosed\].*incident \[stem2361:headSideLEFT:heads\[x19:sig64:id1417:sideLEFT,x20:sig65:id1419:sideLEFT\].*closureWrites \[x20:sig65:LEFT:false->true,x20:sig65:RIGHT:false->true\].*closedValueChanges 2 .*nextHeadOrder 52 nextHeadX 15 nextHeadSig 80 nextHeadInterId 1449 ' "$rows" || \
        ! grep -q '^stemsheadclinkfrontier .*headOrder 51 headX 19 headSig 64 .*cAlias h:19:LEFT:BOTTOM .*lastIndex 1 maxIndex 1 relations 1 .*glyphs 2 .*candidateIdBefore 0 existingGlyph glyph:299 existingActive true existingStem 2361 ' "$rows" || \
        ! grep -q '^stemsheadclinkresult headOrder 51 allocatorBefore 2385 allocatorAfter 2385 .*addedVertices - addedEdges - addedSystemStems - .*linkerChanges \[h:20:LEFT:BOTTOM:true:false->true:true' "$rows"; then
    echo "v51 existing-stem reconciliation contract differs" >&2
    exit 1
fi
: <<'DISABLED_V27_ACTIVE'
...
v50_runner_sha=$(shasum -a 256 "$script_dir/run-stems-head-phase-prefix-v50.sh" | awk '{print $1}')
if [ "$v50_runner_sha" != "edff8b0e4715bb198bc70f2cdaac9320d8940f66a417ff9c7cad532cc96a5910" ]; then
    echo "v51 base v50 runner drifted" >&2
    exit 1
fi
v50_fixture="$repo_root/rust/oracle/stems-head-phase-prefix-chula-system1-v50.txt"
v50_fixture_sha=$(shasum -a 256 "$v50_fixture" | awk '{print $1}')
if [ "$v50_fixture_sha" != "00021c6593a39641e87864cb950a06651a797a76899686c23f9faa009e0d275d" ]; then
    echo "v51 base v50 fixture drifted" >&2
    exit 1
fi
...
out="/private/tmp/stems-head-phase-prefix-chula-system1-v51-audit.txt"
{
    echo '# Java Audiveris 5.11 (Temurin JDK 25.0.3+9 LTS) post-STUMPS head phase v51.'
    echo '# schema: stems-head-phase-prefix-v51'
    echo '# Snapshot-minimized order-51 derivative: orders 1-50 mutate without snapshots; order 51 emits existing-stem reconciliation.'
    echo '# This bounded scope is intentional for deterministic replay under the full-snapshot heap limit.'
    cat "$rows"
    printf '%s\n' \
        "stemsheadphaseprefix summary schema stems-head-phase-prefix-v51 page chula.png#1 system 1 rows $row_count baseProbeSourceSha256 $base_probe_sha baseV50RunnerSourceSha256 $v50_runner_sha baseV50FixtureSha256 $v50_fixture_sha fragmentSourceSha256 $fragment_sha probeSourceSha256 $probe_sha runnerSourceSha256 $runner_sha emittedBodySha256 $body_sha semanticPassSha256 $semantic_sha completeStumpsFixtureSha256 $stumps_sha freshRuns 2 freshRunsByteIdentical true nativeScope BoundedSnapshotMinimizedOrder51ExistingStemReconciliation javaEvidence ReturnedBeforeFiftySecondHead"
} > "$out"
echo "wrote $out"
