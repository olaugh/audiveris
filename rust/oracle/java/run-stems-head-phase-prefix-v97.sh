...
# Read-only snapshot-minimized order97 continuation from v96; prior orders mutate without snapshots.
set -eu
...
tmp_base=/private/tmp/stems-head-phase-prefix-v97
...
awk -v fragment="$fragment18" '
{
    ...
    if (capture_after && index($0, "heads, allLinkers);") != 0) {
        print
        print "            if (headOrder == 7 || headOrder == 22 || headOrder == 27 || headOrder == 34 || headOrder == 36 || headOrder == 37 || headOrder == 38 || headOrder == 39 || headOrder == 40 || headOrder == 41 || headOrder == 42 || headOrder == 43 || headOrder == 44 || headOrder == 45 || headOrder == 46 || headOrder == 47 || headOrder == 48 || headOrder == 49 || headOrder == 50 || headOrder == 51 || headOrder == 52 || headOrder == 53 || headOrder == 54 || headOrder == 55 || headOrder == 56 || headOrder == 57 || headOrder == 58 || headOrder == 59 || headOrder == 60 || headOrder == 61 || headOrder == 62 || headOrder == 63 || headOrder == 64 || headOrder == 65 || headOrder == 66 || headOrder == 67 || headOrder == 68 || headOrder == 69 || headOrder == 70 || headOrder == 71 || headOrder == 72 || headOrder == 73 || headOrder == 74 || headOrder == 75 || headOrder == 76 || headOrder == 77 || headOrder == 78 || headOrder == 79 || headOrder == 80 || headOrder == 81 || headOrder == 82 || headOrder == 83 || headOrder == 84 || headOrder == 85 || headOrder == 86 || headOrder == 87 || headOrder == 88 || headOrder == 89 || headOrder == 90 || headOrder == 91 || headOrder == 92 || headOrder == 93 || headOrder == 94 || headOrder == 95 || headOrder == 96 || headOrder == 97) emitHeadCLinkResult(before, after, headOrder);"
        capture_after = 0
        in_linker = 0
        next
    }
...
if [ "$(grep -c '^stemsheadphasecontinue ' "$rows")" -ne 1 ] || \
        ! grep -q 'headOrder 97 headX 44 headSig 70 headInterId 1429 .*decisions \[LEFT:SkipAlreadyLinked,RIGHT:SkipClosed\].*incident \[stem2377:headSideLEFT:heads\[x44:sig70:id1429:sideLEFT,x45:sig56:id1401:sideLEFT,x46:sig57:id1403:sideLEFT\].*closureWrites \[x45:sig56:LEFT:true->true,x45:sig56:RIGHT:true->true,x46:sig57:LEFT:true->true,x46:sig57:RIGHT:true->true\].*closedValueChanges 0 .*nextHeadOrder 98 nextHeadX 39 nextHeadSig 37 nextHeadInterId 1359 ' "$rows" || \
        ! grep -q '^stemsheadclinkfrontier .*headOrder 97 headX 44 headSig 70 .*cAlias h:44:LEFT:BOTTOM .*lastIndex 1 maxIndex 1 relations 2 .*glyphs 1 .*candidateIdBefore 302 existingGlyph glyph:302 existingActive true existingStem 2377 ' "$rows" || \
        ! grep -q '^stemsheadclinkresult headOrder 97 allocatorBefore 2385 allocatorAfter 2385 .*addedVertices - addedEdges - addedSystemStems - linkerChanges - ' "$rows"; then
    echo "v97 zero-write existing-stem reconciliation contract differs" >&2
    exit 1
fi
: <<'DISABLED_V27_ACTIVE'
...
v96_runner_sha=$(shasum -a 256 "$script_dir/run-stems-head-phase-prefix-v96.sh" | awk '{print $1}')
if [ "$v96_runner_sha" != "1b8c36025bcf6c3790ae8b1cd2026b290d6d8dcb85080139aa9ecaee6ab4a76e" ]; then
    echo "v97 base v96 runner drifted" >&2
    exit 1
fi
v96_fixture="$repo_root/rust/oracle/stems-head-phase-prefix-chula-system1-v96.txt"
v96_fixture_sha=$(shasum -a 256 "$v96_fixture" | awk '{print $1}')
if [ "$v96_fixture_sha" != "5d3ab89d92445460772989ae421f13a6d2abf68546a97835f9f24242c829dba0" ]; then
    echo "v97 base v96 fixture drifted" >&2
    exit 1
fi
...
out="/private/tmp/stems-head-phase-prefix-chula-system1-v97-audit.txt"
{
    echo '# Java Audiveris 5.11 (Temurin JDK 25.0.3+9 LTS) post-STUMPS head phase v97.'
    echo '# schema: stems-head-phase-prefix-v97'
    echo '# Snapshot-minimized order-97 derivative: orders 1-96 mutate without snapshots; order 97 emits the zero-write existing-stem reconciliation.'
    echo '# This bounded scope is intentional for deterministic replay under the full-snapshot heap limit.'
    cat "$rows"
    printf '%s\n' \
        "stemsheadphaseprefix summary schema stems-head-phase-prefix-v97 page chula.png#1 system 1 rows $row_count baseProbeSourceSha256 $base_probe_sha baseV96RunnerSourceSha256 $v96_runner_sha baseV96FixtureSha256 $v96_fixture_sha fragmentSourceSha256 $fragment_sha probeSourceSha256 $probe_sha runnerSourceSha256 $runner_sha emittedBodySha256 $body_sha semanticPassSha256 $semantic_sha completeStumpsFixtureSha256 $stumps_sha freshRuns 2 freshRunsByteIdentical true nativeScope BoundedSnapshotMinimizedOrder97ExistingStemReconciliation javaEvidence ReturnedBeforeNinetyEighthHead"
} > "$out"
echo "wrote $out"
