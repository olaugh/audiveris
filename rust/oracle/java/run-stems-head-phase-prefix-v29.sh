...
# Freeze a snapshot-minimized order29 continuation from v28; prior orders mutate without snapshots.
set -eu
...
tmp_base=/private/tmp/stems-head-phase-prefix-v29
...
if [ "$(grep -c '^stemsheadphasecontinue ' "$rows")" -ne 1 ] || \
        ! grep -q 'headOrder 29 headX 10 headSig 9 headInterId 1303 .*decisions \[LEFT:SkipAlreadyLinked,RIGHT:top=false:bottom=false:branch=Neither\].*incident \[stem2355:headSideLEFT:heads\[x9:sig8:id1301:sideLEFT,x10:sig9:id1303:sideLEFT\]\].*closureWrites \[x9:sig8:LEFT:false->true,x9:sig8:RIGHT:false->true\].*closedValueChanges 2 .*nextHeadOrder 30 nextHeadX 101 nextHeadSig 43 nextHeadInterId 1371 ' "$rows"; then
    echo "v29 minimized closure contract differs" >&2
    exit 1
fi
: <<'DISABLED_V27_ACTIVE'
...
v28_runner_sha=$(shasum -a 256 "$script_dir/run-stems-head-phase-prefix-v28.sh" | awk '{print $1}')
if [ "$v28_runner_sha" != "ec1985d786f0c984f5a09a461008911f12777229b0a08eb71b7e36a39d548d82" ]; then
    echo "v29 base v28 runner drifted" >&2
    exit 1
fi
v28_fixture="$repo_root/rust/oracle/stems-head-phase-prefix-chula-system1-v28.txt"
v28_fixture_sha=$(shasum -a 256 "$v28_fixture" | awk '{print $1}')
if [ "$v28_fixture_sha" != "6f30a5cb8706fb0445b5eb84cee2896dfa1b85236f6870a97177714672ef10b7" ]; then
    echo "v29 base v28 fixture drifted" >&2
    exit 1
fi
...
out="$repo_root/rust/oracle/stems-head-phase-prefix-chula-system1-v29.txt"
{
    echo '# Java Audiveris 5.11 (Temurin JDK 25.0.3+9 LTS) post-STUMPS head phase v29.'
    echo '# schema: stems-head-phase-prefix-v29'
    echo '# Snapshot-minimized order-29 derivative: orders 1-28 mutate without snapshots; order 29 emits the prelinked closure row.'
    echo '# This minimized scope is intentional for deterministic replay under the full-snapshot heap limit.'
    cat "$rows"
    printf '%s\n' \
        "stemsheadphaseprefix summary schema stems-head-phase-prefix-v29 page chula.png#1 system 1 rows $row_count baseProbeSourceSha256 $base_probe_sha baseV28RunnerSourceSha256 $v28_runner_sha baseV28FixtureSha256 $v28_fixture_sha fragmentSourceSha256 $fragment_sha probeSourceSha256 $probe_sha runnerSourceSha256 $runner_sha emittedBodySha256 $body_sha semanticPassSha256 $semantic_sha completeStumpsFixtureSha256 $stumps_sha freshRuns 2 freshRunsByteIdentical true nativeScope BoundedSnapshotMinimizedOrder29PrelinkedClosure javaEvidence ReturnedBeforeThirtiethHead"
} > "$out"
echo "wrote $out"
