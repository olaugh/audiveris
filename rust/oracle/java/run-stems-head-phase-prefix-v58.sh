...
# Read-only snapshot-minimized order58 continuation from v57; prior orders mutate without snapshots.
set -eu
...
tmp_base=/private/tmp/stems-head-phase-prefix-v58
...
awk -v fragment="$fragment18" '
{
    ...
    if (capture_after && index($0, "heads, allLinkers);") != 0) {
        print
        print "            if (headOrder == 7 || headOrder == 22 || headOrder == 27 || headOrder == 34 || headOrder == 36 || headOrder == 37 || headOrder == 38 || headOrder == 39 || headOrder == 40 || headOrder == 41 || headOrder == 42 || headOrder == 43 || headOrder == 44 || headOrder == 45 || headOrder == 46 || headOrder == 47 || headOrder == 48 || headOrder == 49 || headOrder == 50 || headOrder == 51 || headOrder == 52 || headOrder == 53 || headOrder == 54 || headOrder == 55 || headOrder == 56 || headOrder == 57 || headOrder == 58) emitHeadCLinkResult(before, after, headOrder);"
        capture_after = 0
        in_linker = 0
        next
    }
...
if [ "$(grep -c '^stemsheadphasecontinue ' "$rows")" -ne 1 ] || \
        ! grep -q 'headOrder 58 headX 92 headSig 24 headInterId 1333 .*decisions \[LEFT:SkipAlreadyLinked,RIGHT:SkipClosed\].*incident \[stem2342:headSideLEFT:heads\[x92:sig24:id1333:sideLEFT,x93:sig25:id1335:sideLEFT\].*closureWrites \[x93:sig25:LEFT:false->true,x93:sig25:RIGHT:false->true\].*closedValueChanges 2 .*nextHeadOrder 59 nextHeadX 100 nextHeadSig 42 nextHeadInterId 1369 ' "$rows" || \
        ! grep -q '^stemsheadclinkfrontier .*headOrder 58 headX 92 headSig 24 .*cAlias h:92:LEFT:BOTTOM .*lastIndex 3 maxIndex 3 relations 3 .*glyphs 2 .*candidateIdBefore 0 existingGlyph glyph:298 existingActive true existingStem 2342 ' "$rows" || \
        ! grep -q '^stemsheadclinkresult headOrder 58 allocatorBefore 2385 allocatorAfter 2385 .*addedVertices - addedEdges - addedSystemStems - .*linkerChanges \[h:93:LEFT:BOTTOM:true:false->true:true' "$rows"; then
    echo "v58 existing-stem reconciliation contract differs" >&2
    exit 1
fi
: <<'DISABLED_V27_ACTIVE'
...
v57_runner_sha=$(shasum -a 256 "$script_dir/run-stems-head-phase-prefix-v57.sh" | awk '{print $1}')
if [ "$v57_runner_sha" != "a108389f5e9465fb4483ecec852e38dc7985676a1d9f1feb8dcb392b32559fbc" ]; then
    echo "v58 base v57 runner drifted" >&2
    exit 1
fi
v57_fixture="$repo_root/rust/oracle/stems-head-phase-prefix-chula-system1-v57.txt"
v57_fixture_sha=$(shasum -a 256 "$v57_fixture" | awk '{print $1}')
if [ "$v57_fixture_sha" != "c8b44bab7c0e75e74755e3ca2d29f46b72b90cde0e406524fb395214a6fe25d5" ]; then
    echo "v58 base v57 fixture drifted" >&2
    exit 1
fi
...
out="/private/tmp/stems-head-phase-prefix-chula-system1-v58-audit.txt"
{
    echo '# Java Audiveris 5.11 (Temurin JDK 25.0.3+9 LTS) post-STUMPS head phase v58.'
    echo '# schema: stems-head-phase-prefix-v58'
    echo '# Snapshot-minimized order-58 derivative: orders 1-57 mutate without snapshots; order 58 emits existing-stem reconciliation.'
    echo '# This bounded scope is intentional for deterministic replay under the full-snapshot heap limit.'
    cat "$rows"
    printf '%s\n' \
        "stemsheadphaseprefix summary schema stems-head-phase-prefix-v58 page chula.png#1 system 1 rows $row_count baseProbeSourceSha256 $base_probe_sha baseV57RunnerSourceSha256 $v57_runner_sha baseV57FixtureSha256 $v57_fixture_sha fragmentSourceSha256 $fragment_sha probeSourceSha256 $probe_sha runnerSourceSha256 $runner_sha emittedBodySha256 $body_sha semanticPassSha256 $semantic_sha completeStumpsFixtureSha256 $stumps_sha freshRuns 2 freshRunsByteIdentical true nativeScope BoundedSnapshotMinimizedOrder58ExistingStemReconciliation javaEvidence ReturnedBeforeFiftyNinthHead"
} > "$out"
echo "wrote $out"
