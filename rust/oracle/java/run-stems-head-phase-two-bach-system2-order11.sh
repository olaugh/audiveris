...
# Freeze Java Bach system-2 phase-two queue 11's rejected LEFT/TOP retry.
set -eu
...
tmp_dir=$(mktemp -d /private/tmp/stems-head-phase-two-bach-s2-q11.XXXXXX)
...
grep -E '^(stemsheadphase2baseline .* system 2 |stemsheadphase2retry .* system 2 queueIndex 11 )' \
    "$tmp_dir/pass1" > "$tmp_dir/rows1"
grep -E '^(stemsheadphase2baseline .* system 2 |stemsheadphase2retry .* system 2 queueIndex 11 )' \
    "$tmp_dir/pass2" > "$tmp_dir/rows2"
if ! cmp -s "$tmp_dir/rows1" "$tmp_dir/rows2"; then
    echo "fresh Bach system-2 phase-two queue-11 Java passes are not byte-identical" >&2
...
if [ "$(wc -l < "$tmp_dir/rows1" | tr -d ' ')" -ne 2 ] || \
        ...
        ! grep -q '^stemsheadphase2retry page BachInvention5.jpg#1 system 2 queueIndex 11 headX 98 headSig 136 headInterId 3878 grade 3fc70b377918303a append true sidesBefore \[LEFT:false:true,RIGHT:false:true\] decisions \[LEFT:top=true:bottom=false:branch=TopOnly,RIGHT:top=false:bottom=false:branch=Neither\] returned false sidesAfter \[LEFT:false:true,RIGHT:false:true\] undefs \[\] sideChanges \[\] sigVerticesBefore 394 sigVerticesAfter 394 sigEdgesBefore 602 sigEdgesAfter 602 systemStemsBefore 77 systemStemsAfter 77 allocatorBefore 6815 allocatorAfter 6815$' "$tmp_dir/rows1"; then
    echo "Bach system-2 phase-two queue-11 Java contract differs" >&2
    cat "$tmp_dir/rows1" >&2
    exit 1
fi

base_runner="$script_dir/run-stems-head-phase-two-bach-system2-order10.sh"
base_fixture="$repo_root/rust/oracle/stems-head-phase-two-bach-system2-order10.txt"
base_runner_sha=$(shasum -a 256 "$base_runner" | awk '{print $1}')
base_fixture_sha=$(shasum -a 256 "$base_fixture" | awk '{print $1}')
if [ "$base_runner_sha" != "8f1c053c0ae18ae60a65d3269554eac7e0a2e9b8e570e5ea66a5e2ecdc2386a4" ] || \
        [ "$base_fixture_sha" != "d7054c473728180fe1677d8a0bc561594c536387e32163874a897f04b1b8efb5" ]; then
    echo "Bach system-2 phase-two queue-10 predecessor drifted" >&2
    exit 1
fi
...
out="$repo_root/rust/oracle/stems-head-phase-two-bach-system2-order11.txt"
{
    echo '# Java Audiveris 5.11 (Temurin JDK 25.0.3+9 LTS) Bach system-2 phase-two queue 11.'
    echo '# schema: stems-head-phase-two-bach-system2-order11-v1'
    cat "$tmp_dir/rows1"
    printf '%s\n' \
        "stemsheadphase2bachs2q11summary schema stems-head-phase-two-bach-system2-order11-v1 page BachInvention5.jpg#1 system 2 rows 2 inputSha256 $input_sha stemsRetrieverSourceSha256 $stems_source_sha probeSourceSha256 $probe_sha initSourceSha256 $init_sha runnerSourceSha256 $runner_sha baseBoundary252RunnerSha256 $base_runner_sha baseBoundary252FixtureSha256 $base_fixture_sha emittedBodySha256 $body_sha semanticPassSha256 $body_sha freshRuns 2 freshRunsByteIdentical true nativeScope BachSystem2PhaseTwoOrder11RejectedLeftTop javaEvidence ReturnedAfterSystem2RetryIndex11"
} > "$out"
echo "wrote $out"
