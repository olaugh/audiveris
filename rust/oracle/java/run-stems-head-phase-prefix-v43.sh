...
# Freeze a snapshot-minimized order43 existing-stem reconciliation from v42; prior orders mutate without snapshots.
set -eu
...
tmp_base=/private/tmp/stems-head-phase-prefix-v43
...
awk -v fragment="$fragment18" '
{
    ...
    if (index($0, "final boolean returned = linker.linkSides(") != 0) {
        print "            if (headOrder == 7 || headOrder == 22 || headOrder == 27 || headOrder == 34 || headOrder == 36 || headOrder == 37 || headOrder == 38 || headOrder == 39 || headOrder == 40 || headOrder == 41 || headOrder == 42 || headOrder == 43) emitHeadCLinkEnvelope(head, linker, before, headOrder);"
        print
        in_linker = 1
        next
    }
    ...
    if (capture_after && index($0, "heads, allLinkers);") != 0) {
        print
        print "            if (headOrder == 7 || headOrder == 22 || headOrder == 27 || headOrder == 34 || headOrder == 36 || headOrder == 37 || headOrder == 38 || headOrder == 39 || headOrder == 40 || headOrder == 41 || headOrder == 42 || headOrder == 43) emitHeadCLinkResult(before, after, headOrder);"
        capture_after = 0
        in_linker = 0
        next
    }
...
if [ "$(grep -c '^stemsheadphasecontinue ' "$rows")" -ne 1 ] || \
        ! grep -q 'headOrder 43 headX 25 headSig 91 headInterId 1471 .*decisions \[LEFT:SkipAlreadyLinked,RIGHT:top=false:bottom=false:branch=Neither\].*incident \[stem2356:headSideLEFT:heads\[x24:sig90:id1469:sideLEFT,x25:sig91:id1471:sideLEFT\].*closureWrites \[x24:sig90:LEFT:false->true,x24:sig90:RIGHT:false->true\].*closedValueChanges 2 .*nextHeadOrder 44 nextHeadX 83 nextHeadSig 21 nextHeadInterId 1327 ' "$rows" || \
        ! grep -q '^stemsheadclinkfrontier .*headOrder 43 headX 25 headSig 91 .*cAlias h:25:LEFT:BOTTOM .*lastIndex 3 maxIndex 3 relations 3 .*glyphs 2 .*candidateIdBefore 0 existingGlyph glyph:292 existingActive true existingStem 2356 ' "$rows" || \
        ! grep -q '^stemsheadclinkresult headOrder 43 allocatorBefore 2385 allocatorAfter 2385 .*addedVertices - addedEdges - addedSystemStems - .*linkerChanges \[h:24:LEFT:BOTTOM:true:false->true:true' "$rows"; then
    echo "v43 existing-stem reconciliation contract differs" >&2
    exit 1
fi
: <<'DISABLED_V27_ACTIVE'
...
v42_runner_sha=$(shasum -a 256 "$script_dir/run-stems-head-phase-prefix-v42.sh" | awk '{print $1}')
if [ "$v42_runner_sha" != "b3e7d0f4399584faa4a180b87dcc95259114cb4821dd0fcee404739a577c31c0" ]; then
    echo "v43 base v42 runner drifted" >&2
    exit 1
fi
v42_fixture="$repo_root/rust/oracle/stems-head-phase-prefix-chula-system1-v42.txt"
v42_fixture_sha=$(shasum -a 256 "$v42_fixture" | awk '{print $1}')
if [ "$v42_fixture_sha" != "64b55e449e38f7af6ed47c1ca026236772a277ac8c5917bc5eaea397125b332c" ]; then
    echo "v43 base v42 fixture drifted" >&2
    exit 1
fi
...
out="/private/tmp/stems-head-phase-prefix-chula-system1-v43-audit.txt"
{
    echo '# Java Audiveris 5.11 (Temurin JDK 25.0.3+9 LTS) post-STUMPS head phase v43.'
    echo '# schema: stems-head-phase-prefix-v43'
    echo '# Snapshot-minimized order-43 derivative: orders 1-42 mutate without snapshots; order 43 emits existing-stem reconciliation.'
    echo '# This bounded scope is intentional for deterministic replay under the full-snapshot heap limit.'
    cat "$rows"
    printf '%s\n' \
        "stemsheadphaseprefix summary schema stems-head-phase-prefix-v43 page chula.png#1 system 1 rows $row_count baseProbeSourceSha256 $base_probe_sha baseV42RunnerSourceSha256 $v42_runner_sha baseV42FixtureSha256 $v42_fixture_sha fragmentSourceSha256 $fragment_sha probeSourceSha256 $probe_sha runnerSourceSha256 $runner_sha emittedBodySha256 $body_sha semanticPassSha256 $semantic_sha completeStumpsFixtureSha256 $stumps_sha freshRuns 2 freshRunsByteIdentical true nativeScope BoundedSnapshotMinimizedOrder43ExistingStemReconciliation javaEvidence ReturnedBeforeFortyFourthHead"
} > "$out"
echo "wrote $out"
