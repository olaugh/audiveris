...
# Read-only snapshot-minimized order79 continuation from v78; prior orders mutate without snapshots.
set -eu
...
tmp_base=/private/tmp/stems-head-phase-prefix-v79
...
awk -v fragment="$fragment18" '
{
    ...
    if (capture_after && index($0, "heads, allLinkers);") != 0) {
        print
        print "            if (headOrder == 7 || headOrder == 22 || headOrder == 27 || headOrder == 34 || headOrder == 36 || headOrder == 37 || headOrder == 38 || headOrder == 39 || headOrder == 40 || headOrder == 41 || headOrder == 42 || headOrder == 43 || headOrder == 44 || headOrder == 45 || headOrder == 46 || headOrder == 47 || headOrder == 48 || headOrder == 49 || headOrder == 50 || headOrder == 51 || headOrder == 52 || headOrder == 53 || headOrder == 54 || headOrder == 55 || headOrder == 56 || headOrder == 57 || headOrder == 58 || headOrder == 59 || headOrder == 60 || headOrder == 61 || headOrder == 62 || headOrder == 63 || headOrder == 64 || headOrder == 65 || headOrder == 66 || headOrder == 67 || headOrder == 68 || headOrder == 69 || headOrder == 70 || headOrder == 71 || headOrder == 72 || headOrder == 73 || headOrder == 74 || headOrder == 75 || headOrder == 76 || headOrder == 77 || headOrder == 78 || headOrder == 79) emitHeadCLinkResult(before, after, headOrder);"
        capture_after = 0
        in_linker = 0
        next
    }
...
if [ "$(grep -c '^stemsheadphasecontinue ' "$rows")" -ne 1 ] || \
        ! grep -q 'headOrder 79 headX 17 headSig 10 headInterId 1305 .*decisions \[LEFT:SkipAlreadyLinked,RIGHT:SkipClosed\].*incident \[stem2372:headSideLEFT:heads\[x17:sig10:id1305:sideLEFT,x18:sig4:id1293:sideLEFT\].*closureWrites \[x18:sig4:LEFT:false->true,x18:sig4:RIGHT:false->true\].*closedValueChanges 2 .*nextHeadOrder 80 nextHeadX 29 nextHeadSig 66 nextHeadInterId 1421 ' "$rows" || \
        ! grep -q '^stemsheadclinkfrontier .*headOrder 79 headX 17 headSig 10 .*cAlias h:17:LEFT:BOTTOM .*lastIndex 1 maxIndex 1 relations 2 .*glyphs 1 .*candidateIdBefore 310 existingGlyph glyph:310 existingActive true existingStem 2372 ' "$rows" || \
        ! grep -q '^stemsheadclinkresult headOrder 79 allocatorBefore 2385 allocatorAfter 2385 .*addedVertices - addedEdges - addedSystemStems - .*linkerChanges \[h:18:LEFT:BOTTOM:true:false->true:true' "$rows"; then
    echo "v79 existing-stem reconciliation contract differs" >&2
    exit 1
fi
: <<'DISABLED_V27_ACTIVE'
...
v78_runner_sha=$(shasum -a 256 "$script_dir/run-stems-head-phase-prefix-v78.sh" | awk '{print $1}')
if [ "$v78_runner_sha" != "3e8e0b9e02f164e90d64899657208f4e2d21d3ccddb1ad1aeea1bccf43fcb532" ]; then
    echo "v79 base v78 runner drifted" >&2
    exit 1
fi
v78_fixture="$repo_root/rust/oracle/stems-head-phase-prefix-chula-system1-v78.txt"
v78_fixture_sha=$(shasum -a 256 "$v78_fixture" | awk '{print $1}')
if [ "$v78_fixture_sha" != "74fb2c4b16b28528cf5c7767612ec882346f3079fb09c9c4cabf5985364a5497" ]; then
    echo "v79 base v78 fixture drifted" >&2
    exit 1
fi
...
out="/private/tmp/stems-head-phase-prefix-chula-system1-v79-audit.txt"
{
    echo '# Java Audiveris 5.11 (Temurin JDK 25.0.3+9 LTS) post-STUMPS head phase v79.'
    echo '# schema: stems-head-phase-prefix-v79'
    echo '# Snapshot-minimized order-79 derivative: orders 1-78 mutate without snapshots; order 79 emits the existing-stem reconciliation.'
    echo '# This bounded scope is intentional for deterministic replay under the full-snapshot heap limit.'
    cat "$rows"
    printf '%s\n' \
        "stemsheadphaseprefix summary schema stems-head-phase-prefix-v79 page chula.png#1 system 1 rows $row_count baseProbeSourceSha256 $base_probe_sha baseV78RunnerSourceSha256 $v78_runner_sha baseV78FixtureSha256 $v78_fixture_sha fragmentSourceSha256 $fragment_sha probeSourceSha256 $probe_sha runnerSourceSha256 $runner_sha emittedBodySha256 $body_sha semanticPassSha256 $semantic_sha completeStumpsFixtureSha256 $stumps_sha freshRuns 2 freshRunsByteIdentical true nativeScope BoundedSnapshotMinimizedOrder79ExistingStemReconciliation javaEvidence ReturnedBeforeEightiethHead"
} > "$out"
echo "wrote $out"
