...
# Read-only snapshot-minimized order86 continuation from v85; prior orders mutate without snapshots.
set -eu
...
tmp_base=/private/tmp/stems-head-phase-prefix-v86
...
awk -v fragment="$fragment18" '
{
    ...
    if (capture_after && index($0, "heads, allLinkers);") != 0) {
        print
        print "            if (headOrder == 7 || headOrder == 22 || headOrder == 27 || headOrder == 34 || headOrder == 36 || headOrder == 37 || headOrder == 38 || headOrder == 39 || headOrder == 40 || headOrder == 41 || headOrder == 42 || headOrder == 43 || headOrder == 44 || headOrder == 45 || headOrder == 46 || headOrder == 47 || headOrder == 48 || headOrder == 49 || headOrder == 50 || headOrder == 51 || headOrder == 52 || headOrder == 53 || headOrder == 54 || headOrder == 55 || headOrder == 56 || headOrder == 57 || headOrder == 58 || headOrder == 59 || headOrder == 60 || headOrder == 61 || headOrder == 62 || headOrder == 63 || headOrder == 64 || headOrder == 65 || headOrder == 66 || headOrder == 67 || headOrder == 68 || headOrder == 69 || headOrder == 70 || headOrder == 71 || headOrder == 72 || headOrder == 73 || headOrder == 74 || headOrder == 75 || headOrder == 76 || headOrder == 77 || headOrder == 78 || headOrder == 79 || headOrder == 80 || headOrder == 81 || headOrder == 82 || headOrder == 83 || headOrder == 84 || headOrder == 85 || headOrder == 86) emitHeadCLinkResult(before, after, headOrder);"
        capture_after = 0
        in_linker = 0
        next
    }
...
if [ "$(grep -c '^stemsheadphasecontinue ' "$rows")" -ne 1 ] || \
        ! grep -q 'headOrder 86 headX 51 headSig 82 headInterId 1453 .*decisions \[LEFT:SkipAlreadyLinked,RIGHT:SkipClosed\].*incident \[stem2362:headSideLEFT:heads\[x51:sig82:id1453:sideLEFT,x54:sig78:id1445:sideLEFT,x55:sig79:id1447:sideLEFT\].*closureWrites \[x54:sig78:LEFT:true->true,x54:sig78:RIGHT:true->true,x55:sig79:LEFT:false->true,x55:sig79:RIGHT:false->true\].*closedValueChanges 2 .*nextHeadOrder 87 nextHeadX 45 nextHeadSig 56 nextHeadInterId 1401 ' "$rows" || \
        ! grep -q '^stemsheadclinkfrontier .*headOrder 86 headX 51 headSig 82 .*cAlias h:51:LEFT:BOTTOM .*lastIndex 0 maxIndex 0 relations 1 .*glyphs 1 .*candidateIdBefore 334 existingGlyph glyph:334 existingActive true existingStem 2362 ' "$rows" || \
        ! grep -q '^stemsheadclinkresult headOrder 86 allocatorBefore 2385 allocatorAfter 2385 .*addedVertices - addedEdges - addedSystemStems - .*linkerChanges \[h:55:LEFT:BOTTOM:true:false->true:true' "$rows"; then
    echo "v86 three-head existing-stem reconciliation contract differs" >&2
    exit 1
fi
: <<'DISABLED_V27_ACTIVE'
...
v85_runner_sha=$(shasum -a 256 "$script_dir/run-stems-head-phase-prefix-v85.sh" | awk '{print $1}')
if [ "$v85_runner_sha" != "7b604fa112d346eb560e3086ce31b3d19e929cb45cdda52e172c2178ed8d130a" ]; then
    echo "v86 base v85 runner drifted" >&2
    exit 1
fi
v85_fixture="$repo_root/rust/oracle/stems-head-phase-prefix-chula-system1-v85.txt"
v85_fixture_sha=$(shasum -a 256 "$v85_fixture" | awk '{print $1}')
if [ "$v85_fixture_sha" != "ac38c32459c9cc0afeef77572a060e6ef336005671296924424d559e003bcb0f" ]; then
    echo "v86 base v85 fixture drifted" >&2
    exit 1
fi
...
out="/private/tmp/stems-head-phase-prefix-chula-system1-v86-audit.txt"
{
    echo '# Java Audiveris 5.11 (Temurin JDK 25.0.3+9 LTS) post-STUMPS head phase v86.'
    echo '# schema: stems-head-phase-prefix-v86'
    echo '# Snapshot-minimized order-86 derivative: orders 1-85 mutate without snapshots; order 86 emits the three-head existing-stem reconciliation.'
    echo '# This bounded scope is intentional for deterministic replay under the full-snapshot heap limit.'
    cat "$rows"
    printf '%s\n' \
        "stemsheadphaseprefix summary schema stems-head-phase-prefix-v86 page chula.png#1 system 1 rows $row_count baseProbeSourceSha256 $base_probe_sha baseV85RunnerSourceSha256 $v85_runner_sha baseV85FixtureSha256 $v85_fixture_sha fragmentSourceSha256 $fragment_sha probeSourceSha256 $probe_sha runnerSourceSha256 $runner_sha emittedBodySha256 $body_sha semanticPassSha256 $semantic_sha completeStumpsFixtureSha256 $stumps_sha freshRuns 2 freshRunsByteIdentical true nativeScope BoundedSnapshotMinimizedOrder86ExistingStemReconciliation javaEvidence ReturnedBeforeEightySeventhHead"
} > "$out"
echo "wrote $out"
