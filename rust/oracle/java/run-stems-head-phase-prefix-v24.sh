...
# Freeze a bounded order-24 continuation replay from v23, including the
# prelinked existing-stem closure at x93/SIG25. Orders below 18 execute without snapshots.
set -eu
...
tmp_base=/private/tmp/stems-head-phase-prefix-v24
...
fragment18="$script_dir/stems-head-phase-v24-fragment.java"
...
if ! cmp -s "$semantic_one" "$semantic_two"; then
    echo "two fresh post-STUMPS v24 semantic passes are not byte-identical" >&2
...
if [ "$(grep -c '^stemsheadphasecontinue ' "$rows")" -ne 7 ] || \
        ! grep -q 'headOrder 24 headX 93 headSig 25 headInterId 1335 .*decisions \[LEFT:SkipAlreadyLinked,RIGHT:top=false:bottom=false:branch=Neither\].*closureWrites \[x92:sig24:LEFT:false->true,x92:sig24:RIGHT:false->true\].*closedValueChanges 2 .*nextHeadOrder 25 nextHeadX 59 nextHeadSig 74 nextHeadInterId 1437 ' "$rows"; then
...
v23_runner_sha=$(shasum -a 256 "$script_dir/run-stems-head-phase-prefix-v23.sh" | awk '{print $1}')
if [ "$v23_runner_sha" != "b945062e6c069c975f738ee066bc42107b4b6af599b5097abd0423bbb232aa25" ]; then
    echo "v24 base v23 runner drifted" >&2
    exit 1
fi
v23_fixture="$repo_root/rust/oracle/stems-head-phase-prefix-chula-system1-v23.txt"
v23_fixture_sha=$(shasum -a 256 "$v23_fixture" | awk '{print $1}')
if [ "$v23_fixture_sha" != "20731b3ff52e2512407f17c00329e16f015aaedba7bf5c91ec1b0b9907c58e68" ]; then
    echo "v24 base v23 fixture drifted" >&2
    exit 1
fi
...
out="$repo_root/rust/oracle/stems-head-phase-prefix-chula-system1-v24.txt"
{
    echo '# Java Audiveris 5.11 (Temurin JDK 25.0.3+9 LTS) post-STUMPS head phase v24.'
    echo '# schema: stems-head-phase-prefix-v24'
    echo '# Bounded order-24 derivative: order 24 emits the prelinked closure continuation after the v23 predecessor.'
    echo '# This minimized scope is intentional for deterministic replay under the full-snapshot heap limit.'
    cat "$rows"
    printf '%s\n' \
        "stemsheadphaseprefix summary schema stems-head-phase-prefix-v24 page chula.png#1 system 1 rows $row_count baseProbeSourceSha256 $base_probe_sha baseV23RunnerSourceSha256 $v23_runner_sha baseV23FixtureSha256 $v23_fixture_sha fragmentSourceSha256 $fragment_sha probeSourceSha256 $probe_sha runnerSourceSha256 $runner_sha emittedBodySha256 $body_sha semanticPassSha256 $semantic_sha completeStumpsFixtureSha256 $stumps_sha freshRuns 2 freshRunsByteIdentical true nativeScope BoundedSnapshotMinimizedOrder24PrelinkedClosure javaEvidence ReturnedBeforeTwentyFifthHead"
} > "$out"
echo "wrote $out"
