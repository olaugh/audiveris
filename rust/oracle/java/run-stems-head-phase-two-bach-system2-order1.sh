...
# Freeze Java Bach system-2 phase-two queue 1's rejected append retry.
set -eu
...
tmp_dir=$(mktemp -d /private/tmp/stems-head-phase-two-bach-s2-q1.XXXXXX)
...
grep -E '^(stemsheadphase2baseline .* system 2 |stemsheadphase2retry .* system 2 queueIndex 1 )' \
    "$tmp_dir/pass1" > "$tmp_dir/rows1"
grep -E '^(stemsheadphase2baseline .* system 2 |stemsheadphase2retry .* system 2 queueIndex 1 )' \
    "$tmp_dir/pass2" > "$tmp_dir/rows2"
if ! cmp -s "$tmp_dir/rows1" "$tmp_dir/rows2"; then
    echo "fresh Bach system-2 phase-two queue-1 Java passes are not byte-identical" >&2
...
if [ "$(wc -l < "$tmp_dir/rows1" | tr -d ' ')" -ne 2 ] || \
        ...
        ! grep -q '^stemsheadphase2retry page BachInvention5.jpg#1 system 2 queueIndex 1 headX 159 headSig 164 headInterId 3939 grade 3fd6e98b920d30d7 append true sidesBefore \[LEFT:false:true,RIGHT:false:true\] decisions \[LEFT:top=false:bottom=false:branch=Neither,RIGHT:top=false:bottom=true:branch=BottomOnly\] returned false sidesAfter \[LEFT:false:true,RIGHT:false:true\] undefs \[\] sideChanges \[\] sigVerticesBefore 394 sigVerticesAfter 394 sigEdgesBefore 600 sigEdgesAfter 600 systemStemsBefore 77 systemStemsAfter 77 allocatorBefore 6815 allocatorAfter 6815$' "$tmp_dir/rows1"; then
    echo "Bach system-2 phase-two queue-1 Java contract differs" >&2
    cat "$tmp_dir/rows1" >&2
    exit 1
fi

base_runner="$script_dir/run-stems-head-phase-two-bach-system2-order0.sh"
base_fixture="$repo_root/rust/oracle/stems-head-phase-two-bach-system2-order0.txt"
base_runner_sha=$(shasum -a 256 "$base_runner" | awk '{print $1}')
base_fixture_sha=$(shasum -a 256 "$base_fixture" | awk '{print $1}')
if [ "$base_runner_sha" != "98d97209bec4e7fbe3ba8d9d2aae084c3080ce3806352c34bbc7995517c22175" ] || \
        [ "$base_fixture_sha" != "7220313d786c3c917648b6be55abfda26b8b0bfac81a164b98253739fa2b74a1" ]; then
    echo "Bach system-2 phase-two queue-0 predecessor drifted" >&2
    exit 1
fi
...
out="$repo_root/rust/oracle/stems-head-phase-two-bach-system2-order1.txt"
{
    echo '# Java Audiveris 5.11 (Temurin JDK 25.0.3+9 LTS) Bach system-2 phase-two queue 1.'
    echo '# schema: stems-head-phase-two-bach-system2-order1-v1'
    cat "$tmp_dir/rows1"
    printf '%s\n' \
        "stemsheadphase2bachs2q1summary schema stems-head-phase-two-bach-system2-order1-v1 page BachInvention5.jpg#1 system 2 rows 2 inputSha256 $input_sha stemsRetrieverSourceSha256 $stems_source_sha probeSourceSha256 $probe_sha initSourceSha256 $init_sha runnerSourceSha256 $runner_sha baseBoundary242RunnerSha256 $base_runner_sha baseBoundary242FixtureSha256 $base_fixture_sha emittedBodySha256 $body_sha semanticPassSha256 $body_sha freshRuns 2 freshRunsByteIdentical true nativeScope BachSystem2PhaseTwoOrder1RejectedBottomAppend javaEvidence ReturnedAfterSystem2RetryIndex1"
} > "$out"
echo "wrote $out"
