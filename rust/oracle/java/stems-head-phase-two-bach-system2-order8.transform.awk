...
# Bach system 2 queue 8 / Java Inter 3633.
function emit_after_reuse() {
    print "                    if (append && head.getId() == 3633) {"
    print "                        System.out.printf("
    print "                                \"stemsheadphase2bachs2q8reuse headInterId %d lastIndex %d selectedStem %s terminal ReturnedFromReuseStem%n\","
    print "                                head.getId(), lastIndex, rustStem(stem));"
    print "                    }"
}

function emit_reuse_match() {
    print "                            if (head.getId() == 3633) {"
    print "                                final StemInter rustReuse = (StemInter) sig.getOppositeInter(h, r);"
    print "                                System.out.printf("
    print "                                        \"stemsheadphase2bachs2q8reusematch headInterId %d sourceHeadId %d sourceCorner %s sourceSide %s relationGrade %s stem %s terminal SelectedReuseStem%n\","
...
function emit_committed_result() {
    print "                if (append && head.getId() == 3633) {"
    print "                    System.out.printf("
    print "                            \"stemsheadphase2bachs2q8commit headInterId %d linkedStem %s relationRows %s vertices %d edges %d allocator %d terminal ReturnedHeadCLinkTransaction%n\","
...
{
    gsub("head.getId[(][)] == 1777", "head.getId() == 3633")
    gsub("stemsheadphase2x14", "stemsheadphase2bachs2q8")
