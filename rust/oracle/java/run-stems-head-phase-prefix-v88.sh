...
# Read-only snapshot-minimized order88 continuation from v87; prior orders mutate without snapshots.
set -eu
...
tmp_base=/private/tmp/stems-head-phase-prefix-v88
...
awk -v fragment="$fragment18" '
{
    ...
    if (capture_after && index($0, "heads, allLinkers);") != 0) {
        print
        print "            if (headOrder == 7 || headOrder == 22 || headOrder == 27 || headOrder == 34 || headOrder == 36 || headOrder == 37 || headOrder == 38 || headOrder == 39 || headOrder == 40 || headOrder == 41 || headOrder == 42 || headOrder == 43 || headOrder == 44 || headOrder == 45 || headOrder == 46 || headOrder == 47 || headOrder == 48 || headOrder == 49 || headOrder == 50 || headOrder == 51 || headOrder == 52 || headOrder == 53 || headOrder == 54 || headOrder == 55 || headOrder == 56 || headOrder == 57 || headOrder == 58 || headOrder == 59 || headOrder == 60 || headOrder == 61 || headOrder == 62 || headOrder == 63 || headOrder == 64 || headOrder == 65 || headOrder == 66 || headOrder == 67 || headOrder == 68 || headOrder == 69 || headOrder == 70 || headOrder == 71 || headOrder == 72 || headOrder == 73 || headOrder == 74 || headOrder == 75 || headOrder == 76 || headOrder == 77 || headOrder == 78 || headOrder == 79 || headOrder == 80 || headOrder == 81 || headOrder == 82 || headOrder == 83 || headOrder == 84 || headOrder == 85 || headOrder == 86 || headOrder == 87 || headOrder == 88) emitHeadCLinkResult(before, after, headOrder);"
        capture_after = 0
        in_linker = 0
        next
    }
...
if [ "$(grep -c '^stemsheadphasecontinue ' "$rows")" -ne 1 ] || \
        ! grep -q 'headOrder 88 headX 72 headSig 101 headInterId 1491 .*decisions \[LEFT:SkipAlreadyLinked,RIGHT:SkipClosed\].*incident \[stem2380:headSideLEFT:heads\[x76:sig97:id1483:sideLEFT,x75:sig96:id1481:sideLEFT,x72:sig101:id1491:sideLEFT\].*closureWrites \[x76:sig97:LEFT:true->true,x76:sig97:RIGHT:true->true,x75:sig96:LEFT:false->true,x75:sig96:RIGHT:false->true\].*closedValueChanges 2 .*nextHeadOrder 89 nextHeadX 47 nextHeadSig 28 nextHeadInterId 1341 ' "$rows" || \
        ! grep -q '^stemsheadclinkfrontier .*headOrder 88 headX 72 headSig 101 .*cAlias h:72:LEFT:BOTTOM .*lastIndex -1 maxIndex 0 relations 1 .*glyphs 1 .*candidateIdBefore 319 existingGlyph glyph:319 existingActive true existingStem 2380 ' "$rows" || \
        ! grep -q '^stemsheadclinkresult headOrder 88 allocatorBefore 2385 allocatorAfter 2385 .*addedVertices - addedEdges - addedSystemStems - .*linkerChanges \[h:75:LEFT:BOTTOM:true:false->true:true' "$rows"; then
    echo "v88 three-head existing-stem reconciliation contract differs" >&2
    exit 1
fi
: <<'DISABLED_V27_ACTIVE'
...
v87_runner_sha=$(shasum -a 256 "$script_dir/run-stems-head-phase-prefix-v87.sh" | awk '{print $1}')
if [ "$v87_runner_sha" != "ccd2ec6ebe416f3b6310b703ec8ab284c8f5bbb182c310340c31e149c416dac7" ]; then
    echo "v88 base v87 runner drifted" >&2
    exit 1
fi
v87_fixture="$repo_root/rust/oracle/stems-head-phase-prefix-chula-system1-v87.txt"
v87_fixture_sha=$(shasum -a 256 "$v87_fixture" | awk '{print $1}')
if [ "$v87_fixture_sha" != "b750df3442489e2da4afe4dafa3a4fd0ac6e2526e3f8e30406313f4e395d766b" ]; then
    echo "v88 base v87 fixture drifted" >&2
    exit 1
fi
...
out="/private/tmp/stems-head-phase-prefix-chula-system1-v88-audit.txt"
{
    echo '# Java Audiveris 5.11 (Temurin JDK 25.0.3+9 LTS) post-STUMPS head phase v88.'
    echo '# schema: stems-head-phase-prefix-v88'
    echo '# Snapshot-minimized order-88 derivative: orders 1-87 mutate without snapshots; order 88 emits the three-head existing-stem reconciliation.'
    echo '# This bounded scope is intentional for deterministic replay under the full-snapshot heap limit.'
    cat "$rows"
    printf '%s\n' \
        "stemsheadphaseprefix summary schema stems-head-phase-prefix-v88 page chula.png#1 system 1 rows $row_count baseProbeSourceSha256 $base_probe_sha baseV87RunnerSourceSha256 $v87_runner_sha baseV87FixtureSha256 $v87_fixture_sha fragmentSourceSha256 $fragment_sha probeSourceSha256 $probe_sha runnerSourceSha256 $runner_sha emittedBodySha256 $body_sha semanticPassSha256 $semantic_sha completeStumpsFixtureSha256 $stumps_sha freshRuns 2 freshRunsByteIdentical true nativeScope BoundedSnapshotMinimizedOrder88ExistingStemReconciliation javaEvidence ReturnedBeforeEightyNinthHead"
} > "$out"
echo "wrote $out"
