...
# Read-only snapshot-minimized order50 continuation from v49; prior orders mutate without snapshots.
set -eu
...
tmp_base=/private/tmp/stems-head-phase-prefix-v50
...
awk -v fragment="$fragment18" '
{
    ...
    if (index($0, "final boolean returned = linker.linkSides(") != 0) {
        print "            if (headOrder == 7 || headOrder == 22 || headOrder == 27 || headOrder == 34 || headOrder == 36 || headOrder == 37 || headOrder == 38 || headOrder == 39 || headOrder == 40 || headOrder == 41 || headOrder == 42 || headOrder == 43 || headOrder == 44 || headOrder == 45 || headOrder == 46 || headOrder == 47 || headOrder == 48 || headOrder == 49 || headOrder == 50) emitHeadCLinkEnvelope(head, linker, before, headOrder);"
        print
        in_linker = 1
        next
    }
    ...
    if (capture_after && index($0, "heads, allLinkers);") != 0) {
        print
        print "            if (headOrder == 7 || headOrder == 22 || headOrder == 27 || headOrder == 34 || headOrder == 36 || headOrder == 37 || headOrder == 38 || headOrder == 39 || headOrder == 40 || headOrder == 41 || headOrder == 42 || headOrder == 43 || headOrder == 44 || headOrder == 45 || headOrder == 46 || headOrder == 47 || headOrder == 48 || headOrder == 49 || headOrder == 50) emitHeadCLinkResult(before, after, headOrder);"
        capture_after = 0
        in_linker = 0
        next
    }
...
if [ "$(grep -c '^stemsheadphasecontinue ' "$rows")" -ne 1 ] || \
        ! grep -q 'headOrder 50 headX 32 headSig 50 headInterId 1389 .*decisions \[LEFT:top=true:bottom=true:branch=Both,RIGHT:top=true:bottom=false:branch=TopOnly\].*incident - returned false .*undefs \[LEFT\].*closedValueChanges 0 .*nextHeadOrder 51 nextHeadX 19 nextHeadSig 64 nextHeadInterId 1417 ' "$rows" || \
        ! grep -q '^stemsheadclinkfrontier .*headOrder 50 headX 32 headSig 50 .*cAlias h:32:LEFT:BOTTOM .*lastIndex -1 maxIndex 1 relations 1 .*glyphs 2 .*candidateIdBefore 0 existingGlyph glyph:314 existingActive true existingStem 2383 ' "$rows" || \
        ! grep -q '^stemsheadclinkresult headOrder 50 allocatorBefore 2385 allocatorAfter 2385 .*addedVertices - addedEdges - addedSystemStems - linkerChanges - .*terminal ReturnedHeadCLinkTransaction' "$rows"; then
    echo "v50 open-frontier contract differs" >&2
    exit 1
fi
: <<'DISABLED_V27_ACTIVE'
...
v49_runner_sha=$(shasum -a 256 "$script_dir/run-stems-head-phase-prefix-v49.sh" | awk '{print $1}')
if [ "$v49_runner_sha" != "da0ed259bc25e03b3624b96a5078661497fb713f08ca183dd3624bd16e74f406" ]; then
    echo "v50 base v49 runner drifted" >&2
    exit 1
fi
v49_fixture="$repo_root/rust/oracle/stems-head-phase-prefix-chula-system1-v49.txt"
v49_fixture_sha=$(shasum -a 256 "$v49_fixture" | awk '{print $1}')
if [ "$v49_fixture_sha" != "ef4bfca2696caba71b227fb15d36b85adfe34ca519e7d87419d86f2605664147" ]; then
    echo "v50 base v49 fixture drifted" >&2
    exit 1
fi
...
out="/private/tmp/stems-head-phase-prefix-chula-system1-v50-audit.txt"
{
    echo '# Java Audiveris 5.11 (Temurin JDK 25.0.3+9 LTS) post-STUMPS head phase v50.'
    echo '# schema: stems-head-phase-prefix-v50'
    echo '# Snapshot-minimized order-50 derivative: orders 1-49 mutate without snapshots; order 50 emits the open-frontier transaction.'
    echo '# This bounded scope is intentional for deterministic replay under the full-snapshot heap limit.'
    cat "$rows"
    printf '%s\n' \
        "stemsheadphaseprefix summary schema stems-head-phase-prefix-v50 page chula.png#1 system 1 rows $row_count baseProbeSourceSha256 $base_probe_sha baseV49RunnerSourceSha256 $v49_runner_sha baseV49FixtureSha256 $v49_fixture_sha fragmentSourceSha256 $fragment_sha probeSourceSha256 $probe_sha runnerSourceSha256 $runner_sha emittedBodySha256 $body_sha semanticPassSha256 $semantic_sha completeStumpsFixtureSha256 $stumps_sha freshRuns 2 freshRunsByteIdentical true nativeScope BoundedSnapshotMinimizedOrder50OpenFrontier javaEvidence ReturnedBeforeFiftyFirstHead"
} > "$out"
echo "wrote $out"
