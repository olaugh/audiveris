...
# Freeze Java Bach system-2 phase-two queue 10's no-link retry.
set -eu
...
tmp_dir=$(mktemp -d /private/tmp/stems-head-phase-two-bach-s2-q10.XXXXXX)
...
grep -E '^(stemsheadphase2baseline .* system 2 |stemsheadphase2retry .* system 2 queueIndex 10 )' \
    "$tmp_dir/pass1" > "$tmp_dir/rows1"
grep -E '^(stemsheadphase2baseline .* system 2 |stemsheadphase2retry .* system 2 queueIndex 10 )' \
    "$tmp_dir/pass2" > "$tmp_dir/rows2"
if ! cmp -s "$tmp_dir/rows1" "$tmp_dir/rows2"; then
    echo "fresh Bach system-2 phase-two queue-10 Java passes are not byte-identical" >&2
...
if [ "$(wc -l < "$tmp_dir/rows1" | tr -d ' ')" -ne 2 ] || \
        ...
        ! grep -q '^stemsheadphase2retry page BachInvention5.jpg#1 system 2 queueIndex 10 headX 190 headSig 214 headInterId 4036 grade 3fc857b6c55b3c0d append true sidesBefore \[LEFT:false:true,RIGHT:false:true\] decisions \[LEFT:top=false:bottom=false:branch=Neither,RIGHT:top=false:bottom=false:branch=Neither\] returned false sidesAfter \[LEFT:false:true,RIGHT:false:true\] undefs \[\] sideChanges \[\] sigVerticesBefore 394 sigVerticesAfter 394 sigEdgesBefore 602 sigEdgesAfter 602 systemStemsBefore 77 systemStemsAfter 77 allocatorBefore 6815 allocatorAfter 6815$' "$tmp_dir/rows1"; then
    echo "Bach system-2 phase-two queue-10 Java contract differs" >&2
    cat "$tmp_dir/rows1" >&2
    exit 1
fi

base_runner="$script_dir/run-stems-head-phase-two-bach-system2-order9.sh"
base_fixture="$repo_root/rust/oracle/stems-head-phase-two-bach-system2-order9.txt"
base_runner_sha=$(shasum -a 256 "$base_runner" | awk '{print $1}')
base_fixture_sha=$(shasum -a 256 "$base_fixture" | awk '{print $1}')
if [ "$base_runner_sha" != "e167db773c00127cb22f4c1982037f1e56b66ee33cdfe6895ad1d2e675c8dff4" ] || \
        [ "$base_fixture_sha" != "2de3f797787eeb64ecdf4167c92d2f6f2582b57577017146f7268f72f0203f05" ]; then
    echo "Bach system-2 phase-two queue-9 predecessor drifted" >&2
    exit 1
fi
...
out="$repo_root/rust/oracle/stems-head-phase-two-bach-system2-order10.txt"
{
    echo '# Java Audiveris 5.11 (Temurin JDK 25.0.3+9 LTS) Bach system-2 phase-two queue 10.'
    echo '# schema: stems-head-phase-two-bach-system2-order10-v1'
    cat "$tmp_dir/rows1"
    printf '%s\n' \
        "stemsheadphase2bachs2q10summary schema stems-head-phase-two-bach-system2-order10-v1 page BachInvention5.jpg#1 system 2 rows 2 inputSha256 $input_sha stemsRetrieverSourceSha256 $stems_source_sha probeSourceSha256 $probe_sha initSourceSha256 $init_sha runnerSourceSha256 $runner_sha baseBoundary251RunnerSha256 $base_runner_sha baseBoundary251FixtureSha256 $base_fixture_sha emittedBodySha256 $body_sha semanticPassSha256 $body_sha freshRuns 2 freshRunsByteIdentical true nativeScope BachSystem2PhaseTwoOrder10NoLinkRetry javaEvidence ReturnedAfterSystem2RetryIndex10"
} > "$out"
echo "wrote $out"
