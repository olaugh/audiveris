...
# Freeze the v7 head prefix plus the next order-8 prelinked continuation.
set -eu
...
tmp_base=/private/tmp/stems-head-phase-prefix-v8
...
if [ "$(wc -l < "$rows" | tr -d ' ')" -ne 16 ] || \
        [ "$(grep -c '^stemsheadphasecontinue ' "$rows")" -ne 8 ] || \
        ...
        ! grep -q 'headOrder 8 headX 95 headSig 100 headInterId 1489 .*decisions \[LEFT:SkipAlreadyLinked,RIGHT:top=false:bottom=false:branch=Neither\] incident \[stem2364:headSideLEFT:heads\[x91:sig98:id1485:sideLEFT,x94:sig99:id1487:sideLEFT,x95:sig100:id1489:sideLEFT\]\] returned true .*closureWrites \[x91:sig98:LEFT:false->true,x91:sig98:RIGHT:false->true,x94:sig99:LEFT:false->true,x94:sig99:RIGHT:false->true\] closedValueChanges 4 unlinkedCount 0 .*nextHeadOrder 9 nextHeadX 42 nextHeadSig 93 nextHeadInterId 1475 ' "$rows"; then
    echo "v8 order-8 continuation contract differs" >&2
    exit 1
fi
...
v7_runner_sha=$(shasum -a 256 "$script_dir/run-stems-head-phase-prefix-v7.sh" | awk '{print $1}')
if [ "$v7_runner_sha" != "87a12b97b6d9c79e6c0d346f8187b426505ab5e0e7785bd07a5984a03a18c197" ]; then
    echo "v8 base v7 runner drifted" >&2
    exit 1
fi
v7_fixture="$repo_root/rust/oracle/stems-head-phase-prefix-chula-system1-v7.txt"
v7_fixture_sha=$(shasum -a 256 "$v7_fixture" | awk '{print $1}')
if [ "$v7_fixture_sha" != "8df7d36e780e90e569fcc37144bd48ff43e5b647f9cdc240d899ee10386b153d" ]; then
    echo "v8 base v7 fixture drifted" >&2
    exit 1
fi
...
out="$repo_root/rust/oracle/stems-head-phase-prefix-chula-system1-v8.txt"
{
    echo '# Java Audiveris 5.11 (Temurin JDK 25.0.3+9 LTS) post-STUMPS head phase v8.'
    echo '# schema: stems-head-phase-prefix-v8'
    echo '# v7 CLinker envelope plus the next real order-8 phase-1 linkSides call.'
    echo '# Expected rows stay unread until the native transaction returns.'
    cat "$rows"
    printf '%s\n' \
        "stemsheadphaseprefix summary schema stems-head-phase-prefix-v8 page chula.png#1 system 1 rows 16 baseProbeSourceSha256 $base_probe_sha baseV7RunnerSourceSha256 $v7_runner_sha baseV7FixtureSha256 $v7_fixture_sha probeSourceSha256 $probe_sha runnerSourceSha256 $runner_sha emittedBodySha256 $body_sha semanticPassSha256 $semantic_sha completeStumpsFixtureSha256 $stumps_sha freshRuns 2 freshRunsByteIdentical true nativeScope ReturnedOrder8HeadContinuation javaEvidence ReturnedBeforeTenthHead"
} > "$out"
echo "wrote $out"
