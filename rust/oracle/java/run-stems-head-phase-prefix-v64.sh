...
# Read-only snapshot-minimized order64 continuation from v63; prior orders mutate without snapshots.
set -eu
...
tmp_base=/private/tmp/stems-head-phase-prefix-v64
...
awk -v fragment="$fragment18" '
{
    ...
    if (capture_after && index($0, "heads, allLinkers);") != 0) {
        print
        print "            if (headOrder == 7 || headOrder == 22 || headOrder == 27 || headOrder == 34 || headOrder == 36 || headOrder == 37 || headOrder == 38 || headOrder == 39 || headOrder == 40 || headOrder == 41 || headOrder == 42 || headOrder == 43 || headOrder == 44 || headOrder == 45 || headOrder == 46 || headOrder == 47 || headOrder == 48 || headOrder == 49 || headOrder == 50 || headOrder == 51 || headOrder == 52 || headOrder == 53 || headOrder == 54 || headOrder == 55 || headOrder == 56 || headOrder == 57 || headOrder == 58 || headOrder == 59 || headOrder == 60 || headOrder == 61 || headOrder == 62 || headOrder == 63 || headOrder == 64) emitHeadCLinkResult(before, after, headOrder);"
        capture_after = 0
        in_linker = 0
        next
    }
...
if [ "$(grep -c '^stemsheadphasecontinue ' "$rows")" -ne 1 ] || \
        ! grep -q 'headOrder 64 headX 3 headSig 6 headInterId 1297 .*decisions \[LEFT:SkipAlreadyLinked,RIGHT:SkipClosed\].*incident \[stem2354:headSideLEFT:heads\[x4:sig7:id1299:sideLEFT,x3:sig6:id1297:sideLEFT\].*closureWrites \[x4:sig7:LEFT:false->true,x4:sig7:RIGHT:false->true\].*closedValueChanges 2 .*nextHeadOrder 65 nextHeadX 58 nextHeadSig 73 nextHeadInterId 1435 ' "$rows" || \
        ! grep -q '^stemsheadclinkfrontier .*headOrder 64 headX 3 headSig 6 .*cAlias h:3:LEFT:BOTTOM .*lastIndex 2 maxIndex 2 relations 2 .*glyphs 2 .*candidateIdBefore 0 existingGlyph glyph:315 existingActive true existingStem 2354 ' "$rows" || \
        ! grep -q '^stemsheadclinkresult headOrder 64 allocatorBefore 2385 allocatorAfter 2385 .*addedVertices - addedEdges - addedSystemStems - .*linkerChanges \[h:4:LEFT:BOTTOM:true:false->true:true' "$rows"; then
    echo "v64 existing-stem reconciliation contract differs" >&2
    exit 1
fi
: <<'DISABLED_V27_ACTIVE'
...
v63_runner_sha=$(shasum -a 256 "$script_dir/run-stems-head-phase-prefix-v63.sh" | awk '{print $1}')
if [ "$v63_runner_sha" != "d535607b178a298755a554bcc878cfd2bff32845960c5d39660a8ddc62992ece" ]; then
    echo "v64 base v63 runner drifted" >&2
    exit 1
fi
v63_fixture="$repo_root/rust/oracle/stems-head-phase-prefix-chula-system1-v63.txt"
v63_fixture_sha=$(shasum -a 256 "$v63_fixture" | awk '{print $1}')
if [ "$v63_fixture_sha" != "6f57b1db06a5319133c90cbcb88ddd316c6d6741cde0fb23025aab0ac31c7fcf" ]; then
    echo "v64 base v63 fixture drifted" >&2
    exit 1
fi
...
out="/private/tmp/stems-head-phase-prefix-chula-system1-v64-audit.txt"
{
    echo '# Java Audiveris 5.11 (Temurin JDK 25.0.3+9 LTS) post-STUMPS head phase v64.'
    echo '# schema: stems-head-phase-prefix-v64'
    echo '# Snapshot-minimized order-64 derivative: orders 1-63 mutate without snapshots; order 64 emits existing-stem reconciliation.'
    echo '# This bounded scope is intentional for deterministic replay under the full-snapshot heap limit.'
    cat "$rows"
    printf '%s\n' \
        "stemsheadphaseprefix summary schema stems-head-phase-prefix-v64 page chula.png#1 system 1 rows $row_count baseProbeSourceSha256 $base_probe_sha baseV63RunnerSourceSha256 $v63_runner_sha baseV63FixtureSha256 $v63_fixture_sha fragmentSourceSha256 $fragment_sha probeSourceSha256 $probe_sha runnerSourceSha256 $runner_sha emittedBodySha256 $body_sha semanticPassSha256 $semantic_sha completeStumpsFixtureSha256 $stumps_sha freshRuns 2 freshRunsByteIdentical true nativeScope BoundedSnapshotMinimizedOrder64ExistingStemReconciliation javaEvidence ReturnedBeforeSixtyFifthHead"
} > "$out"
echo "wrote $out"
