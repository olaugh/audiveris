...
# Read-only snapshot-minimized order71 continuation from v70; prior orders mutate without snapshots.
set -eu
...
tmp_base=/private/tmp/stems-head-phase-prefix-v71
...
awk -v fragment="$fragment18" '
{
    ...
    if (capture_after && index($0, "heads, allLinkers);") != 0) {
        print
        print "            if (headOrder == 7 || headOrder == 22 || headOrder == 27 || headOrder == 34 || headOrder == 36 || headOrder == 37 || headOrder == 38 || headOrder == 39 || headOrder == 40 || headOrder == 41 || headOrder == 42 || headOrder == 43 || headOrder == 44 || headOrder == 45 || headOrder == 46 || headOrder == 47 || headOrder == 48 || headOrder == 49 || headOrder == 50 || headOrder == 51 || headOrder == 52 || headOrder == 53 || headOrder == 54 || headOrder == 55 || headOrder == 56 || headOrder == 57 || headOrder == 58 || headOrder == 59 || headOrder == 60 || headOrder == 61 || headOrder == 62 || headOrder == 63 || headOrder == 64 || headOrder == 65 || headOrder == 66 || headOrder == 67 || headOrder == 68 || headOrder == 69 || headOrder == 70 || headOrder == 71) emitHeadCLinkResult(before, after, headOrder);"
        capture_after = 0
        in_linker = 0
        next
    }
...
if [ "$(grep -c '^stemsheadphasecontinue ' "$rows")" -ne 1 ] || \
        ! grep -q 'headOrder 71 headX 77 headSig 38 headInterId 1361 .*decisions \[LEFT:SkipAlreadyLinked,RIGHT:SkipClosed\].*incident \[stem2370:headSideLEFT:heads\[x77:sig38:id1361:sideLEFT,x78:sig39:id1363:sideLEFT\].*closureWrites \[x78:sig39:LEFT:false->true,x78:sig39:RIGHT:false->true\].*closedValueChanges 2 .*nextHeadOrder 72 nextHeadX 26 nextHeadSig 13 nextHeadInterId 1311 ' "$rows" || \
        ! grep -q '^stemsheadclinkfrontier .*headOrder 71 headX 77 headSig 38 .*cAlias h:77:LEFT:BOTTOM .*lastIndex 2 maxIndex 2 relations 3 .*glyphs 1 .*candidateIdBefore 309 existingGlyph glyph:309 existingActive true existingStem 2370 ' "$rows" || \
        ! grep -q '^stemsheadclinkresult headOrder 71 allocatorBefore 2385 allocatorAfter 2385 .*addedVertices - addedEdges - addedSystemStems - .*linkerChanges \[h:78:LEFT:BOTTOM:true:false->true:true' "$rows"; then
    echo "v71 existing-stem reconciliation contract differs" >&2
    exit 1
fi
: <<'DISABLED_V27_ACTIVE'
...
v70_runner_sha=$(shasum -a 256 "$script_dir/run-stems-head-phase-prefix-v70.sh" | awk '{print $1}')
if [ "$v70_runner_sha" != "75be33c3f6bf1e2eae337d9808f967172e7633ffad7fe48e7c48441d7e0740be" ]; then
    echo "v71 base v70 runner drifted" >&2
    exit 1
fi
v70_fixture="$repo_root/rust/oracle/stems-head-phase-prefix-chula-system1-v70.txt"
v70_fixture_sha=$(shasum -a 256 "$v70_fixture" | awk '{print $1}')
if [ "$v70_fixture_sha" != "cf883421cfd392f124598143d6d48e83019c9ddc6cfd416ccc1a676b9558e142" ]; then
    echo "v71 base v70 fixture drifted" >&2
    exit 1
fi
...
out="/private/tmp/stems-head-phase-prefix-chula-system1-v71-audit.txt"
{
    echo '# Java Audiveris 5.11 (Temurin JDK 25.0.3+9 LTS) post-STUMPS head phase v71.'
    echo '# schema: stems-head-phase-prefix-v71'
    echo '# Snapshot-minimized order-71 derivative: orders 1-70 mutate without snapshots; order 71 emits existing-stem reconciliation.'
    echo '# This bounded scope is intentional for deterministic replay under the full-snapshot heap limit.'
    cat "$rows"
    printf '%s\n' \
        "stemsheadphaseprefix summary schema stems-head-phase-prefix-v71 page chula.png#1 system 1 rows $row_count baseProbeSourceSha256 $base_probe_sha baseV70RunnerSourceSha256 $v70_runner_sha baseV70FixtureSha256 $v70_fixture_sha fragmentSourceSha256 $fragment_sha probeSourceSha256 $probe_sha runnerSourceSha256 $runner_sha emittedBodySha256 $body_sha semanticPassSha256 $semantic_sha completeStumpsFixtureSha256 $stumps_sha freshRuns 2 freshRunsByteIdentical true nativeScope BoundedSnapshotMinimizedOrder71ExistingStemReconciliation javaEvidence ReturnedBeforeSeventySecondHead"
} > "$out"
echo "wrote $out"
