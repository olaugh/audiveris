...
# Read-only snapshot-minimized order47 continuation from v46; prior orders mutate without snapshots.
set -eu
...
tmp_base=/private/tmp/stems-head-phase-prefix-v47
...
awk -v fragment="$fragment18" '
{
    ...
    if (index($0, "final boolean returned = linker.linkSides(") != 0) {
        print "            if (headOrder == 7 || headOrder == 22 || headOrder == 27 || headOrder == 34 || headOrder == 36 || headOrder == 37 || headOrder == 38 || headOrder == 39 || headOrder == 40 || headOrder == 41 || headOrder == 42 || headOrder == 43 || headOrder == 44 || headOrder == 45 || headOrder == 46 || headOrder == 47) emitHeadCLinkEnvelope(head, linker, before, headOrder);"
        print
        in_linker = 1
        next
    }
    ...
    if (capture_after && index($0, "heads, allLinkers);") != 0) {
        print
        print "            if (headOrder == 7 || headOrder == 22 || headOrder == 27 || headOrder == 34 || headOrder == 36 || headOrder == 37 || headOrder == 38 || headOrder == 39 || headOrder == 40 || headOrder == 41 || headOrder == 42 || headOrder == 43 || headOrder == 44 || headOrder == 45 || headOrder == 46 || headOrder == 47) emitHeadCLinkResult(before, after, headOrder);"
        capture_after = 0
        in_linker = 0
        next
    }
...
if [ "$(grep -c '^stemsheadphasecontinue ' "$rows")" -ne 1 ] || \
        ! grep -q 'headOrder 47 headX 89 headSig 22 headInterId 1329 .*decisions \[LEFT:SkipAlreadyLinked,RIGHT:SkipClosed\].*incident \[stem2359:headSideLEFT:heads\[x89:sig22:id1329:sideLEFT,x90:sig23:id1331:sideLEFT\].*closureWrites \[x90:sig23:LEFT:false->true,x90:sig23:RIGHT:false->true\].*closedValueChanges 2 .*nextHeadOrder 48 nextHeadX 52 nextHeadSig 2 nextHeadInterId 1289 ' "$rows" || \
        ! grep -q '^stemsheadclinkfrontier .*headOrder 47 headX 89 headSig 22 .*cAlias h:89:LEFT:BOTTOM .*lastIndex 0 maxIndex 0 relations 1 .*glyphs 1 .*candidateIdBefore 304 existingGlyph glyph:304 existingActive true existingStem 2359 ' "$rows" || \
        ! grep -q '^stemsheadclinkresult headOrder 47 allocatorBefore 2385 allocatorAfter 2385 .*addedVertices - addedEdges - addedSystemStems - .*linkerChanges \[h:90:LEFT:BOTTOM:true:false->true:true' "$rows"; then
    echo "v47 existing-stem reconciliation contract differs" >&2
    exit 1
fi
: <<'DISABLED_V27_ACTIVE'
...
v46_runner_sha=$(shasum -a 256 "$script_dir/run-stems-head-phase-prefix-v46.sh" | awk '{print $1}')
if [ "$v46_runner_sha" != "aaff9b381d5268c42f9688658071bcafd31736c7987c21a79726eb516483fa78" ]; then
    echo "v47 base v46 runner drifted" >&2
    exit 1
fi
v46_fixture="$repo_root/rust/oracle/stems-head-phase-prefix-chula-system1-v46.txt"
v46_fixture_sha=$(shasum -a 256 "$v46_fixture" | awk '{print $1}')
if [ "$v46_fixture_sha" != "017cfeddc3faeedda3aca5308c82251135bd0c3308854385f77271cb7fc76f8d" ]; then
    echo "v47 base v46 fixture drifted" >&2
    exit 1
fi
...
out="/private/tmp/stems-head-phase-prefix-chula-system1-v47-audit.txt"
{
    echo '# Java Audiveris 5.11 (Temurin JDK 25.0.3+9 LTS) post-STUMPS head phase v47.'
    echo '# schema: stems-head-phase-prefix-v47'
    echo '# Snapshot-minimized order-47 derivative: orders 1-46 mutate without snapshots; order 47 emits existing-stem reconciliation.'
    echo '# This bounded scope is intentional for deterministic replay under the full-snapshot heap limit.'
    cat "$rows"
    printf '%s\n' \
        "stemsheadphaseprefix summary schema stems-head-phase-prefix-v47 page chula.png#1 system 1 rows $row_count baseProbeSourceSha256 $base_probe_sha baseV46RunnerSourceSha256 $v46_runner_sha baseV46FixtureSha256 $v46_fixture_sha fragmentSourceSha256 $fragment_sha probeSourceSha256 $probe_sha runnerSourceSha256 $runner_sha emittedBodySha256 $body_sha semanticPassSha256 $semantic_sha completeStumpsFixtureSha256 $stumps_sha freshRuns 2 freshRunsByteIdentical true nativeScope BoundedSnapshotMinimizedOrder47ExistingStemReconciliation javaEvidence ReturnedBeforeFortyEighthHead"
} > "$out"
echo "wrote $out"
