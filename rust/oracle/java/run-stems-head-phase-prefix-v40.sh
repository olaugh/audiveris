...
# Freeze a snapshot-minimized order40 existing-stem reconciliation from v39; prior orders mutate without snapshots.
set -eu
...
tmp_base=/private/tmp/stems-head-phase-prefix-v40
...
awk -v fragment="$fragment18" '
{
    ...
    if (index($0, "final boolean returned = linker.linkSides(") != 0) {
        print "            if (headOrder == 7 || headOrder == 22 || headOrder == 27 || headOrder == 34 || headOrder == 36 || headOrder == 37 || headOrder == 38 || headOrder == 39 || headOrder == 40) emitHeadCLinkEnvelope(head, linker, before, headOrder);"
        print
        in_linker = 1
        next
    }
    ...
    if (capture_after && index($0, "heads, allLinkers);") != 0) {
        print
        print "            if (headOrder == 7 || headOrder == 22 || headOrder == 27 || headOrder == 34 || headOrder == 36 || headOrder == 37 || headOrder == 38 || headOrder == 39 || headOrder == 40) emitHeadCLinkResult(before, after, headOrder);"
        capture_after = 0
        in_linker = 0
        next
    }
...
if [ "$(grep -c '^stemsheadphasecontinue ' "$rows")" -ne 1 ] || \
        ! grep -q 'headOrder 40 headX 6 headSig 89 headInterId 1467 .*decisions \[LEFT:SkipAlreadyLinked,RIGHT:top=false:bottom=false:branch=Neither\].*incident \[stem2348:headSideLEFT:heads\[x5:sig88:id1465:sideLEFT,x6:sig89:id1467:sideLEFT\]\].*closureWrites \[x5:sig88:LEFT:false->true,x5:sig88:RIGHT:false->true\].*closedValueChanges 2 .*nextHeadOrder 41 nextHeadX 30 nextHeadSig 67 nextHeadInterId 1423 ' "$rows" || \
        ! grep -q '^stemsheadclinkfrontier .*headOrder 40 headX 6 headSig 89 .*cAlias h:6:LEFT:BOTTOM .*lastIndex 2 maxIndex 2 relations 3 .*glyphs 1 .*candidateIdBefore 290 existingGlyph glyph:290 existingActive true existingStem 2348 ' "$rows" || \
        ! grep -q '^stemsheadclinkresult headOrder 40 allocatorBefore 2385 allocatorAfter 2385 .*addedVertices - addedEdges - addedSystemStems - .*linkerChanges \[h:5:LEFT:BOTTOM:true:false->true:true' "$rows"; then
    echo "v40 existing-stem reconciliation contract differs" >&2
    exit 1
fi
: <<'DISABLED_V27_ACTIVE'
...
v39_runner_sha=$(shasum -a 256 "$script_dir/run-stems-head-phase-prefix-v39.sh" | awk '{print $1}')
if [ "$v39_runner_sha" != "bf7855c0d53d59cea3593de72f51f7272168f488e65148267ebd55e9f70110c7" ]; then
    echo "v40 base v39 runner drifted" >&2
    exit 1
fi
v39_fixture="$repo_root/rust/oracle/stems-head-phase-prefix-chula-system1-v39.txt"
v39_fixture_sha=$(shasum -a 256 "$v39_fixture" | awk '{print $1}')
if [ "$v39_fixture_sha" != "771b7816918d098e66fa1c599df1a68bfb3e24d1724ea6f701ba3bcc59b031fa" ]; then
    echo "v40 base v39 fixture drifted" >&2
    exit 1
fi
...
out="$repo_root/rust/oracle/stems-head-phase-prefix-chula-system1-v40.txt"
{
    echo '# Java Audiveris 5.11 (Temurin JDK 25.0.3+9 LTS) post-STUMPS head phase v40.'
    echo '# schema: stems-head-phase-prefix-v40'
    echo '# Snapshot-minimized order-40 derivative: orders 1-39 mutate without snapshots; order 40 emits existing-stem reconciliation.'
    echo '# This minimized scope is intentional for deterministic replay under the full-snapshot heap limit.'
    cat "$rows"
    printf '%s\n' \
        "stemsheadphaseprefix summary schema stems-head-phase-prefix-v40 page chula.png#1 system 1 rows $row_count baseProbeSourceSha256 $base_probe_sha baseV39RunnerSourceSha256 $v39_runner_sha baseV39FixtureSha256 $v39_fixture_sha fragmentSourceSha256 $fragment_sha probeSourceSha256 $probe_sha runnerSourceSha256 $runner_sha emittedBodySha256 $body_sha semanticPassSha256 $semantic_sha completeStumpsFixtureSha256 $stumps_sha freshRuns 2 freshRunsByteIdentical true nativeScope BoundedSnapshotMinimizedOrder40ExistingStemReconciliation javaEvidence ReturnedBeforeFortyFirstHead"
} > "$out"
echo "wrote $out"
