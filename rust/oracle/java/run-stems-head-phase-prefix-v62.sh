...
# Read-only snapshot-minimized order62 continuation from v61; prior orders mutate without snapshots.
set -eu
...
tmp_base=/private/tmp/stems-head-phase-prefix-v62
...
awk -v fragment="$fragment18" '
{
    ...
    if (capture_after && index($0, "heads, allLinkers);") != 0) {
        print
        print "            if (headOrder == 7 || headOrder == 22 || headOrder == 27 || headOrder == 34 || headOrder == 36 || headOrder == 37 || headOrder == 38 || headOrder == 39 || headOrder == 40 || headOrder == 41 || headOrder == 42 || headOrder == 43 || headOrder == 44 || headOrder == 45 || headOrder == 46 || headOrder == 47 || headOrder == 48 || headOrder == 49 || headOrder == 50 || headOrder == 51 || headOrder == 52 || headOrder == 53 || headOrder == 54 || headOrder == 55 || headOrder == 56 || headOrder == 57 || headOrder == 58 || headOrder == 59 || headOrder == 60 || headOrder == 61 || headOrder == 62) emitHeadCLinkResult(before, after, headOrder);"
        capture_after = 0
        in_linker = 0
        next
    }
...
if [ "$(grep -c '^stemsheadphasecontinue ' "$rows")" -ne 1 ] || \
        ! grep -q 'headOrder 62 headX 9 headSig 8 headInterId 1301 .*decisions \[LEFT:SkipAlreadyLinked,RIGHT:SkipClosed\].*incident \[stem2355:headSideLEFT:heads\[x9:sig8:id1301:sideLEFT,x10:sig9:id1303:sideLEFT\].*closureWrites \[x10:sig9:LEFT:false->true,x10:sig9:RIGHT:false->true\].*closedValueChanges 2 .*nextHeadOrder 63 nextHeadX 41 nextHeadSig 92 nextHeadInterId 1473 ' "$rows" || \
        ! grep -q '^stemsheadclinkfrontier .*headOrder 62 headX 9 headSig 8 .*cAlias h:9:LEFT:BOTTOM .*lastIndex 3 maxIndex 3 relations 4 .*glyphs 1 .*candidateIdBefore 318 existingGlyph glyph:318 existingActive true existingStem 2355 ' "$rows" || \
        ! grep -q '^stemsheadclinkresult headOrder 62 allocatorBefore 2385 allocatorAfter 2385 .*addedVertices - addedEdges - addedSystemStems - .*linkerChanges \[h:10:LEFT:BOTTOM:true:false->true:true' "$rows"; then
    echo "v62 existing-stem reconciliation contract differs" >&2
    exit 1
fi
: <<'DISABLED_V27_ACTIVE'
...
v61_runner_sha=$(shasum -a 256 "$script_dir/run-stems-head-phase-prefix-v61.sh" | awk '{print $1}')
if [ "$v61_runner_sha" != "092987d133085a52c2782db96521258df39f05caefff69ec1a143c560ab3beed" ]; then
    echo "v62 base v61 runner drifted" >&2
    exit 1
fi
v61_fixture="$repo_root/rust/oracle/stems-head-phase-prefix-chula-system1-v61.txt"
v61_fixture_sha=$(shasum -a 256 "$v61_fixture" | awk '{print $1}')
if [ "$v61_fixture_sha" != "205b8875f6846384245629122645b6ac8cbef4bf18573c2ec539552d7799416d" ]; then
    echo "v62 base v61 fixture drifted" >&2
    exit 1
fi
...
out="/private/tmp/stems-head-phase-prefix-chula-system1-v62-audit.txt"
{
    echo '# Java Audiveris 5.11 (Temurin JDK 25.0.3+9 LTS) post-STUMPS head phase v62.'
    echo '# schema: stems-head-phase-prefix-v62'
    echo '# Snapshot-minimized order-62 derivative: orders 1-61 mutate without snapshots; order 62 emits existing-stem reconciliation.'
    echo '# This bounded scope is intentional for deterministic replay under the full-snapshot heap limit.'
    cat "$rows"
    printf '%s\n' \
        "stemsheadphaseprefix summary schema stems-head-phase-prefix-v62 page chula.png#1 system 1 rows $row_count baseProbeSourceSha256 $base_probe_sha baseV61RunnerSourceSha256 $v61_runner_sha baseV61FixtureSha256 $v61_fixture_sha fragmentSourceSha256 $fragment_sha probeSourceSha256 $probe_sha runnerSourceSha256 $runner_sha emittedBodySha256 $body_sha semanticPassSha256 $semantic_sha completeStumpsFixtureSha256 $stumps_sha freshRuns 2 freshRunsByteIdentical true nativeScope BoundedSnapshotMinimizedOrder62ExistingStemReconciliation javaEvidence ReturnedBeforeSixtyThirdHead"
} > "$out"
echo "wrote $out"
