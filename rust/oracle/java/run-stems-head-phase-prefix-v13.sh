...
# Freeze a bounded order-13 continuation replay. Orders below 13 are
# executed without persistent snapshots; only the order-13 row is emitted.
set -eu
...
tmp_base=/private/tmp/stems-head-phase-prefix-v13
...
if ! cmp -s "$semantic_one" "$semantic_two"; then
    echo "two fresh post-STUMPS v13 semantic passes are not byte-identical" >&2
...
if [ "$(grep -c '^stemsheadphasecontinue ' "$rows")" -ne 1 ] || \
        ! grep -q 'headOrder 13 headX 53 headSig 3 headInterId 1291 ' "$rows"; then
    echo "v13 bounded order-13 continuation contract differs" >&2
    exit 1
fi
...
v12_runner_sha=$(shasum -a 256 "$script_dir/run-stems-head-phase-prefix-v12.sh" | awk '{print $1}')
if [ "$v12_runner_sha" != "74b6ba4f84c046ae2ca08e270ce9726acee42a14f4b639282bfbccd3c8b654d1" ]; then
    echo "v13 base v12 runner drifted" >&2
    exit 1
fi
v12_fixture="$repo_root/rust/oracle/stems-head-phase-prefix-chula-system1-v12.txt"
v12_fixture_sha=$(shasum -a 256 "$v12_fixture" | awk '{print $1}')
if [ "$v12_fixture_sha" != "e8b19156d29722a74b41e6d07d1591edd78b3077844f6be7268fa78754a1acd2" ]; then
    echo "v13 base v12 fixture drifted" >&2
    exit 1
fi
...
out="$repo_root/rust/oracle/stems-head-phase-prefix-chula-system1-v13.txt"
{
    echo '# Java Audiveris 5.11 (Temurin JDK 25.0.3+9 LTS) post-STUMPS head phase v13.'
    echo '# schema: stems-head-phase-prefix-v13'
    echo '# Bounded order-13 derivative: orders 1-12 execute linkSides without persistent snapshots; only order 13 is emitted.'
    echo '# This minimized scope is intentional for deterministic replay under the full-snapshot heap limit.'
    cat "$rows"
    printf '%s\n' \
        "stemsheadphaseprefix summary schema stems-head-phase-prefix-v13 page chula.png#1 system 1 rows $row_count baseProbeSourceSha256 $base_probe_sha baseV12RunnerSourceSha256 $v12_runner_sha baseV12FixtureSha256 $v12_fixture_sha probeSourceSha256 $probe_sha runnerSourceSha256 $runner_sha emittedBodySha256 $body_sha semanticPassSha256 $semantic_sha completeStumpsFixtureSha256 $stumps_sha freshRuns 2 freshRunsByteIdentical true nativeScope BoundedSnapshotMinimizedOrder13 javaEvidence ReturnedBeforeFourteenthHead"
} > "$out"
echo "wrote $out"
