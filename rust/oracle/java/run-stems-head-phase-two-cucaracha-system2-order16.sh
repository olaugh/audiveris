...
# Freeze Java system-2 queue 16's reused-stem append.
set -eu
...
tmp_dir=$(mktemp -d /private/tmp/stems-head-phase-two-cucaracha-s2-q16.XXXXXX)
...
retarget_transform="$script_dir/stems-head-phase-two-cucaracha-system2-order16.transform.awk"
...
grep -E '^(stemsheadphase2cucarachas2q16|stemsheadphase2baseline .* system 2 |stemsheadphase2retry .* system 2 queueIndex 16 )' \
    "$tmp_dir/pass1" > "$tmp_dir/rows1"
grep -E '^(stemsheadphase2cucarachas2q16|stemsheadphase2baseline .* system 2 |stemsheadphase2retry .* system 2 queueIndex 16 )' \
    "$tmp_dir/pass2" > "$tmp_dir/rows2"
if ! cmp -s "$tmp_dir/rows1" "$tmp_dir/rows2"; then
    echo "fresh Cucaracha system-2 queue-16 Java passes are not byte-identical" >&2
...
if [ "$(wc -l < "$tmp_dir/rows1" | tr -d ' ')" -ne 7 ] || \
        ...
        ! grep -q '^stemsheadphase2cucarachas2q16frontier headInterId 1394 corner BL hSide LEFT vSide BOTTOM .*lastIndex 3 maxIndex 3 relations 3 .*glyphs 2 selected \[id253:1811:1213:4:128:weight344,id2575:1811:1213:4:128:weight269\] .*existingStem id2654:.*verticesBefore 255 edgesBefore 350 allocatorBefore 2659 terminal ReadyForHeadCreateStem$' "$tmp_dir/rows1" || \
        ! grep -q '^stemsheadphase2cucarachas2q16reusematch headInterId 1394 sourceHeadId 1453 sourceCorner BL sourceSide LEFT relationGrade 3fedcb3654b8677f stem id2654:.*terminal SelectedReuseStem$' "$tmp_dir/rows1" || \
        ! grep -q '^stemsheadphase2cucarachas2q16result headInterId 1394 linkedStem id2654:.*reusedExisting true applied grade3fef148d14458919:dxbf9734df7f4c3cf4:.*verticesBefore 255 verticesAfter 255 edgesBefore 350 edgesAfter 351 allocatorBefore 2659 allocatorAfter 2659 terminal ReturnedHeadCLinkTransaction$' "$tmp_dir/rows1" || \
        ! grep -q '^stemsheadphase2retry page cucaracha.png#1 system 2 queueIndex 16 headX 109 headSig 81 headInterId 1394 grade 3fd65df4633ba537 append true .*decisions \[LEFT:top=false:bottom=true:branch=BottomOnly,RIGHT:top=false:bottom=true:branch=BottomOnly\] returned true .*sideChanges \[x109:sig81:LEFT:false:true->true:true\] .*sigEdgesBefore 350 sigEdgesAfter 351 .*allocatorBefore 2659 allocatorAfter 2659$' "$tmp_dir/rows1"; then
    echo "Cucaracha system-2 queue-16 Java contract differs" >&2
    cat "$tmp_dir/rows1" >&2
    exit 1
fi

base_runner="$script_dir/run-stems-head-phase-two-cucaracha-system2-order10.sh"
base_fixture="$repo_root/rust/oracle/stems-head-phase-two-cucaracha-system2-order10.txt"
base_retarget_transform="$script_dir/stems-head-phase-two-cucaracha-system2-order10.transform.awk"
base_runner_sha=$(shasum -a 256 "$base_runner" | awk '{print $1}')
base_fixture_sha=$(shasum -a 256 "$base_fixture" | awk '{print $1}')
base_retarget_transform_sha=$(shasum -a 256 "$base_retarget_transform" | awk '{print $1}')
if [ "$base_runner_sha" != "12ee38de83ff0a4aae421be6c7c608b0fa91324d117af333dd4f9458cb565906" ] || \
        [ "$base_fixture_sha" != "5e7de8f4ffd9de54985f1e8e1e1c44c0b57330b40b643daf64241996e49a360c" ] || \
        [ "$base_retarget_transform_sha" != "3d076bd7c6ff7e43145545af6969a36b2c415ac4067a317c6f169735c28639e0" ]; then
    echo "Cucaracha system-2 queue-16 strict predecessor drifted" >&2
    exit 1
fi
...
out="$repo_root/rust/oracle/stems-head-phase-two-cucaracha-system2-order16.txt"
{
    echo '# Java Audiveris 5.11 (Temurin JDK 25.0.3+9 LTS) Cucaracha system-2 phase-two queue 16.'
    echo '# schema: stems-head-phase-two-cucaracha-system2-order16-v1'
    cat "$tmp_dir/rows1"
    printf '%s\n' \
        "stemsheadphase2cucarachas2q16summary schema stems-head-phase-two-cucaracha-system2-order16-v1 page cucaracha.png#1 system 2 rows 7 inputSha256 $input_sha baseProbeSourceSha256 $base_probe_sha headLinkerSourceSha256 $head_linker_sha baseTransformSourceSha256 $base_transform_sha retargetTransformSourceSha256 $retarget_transform_sha probeSourceSha256 $probe_sha transformedHeadLinkerSourceSha256 $transformed_head_linker_sha initSourceSha256 $init_sha runnerSourceSha256 $runner_sha baseOrder10RunnerSha256 $base_runner_sha baseOrder10FixtureSha256 $base_fixture_sha baseOrder10RetargetTransformSha256 $base_retarget_transform_sha emittedBodySha256 $body_sha semanticPassSha256 $body_sha freshRuns 2 freshRunsByteIdentical true nativeScope CucarachaSystem2PhaseTwoOrder16ReusedStemAppend javaEvidence ReturnedAfterSystem2RetryIndex16"
} > "$out"
echo "wrote $out"
