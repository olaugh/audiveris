...
# Read-only snapshot-minimized order65 continuation from v64; prior orders mutate without snapshots.
set -eu
...
tmp_base=/private/tmp/stems-head-phase-prefix-v65
...
awk -v fragment="$fragment18" '
{
    ...
    if (capture_after && index($0, "heads, allLinkers);") != 0) {
        print
        print "            if (headOrder == 7 || headOrder == 22 || headOrder == 27 || headOrder == 34 || headOrder == 36 || headOrder == 37 || headOrder == 38 || headOrder == 39 || headOrder == 40 || headOrder == 41 || headOrder == 42 || headOrder == 43 || headOrder == 44 || headOrder == 45 || headOrder == 46 || headOrder == 47 || headOrder == 48 || headOrder == 49 || headOrder == 50 || headOrder == 51 || headOrder == 52 || headOrder == 53 || headOrder == 54 || headOrder == 55 || headOrder == 56 || headOrder == 57 || headOrder == 58 || headOrder == 59 || headOrder == 60 || headOrder == 61 || headOrder == 62 || headOrder == 63 || headOrder == 64 || headOrder == 65) emitHeadCLinkResult(before, after, headOrder);"
        capture_after = 0
        in_linker = 0
        next
    }
...
if [ "$(grep -c '^stemsheadphasecontinue ' "$rows")" -ne 1 ] || \
        ! grep -q 'headOrder 65 headX 58 headSig 73 headInterId 1435 .*decisions \[LEFT:SkipAlreadyLinked,RIGHT:SkipClosed\].*incident \[stem2363:headSideLEFT:heads\[x58:sig73:id1435:sideLEFT,x59:sig74:id1437:sideLEFT\].*closureWrites \[x59:sig74:LEFT:false->true,x59:sig74:RIGHT:false->true\].*closedValueChanges 2 .*nextHeadOrder 66 nextHeadX 13 nextHeadSig 0 nextHeadInterId 1285 ' "$rows" || \
        ! grep -q '^stemsheadclinkfrontier .*headOrder 65 headX 58 headSig 73 .*cAlias h:58:LEFT:BOTTOM .*lastIndex 0 maxIndex 0 relations 1 .*glyphs 1 .*candidateIdBefore 311 existingGlyph glyph:311 existingActive true existingStem 2363 ' "$rows" || \
        ! grep -q '^stemsheadclinkresult headOrder 65 allocatorBefore 2385 allocatorAfter 2385 .*addedVertices - addedEdges - addedSystemStems - .*linkerChanges \[h:59:LEFT:BOTTOM:true:false->true:true' "$rows"; then
    echo "v65 existing-stem reconciliation contract differs" >&2
    exit 1
fi
: <<'DISABLED_V27_ACTIVE'
...
v64_runner_sha=$(shasum -a 256 "$script_dir/run-stems-head-phase-prefix-v64.sh" | awk '{print $1}')
if [ "$v64_runner_sha" != "b9f580dbe3fdb53503ee6699a4ab314996bb0e1b176f19487fb08f810f11cd05" ]; then
    echo "v65 base v64 runner drifted" >&2
    exit 1
fi
v64_fixture="$repo_root/rust/oracle/stems-head-phase-prefix-chula-system1-v64.txt"
v64_fixture_sha=$(shasum -a 256 "$v64_fixture" | awk '{print $1}')
if [ "$v64_fixture_sha" != "2ef4ffd1e739b21998ee8a45652604557b42f41abe185407a9fd254d1e02ed5c" ]; then
    echo "v65 base v64 fixture drifted" >&2
    exit 1
fi
...
out="/private/tmp/stems-head-phase-prefix-chula-system1-v65-audit.txt"
{
    echo '# Java Audiveris 5.11 (Temurin JDK 25.0.3+9 LTS) post-STUMPS head phase v65.'
    echo '# schema: stems-head-phase-prefix-v65'
    echo '# Snapshot-minimized order-65 derivative: orders 1-64 mutate without snapshots; order 65 emits existing-stem reconciliation.'
    echo '# This bounded scope is intentional for deterministic replay under the full-snapshot heap limit.'
    cat "$rows"
    printf '%s\n' \
        "stemsheadphaseprefix summary schema stems-head-phase-prefix-v65 page chula.png#1 system 1 rows $row_count baseProbeSourceSha256 $base_probe_sha baseV64RunnerSourceSha256 $v64_runner_sha baseV64FixtureSha256 $v64_fixture_sha fragmentSourceSha256 $fragment_sha probeSourceSha256 $probe_sha runnerSourceSha256 $runner_sha emittedBodySha256 $body_sha semanticPassSha256 $semantic_sha completeStumpsFixtureSha256 $stumps_sha freshRuns 2 freshRunsByteIdentical true nativeScope BoundedSnapshotMinimizedOrder65ExistingStemReconciliation javaEvidence ReturnedBeforeSixtySixthHead"
} > "$out"
echo "wrote $out"
