...
# Freeze a snapshot-minimized order38 existing-stem reconciliation from v37; prior orders mutate without snapshots.
set -eu
...
tmp_base=/private/tmp/stems-head-phase-prefix-v38
...
awk -v fragment="$fragment18" '
{
    ...
    if (index($0, "final boolean returned = linker.linkSides(") != 0) {
        print "            if (headOrder == 7 || headOrder == 22 || headOrder == 27 || headOrder == 34 || headOrder == 36 || headOrder == 37 || headOrder == 38) emitHeadCLinkEnvelope(head, linker, before, headOrder);"
        print
        in_linker = 1
        next
    }
    ...
    if (capture_after && index($0, "heads, allLinkers);") != 0) {
        print
        print "            if (headOrder == 7 || headOrder == 22 || headOrder == 27 || headOrder == 34 || headOrder == 36 || headOrder == 37 || headOrder == 38) emitHeadCLinkResult(before, after, headOrder);"
        capture_after = 0
        in_linker = 0
        next
    }
...
if [ "$(grep -c '^stemsheadphasecontinue ' "$rows")" -ne 1 ] || \
        ! grep -q 'headOrder 38 headX 18 headSig 4 headInterId 1293 .*decisions \[LEFT:SkipAlreadyLinked,RIGHT:top=false:bottom=false:branch=Neither\].*incident \[stem2372:headSideLEFT:heads\[x17:sig10:id1305:sideLEFT,x18:sig4:id1293:sideLEFT\]\].*closureWrites \[x17:sig10:LEFT:false->true,x17:sig10:RIGHT:false->true\].*closedValueChanges 2 .*nextHeadOrder 39 nextHeadX 97 nextHeadSig 34 nextHeadInterId 1353 ' "$rows" || \
        ! grep -q '^stemsheadclinkfrontier .*headOrder 38 headX 18 headSig 4 .*cAlias h:18:LEFT:BOTTOM .*lastIndex 1 maxIndex 1 relations 2 .*glyphs 1 .*candidateIdBefore 310 existingGlyph glyph:310 existingActive true existingStem 2372 ' "$rows" || \
        ! grep -q '^stemsheadclinkresult headOrder 38 allocatorBefore 2385 allocatorAfter 2385 .*addedVertices - addedEdges - addedSystemStems - .*linkerChanges \[h:17:LEFT:BOTTOM:true:false->true:true' "$rows"; then
    echo "v38 existing-stem reconciliation contract differs" >&2
    exit 1
fi
: <<'DISABLED_V27_ACTIVE'
...
v37_runner_sha=$(shasum -a 256 "$script_dir/run-stems-head-phase-prefix-v37.sh" | awk '{print $1}')
if [ "$v37_runner_sha" != "2fac40e0bf6f49186a994bae499aa371be8bee2152297d325bae067c3f8d5bc1" ]; then
    echo "v38 base v37 runner drifted" >&2
    exit 1
fi
v37_fixture="$repo_root/rust/oracle/stems-head-phase-prefix-chula-system1-v37.txt"
v37_fixture_sha=$(shasum -a 256 "$v37_fixture" | awk '{print $1}')
if [ "$v37_fixture_sha" != "5af8e1928df00217e1780e2e6e0d057c4202b0f1cf46f25d5d889678c5fdf2b8" ]; then
    echo "v38 base v37 fixture drifted" >&2
    exit 1
fi
...
out="$repo_root/rust/oracle/stems-head-phase-prefix-chula-system1-v38.txt"
{
    echo '# Java Audiveris 5.11 (Temurin JDK 25.0.3+9 LTS) post-STUMPS head phase v38.'
    echo '# schema: stems-head-phase-prefix-v38'
    echo '# Snapshot-minimized order-38 derivative: orders 1-37 mutate without snapshots; order 38 emits existing-stem reconciliation.'
    echo '# This minimized scope is intentional for deterministic replay under the full-snapshot heap limit.'
    cat "$rows"
    printf '%s\n' \
        "stemsheadphaseprefix summary schema stems-head-phase-prefix-v38 page chula.png#1 system 1 rows $row_count baseProbeSourceSha256 $base_probe_sha baseV37RunnerSourceSha256 $v37_runner_sha baseV37FixtureSha256 $v37_fixture_sha fragmentSourceSha256 $fragment_sha probeSourceSha256 $probe_sha runnerSourceSha256 $runner_sha emittedBodySha256 $body_sha semanticPassSha256 $semantic_sha completeStumpsFixtureSha256 $stumps_sha freshRuns 2 freshRunsByteIdentical true nativeScope BoundedSnapshotMinimizedOrder38ExistingStemReconciliation javaEvidence ReturnedBeforeThirtyNinthHead"
} > "$out"
echo "wrote $out"
