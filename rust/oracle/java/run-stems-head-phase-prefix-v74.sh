...
# Read-only snapshot-minimized order74 continuation from v73; prior orders mutate without snapshots.
set -eu
...
tmp_base=/private/tmp/stems-head-phase-prefix-v74
...
awk -v fragment="$fragment18" '
{
    ...
    if (capture_after && index($0, "heads, allLinkers);") != 0) {
        print
        print "            if (headOrder == 7 || headOrder == 22 || headOrder == 27 || headOrder == 34 || headOrder == 36 || headOrder == 37 || headOrder == 38 || headOrder == 39 || headOrder == 40 || headOrder == 41 || headOrder == 42 || headOrder == 43 || headOrder == 44 || headOrder == 45 || headOrder == 46 || headOrder == 47 || headOrder == 48 || headOrder == 49 || headOrder == 50 || headOrder == 51 || headOrder == 52 || headOrder == 53 || headOrder == 54 || headOrder == 55 || headOrder == 56 || headOrder == 57 || headOrder == 58 || headOrder == 59 || headOrder == 60 || headOrder == 61 || headOrder == 62 || headOrder == 63 || headOrder == 64 || headOrder == 65 || headOrder == 66 || headOrder == 67 || headOrder == 68 || headOrder == 69 || headOrder == 70 || headOrder == 71 || headOrder == 72 || headOrder == 73 || headOrder == 74) emitHeadCLinkResult(before, after, headOrder);"
        capture_after = 0
        in_linker = 0
        next
    }
...
if [ "$(grep -c '^stemsheadphasecontinue ' "$rows")" -ne 1 ] || \
        ! grep -q 'headOrder 74 headX 49 headSig 71 headInterId 1431 .*decisions \[LEFT:SkipAlreadyLinked,RIGHT:SkipClosed\].*incident \[stem2353:headSideLEFT:heads\[x49:sig71:id1431:sideLEFT,x50:sig72:id1433:sideLEFT\].*closureWrites \[x50:sig72:LEFT:false->true,x50:sig72:RIGHT:false->true\].*closedValueChanges 2 .*nextHeadOrder 75 nextHeadX 31 nextHeadSig 47 nextHeadInterId 1381 ' "$rows" || \
        ! grep -q '^stemsheadclinkfrontier .*headOrder 74 headX 49 headSig 71 .*cAlias h:49:LEFT:BOTTOM .*lastIndex 1 maxIndex 1 relations 1 .*glyphs 2 .*candidateIdBefore 0 existingGlyph glyph:317 existingActive true existingStem 2353 ' "$rows" || \
        ! grep -q '^stemsheadclinkresult headOrder 74 allocatorBefore 2385 allocatorAfter 2385 .*addedVertices - addedEdges - addedSystemStems - .*linkerChanges \[h:50:LEFT:BOTTOM:true:false->true:true' "$rows"; then
    echo "v74 existing-stem reconciliation contract differs" >&2
    exit 1
fi
: <<'DISABLED_V27_ACTIVE'
...
v73_runner_sha=$(shasum -a 256 "$script_dir/run-stems-head-phase-prefix-v73.sh" | awk '{print $1}')
if [ "$v73_runner_sha" != "b9ff88d207e2e54e3f011598ed2224d092794c23eac26caff84fa8d4544131f1" ]; then
    echo "v74 base v73 runner drifted" >&2
    exit 1
fi
v73_fixture="$repo_root/rust/oracle/stems-head-phase-prefix-chula-system1-v73.txt"
v73_fixture_sha=$(shasum -a 256 "$v73_fixture" | awk '{print $1}')
if [ "$v73_fixture_sha" != "52e10c12f512c6ab5a3a495a774f717f0354281342a88a1ffcaa079073333e57" ]; then
    echo "v74 base v73 fixture drifted" >&2
    exit 1
fi
...
out="/private/tmp/stems-head-phase-prefix-chula-system1-v74-audit.txt"
{
    echo '# Java Audiveris 5.11 (Temurin JDK 25.0.3+9 LTS) post-STUMPS head phase v74.'
    echo '# schema: stems-head-phase-prefix-v74'
    echo '# Snapshot-minimized order-74 derivative: orders 1-73 mutate without snapshots; order 74 emits the existing-stem reconciliation.'
    echo '# This bounded scope is intentional for deterministic replay under the full-snapshot heap limit.'
    cat "$rows"
    printf '%s\n' \
        "stemsheadphaseprefix summary schema stems-head-phase-prefix-v74 page chula.png#1 system 1 rows $row_count baseProbeSourceSha256 $base_probe_sha baseV73RunnerSourceSha256 $v73_runner_sha baseV73FixtureSha256 $v73_fixture_sha fragmentSourceSha256 $fragment_sha probeSourceSha256 $probe_sha runnerSourceSha256 $runner_sha emittedBodySha256 $body_sha semanticPassSha256 $semantic_sha completeStumpsFixtureSha256 $stumps_sha freshRuns 2 freshRunsByteIdentical true nativeScope BoundedSnapshotMinimizedOrder74ExistingStemReconciliation javaEvidence ReturnedBeforeSeventyFifthHead"
} > "$out"
echo "wrote $out"
