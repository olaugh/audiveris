...
# Freeze a snapshot-minimized order39 existing-stem reconciliation from v38; prior orders mutate without snapshots.
set -eu
...
tmp_base=/private/tmp/stems-head-phase-prefix-v39
...
awk -v fragment="$fragment18" '
{
    ...
    if (index($0, "final boolean returned = linker.linkSides(") != 0) {
        print "            if (headOrder == 7 || headOrder == 22 || headOrder == 27 || headOrder == 34 || headOrder == 36 || headOrder == 37 || headOrder == 38 || headOrder == 39) emitHeadCLinkEnvelope(head, linker, before, headOrder);"
        print
        in_linker = 1
        next
    }
    ...
    if (capture_after && index($0, "heads, allLinkers);") != 0) {
        print
        print "            if (headOrder == 7 || headOrder == 22 || headOrder == 27 || headOrder == 34 || headOrder == 36 || headOrder == 37 || headOrder == 38 || headOrder == 39) emitHeadCLinkResult(before, after, headOrder);"
        capture_after = 0
        in_linker = 0
        next
    }
...
if [ "$(grep -c '^stemsheadphasecontinue ' "$rows")" -ne 1 ] || \
        ! grep -q 'headOrder 39 headX 97 headSig 34 headInterId 1353 .*decisions \[LEFT:SkipAlreadyLinked,RIGHT:top=false:bottom=false:branch=Neither\].*incident \[stem2373:headSideLEFT:heads\[x96:sig41:id1367:sideLEFT,x97:sig34:id1353:sideLEFT\]\].*closureWrites \[x96:sig41:LEFT:false->true,x96:sig41:RIGHT:false->true\].*closedValueChanges 2 .*nextHeadOrder 40 nextHeadX 6 nextHeadSig 89 nextHeadInterId 1467 ' "$rows" || \
        ! grep -q '^stemsheadclinkfrontier .*headOrder 39 headX 97 headSig 34 .*cAlias h:97:LEFT:BOTTOM .*lastIndex 1 maxIndex 1 relations 2 .*glyphs 1 .*candidateIdBefore 321 existingGlyph glyph:321 existingActive true existingStem 2373 ' "$rows" || \
        ! grep -q '^stemsheadclinkresult headOrder 39 allocatorBefore 2385 allocatorAfter 2385 .*addedVertices - addedEdges - addedSystemStems - .*linkerChanges \[h:96:LEFT:BOTTOM:true:false->true:true' "$rows"; then
    echo "v39 existing-stem reconciliation contract differs" >&2
    exit 1
fi
: <<'DISABLED_V27_ACTIVE'
...
v38_runner_sha=$(shasum -a 256 "$script_dir/run-stems-head-phase-prefix-v38.sh" | awk '{print $1}')
if [ "$v38_runner_sha" != "ad2edbfdf046db3a27b67d81da23f6f30d254cde9c91eb92063df72da10c7551" ]; then
    echo "v39 base v38 runner drifted" >&2
    exit 1
fi
v38_fixture="$repo_root/rust/oracle/stems-head-phase-prefix-chula-system1-v38.txt"
v38_fixture_sha=$(shasum -a 256 "$v38_fixture" | awk '{print $1}')
if [ "$v38_fixture_sha" != "98c8d3c19d50df531d756d6fd50ddbc9f07ce7db24bea47849fff731d5271b0f" ]; then
    echo "v39 base v38 fixture drifted" >&2
    exit 1
fi
...
out="$repo_root/rust/oracle/stems-head-phase-prefix-chula-system1-v39.txt"
{
    echo '# Java Audiveris 5.11 (Temurin JDK 25.0.3+9 LTS) post-STUMPS head phase v39.'
    echo '# schema: stems-head-phase-prefix-v39'
    echo '# Snapshot-minimized order-39 derivative: orders 1-38 mutate without snapshots; order 39 emits existing-stem reconciliation.'
    echo '# This minimized scope is intentional for deterministic replay under the full-snapshot heap limit.'
    cat "$rows"
    printf '%s\n' \
        "stemsheadphaseprefix summary schema stems-head-phase-prefix-v39 page chula.png#1 system 1 rows $row_count baseProbeSourceSha256 $base_probe_sha baseV38RunnerSourceSha256 $v38_runner_sha baseV38FixtureSha256 $v38_fixture_sha fragmentSourceSha256 $fragment_sha probeSourceSha256 $probe_sha runnerSourceSha256 $runner_sha emittedBodySha256 $body_sha semanticPassSha256 $semantic_sha completeStumpsFixtureSha256 $stumps_sha freshRuns 2 freshRunsByteIdentical true nativeScope BoundedSnapshotMinimizedOrder39ExistingStemReconciliation javaEvidence ReturnedBeforeFortiethHead"
} > "$out"
echo "wrote $out"
