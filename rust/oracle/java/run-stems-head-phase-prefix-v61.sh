...
# Read-only snapshot-minimized order61 continuation from v60; prior orders mutate without snapshots.
set -eu
...
tmp_base=/private/tmp/stems-head-phase-prefix-v61
...
awk -v fragment="$fragment18" '
{
    ...
    if (capture_after && index($0, "heads, allLinkers);") != 0) {
        print
        print "            if (headOrder == 7 || headOrder == 22 || headOrder == 27 || headOrder == 34 || headOrder == 36 || headOrder == 37 || headOrder == 38 || headOrder == 39 || headOrder == 40 || headOrder == 41 || headOrder == 42 || headOrder == 43 || headOrder == 44 || headOrder == 45 || headOrder == 46 || headOrder == 47 || headOrder == 48 || headOrder == 49 || headOrder == 50 || headOrder == 51 || headOrder == 52 || headOrder == 53 || headOrder == 54 || headOrder == 55 || headOrder == 56 || headOrder == 57 || headOrder == 58 || headOrder == 59 || headOrder == 60 || headOrder == 61) emitHeadCLinkResult(before, after, headOrder);"
        capture_after = 0
        in_linker = 0
        next
    }
...
if [ "$(grep -c '^stemsheadphasecontinue ' "$rows")" -ne 1 ] || \
        ! grep -q 'headOrder 61 headX 70 headSig 46 headInterId 1377 .*decisions \[LEFT:top=true:bottom=true:branch=Both,RIGHT:top=true:bottom=false:branch=TopOnly\].*incident - returned false .*undefs \[LEFT\].*closedValueChanges 0 .*nextHeadOrder 62 nextHeadX 9 nextHeadSig 8 nextHeadInterId 1301 ' "$rows" || \
        ! grep -q '^stemsheadclinkfrontier .*headOrder 61 headX 70 headSig 46 .*cAlias h:70:LEFT:BOTTOM .*lastIndex -1 maxIndex 1 relations 1 .*glyphs 2 .*candidateIdBefore 0 existingGlyph glyph:332 existingActive true existingStem 2382 ' "$rows" || \
        ! grep -q '^stemsheadclinkresult headOrder 61 allocatorBefore 2385 allocatorAfter 2385 .*addedVertices - addedEdges - addedSystemStems - linkerChanges - .*terminal ReturnedHeadCLinkTransaction' "$rows"; then
    echo "v61 open-frontier contract differs" >&2
    exit 1
fi
: <<'DISABLED_V27_ACTIVE'
...
v60_runner_sha=$(shasum -a 256 "$script_dir/run-stems-head-phase-prefix-v60.sh" | awk '{print $1}')
if [ "$v60_runner_sha" != "89c0d77c4e5829e6faf91179f801812bf97e27df06f0d50d9afcf4aaba63282b" ]; then
    echo "v61 base v60 runner drifted" >&2
    exit 1
fi
v60_fixture="$repo_root/rust/oracle/stems-head-phase-prefix-chula-system1-v60.txt"
v60_fixture_sha=$(shasum -a 256 "$v60_fixture" | awk '{print $1}')
if [ "$v60_fixture_sha" != "5e031afa32387b6b8a3f097d98b504b32c4216b62b1d67545fe76d3e05b39a28" ]; then
    echo "v61 base v60 fixture drifted" >&2
    exit 1
fi
...
out="/private/tmp/stems-head-phase-prefix-chula-system1-v61-audit.txt"
{
    echo '# Java Audiveris 5.11 (Temurin JDK 25.0.3+9 LTS) post-STUMPS head phase v61.'
    echo '# schema: stems-head-phase-prefix-v61'
    echo '# Snapshot-minimized order-61 derivative: orders 1-60 mutate without snapshots; order 61 emits the open-frontier transaction.'
    echo '# This bounded scope is intentional for deterministic replay under the full-snapshot heap limit.'
    cat "$rows"
    printf '%s\n' \
        "stemsheadphaseprefix summary schema stems-head-phase-prefix-v61 page chula.png#1 system 1 rows $row_count baseProbeSourceSha256 $base_probe_sha baseV60RunnerSourceSha256 $v60_runner_sha baseV60FixtureSha256 $v60_fixture_sha fragmentSourceSha256 $fragment_sha probeSourceSha256 $probe_sha runnerSourceSha256 $runner_sha emittedBodySha256 $body_sha semanticPassSha256 $semantic_sha completeStumpsFixtureSha256 $stumps_sha freshRuns 2 freshRunsByteIdentical true nativeScope BoundedSnapshotMinimizedOrder61OpenFrontier javaEvidence ReturnedBeforeSixtySecondHead"
} > "$out"
echo "wrote $out"
