...
# Freeze a snapshot-minimized order30 continuation from v29; prior orders mutate without snapshots.
set -eu
...
tmp_base=/private/tmp/stems-head-phase-prefix-v30
...
if [ "$(grep -c '^stemsheadphasecontinue ' "$rows")" -ne 1 ] || \
        ! grep -q 'headOrder 30 headX 101 headSig 43 headInterId 1371 .*decisions \[LEFT:SkipAlreadyLinked,RIGHT:top=false:bottom=false:branch=Neither\].*incident \[stem2343:headSideLEFT:heads\[x100:sig42:id1369:sideLEFT,x101:sig43:id1371:sideLEFT\]\].*closureWrites \[x100:sig42:LEFT:false->true,x100:sig42:RIGHT:false->true\].*closedValueChanges 2 .*nextHeadOrder 31 nextHeadX 16 nextHeadSig 81 nextHeadInterId 1451 ' "$rows"; then
    echo "v30 minimized closure contract differs" >&2
    exit 1
fi
: <<'DISABLED_V27_ACTIVE'
...
v29_runner_sha=$(shasum -a 256 "$script_dir/run-stems-head-phase-prefix-v29.sh" | awk '{print $1}')
if [ "$v29_runner_sha" != "0ae5afb409d11eef138ed62bb8adbefb04eabfa99c0581cad7a6952ecb5e1d4c" ]; then
    echo "v30 base v29 runner drifted" >&2
    exit 1
fi
v29_fixture="$repo_root/rust/oracle/stems-head-phase-prefix-chula-system1-v29.txt"
v29_fixture_sha=$(shasum -a 256 "$v29_fixture" | awk '{print $1}')
if [ "$v29_fixture_sha" != "a88b9fd3c27133c3c8bdcc839308365557c0e95c2ac3ea83fe348dc0d1ffa270" ]; then
    echo "v30 base v29 fixture drifted" >&2
    exit 1
fi
...
out="$repo_root/rust/oracle/stems-head-phase-prefix-chula-system1-v30.txt"
{
    echo '# Java Audiveris 5.11 (Temurin JDK 25.0.3+9 LTS) post-STUMPS head phase v30.'
    echo '# schema: stems-head-phase-prefix-v30'
    echo '# Snapshot-minimized order-30 derivative: orders 1-29 mutate without snapshots; order 30 emits the prelinked closure row.'
    echo '# This minimized scope is intentional for deterministic replay under the full-snapshot heap limit.'
    cat "$rows"
    printf '%s\n' \
        "stemsheadphaseprefix summary schema stems-head-phase-prefix-v30 page chula.png#1 system 1 rows $row_count baseProbeSourceSha256 $base_probe_sha baseV29RunnerSourceSha256 $v29_runner_sha baseV29FixtureSha256 $v29_fixture_sha fragmentSourceSha256 $fragment_sha probeSourceSha256 $probe_sha runnerSourceSha256 $runner_sha emittedBodySha256 $body_sha semanticPassSha256 $semantic_sha completeStumpsFixtureSha256 $stumps_sha freshRuns 2 freshRunsByteIdentical true nativeScope BoundedSnapshotMinimizedOrder30PrelinkedClosure javaEvidence ReturnedBeforeThirtyFirstHead"
} > "$out"
echo "wrote $out"
