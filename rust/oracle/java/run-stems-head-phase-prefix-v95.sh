...
# Read-only snapshot-minimized order95 continuation from v94; prior orders mutate without snapshots.
set -eu
...
tmp_base=/private/tmp/stems-head-phase-prefix-v95
...
awk -v fragment="$fragment18" '
{
    ...
    if (capture_after && index($0, "heads, allLinkers);") != 0) {
        print
        print "            if (headOrder == 7 || headOrder == 22 || headOrder == 27 || headOrder == 34 || headOrder == 36 || headOrder == 37 || headOrder == 38 || headOrder == 39 || headOrder == 40 || headOrder == 41 || headOrder == 42 || headOrder == 43 || headOrder == 44 || headOrder == 45 || headOrder == 46 || headOrder == 47 || headOrder == 48 || headOrder == 49 || headOrder == 50 || headOrder == 51 || headOrder == 52 || headOrder == 53 || headOrder == 54 || headOrder == 55 || headOrder == 56 || headOrder == 57 || headOrder == 58 || headOrder == 59 || headOrder == 60 || headOrder == 61 || headOrder == 62 || headOrder == 63 || headOrder == 64 || headOrder == 65 || headOrder == 66 || headOrder == 67 || headOrder == 68 || headOrder == 69 || headOrder == 70 || headOrder == 71 || headOrder == 72 || headOrder == 73 || headOrder == 74 || headOrder == 75 || headOrder == 76 || headOrder == 77 || headOrder == 78 || headOrder == 79 || headOrder == 80 || headOrder == 81 || headOrder == 82 || headOrder == 83 || headOrder == 84 || headOrder == 85 || headOrder == 86 || headOrder == 87 || headOrder == 88 || headOrder == 89 || headOrder == 90 || headOrder == 91 || headOrder == 92 || headOrder == 93 || headOrder == 94 || headOrder == 95) emitHeadCLinkResult(before, after, headOrder);"
        capture_after = 0
        in_linker = 0
        next
    }
...
if [ "$(grep -c '^stemsheadphasecontinue ' "$rows")" -ne 1 ] || \
        ! grep -q 'headOrder 95 headX 7 headSig 52 headInterId 1393 .*decisions \[LEFT:SkipAlreadyLinked,RIGHT:SkipClosed\].*incident \[stem2376:headSideLEFT:heads\[x8:sig53:id1395:sideLEFT,x7:sig52:id1393:sideLEFT\].*closureWrites \[x8:sig53:LEFT:false->true,x8:sig53:RIGHT:false->true\].*closedValueChanges 2 .*nextHeadOrder 96 nextHeadX 60 nextHeadSig 30 nextHeadInterId 1345 ' "$rows" || \
        ! grep -q '^stemsheadclinkfrontier .*headOrder 95 headX 7 headSig 52 .*cAlias h:7:LEFT:BOTTOM .*lastIndex 1 maxIndex 1 relations 2 .*glyphs 1 .*candidateIdBefore 305 existingGlyph glyph:305 existingActive true existingStem 2376 ' "$rows" || \
        ! grep -q '^stemsheadclinkresult headOrder 95 allocatorBefore 2385 allocatorAfter 2385 .*addedVertices - addedEdges - addedSystemStems - .*linkerChanges \[h:8:LEFT:BOTTOM:true:false->true:true' "$rows"; then
    echo "v95 existing-stem reconciliation contract differs" >&2
    exit 1
fi
: <<'DISABLED_V27_ACTIVE'
...
v94_runner_sha=$(shasum -a 256 "$script_dir/run-stems-head-phase-prefix-v94.sh" | awk '{print $1}')
if [ "$v94_runner_sha" != "d57ea24bda43d732338ad2aa89aeaec9d5f863b733fd9e5eb17853cc76aa4720" ]; then
    echo "v95 base v94 runner drifted" >&2
    exit 1
fi
v94_fixture="$repo_root/rust/oracle/stems-head-phase-prefix-chula-system1-v94.txt"
v94_fixture_sha=$(shasum -a 256 "$v94_fixture" | awk '{print $1}')
if [ "$v94_fixture_sha" != "2ea83650480f971d7a2b79e3c1544dc05707e020d2ffa5f0e480550bdcce095c" ]; then
    echo "v95 base v94 fixture drifted" >&2
    exit 1
fi
...
out="/private/tmp/stems-head-phase-prefix-chula-system1-v95-audit.txt"
{
    echo '# Java Audiveris 5.11 (Temurin JDK 25.0.3+9 LTS) post-STUMPS head phase v95.'
    echo '# schema: stems-head-phase-prefix-v95'
    echo '# Snapshot-minimized order-95 derivative: orders 1-94 mutate without snapshots; order 95 emits the existing-stem reconciliation.'
    echo '# This bounded scope is intentional for deterministic replay under the full-snapshot heap limit.'
    cat "$rows"
    printf '%s\n' \
        "stemsheadphaseprefix summary schema stems-head-phase-prefix-v95 page chula.png#1 system 1 rows $row_count baseProbeSourceSha256 $base_probe_sha baseV94RunnerSourceSha256 $v94_runner_sha baseV94FixtureSha256 $v94_fixture_sha fragmentSourceSha256 $fragment_sha probeSourceSha256 $probe_sha runnerSourceSha256 $runner_sha emittedBodySha256 $body_sha semanticPassSha256 $semantic_sha completeStumpsFixtureSha256 $stumps_sha freshRuns 2 freshRunsByteIdentical true nativeScope BoundedSnapshotMinimizedOrder95ExistingStemReconciliation javaEvidence ReturnedBeforeNinetySixthHead"
} > "$out"
echo "wrote $out"
