...
# Read-only snapshot-minimized order69 continuation from v68; prior orders mutate without snapshots.
set -eu
...
tmp_base=/private/tmp/stems-head-phase-prefix-v69
...
awk -v fragment="$fragment18" '
{
    ...
    if (capture_after && index($0, "heads, allLinkers);") != 0) {
        print
        print "            if (headOrder == 7 || headOrder == 22 || headOrder == 27 || headOrder == 34 || headOrder == 36 || headOrder == 37 || headOrder == 38 || headOrder == 39 || headOrder == 40 || headOrder == 41 || headOrder == 42 || headOrder == 43 || headOrder == 44 || headOrder == 45 || headOrder == 46 || headOrder == 47 || headOrder == 48 || headOrder == 49 || headOrder == 50 || headOrder == 51 || headOrder == 52 || headOrder == 53 || headOrder == 54 || headOrder == 55 || headOrder == 56 || headOrder == 57 || headOrder == 58 || headOrder == 59 || headOrder == 60 || headOrder == 61 || headOrder == 62 || headOrder == 63 || headOrder == 64 || headOrder == 65 || headOrder == 66 || headOrder == 67 || headOrder == 68 || headOrder == 69) emitHeadCLinkResult(before, after, headOrder);"
        capture_after = 0
        in_linker = 0
        next
    }
...
if [ "$(grep -c '^stemsheadphasecontinue ' "$rows")" -ne 1 ] || \
        ! grep -q 'headOrder 69 headX 87 headSig 83 headInterId 1455 .*decisions \[LEFT:SkipAlreadyLinked,RIGHT:SkipClosed\].*incident \[stem2367:headSideLEFT:heads\[x87:sig83:id1455:sideLEFT,x88:sig84:id1457:sideLEFT\].*closureWrites \[x88:sig84:LEFT:false->true,x88:sig84:RIGHT:false->true\].*closedValueChanges 2 .*nextHeadOrder 70 nextHeadX 1 nextHeadSig 35 nextHeadInterId 1355 ' "$rows" || \
        ! grep -q '^stemsheadclinkfrontier .*headOrder 69 headX 87 headSig 83 .*cAlias h:87:LEFT:BOTTOM .*lastIndex 0 maxIndex 0 relations 1 .*glyphs 1 .*candidateIdBefore 295 existingGlyph glyph:295 existingActive true existingStem 2367 ' "$rows" || \
        ! grep -q '^stemsheadclinkresult headOrder 69 allocatorBefore 2385 allocatorAfter 2385 .*addedVertices - addedEdges - addedSystemStems - .*linkerChanges \[h:88:LEFT:BOTTOM:true:false->true:true' "$rows"; then
    echo "v69 existing-stem reconciliation contract differs" >&2
    exit 1
fi
: <<'DISABLED_V27_ACTIVE'
...
v68_runner_sha=$(shasum -a 256 "$script_dir/run-stems-head-phase-prefix-v68.sh" | awk '{print $1}')
if [ "$v68_runner_sha" != "0316c6aa084df10326c34f886443420360b82e81918d935ca0da7187b9f4acbf" ]; then
    echo "v69 base v68 runner drifted" >&2
    exit 1
fi
v68_fixture="$repo_root/rust/oracle/stems-head-phase-prefix-chula-system1-v68.txt"
v68_fixture_sha=$(shasum -a 256 "$v68_fixture" | awk '{print $1}')
if [ "$v68_fixture_sha" != "b6be7975fc45961e8d5ff869151c6b6fc03ceac63dd13cefb700c05a75d05e48" ]; then
    echo "v69 base v68 fixture drifted" >&2
    exit 1
fi
...
out="/private/tmp/stems-head-phase-prefix-chula-system1-v69-audit.txt"
{
    echo '# Java Audiveris 5.11 (Temurin JDK 25.0.3+9 LTS) post-STUMPS head phase v69.'
    echo '# schema: stems-head-phase-prefix-v69'
    echo '# Snapshot-minimized order-69 derivative: orders 1-68 mutate without snapshots; order 69 emits existing-stem reconciliation.'
    echo '# This bounded scope is intentional for deterministic replay under the full-snapshot heap limit.'
    cat "$rows"
    printf '%s\n' \
        "stemsheadphaseprefix summary schema stems-head-phase-prefix-v69 page chula.png#1 system 1 rows $row_count baseProbeSourceSha256 $base_probe_sha baseV68RunnerSourceSha256 $v68_runner_sha baseV68FixtureSha256 $v68_fixture_sha fragmentSourceSha256 $fragment_sha probeSourceSha256 $probe_sha runnerSourceSha256 $runner_sha emittedBodySha256 $body_sha semanticPassSha256 $semantic_sha completeStumpsFixtureSha256 $stumps_sha freshRuns 2 freshRunsByteIdentical true nativeScope BoundedSnapshotMinimizedOrder69ExistingStemReconciliation javaEvidence ReturnedBeforeSeventiethHead"
} > "$out"
echo "wrote $out"
