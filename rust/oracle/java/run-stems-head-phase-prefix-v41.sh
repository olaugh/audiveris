...
# Freeze a snapshot-minimized order41 existing-stem reconciliation from v40; prior orders mutate without snapshots.
set -eu
...
tmp_base=/private/tmp/stems-head-phase-prefix-v41
...
awk -v fragment="$fragment18" '
{
    ...
    if (index($0, "final boolean returned = linker.linkSides(") != 0) {
        print "            if (headOrder == 7 || headOrder == 22 || headOrder == 27 || headOrder == 34 || headOrder == 36 || headOrder == 37 || headOrder == 38 || headOrder == 39 || headOrder == 40 || headOrder == 41) emitHeadCLinkEnvelope(head, linker, before, headOrder);"
        print
        in_linker = 1
        next
    }
    ...
    if (capture_after && index($0, "heads, allLinkers);") != 0) {
        print
        print "            if (headOrder == 7 || headOrder == 22 || headOrder == 27 || headOrder == 34 || headOrder == 36 || headOrder == 37 || headOrder == 38 || headOrder == 39 || headOrder == 40 || headOrder == 41) emitHeadCLinkResult(before, after, headOrder);"
        capture_after = 0
        in_linker = 0
        next
    }
...
if [ "$(grep -c '^stemsheadphasecontinue ' "$rows")" -ne 1 ] || \
        ! grep -q 'headOrder 41 headX 30 headSig 67 headInterId 1423 .*decisions \[LEFT:SkipAlreadyLinked,RIGHT:top=false:bottom=false:branch=Neither\].*incident \[stem2357:headSideLEFT:heads\[x29:sig66:id1421:sideLEFT,x30:sig67:id1423:sideLEFT\]\].*closureWrites \[x29:sig66:LEFT:false->true,x29:sig66:RIGHT:false->true\].*closedValueChanges 2 .*nextHeadOrder 42 nextHeadX 43 nextHeadSig 48 nextHeadInterId 1385 ' "$rows" || \
        ! grep -q '^stemsheadclinkfrontier .*headOrder 41 headX 30 headSig 67 .*cAlias h:30:LEFT:BOTTOM .*lastIndex 4 maxIndex 4 relations 4 .*glyphs 2 .*candidateIdBefore 0 existingGlyph glyph:313 existingActive true existingStem 2357 ' "$rows" || \
        ! grep -q '^stemsheadclinkresult headOrder 41 allocatorBefore 2385 allocatorAfter 2385 .*addedVertices - addedEdges - addedSystemStems - .*linkerChanges \[h:29:LEFT:BOTTOM:true:false->true:true' "$rows"; then
    echo "v41 existing-stem reconciliation contract differs" >&2
    exit 1
fi
: <<'DISABLED_V27_ACTIVE'
...
v40_runner_sha=$(shasum -a 256 "$script_dir/run-stems-head-phase-prefix-v40.sh" | awk '{print $1}')
if [ "$v40_runner_sha" != "7caaaf046770aafb327359fc587ed54509a83ec867a90a8c53cd254b2de5cb45" ]; then
    echo "v41 base v40 runner drifted" >&2
    exit 1
fi
v40_fixture="$repo_root/rust/oracle/stems-head-phase-prefix-chula-system1-v40.txt"
v40_fixture_sha=$(shasum -a 256 "$v40_fixture" | awk '{print $1}')
if [ "$v40_fixture_sha" != "26e4a2ecbd547829c573c4c7737331e4773f6faf64581ecfdf380a6b87283fa9" ]; then
    echo "v41 base v40 fixture drifted" >&2
    exit 1
fi
...
out="$repo_root/rust/oracle/stems-head-phase-prefix-chula-system1-v41.txt"
{
    echo '# Java Audiveris 5.11 (Temurin JDK 25.0.3+9 LTS) post-STUMPS head phase v41.'
    echo '# schema: stems-head-phase-prefix-v41'
    echo '# Snapshot-minimized order-41 derivative: orders 1-40 mutate without snapshots; order 41 emits existing-stem reconciliation.'
    echo '# This minimized scope is intentional for deterministic replay under the full-snapshot heap limit.'
    cat "$rows"
    printf '%s\n' \
        "stemsheadphaseprefix summary schema stems-head-phase-prefix-v41 page chula.png#1 system 1 rows $row_count baseProbeSourceSha256 $base_probe_sha baseV40RunnerSourceSha256 $v40_runner_sha baseV40FixtureSha256 $v40_fixture_sha fragmentSourceSha256 $fragment_sha probeSourceSha256 $probe_sha runnerSourceSha256 $runner_sha emittedBodySha256 $body_sha semanticPassSha256 $semantic_sha completeStumpsFixtureSha256 $stumps_sha freshRuns 2 freshRunsByteIdentical true nativeScope BoundedSnapshotMinimizedOrder41ExistingStemReconciliation javaEvidence ReturnedBeforeFortySecondHead"
} > "$out"
echo "wrote $out"
