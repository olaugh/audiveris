...
# Freeze a bounded order-27 continuation replay from v26, including the
# both-open C-link envelope/result at x33/SIG26. Orders below 18 execute without snapshots.
set -eu
...
tmp_base=/private/tmp/stems-head-phase-prefix-v27
...
fragment18="$script_dir/stems-head-phase-v27-fragment.java"
...
v26_runner_sha=$(shasum -a 256 "$script_dir/run-stems-head-phase-prefix-v26.sh" | awk '{print $1}')
if [ "$v26_runner_sha" != "afe60083e9b34076c7aab0106216eb5dac7ba689c63ef388112f7b700f842ed0" ]; then
    echo "v27 base v26 runner drifted" >&2
    exit 1
fi
v26_fixture="$repo_root/rust/oracle/stems-head-phase-prefix-chula-system1-v26.txt"
v26_fixture_sha=$(shasum -a 256 "$v26_fixture" | awk '{print $1}')
if [ "$v26_fixture_sha" != "a5e6a9cb07b49ecf1753fbe10ba709a63d274dce5393887acddc123e55342c36" ]; then
    echo "v27 base v26 fixture drifted" >&2
    exit 1
fi
...
out="$repo_root/rust/oracle/stems-head-phase-prefix-chula-system1-v27.txt"
{
    echo '# Java Audiveris 5.11 (Temurin JDK 25.0.3+9 LTS) post-STUMPS head phase v27.'
    echo '# schema: stems-head-phase-prefix-v27'
    echo '# Bounded order-27 derivative: order 27 emits both-open C-link envelope/result plus continuation.'
    echo '# This minimized scope is intentional for deterministic replay under the full-snapshot heap limit.'
    cat "$rows"
    printf '%s\n' \
        "stemsheadphaseprefix summary schema stems-head-phase-prefix-v27 page chula.png#1 system 1 rows $row_count baseProbeSourceSha256 $base_probe_sha baseV26RunnerSourceSha256 $v26_runner_sha baseV26FixtureSha256 $v26_fixture_sha fragmentSourceSha256 $fragment_sha probeSourceSha256 $probe_sha runnerSourceSha256 $runner_sha emittedBodySha256 $body_sha semanticPassSha256 $semantic_sha completeStumpsFixtureSha256 $stumps_sha freshRuns 2 freshRunsByteIdentical true nativeScope BoundedSnapshotMinimizedOrder27BothOpenCLink javaEvidence ReturnedBeforeTwentyEighthHead"
} > "$out"
echo "wrote $out"
