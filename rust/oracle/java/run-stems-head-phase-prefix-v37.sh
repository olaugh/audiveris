...
# Freeze a snapshot-minimized order37 existing-stem reconciliation from v36; prior orders mutate without snapshots.
set -eu
...
tmp_base=/private/tmp/stems-head-phase-prefix-v37
...
awk -v fragment="$fragment18" '
{
    ...
    if (index($0, "final boolean returned = linker.linkSides(") != 0) {
        print "            if (headOrder == 7 || headOrder == 22 || headOrder == 27 || headOrder == 34 || headOrder == 36 || headOrder == 37) emitHeadCLinkEnvelope(head, linker, before, headOrder);"
        print
        in_linker = 1
        next
    }
    ...
    if (capture_after && index($0, "heads, allLinkers);") != 0) {
        print
        print "            if (headOrder == 7 || headOrder == 22 || headOrder == 27 || headOrder == 34 || headOrder == 36 || headOrder == 37) emitHeadCLinkResult(before, after, headOrder);"
        capture_after = 0
        in_linker = 0
        next
    }
...
if [ "$(grep -c '^stemsheadphasecontinue ' "$rows")" -ne 1 ] || \
        ! grep -q 'headOrder 37 headX 14 headSig 1 headInterId 1287 .*decisions \[LEFT:SkipAlreadyLinked,RIGHT:top=false:bottom=false:branch=Neither\].*incident \[stem2340:headSideLEFT:heads\[x13:sig0:id1285:sideLEFT,x14:sig1:id1287:sideLEFT\]\].*closureWrites \[x13:sig0:LEFT:false->true,x13:sig0:RIGHT:false->true\].*closedValueChanges 2 .*nextHeadOrder 38 nextHeadX 18 nextHeadSig 4 nextHeadInterId 1293 ' "$rows" || \
        ! grep -q '^stemsheadclinkfrontier .*headOrder 37 headX 14 headSig 1 .*cAlias h:14:LEFT:BOTTOM .*lastIndex 4 maxIndex 4 relations 4 .*glyphs 2 .*candidateIdBefore 0 existingGlyph glyph:294 existingActive true existingStem 2340 ' "$rows" || \
        ! grep -q '^stemsheadclinkresult headOrder 37 allocatorBefore 2385 allocatorAfter 2385 .*addedVertices - addedEdges - addedSystemStems - .*linkerChanges \[h:13:LEFT:BOTTOM:true:false->true:true' "$rows"; then
    echo "v37 existing-stem reconciliation contract differs" >&2
    exit 1
fi
: <<'DISABLED_V27_ACTIVE'
...
v36_runner_sha=$(shasum -a 256 "$script_dir/run-stems-head-phase-prefix-v36.sh" | awk '{print $1}')
if [ "$v36_runner_sha" != "3176407de9bdd88f167e925a2b901f811f230f6b83c5a120ddf031a42ec49fd4" ]; then
    echo "v37 base v36 runner drifted" >&2
    exit 1
fi
v36_fixture="$repo_root/rust/oracle/stems-head-phase-prefix-chula-system1-v36.txt"
v36_fixture_sha=$(shasum -a 256 "$v36_fixture" | awk '{print $1}')
if [ "$v36_fixture_sha" != "7d7d0d17e51c03a145bdff3a739da3aaaa05fb0c5bba20cd9a46468742eb26e7" ]; then
    echo "v37 base v36 fixture drifted" >&2
    exit 1
fi
...
out="$repo_root/rust/oracle/stems-head-phase-prefix-chula-system1-v37.txt"
{
    echo '# Java Audiveris 5.11 (Temurin JDK 25.0.3+9 LTS) post-STUMPS head phase v37.'
    echo '# schema: stems-head-phase-prefix-v37'
    echo '# Snapshot-minimized order-37 derivative: orders 1-36 mutate without snapshots; order 37 emits existing-stem reconciliation.'
    echo '# This minimized scope is intentional for deterministic replay under the full-snapshot heap limit.'
    cat "$rows"
    printf '%s\n' \
        "stemsheadphaseprefix summary schema stems-head-phase-prefix-v37 page chula.png#1 system 1 rows $row_count baseProbeSourceSha256 $base_probe_sha baseV36RunnerSourceSha256 $v36_runner_sha baseV36FixtureSha256 $v36_fixture_sha fragmentSourceSha256 $fragment_sha probeSourceSha256 $probe_sha runnerSourceSha256 $runner_sha emittedBodySha256 $body_sha semanticPassSha256 $semantic_sha completeStumpsFixtureSha256 $stumps_sha freshRuns 2 freshRunsByteIdentical true nativeScope BoundedSnapshotMinimizedOrder37ExistingStemReconciliation javaEvidence ReturnedBeforeThirtyEighthHead"
} > "$out"
echo "wrote $out"
