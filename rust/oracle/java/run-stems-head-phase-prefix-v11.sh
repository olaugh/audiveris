...
# Freeze the v10 head prefix plus the next order-11 prelinked continuation.
set -eu
...
tmp_base=/private/tmp/stems-head-phase-prefix-v11
...
if ! cmp -s "$semantic_one" "$semantic_two"; then
    echo "two fresh post-STUMPS v11 semantic passes are not byte-identical" >&2
...
if [ "$(wc -l < "$rows" | tr -d ' ')" -ne 19 ] || \
        [ "$(grep -c '^stemsheadphasecontinue ' "$rows")" -ne 11 ] || \
        ...
        ! grep -q 'headOrder 10 headX 65 headSig 95 headInterId 1479 .*decisions \[LEFT:SkipAlreadyLinked,RIGHT:top=false:bottom=false:branch=Neither\] incident \[stem2346:headSideLEFT:heads\[x64:sig94:id1477:sideLEFT,x65:sig95:id1479:sideLEFT\]\] returned true .*closureWrites \[x64:sig94:LEFT:false->true,x64:sig94:RIGHT:false->true\] closedValueChanges 2 unlinkedCount 0 .*nextHeadOrder 11 nextHeadX 46 nextHeadSig 57 nextHeadInterId 1403 ' "$rows" || \
        ! grep -q 'headOrder 11 headX 46 headSig 57 headInterId 1403 .*decisions \[LEFT:SkipAlreadyLinked,RIGHT:top=false:bottom=false:branch=Neither\] incident \[stem2377:headSideLEFT:heads\[x44:sig70:id1429:sideLEFT,x45:sig56:id1401:sideLEFT,x46:sig57:id1403:sideLEFT\]\] returned true .*closureWrites \[x44:sig70:LEFT:false->true,x44:sig70:RIGHT:false->true,x45:sig56:LEFT:false->true,x45:sig56:RIGHT:false->true\] closedValueChanges 4 unlinkedCount 0 .*nextHeadOrder 12 nextHeadX 55 nextHeadSig 79 nextHeadInterId 1447 ' "$rows"; then
    echo "v11 order-11 continuation contract differs" >&2
    exit 1
fi
...
v10_runner_sha=$(shasum -a 256 "$script_dir/run-stems-head-phase-prefix-v10.sh" | awk '{print $1}')
if [ "$v10_runner_sha" != "ddf5b4c3f6d726c3e7d91de33d077930ff29254f1e7e84751ee391614978c464" ]; then
    echo "v11 base v10 runner drifted" >&2
    exit 1
fi
v10_fixture="$repo_root/rust/oracle/stems-head-phase-prefix-chula-system1-v10.txt"
v10_fixture_sha=$(shasum -a 256 "$v10_fixture" | awk '{print $1}')
if [ "$v10_fixture_sha" != "7b0bf32fcf75cf792eb67c2c8a52ae9702de215078a54bea7edc7cde853869d0" ]; then
    echo "v11 base v10 fixture drifted" >&2
    exit 1
fi
...
out="$repo_root/rust/oracle/stems-head-phase-prefix-chula-system1-v11.txt"
{
    echo '# Java Audiveris 5.11 (Temurin JDK 25.0.3+9 LTS) post-STUMPS head phase v11.'
    echo '# schema: stems-head-phase-prefix-v11'
    echo '# v10 head continuation plus the next real order-11 phase-1 linkSides call.'
    echo '# Expected rows stay unread until the native transaction returns.'
    cat "$rows"
    printf '%s\n' \
        "stemsheadphaseprefix summary schema stems-head-phase-prefix-v11 page chula.png#1 system 1 rows 19 baseProbeSourceSha256 $base_probe_sha baseV10RunnerSourceSha256 $v10_runner_sha baseV10FixtureSha256 $v10_fixture_sha probeSourceSha256 $probe_sha runnerSourceSha256 $runner_sha emittedBodySha256 $body_sha semanticPassSha256 $semantic_sha completeStumpsFixtureSha256 $stumps_sha freshRuns 2 freshRunsByteIdentical true nativeScope ReturnedOrder11HeadContinuation javaEvidence ReturnedBeforeThirteenthHead"
} > "$out"
echo "wrote $out"
