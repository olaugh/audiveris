...
# Freeze a bounded order-17 continuation replay. Orders below 17 are
# executed without persistent snapshots; only the order-17 row is emitted.
set -eu
...
tmp_base=/private/tmp/stems-head-phase-prefix-v17
...
if ! cmp -s "$semantic_one" "$semantic_two"; then
    echo "two fresh post-STUMPS v17 semantic passes are not byte-identical" >&2
...
if [ "$(grep -c '^stemsheadphasecontinue ' "$rows")" -ne 1 ] || \
        ! grep -q 'headOrder 17 headX 48 headSig 29 headInterId 1343 ' "$rows"; then
    echo "v17 bounded order-17 continuation contract differs" >&2
    exit 1
fi
...
v16_runner_sha=$(shasum -a 256 "$script_dir/run-stems-head-phase-prefix-v16.sh" | awk '{print $1}')
if [ "$v16_runner_sha" != "d6edd52b746acd625c2e516f328c4b43253e23bbbe906ffcdae0b3674eae1dcf" ]; then
    echo "v17 base v16 runner drifted" >&2
    exit 1
fi
v16_fixture="$repo_root/rust/oracle/stems-head-phase-prefix-chula-system1-v16.txt"
v16_fixture_sha=$(shasum -a 256 "$v16_fixture" | awk '{print $1}')
if [ "$v16_fixture_sha" != "04d35bb21c808dc38edd93c0631b3a01af9931efc8f500422646adf8f7123de4" ]; then
    echo "v17 base v16 fixture drifted" >&2
    exit 1
fi
...
out="$repo_root/rust/oracle/stems-head-phase-prefix-chula-system1-v17.txt"
{
    echo '# Java Audiveris 5.11 (Temurin JDK 25.0.3+9 LTS) post-STUMPS head phase v17.'
    echo '# schema: stems-head-phase-prefix-v17'
    echo '# Bounded order-17 derivative: orders 1-16 execute linkSides without persistent snapshots; only order 17 is emitted.'
    echo '# This minimized scope is intentional for deterministic replay under the full-snapshot heap limit.'
    cat "$rows"
    printf '%s\n' \
        "stemsheadphaseprefix summary schema stems-head-phase-prefix-v17 page chula.png#1 system 1 rows $row_count baseProbeSourceSha256 $base_probe_sha baseV16RunnerSourceSha256 $v16_runner_sha baseV16FixtureSha256 $v16_fixture_sha probeSourceSha256 $probe_sha runnerSourceSha256 $runner_sha emittedBodySha256 $body_sha semanticPassSha256 $semantic_sha completeStumpsFixtureSha256 $stumps_sha freshRuns 2 freshRunsByteIdentical true nativeScope BoundedSnapshotMinimizedOrder17 javaEvidence ReturnedBeforeEighteenthHead"
} > "$out"
echo "wrote $out"
