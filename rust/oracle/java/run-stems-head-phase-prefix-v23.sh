...
# Freeze a bounded order-23 continuation replay from v22, including the
# prelinked existing-stem closure at x78/SIG39. Orders below 18 execute without snapshots.
set -eu
...
tmp_base=/private/tmp/stems-head-phase-prefix-v23
...
fragment18="$script_dir/stems-head-phase-v23-fragment.java"
...
if ! cmp -s "$semantic_one" "$semantic_two"; then
    echo "two fresh post-STUMPS v23 semantic passes are not byte-identical" >&2
...
if [ "$(grep -c '^stemsheadphasecontinue ' "$rows")" -ne 6 ] || \
        ...
        ! grep -q 'headOrder 23 headX 78 headSig 39 headInterId 1363 .*decisions \[LEFT:SkipAlreadyLinked,RIGHT:top=false:bottom=false:branch=Neither\].*incident \[stem2370:headSideLEFT:heads\[x77:sig38:id1361:sideLEFT,x78:sig39:id1363:sideLEFT\]\].*closureWrites \[x77:sig38:LEFT:false->true,x77:sig38:RIGHT:false->true\].*closedValueChanges 2 unlinkedCount 0 sigVerticesBefore 682 sigVerticesAfter 682 sigEdgesBefore 693 sigEdgesAfter 693 systemStemsBefore 43 systemStemsAfter 43 relationStateHashBefore 3c50fb8ad029b80132262f4607e856ffcfadd6aa777d03bd3795288018faac9d relationStateHashAfter 3c50fb8ad029b80132262f4607e856ffcfadd6aa777d03bd3795288018faac9d .*nextHeadOrder 24 nextHeadX 93 nextHeadSig 25 nextHeadInterId 1335 .*nextSides \[LEFT:true:false,RIGHT:false:false\]' "$rows"; then
    echo "v23 bounded prelinked-closure contract differs" >&2
    exit 1
fi
...
v22_runner_sha=$(shasum -a 256 "$script_dir/run-stems-head-phase-prefix-v22.sh" | awk '{print $1}')
if [ "$v22_runner_sha" != "be1091ab266ea190a507291351f50bec4842f50003c75fb048f6bb96537ceebc" ]; then
    echo "v23 base v22 runner drifted" >&2
    exit 1
fi
v22_fixture="$repo_root/rust/oracle/stems-head-phase-prefix-chula-system1-v22.txt"
v22_fixture_sha=$(shasum -a 256 "$v22_fixture" | awk '{print $1}')
if [ "$v22_fixture_sha" != "e7bd66417228bf8fed7fe0c04d904e81ade4026fb00b4c17270b73947f85a1a4" ]; then
    echo "v23 base v22 fixture drifted" >&2
    exit 1
fi
...
out="$repo_root/rust/oracle/stems-head-phase-prefix-chula-system1-v23.txt"
{
    echo '# Java Audiveris 5.11 (Temurin JDK 25.0.3+9 LTS) post-STUMPS head phase v23.'
    echo '# schema: stems-head-phase-prefix-v23'
    echo '# Bounded order-23 derivative: order 22 emits existing-stem retry envelope/result; order 23 emits the prelinked closure continuation.'
    echo '# This minimized scope is intentional for deterministic replay under the full-snapshot heap limit.'
    cat "$rows"
    printf '%s\n' \
        "stemsheadphaseprefix summary schema stems-head-phase-prefix-v23 page chula.png#1 system 1 rows $row_count baseProbeSourceSha256 $base_probe_sha baseV22RunnerSourceSha256 $v22_runner_sha baseV22FixtureSha256 $v22_fixture_sha fragmentSourceSha256 $fragment_sha probeSourceSha256 $probe_sha runnerSourceSha256 $runner_sha emittedBodySha256 $body_sha semanticPassSha256 $semantic_sha completeStumpsFixtureSha256 $stumps_sha freshRuns 2 freshRunsByteIdentical true nativeScope BoundedSnapshotMinimizedOrder23PrelinkedClosure javaEvidence ReturnedBeforeTwentyFourthHead"
} > "$out"
echo "wrote $out"
