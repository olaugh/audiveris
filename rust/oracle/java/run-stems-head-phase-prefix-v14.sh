...
# Freeze a bounded order-14 continuation replay. Orders below 14 are
# executed without persistent snapshots; only the order-14 row is emitted.
set -eu
...
tmp_base=/private/tmp/stems-head-phase-prefix-v14
...
if ! cmp -s "$semantic_one" "$semantic_two"; then
    echo "two fresh post-STUMPS v14 semantic passes are not byte-identical" >&2
...
if [ "$(grep -c '^stemsheadphasecontinue ' "$rows")" -ne 1 ] || \
        ! grep -q 'headOrder 14 headX 12 headSig 63 headInterId 1415 ' "$rows"; then
    echo "v14 bounded order-14 continuation contract differs" >&2
    exit 1
fi
...
v13_runner_sha=$(shasum -a 256 "$script_dir/run-stems-head-phase-prefix-v13.sh" | awk '{print $1}')
if [ "$v13_runner_sha" != "675bce84bfa4e76ed78cc72592da9f8fe95571752d424da99bd4be93af7478f8" ]; then
    echo "v14 base v13 runner drifted" >&2
    exit 1
fi
v13_fixture="$repo_root/rust/oracle/stems-head-phase-prefix-chula-system1-v13.txt"
v13_fixture_sha=$(shasum -a 256 "$v13_fixture" | awk '{print $1}')
if [ "$v13_fixture_sha" != "ff27fa03e80e44e554d46682c827097ecec1d463abf0c0e131a6ab1beccfbb5e" ]; then
    echo "v14 base v13 fixture drifted" >&2
    exit 1
fi
...
out="$repo_root/rust/oracle/stems-head-phase-prefix-chula-system1-v14.txt"
{
    echo '# Java Audiveris 5.11 (Temurin JDK 25.0.3+9 LTS) post-STUMPS head phase v14.'
    echo '# schema: stems-head-phase-prefix-v14'
    echo '# Bounded order-14 derivative: orders 1-13 execute linkSides without persistent snapshots; only order 14 is emitted.'
    echo '# This minimized scope is intentional for deterministic replay under the full-snapshot heap limit.'
    cat "$rows"
    printf '%s\n' \
        "stemsheadphaseprefix summary schema stems-head-phase-prefix-v14 page chula.png#1 system 1 rows $row_count baseProbeSourceSha256 $base_probe_sha baseV13RunnerSourceSha256 $v13_runner_sha baseV13FixtureSha256 $v13_fixture_sha probeSourceSha256 $probe_sha runnerSourceSha256 $runner_sha emittedBodySha256 $body_sha semanticPassSha256 $semantic_sha completeStumpsFixtureSha256 $stumps_sha freshRuns 2 freshRunsByteIdentical true nativeScope BoundedSnapshotMinimizedOrder14 javaEvidence ReturnedBeforeFifteenthHead"
} > "$out"
echo "wrote $out"
