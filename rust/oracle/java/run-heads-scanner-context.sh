...
probe_classes=$(mktemp -d /private/tmp/heads-scanner-context-classes.XXXXXX)
...
run_probe ()
{
    ...
    (
        ...
        env -u JAVA_TOOL_OPTIONS "$JAVA_HOME/bin/java" \
            ...
            org.audiveris.omr.rustport.HeadsScannerContextProbe "$target"
    )
}

env -u JAVA_TOOL_OPTIONS "$JAVA_HOME/bin/java" \
    -cp "$probe_classes:$probe_cp" \
    org.audiveris.omr.rustport.HeadsScannerContextProbe --header
