...
# Read-only snapshot-minimized order63 continuation from v62; prior orders mutate without snapshots.
set -eu
...
tmp_base=/private/tmp/stems-head-phase-prefix-v63
...
awk -v fragment="$fragment18" '
{
    ...
    if (capture_after && index($0, "heads, allLinkers);") != 0) {
        print
        print "            if (headOrder == 7 || headOrder == 22 || headOrder == 27 || headOrder == 34 || headOrder == 36 || headOrder == 37 || headOrder == 38 || headOrder == 39 || headOrder == 40 || headOrder == 41 || headOrder == 42 || headOrder == 43 || headOrder == 44 || headOrder == 45 || headOrder == 46 || headOrder == 47 || headOrder == 48 || headOrder == 49 || headOrder == 50 || headOrder == 51 || headOrder == 52 || headOrder == 53 || headOrder == 54 || headOrder == 55 || headOrder == 56 || headOrder == 57 || headOrder == 58 || headOrder == 59 || headOrder == 60 || headOrder == 61 || headOrder == 62 || headOrder == 63) emitHeadCLinkResult(before, after, headOrder);"
        capture_after = 0
        in_linker = 0
        next
    }
...
if [ "$(grep -c '^stemsheadphasecontinue ' "$rows")" -ne 1 ] || \
        ! grep -q 'headOrder 63 headX 41 headSig 92 headInterId 1473 .*decisions \[LEFT:SkipAlreadyLinked,RIGHT:SkipClosed\].*incident \[stem2352:headSideLEFT:heads\[x41:sig92:id1473:sideLEFT,x42:sig93:id1475:sideLEFT\].*closureWrites \[x42:sig93:LEFT:false->true,x42:sig93:RIGHT:false->true\].*closedValueChanges 2 .*nextHeadOrder 64 nextHeadX 3 nextHeadSig 6 nextHeadInterId 1297 ' "$rows" || \
        ! grep -q '^stemsheadclinkfrontier .*headOrder 63 headX 41 headSig 92 .*cAlias h:41:LEFT:BOTTOM .*lastIndex 3 maxIndex 3 relations 3 .*glyphs 2 .*candidateIdBefore 0 existingGlyph glyph:293 existingActive true existingStem 2352 ' "$rows" || \
        ! grep -q '^stemsheadclinkresult headOrder 63 allocatorBefore 2385 allocatorAfter 2385 .*addedVertices - addedEdges - addedSystemStems - .*linkerChanges \[h:42:LEFT:BOTTOM:true:false->true:true' "$rows"; then
    echo "v63 existing-stem reconciliation contract differs" >&2
    exit 1
fi
: <<'DISABLED_V27_ACTIVE'
...
v62_runner_sha=$(shasum -a 256 "$script_dir/run-stems-head-phase-prefix-v62.sh" | awk '{print $1}')
if [ "$v62_runner_sha" != "8a5762e8b8d569095af4d218211be7ad476ba6b9fb0105f757dd022bc0db4ad5" ]; then
    echo "v63 base v62 runner drifted" >&2
    exit 1
fi
v62_fixture="$repo_root/rust/oracle/stems-head-phase-prefix-chula-system1-v62.txt"
v62_fixture_sha=$(shasum -a 256 "$v62_fixture" | awk '{print $1}')
if [ "$v62_fixture_sha" != "fa45b09cf2503942f7510482a4489ecaf7b3bb82b6d6efb76eb043c28b87889e" ]; then
    echo "v63 base v62 fixture drifted" >&2
    exit 1
fi
...
out="/private/tmp/stems-head-phase-prefix-chula-system1-v63-audit.txt"
{
    echo '# Java Audiveris 5.11 (Temurin JDK 25.0.3+9 LTS) post-STUMPS head phase v63.'
    echo '# schema: stems-head-phase-prefix-v63'
    echo '# Snapshot-minimized order-63 derivative: orders 1-62 mutate without snapshots; order 63 emits existing-stem reconciliation.'
    echo '# This bounded scope is intentional for deterministic replay under the full-snapshot heap limit.'
    cat "$rows"
    printf '%s\n' \
        "stemsheadphaseprefix summary schema stems-head-phase-prefix-v63 page chula.png#1 system 1 rows $row_count baseProbeSourceSha256 $base_probe_sha baseV62RunnerSourceSha256 $v62_runner_sha baseV62FixtureSha256 $v62_fixture_sha fragmentSourceSha256 $fragment_sha probeSourceSha256 $probe_sha runnerSourceSha256 $runner_sha emittedBodySha256 $body_sha semanticPassSha256 $semantic_sha completeStumpsFixtureSha256 $stumps_sha freshRuns 2 freshRunsByteIdentical true nativeScope BoundedSnapshotMinimizedOrder63ExistingStemReconciliation javaEvidence ReturnedBeforeSixtyFourthHead"
} > "$out"
echo "wrote $out"
