...
# Freeze Java Bach system-2 phase-two queue 3's prelinked retry.
set -eu
...
tmp_dir=$(mktemp -d /private/tmp/stems-head-phase-two-bach-s2-q3.XXXXXX)
...
grep -E '^(stemsheadphase2baseline .* system 2 |stemsheadphase2retry .* system 2 queueIndex 3 )' \
    "$tmp_dir/pass1" > "$tmp_dir/rows1"
grep -E '^(stemsheadphase2baseline .* system 2 |stemsheadphase2retry .* system 2 queueIndex 3 )' \
    "$tmp_dir/pass2" > "$tmp_dir/rows2"
if ! cmp -s "$tmp_dir/rows1" "$tmp_dir/rows2"; then
    echo "fresh Bach system-2 phase-two queue-3 Java passes are not byte-identical" >&2
...
if [ "$(wc -l < "$tmp_dir/rows1" | tr -d ' ')" -ne 2 ] || \
        ...
        ! grep -q '^stemsheadphase2retry page BachInvention5.jpg#1 system 2 queueIndex 3 headX 163 headSig 170 headInterId 3951 grade 3fd2894c99225f13 append true sidesBefore \[LEFT:true:true,RIGHT:false:true\] decisions \[LEFT:SkipAlreadyLinked,RIGHT:top=false:bottom=false:branch=Neither\] returned true sidesAfter \[LEFT:true:true,RIGHT:false:true\] undefs \[LEFT\] sideChanges \[\] sigVerticesBefore 394 sigVerticesAfter 394 sigEdgesBefore 600 sigEdgesAfter 600 systemStemsBefore 77 systemStemsAfter 77 allocatorBefore 6815 allocatorAfter 6815$' "$tmp_dir/rows1"; then
    echo "Bach system-2 phase-two queue-3 Java contract differs" >&2
    cat "$tmp_dir/rows1" >&2
    exit 1
fi

base_runner="$script_dir/run-stems-head-phase-two-bach-system2-order2.sh"
...
out="$repo_root/rust/oracle/stems-head-phase-two-bach-system2-order3.txt"
{
    echo '# Java Audiveris 5.11 (Temurin JDK 25.0.3+9 LTS) Bach system-2 phase-two queue 3.'
    echo '# schema: stems-head-phase-two-bach-system2-order3-v1'
    cat "$tmp_dir/rows1"
    printf '%s\n' \
        "stemsheadphase2bachs2q3summary schema stems-head-phase-two-bach-system2-order3-v1 page BachInvention5.jpg#1 system 2 rows 2 inputSha256 $input_sha stemsRetrieverSourceSha256 $stems_source_sha probeSourceSha256 $probe_sha initSourceSha256 $init_sha runnerSourceSha256 $runner_sha baseBoundary244RunnerSha256 $base_runner_sha baseBoundary244FixtureSha256 $base_fixture_sha emittedBodySha256 $body_sha semanticPassSha256 $body_sha freshRuns 2 freshRunsByteIdentical true nativeScope BachSystem2PhaseTwoOrder3PrelinkedClosure javaEvidence ReturnedAfterSystem2RetryIndex3"
} > "$out"
echo "wrote $out"
