...
# Read-only snapshot-minimized order70 continuation from v69; prior orders mutate without snapshots.
set -eu
...
tmp_base=/private/tmp/stems-head-phase-prefix-v70
...
awk -v fragment="$fragment18" '
{
    ...
    if (capture_after && index($0, "heads, allLinkers);") != 0) {
        print
        print "            if (headOrder == 7 || headOrder == 22 || headOrder == 27 || headOrder == 34 || headOrder == 36 || headOrder == 37 || headOrder == 38 || headOrder == 39 || headOrder == 40 || headOrder == 41 || headOrder == 42 || headOrder == 43 || headOrder == 44 || headOrder == 45 || headOrder == 46 || headOrder == 47 || headOrder == 48 || headOrder == 49 || headOrder == 50 || headOrder == 51 || headOrder == 52 || headOrder == 53 || headOrder == 54 || headOrder == 55 || headOrder == 56 || headOrder == 57 || headOrder == 58 || headOrder == 59 || headOrder == 60 || headOrder == 61 || headOrder == 62 || headOrder == 63 || headOrder == 64 || headOrder == 65 || headOrder == 66 || headOrder == 67 || headOrder == 68 || headOrder == 69 || headOrder == 70) emitHeadCLinkResult(before, after, headOrder);"
        capture_after = 0
        in_linker = 0
        next
    }
...
if [ "$(grep -c '^stemsheadphasecontinue ' "$rows")" -ne 1 ] || \
        ! grep -q 'headOrder 70 headX 1 headSig 35 headInterId 1355 .*decisions \[LEFT:top=false:bottom=true:branch=BottomOnly,RIGHT:top=false:bottom=false:branch=Neither\].*incident - returned true .*undefs \[\].*closureWrites - closedValueChanges 0 .*sigEdgesBefore 700 sigEdgesAfter 702 .*nextHeadOrder 71 nextHeadX 77 nextHeadSig 38 nextHeadInterId 1361 ' "$rows" || \
        ! grep -q '^stemsheadclinkfrontier .*headOrder 70 headX 1 headSig 35 .*cAlias h:1:LEFT:BOTTOM .*lastIndex 2 maxIndex 2 relations 2 .*glyphs 2 .*candidateIdBefore 0 existingGlyph glyph:322 existingActive true existingStem 2384 ' "$rows" || \
        ! grep -q '^stemsheadclinkresult headOrder 70 allocatorBefore 2385 allocatorAfter 2385 .*addedVertices - addedEdges \[system1:sourceId1355:targetId2384:org.audiveris.omr.sig.relation.HeadStemRelation.*sourceId1390:targetId2384.*addedSystemStems - linkerChanges \[h:0:LEFT:BOTTOM:false:false->true:true' "$rows"; then
    echo "v70 multi-head reuse C-link contract differs" >&2
    exit 1
fi
: <<'DISABLED_V27_ACTIVE'
...
v69_runner_sha=$(shasum -a 256 "$script_dir/run-stems-head-phase-prefix-v69.sh" | awk '{print $1}')
if [ "$v69_runner_sha" != "6b1277bfd4169afb25a94034e52a31e938dcac3f02c5b98df5e0e3e383f4a40e" ]; then
    echo "v70 base v69 runner drifted" >&2
    exit 1
fi
v69_fixture="$repo_root/rust/oracle/stems-head-phase-prefix-chula-system1-v69.txt"
v69_fixture_sha=$(shasum -a 256 "$v69_fixture" | awk '{print $1}')
if [ "$v69_fixture_sha" != "fff308360cc99c0039fc973b5c112ba68e1b5f24ff422461214cfa580063dc4b" ]; then
    echo "v70 base v69 fixture drifted" >&2
    exit 1
fi
...
out="/private/tmp/stems-head-phase-prefix-chula-system1-v70-audit.txt"
{
    echo '# Java Audiveris 5.11 (Temurin JDK 25.0.3+9 LTS) post-STUMPS head phase v70.'
    echo '# schema: stems-head-phase-prefix-v70'
    echo '# Snapshot-minimized order-70 derivative: orders 1-69 mutate without snapshots; order 70 emits the multi-head existing-stem C-link.'
    echo '# This bounded scope is intentional for deterministic replay under the full-snapshot heap limit.'
    cat "$rows"
    printf '%s\n' \
        "stemsheadphaseprefix summary schema stems-head-phase-prefix-v70 page chula.png#1 system 1 rows $row_count baseProbeSourceSha256 $base_probe_sha baseV69RunnerSourceSha256 $v69_runner_sha baseV69FixtureSha256 $v69_fixture_sha fragmentSourceSha256 $fragment_sha probeSourceSha256 $probe_sha runnerSourceSha256 $runner_sha emittedBodySha256 $body_sha semanticPassSha256 $semantic_sha completeStumpsFixtureSha256 $stumps_sha freshRuns 2 freshRunsByteIdentical true nativeScope BoundedSnapshotMinimizedOrder70MultiHeadReuseCLink javaEvidence ReturnedBeforeSeventyFirstHead"
} > "$out"
echo "wrote $out"
