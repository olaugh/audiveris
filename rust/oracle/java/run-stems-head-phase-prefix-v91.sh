...
# Read-only snapshot-minimized order91 continuation from v90; prior orders mutate without snapshots.
set -eu
...
tmp_base=/private/tmp/stems-head-phase-prefix-v91
...
awk -v fragment="$fragment18" '
{
    ...
    if (capture_after && index($0, "heads, allLinkers);") != 0) {
        print
        print "            if (headOrder == 7 || headOrder == 22 || headOrder == 27 || headOrder == 34 || headOrder == 36 || headOrder == 37 || headOrder == 38 || headOrder == 39 || headOrder == 40 || headOrder == 41 || headOrder == 42 || headOrder == 43 || headOrder == 44 || headOrder == 45 || headOrder == 46 || headOrder == 47 || headOrder == 48 || headOrder == 49 || headOrder == 50 || headOrder == 51 || headOrder == 52 || headOrder == 53 || headOrder == 54 || headOrder == 55 || headOrder == 56 || headOrder == 57 || headOrder == 58 || headOrder == 59 || headOrder == 60 || headOrder == 61 || headOrder == 62 || headOrder == 63 || headOrder == 64 || headOrder == 65 || headOrder == 66 || headOrder == 67 || headOrder == 68 || headOrder == 69 || headOrder == 70 || headOrder == 71 || headOrder == 72 || headOrder == 73 || headOrder == 74 || headOrder == 75 || headOrder == 76 || headOrder == 77 || headOrder == 78 || headOrder == 79 || headOrder == 80 || headOrder == 81 || headOrder == 82 || headOrder == 83 || headOrder == 84 || headOrder == 85 || headOrder == 86 || headOrder == 87 || headOrder == 88 || headOrder == 89 || headOrder == 90 || headOrder == 91) emitHeadCLinkResult(before, after, headOrder);"
        capture_after = 0
        in_linker = 0
        next
    }
...
if [ "$(grep -c '^stemsheadphasecontinue ' "$rows")" -ne 1 ] || \
        ! grep -q 'headOrder 91 headX 91 headSig 98 headInterId 1485 .*decisions \[LEFT:SkipAlreadyLinked,RIGHT:SkipClosed\].*incident \[stem2364:headSideLEFT:heads\[x91:sig98:id1485:sideLEFT,x94:sig99:id1487:sideLEFT,x95:sig100:id1489:sideLEFT\].*closureWrites \[x94:sig99:LEFT:true->true,x94:sig99:RIGHT:true->true,x95:sig100:LEFT:true->true,x95:sig100:RIGHT:true->true\].*closedValueChanges 0 .*nextHeadOrder 92 nextHeadX 54 nextHeadSig 78 nextHeadInterId 1445 ' "$rows" || \
        ! grep -q '^stemsheadclinkfrontier .*headOrder 91 headX 91 headSig 98 .*cAlias h:91:LEFT:BOTTOM .*lastIndex 0 maxIndex 0 relations 1 .*glyphs 1 .*candidateIdBefore 297 existingGlyph glyph:297 existingActive true existingStem 2364 ' "$rows" || \
        ! grep -q '^stemsheadclinkresult headOrder 91 allocatorBefore 2385 allocatorAfter 2385 .*addedVertices - addedEdges - addedSystemStems - linkerChanges - ' "$rows"; then
    echo "v91 zero-write existing-stem reconciliation contract differs" >&2
    exit 1
fi
: <<'DISABLED_V27_ACTIVE'
...
v90_runner_sha=$(shasum -a 256 "$script_dir/run-stems-head-phase-prefix-v90.sh" | awk '{print $1}')
if [ "$v90_runner_sha" != "a1bef0b7346d9b685e3b20bb9a1caa6ed96f9f1f2e90d9f3bd6bbf4c188a015f" ]; then
    echo "v91 base v90 runner drifted" >&2
    exit 1
fi
v90_fixture="$repo_root/rust/oracle/stems-head-phase-prefix-chula-system1-v90.txt"
v90_fixture_sha=$(shasum -a 256 "$v90_fixture" | awk '{print $1}')
if [ "$v90_fixture_sha" != "a7ab4b588aaea28c4874c6d2ad2cc39520b83069561b14b49e9ce01644cb6784" ]; then
    echo "v91 base v90 fixture drifted" >&2
    exit 1
fi
...
out="/private/tmp/stems-head-phase-prefix-chula-system1-v91-audit.txt"
{
    echo '# Java Audiveris 5.11 (Temurin JDK 25.0.3+9 LTS) post-STUMPS head phase v91.'
    echo '# schema: stems-head-phase-prefix-v91'
    echo '# Snapshot-minimized order-91 derivative: orders 1-90 mutate without snapshots; order 91 emits the zero-write existing-stem reconciliation.'
    echo '# This bounded scope is intentional for deterministic replay under the full-snapshot heap limit.'
    cat "$rows"
    printf '%s\n' \
        "stemsheadphaseprefix summary schema stems-head-phase-prefix-v91 page chula.png#1 system 1 rows $row_count baseProbeSourceSha256 $base_probe_sha baseV90RunnerSourceSha256 $v90_runner_sha baseV90FixtureSha256 $v90_fixture_sha fragmentSourceSha256 $fragment_sha probeSourceSha256 $probe_sha runnerSourceSha256 $runner_sha emittedBodySha256 $body_sha semanticPassSha256 $semantic_sha completeStumpsFixtureSha256 $stumps_sha freshRuns 2 freshRunsByteIdentical true nativeScope BoundedSnapshotMinimizedOrder91ExistingStemReconciliation javaEvidence ReturnedBeforeNinetySecondHead"
} > "$out"
echo "wrote $out"
