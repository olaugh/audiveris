...
# Read-only snapshot-minimized order83 continuation from v82; prior orders mutate without snapshots.
set -eu
...
tmp_base=/private/tmp/stems-head-phase-prefix-v83
...
if [ "$(grep -c '^stemsheadphasecontinue ' "$rows")" -ne 1 ] || \
        ! grep -q 'headOrder 83 headX 24 headSig 90 headInterId 1469 .*decisions \[LEFT:SkipAlreadyLinked,RIGHT:SkipClosed\].*incident \[stem2356:headSideLEFT:heads\[x24:sig90:id1469:sideLEFT,x25:sig91:id1471:sideLEFT\].*closureWrites \[x25:sig91:LEFT:false->true,x25:sig91:RIGHT:false->true\].*closedValueChanges 2 .*nextHeadOrder 84 nextHeadX 94 nextHeadSig 99 nextHeadInterId 1487 ' "$rows" || \
        ! grep -q '^stemsheadclinkfrontier .*headOrder 83 headX 24 headSig 90 .*cAlias h:24:LEFT:BOTTOM .*lastIndex 3 maxIndex 3 relations 3 .*glyphs 2 .*candidateIdBefore 0 existingGlyph glyph:292 existingActive true existingStem 2356 ' "$rows" || \
        ! grep -q '^stemsheadclinkresult headOrder 83 allocatorBefore 2385 allocatorAfter 2385 .*addedVertices - addedEdges - addedSystemStems - .*linkerChanges \[h:25:LEFT:BOTTOM:true:false->true:true' "$rows"; then
    echo "v83 existing-stem reconciliation contract differs" >&2
    exit 1
fi
: <<'DISABLED_V27_ACTIVE'
...
v82_runner_sha=$(shasum -a 256 "$script_dir/run-stems-head-phase-prefix-v82.sh" | awk '{print $1}')
if [ "$v82_runner_sha" != "28e24f4e4484ecd6967c627a6745be9ef150de32637bf3f3eac8523f10716ba8" ]; then
    echo "v83 base v82 runner drifted" >&2
    exit 1
fi
v82_fixture="$repo_root/rust/oracle/stems-head-phase-prefix-chula-system1-v82.txt"
v82_fixture_sha=$(shasum -a 256 "$v82_fixture" | awk '{print $1}')
if [ "$v82_fixture_sha" != "79497db4a9c58519a8df51aafeed6be1eca9f85773e2a53f049abead8aebd426" ]; then
    echo "v83 base v82 fixture drifted" >&2
    exit 1
fi
...
out="/private/tmp/stems-head-phase-prefix-chula-system1-v83-audit.txt"
{
    echo '# Java Audiveris 5.11 (Temurin JDK 25.0.3+9 LTS) post-STUMPS head phase v83.'
    echo '# schema: stems-head-phase-prefix-v83'
    echo '# Snapshot-minimized order-83 derivative: orders 1-82 mutate without snapshots; order 83 emits the existing-stem reconciliation.'
    echo '# This bounded scope is intentional for deterministic replay under the full-snapshot heap limit.'
    cat "$rows"
    printf '%s\n' \
        "stemsheadphaseprefix summary schema stems-head-phase-prefix-v83 page chula.png#1 system 1 rows $row_count baseProbeSourceSha256 $base_probe_sha baseV82RunnerSourceSha256 $v82_runner_sha baseV82FixtureSha256 $v82_fixture_sha fragmentSourceSha256 $fragment_sha probeSourceSha256 $probe_sha runnerSourceSha256 $runner_sha emittedBodySha256 $body_sha semanticPassSha256 $semantic_sha completeStumpsFixtureSha256 $stumps_sha freshRuns 2 freshRunsByteIdentical true nativeScope BoundedSnapshotMinimizedOrder83ExistingStemReconciliation javaEvidence ReturnedBeforeEightyFourthHead"
} > "$out"
echo "wrote $out"
