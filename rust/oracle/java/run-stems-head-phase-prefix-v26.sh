...
# Freeze a bounded order-26 continuation replay from v25, including the
# prelinked existing-stem closure at x61/SIG31. Orders below 18 execute without snapshots.
set -eu
...
tmp_base=/private/tmp/stems-head-phase-prefix-v26
...
fragment18="$script_dir/stems-head-phase-v26-fragment.java"
...
if ! cmp -s "$semantic_one" "$semantic_two"; then
    echo "two fresh post-STUMPS v26 semantic passes are not byte-identical" >&2
...
if [ "$(grep -c '^stemsheadphasecontinue ' "$rows")" -ne 9 ] || \
...
v25_runner_sha=$(shasum -a 256 "$script_dir/run-stems-head-phase-prefix-v25.sh" | awk '{print $1}')
if [ "$v25_runner_sha" != "d9bb5989503627cf7486f6c3286ffe78754a1a089d1d18087fef1e6d15389c68" ]; then
    echo "v26 base v25 runner drifted" >&2
    exit 1
fi
v25_fixture="$repo_root/rust/oracle/stems-head-phase-prefix-chula-system1-v25.txt"
v25_fixture_sha=$(shasum -a 256 "$v25_fixture" | awk '{print $1}')
if [ "$v25_fixture_sha" != "39ccb74b6231aa2ce3f77a41adb59d18ae64c736598917523f4c4f8835722d2d" ]; then
    echo "v26 base v25 fixture drifted" >&2
    exit 1
fi
...
out="$repo_root/rust/oracle/stems-head-phase-prefix-chula-system1-v26.txt"
{
    echo '# Java Audiveris 5.11 (Temurin JDK 25.0.3+9 LTS) post-STUMPS head phase v26.'
    echo '# schema: stems-head-phase-prefix-v26'
    echo '# Bounded order-26 derivative: order 26 emits the prelinked closure continuation after the v25 predecessor.'
    echo '# This minimized scope is intentional for deterministic replay under the full-snapshot heap limit.'
    cat "$rows"
    printf '%s\n' \
        "stemsheadphaseprefix summary schema stems-head-phase-prefix-v26 page chula.png#1 system 1 rows $row_count baseProbeSourceSha256 $base_probe_sha baseV25RunnerSourceSha256 $v25_runner_sha baseV25FixtureSha256 $v25_fixture_sha fragmentSourceSha256 $fragment_sha probeSourceSha256 $probe_sha runnerSourceSha256 $runner_sha emittedBodySha256 $body_sha semanticPassSha256 $semantic_sha completeStumpsFixtureSha256 $stumps_sha freshRuns 2 freshRunsByteIdentical true nativeScope BoundedSnapshotMinimizedOrder26PrelinkedClosure javaEvidence ReturnedBeforeTwentySeventhHead"
} > "$out"
echo "wrote $out"
