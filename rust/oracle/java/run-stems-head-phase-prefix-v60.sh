...
# Read-only snapshot-minimized order60 continuation from v59; prior orders mutate without snapshots.
set -eu
...
tmp_base=/private/tmp/stems-head-phase-prefix-v60
...
awk -v fragment="$fragment18" '
{
    ...
    if (capture_after && index($0, "heads, allLinkers);") != 0) {
        print
        print "            if (headOrder == 7 || headOrder == 22 || headOrder == 27 || headOrder == 34 || headOrder == 36 || headOrder == 37 || headOrder == 38 || headOrder == 39 || headOrder == 40 || headOrder == 41 || headOrder == 42 || headOrder == 43 || headOrder == 44 || headOrder == 45 || headOrder == 46 || headOrder == 47 || headOrder == 48 || headOrder == 49 || headOrder == 50 || headOrder == 51 || headOrder == 52 || headOrder == 53 || headOrder == 54 || headOrder == 55 || headOrder == 56 || headOrder == 57 || headOrder == 58 || headOrder == 59 || headOrder == 60) emitHeadCLinkResult(before, after, headOrder);"
        capture_after = 0
        in_linker = 0
        next
    }
...
if [ "$(grep -c '^stemsheadphasecontinue ' "$rows")" -ne 1 ] || \
        ! grep -q 'headOrder 60 headX 71 headSig 49 headInterId 1387 .*decisions \[LEFT:top=true:bottom=true:branch=Both,RIGHT:top=true:bottom=false:branch=TopOnly\].*incident - returned false .*undefs \[LEFT\].*closedValueChanges 0 .*nextHeadOrder 61 nextHeadX 70 nextHeadSig 46 nextHeadInterId 1377 ' "$rows" || \
        ! grep -q '^stemsheadclinkfrontier .*headOrder 60 headX 71 headSig 49 .*cAlias h:71:LEFT:BOTTOM .*lastIndex -1 maxIndex 1 relations 1 .*glyphs 2 .*candidateIdBefore 0 existingGlyph glyph:332 existingActive true existingStem 2382 ' "$rows" || \
        ! grep -q '^stemsheadclinkresult headOrder 60 allocatorBefore 2385 allocatorAfter 2385 .*addedVertices - addedEdges - addedSystemStems - linkerChanges - .*terminal ReturnedHeadCLinkTransaction' "$rows"; then
    echo "v60 open-frontier contract differs" >&2
    exit 1
fi
: <<'DISABLED_V27_ACTIVE'
...
v59_runner_sha=$(shasum -a 256 "$script_dir/run-stems-head-phase-prefix-v59.sh" | awk '{print $1}')
if [ "$v59_runner_sha" != "c723a76e61c170f302575b609c5f6a81dbc99ed1b464d8f2558773f846070dcc" ]; then
    echo "v60 base v59 runner drifted" >&2
    exit 1
fi
v59_fixture="$repo_root/rust/oracle/stems-head-phase-prefix-chula-system1-v59.txt"
v59_fixture_sha=$(shasum -a 256 "$v59_fixture" | awk '{print $1}')
if [ "$v59_fixture_sha" != "53bb43246e40ee07c40ffdbf091c1b8521a114f4ef77f4b16a762ebfc7f5c3be" ]; then
    echo "v60 base v59 fixture drifted" >&2
    exit 1
fi
...
out="/private/tmp/stems-head-phase-prefix-chula-system1-v60-audit.txt"
{
    echo '# Java Audiveris 5.11 (Temurin JDK 25.0.3+9 LTS) post-STUMPS head phase v60.'
    echo '# schema: stems-head-phase-prefix-v60'
    echo '# Snapshot-minimized order-60 derivative: orders 1-59 mutate without snapshots; order 60 emits the open-frontier transaction.'
    echo '# This bounded scope is intentional for deterministic replay under the full-snapshot heap limit.'
    cat "$rows"
    printf '%s\n' \
        "stemsheadphaseprefix summary schema stems-head-phase-prefix-v60 page chula.png#1 system 1 rows $row_count baseProbeSourceSha256 $base_probe_sha baseV59RunnerSourceSha256 $v59_runner_sha baseV59FixtureSha256 $v59_fixture_sha fragmentSourceSha256 $fragment_sha probeSourceSha256 $probe_sha runnerSourceSha256 $runner_sha emittedBodySha256 $body_sha semanticPassSha256 $semantic_sha completeStumpsFixtureSha256 $stumps_sha freshRuns 2 freshRunsByteIdentical true nativeScope BoundedSnapshotMinimizedOrder60OpenFrontier javaEvidence ReturnedBeforeSixtyFirstHead"
} > "$out"
echo "wrote $out"
