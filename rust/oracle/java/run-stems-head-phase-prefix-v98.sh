...
# Read-only snapshot-minimized order98 continuation from v97; prior orders mutate without snapshots.
set -eu
...
tmp_base=/private/tmp/stems-head-phase-prefix-v98
...
awk -v fragment="$fragment18" '
{
    ...
    if (capture_after && index($0, "heads, allLinkers);") != 0) {
        print
        print "            if (headOrder == 7 || headOrder == 22 || headOrder == 27 || headOrder == 34 || headOrder == 36 || headOrder == 37 || headOrder == 38 || headOrder == 39 || headOrder == 40 || headOrder == 41 || headOrder == 42 || headOrder == 43 || headOrder == 44 || headOrder == 45 || headOrder == 46 || headOrder == 47 || headOrder == 48 || headOrder == 49 || headOrder == 50 || headOrder == 51 || headOrder == 52 || headOrder == 53 || headOrder == 54 || headOrder == 55 || headOrder == 56 || headOrder == 57 || headOrder == 58 || headOrder == 59 || headOrder == 60 || headOrder == 61 || headOrder == 62 || headOrder == 63 || headOrder == 64 || headOrder == 65 || headOrder == 66 || headOrder == 67 || headOrder == 68 || headOrder == 69 || headOrder == 70 || headOrder == 71 || headOrder == 72 || headOrder == 73 || headOrder == 74 || headOrder == 75 || headOrder == 76 || headOrder == 77 || headOrder == 78 || headOrder == 79 || headOrder == 80 || headOrder == 81 || headOrder == 82 || headOrder == 83 || headOrder == 84 || headOrder == 85 || headOrder == 86 || headOrder == 87 || headOrder == 88 || headOrder == 89 || headOrder == 90 || headOrder == 91 || headOrder == 92 || headOrder == 93 || headOrder == 94 || headOrder == 95 || headOrder == 96 || headOrder == 97 || headOrder == 98) emitHeadCLinkResult(before, after, headOrder);"
        capture_after = 0
        in_linker = 0
        next
    }
...
if [ "$(grep -c '^stemsheadphasecontinue ' "$rows")" -ne 1 ] || \
        ! grep -q 'headOrder 98 headX 39 headSig 37 headInterId 1359 .*decisions \[LEFT:SkipAlreadyLinked,RIGHT:SkipClosed\].*incident \[stem2350:headSideLEFT:heads\[x39:sig37:id1359:sideLEFT,x40:sig27:id1339:sideLEFT,x43:sig48:id1385:sideLEFT\].*closureWrites \[x40:sig27:LEFT:true->true,x40:sig27:RIGHT:true->true,x43:sig48:LEFT:true->true,x43:sig48:RIGHT:true->true\].*closedValueChanges 0 .*nextHeadOrder 99 nextHeadX 56 nextHeadSig 15 nextHeadInterId 1315 ' "$rows" || \
        ! grep -q '^stemsheadclinkfrontier .*headOrder 98 headX 39 headSig 37 .*cAlias h:39:LEFT:BOTTOM .*lastIndex 1 maxIndex 1 relations 1 .*glyphs 2 .*candidateIdBefore 0 existingGlyph glyph:326 existingActive true existingStem 2350 ' "$rows" || \
        ! grep -q '^stemsheadclinkresult headOrder 98 allocatorBefore 2385 allocatorAfter 2385 .*addedVertices - addedEdges - addedSystemStems - linkerChanges - ' "$rows"; then
    echo "v98 zero-write existing-stem reconciliation contract differs" >&2
    exit 1
fi
: <<'DISABLED_V27_ACTIVE'
...
v97_runner_sha=$(shasum -a 256 "$script_dir/run-stems-head-phase-prefix-v97.sh" | awk '{print $1}')
if [ "$v97_runner_sha" != "a5a0e4100f6a32f15a4c7b414bd557815272018cf1d4f90fb9c34745c57a7976" ]; then
    echo "v98 base v97 runner drifted" >&2
    exit 1
fi
v97_fixture="$repo_root/rust/oracle/stems-head-phase-prefix-chula-system1-v97.txt"
v97_fixture_sha=$(shasum -a 256 "$v97_fixture" | awk '{print $1}')
if [ "$v97_fixture_sha" != "b7b981609d22812970cd60bcacd5eaf870d4c34475fad8f97cbbbadf38b2fe89" ]; then
    echo "v98 base v97 fixture drifted" >&2
    exit 1
fi
...
out="/private/tmp/stems-head-phase-prefix-chula-system1-v98-audit.txt"
{
    echo '# Java Audiveris 5.11 (Temurin JDK 25.0.3+9 LTS) post-STUMPS head phase v98.'
    echo '# schema: stems-head-phase-prefix-v98'
    echo '# Snapshot-minimized order-98 derivative: orders 1-97 mutate without snapshots; order 98 emits the zero-write existing-stem reconciliation.'
    echo '# This bounded scope is intentional for deterministic replay under the full-snapshot heap limit.'
    cat "$rows"
    printf '%s\n' \
        "stemsheadphaseprefix summary schema stems-head-phase-prefix-v98 page chula.png#1 system 1 rows $row_count baseProbeSourceSha256 $base_probe_sha baseV97RunnerSourceSha256 $v97_runner_sha baseV97FixtureSha256 $v97_fixture_sha fragmentSourceSha256 $fragment_sha probeSourceSha256 $probe_sha runnerSourceSha256 $runner_sha emittedBodySha256 $body_sha semanticPassSha256 $semantic_sha completeStumpsFixtureSha256 $stumps_sha freshRuns 2 freshRunsByteIdentical true nativeScope BoundedSnapshotMinimizedOrder98ExistingStemReconciliation javaEvidence ReturnedBeforeNinetyNinthHead"
} > "$out"
echo "wrote $out"
