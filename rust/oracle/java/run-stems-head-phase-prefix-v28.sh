...
# Freeze a snapshot-minimized order28 continuation from v27; prior orders mutate without snapshots.
set -eu
...
tmp_base=/private/tmp/stems-head-phase-prefix-v28
...
if [ "$(grep -c '^stemsheadphasecontinue ' "$rows")" -ne 1 ] || \
        ! grep -q 'headOrder 28 headX 85 headSig 87 headInterId 1463 .*decisions \[LEFT:SkipAlreadyLinked,RIGHT:top=false:bottom=false:branch=Neither\].*incident \[stem2366:headSideLEFT:heads\[x84:sig86:id1461:sideLEFT,x85:sig87:id1463:sideLEFT,x86:sig85:id1459:sideLEFT\]\].*closureWrites \[x84:sig86:LEFT:false->true,x84:sig86:RIGHT:false->true,x86:sig85:LEFT:false->true,x86:sig85:RIGHT:false->true\].*closedValueChanges 4 .*nextHeadOrder 29 nextHeadX 10 nextHeadSig 9 nextHeadInterId 1303 ' "$rows"; then
    echo "v28 minimized closure contract differs" >&2
    exit 1
fi
: <<'DISABLED_V27_ACTIVE'
...
v27_runner_sha=$(shasum -a 256 "$script_dir/run-stems-head-phase-prefix-v27.sh" | awk '{print $1}')
if [ "$v27_runner_sha" != "f2c1942b3ff6f00a75bb876b6d6d4b53ba2d999bcb5ddaeb88f6dc86850fcdc5" ]; then
    echo "v28 base v27 runner drifted" >&2
    exit 1
fi
v27_fixture="$repo_root/rust/oracle/stems-head-phase-prefix-chula-system1-v27.txt"
v27_fixture_sha=$(shasum -a 256 "$v27_fixture" | awk '{print $1}')
if [ "$v27_fixture_sha" != "1ba59491992fdd7bd2355e2617b437b84433d3c449cc8f7606cdc0a1e70ac0aa" ]; then
    echo "v28 base v27 fixture drifted" >&2
    exit 1
fi
...
out="$repo_root/rust/oracle/stems-head-phase-prefix-chula-system1-v28.txt"
{
    echo '# Java Audiveris 5.11 (Temurin JDK 25.0.3+9 LTS) post-STUMPS head phase v28.'
    echo '# schema: stems-head-phase-prefix-v28'
    echo '# Snapshot-minimized order-28 derivative: orders 1-27 mutate without snapshots; order 28 emits the prelinked closure row.'
    echo '# This minimized scope is intentional for deterministic replay under the full-snapshot heap limit.'
    cat "$rows"
    printf '%s\n' \
        "stemsheadphaseprefix summary schema stems-head-phase-prefix-v28 page chula.png#1 system 1 rows $row_count baseProbeSourceSha256 $base_probe_sha baseV27RunnerSourceSha256 $v27_runner_sha baseV27FixtureSha256 $v27_fixture_sha fragmentSourceSha256 $fragment_sha probeSourceSha256 $probe_sha runnerSourceSha256 $runner_sha emittedBodySha256 $body_sha semanticPassSha256 $semantic_sha completeStumpsFixtureSha256 $stumps_sha freshRuns 2 freshRunsByteIdentical true nativeScope BoundedSnapshotMinimizedOrder28PrelinkedClosure javaEvidence ReturnedBeforeTwentyNinthHead"
} > "$out"
echo "wrote $out"
