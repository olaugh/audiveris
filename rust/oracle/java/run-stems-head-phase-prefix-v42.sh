...
# Freeze a snapshot-minimized order42 existing-stem reconciliation from v41; prior orders mutate without snapshots.
set -eu
...
tmp_base=/private/tmp/stems-head-phase-prefix-v42
...
awk -v fragment="$fragment18" '
{
    ...
    if (index($0, "final boolean returned = linker.linkSides(") != 0) {
        print "            if (headOrder == 7 || headOrder == 22 || headOrder == 27 || headOrder == 34 || headOrder == 36 || headOrder == 37 || headOrder == 38 || headOrder == 39 || headOrder == 40 || headOrder == 41 || headOrder == 42) emitHeadCLinkEnvelope(head, linker, before, headOrder);"
        print
        in_linker = 1
        next
    }
    ...
    if (capture_after && index($0, "heads, allLinkers);") != 0) {
        print
        print "            if (headOrder == 7 || headOrder == 22 || headOrder == 27 || headOrder == 34 || headOrder == 36 || headOrder == 37 || headOrder == 38 || headOrder == 39 || headOrder == 40 || headOrder == 41 || headOrder == 42) emitHeadCLinkResult(before, after, headOrder);"
        capture_after = 0
        in_linker = 0
        next
    }
...
if [ "$(grep -c '^stemsheadphasecontinue ' "$rows")" -ne 1 ] || \
        ! grep -q 'headOrder 42 headX 43 headSig 48 headInterId 1385 .*decisions \[LEFT:SkipAlreadyLinked,RIGHT:top=false:bottom=false:branch=Neither\].*incident \[stem2350:headSideLEFT:heads\[x39:sig37:id1359:sideLEFT,x40:sig27:id1339:sideLEFT,x43:sig48:id1385:sideLEFT\]\].*closureWrites \[x39:sig37:LEFT:false->true,x39:sig37:RIGHT:false->true,x40:sig27:LEFT:false->true,x40:sig27:RIGHT:false->true\].*closedValueChanges 4 .*nextHeadOrder 43 nextHeadX 25 nextHeadSig 91 nextHeadInterId 1471 ' "$rows" || \
        ! grep -q '^stemsheadclinkfrontier .*headOrder 42 headX 43 headSig 48 .*cAlias h:43:LEFT:BOTTOM .*lastIndex 2 maxIndex 2 relations 2 .*glyphs 2 .*candidateIdBefore 0 existingGlyph glyph:326 existingActive true existingStem 2350 ' "$rows" || \
        ! grep -q '^stemsheadclinkresult headOrder 42 allocatorBefore 2385 allocatorAfter 2385 .*addedVertices - addedEdges - addedSystemStems - .*linkerChanges \[h:39:LEFT:BOTTOM:true:false->true:true' "$rows"; then
    echo "v42 existing-stem reconciliation contract differs" >&2
    exit 1
fi
: <<'DISABLED_V27_ACTIVE'
...
v41_runner_sha=$(shasum -a 256 "$script_dir/run-stems-head-phase-prefix-v41.sh" | awk '{print $1}')
if [ "$v41_runner_sha" != "62be1da6161918739869d9aff57dd324b2145e6bfd6a96eb16fa8c64660c6a12" ]; then
    echo "v42 base v41 runner drifted" >&2
    exit 1
fi
v41_fixture="$repo_root/rust/oracle/stems-head-phase-prefix-chula-system1-v41.txt"
v41_fixture_sha=$(shasum -a 256 "$v41_fixture" | awk '{print $1}')
if [ "$v41_fixture_sha" != "7bb4ebb479617804363078144c55570d1c76229de551492c7cb14050641f1962" ]; then
    echo "v42 base v41 fixture drifted" >&2
    exit 1
fi
...
out="$repo_root/rust/oracle/stems-head-phase-prefix-chula-system1-v42.txt"
{
    echo '# Java Audiveris 5.11 (Temurin JDK 25.0.3+9 LTS) post-STUMPS head phase v42.'
    echo '# schema: stems-head-phase-prefix-v42'
    echo '# Snapshot-minimized order-42 derivative: orders 1-41 mutate without snapshots; order 42 emits existing-stem reconciliation.'
    echo '# This minimized scope is intentional for deterministic replay under the full-snapshot heap limit.'
    cat "$rows"
    printf '%s\n' \
        "stemsheadphaseprefix summary schema stems-head-phase-prefix-v42 page chula.png#1 system 1 rows $row_count baseProbeSourceSha256 $base_probe_sha baseV41RunnerSourceSha256 $v41_runner_sha baseV41FixtureSha256 $v41_fixture_sha fragmentSourceSha256 $fragment_sha probeSourceSha256 $probe_sha runnerSourceSha256 $runner_sha emittedBodySha256 $body_sha semanticPassSha256 $semantic_sha completeStumpsFixtureSha256 $stumps_sha freshRuns 2 freshRunsByteIdentical true nativeScope BoundedSnapshotMinimizedOrder42ExistingStemReconciliation javaEvidence ReturnedBeforeFortyThirdHead"
} > "$out"
echo "wrote $out"
