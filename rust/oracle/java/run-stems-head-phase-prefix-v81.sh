...
# Read-only snapshot-minimized order81 continuation from v80; prior orders mutate without snapshots.
set -eu
...
tmp_base=/private/tmp/stems-head-phase-prefix-v81
...
awk -v fragment="$fragment18" '
{
    ...
    if (capture_after && index($0, "heads, allLinkers);") != 0) {
        print
        print "            if (headOrder == 7 || headOrder == 22 || headOrder == 27 || headOrder == 34 || headOrder == 36 || headOrder == 37 || headOrder == 38 || headOrder == 39 || headOrder == 40 || headOrder == 41 || headOrder == 42 || headOrder == 43 || headOrder == 44 || headOrder == 45 || headOrder == 46 || headOrder == 47 || headOrder == 48 || headOrder == 49 || headOrder == 50 || headOrder == 51 || headOrder == 52 || headOrder == 53 || headOrder == 54 || headOrder == 55 || headOrder == 56 || headOrder == 57 || headOrder == 58 || headOrder == 59 || headOrder == 60 || headOrder == 61 || headOrder == 62 || headOrder == 63 || headOrder == 64 || headOrder == 65 || headOrder == 66 || headOrder == 67 || headOrder == 68 || headOrder == 69 || headOrder == 70 || headOrder == 71 || headOrder == 72 || headOrder == 73 || headOrder == 74 || headOrder == 75 || headOrder == 76 || headOrder == 77 || headOrder == 78 || headOrder == 79 || headOrder == 80 || headOrder == 81) emitHeadCLinkResult(before, after, headOrder);"
        capture_after = 0
        in_linker = 0
        next
    }
...
if [ "$(grep -c '^stemsheadphasecontinue ' "$rows")" -ne 1 ] || \
        ! grep -q 'headOrder 81 headX 98 headSig 60 headInterId 1409 .*decisions \[LEFT:SkipAlreadyLinked,RIGHT:SkipClosed\].*incident \[stem2365:headSideLEFT:heads\[x98:sig60:id1409:sideLEFT,x99:sig61:id1411:sideLEFT\].*closureWrites \[x99:sig61:LEFT:false->true,x99:sig61:RIGHT:false->true\].*closedValueChanges 2 .*nextHeadOrder 82 nextHeadX 80 nextHeadSig 32 nextHeadInterId 1349 ' "$rows" || \
        ! grep -q '^stemsheadclinkfrontier .*headOrder 81 headX 98 headSig 60 .*cAlias h:98:LEFT:BOTTOM .*lastIndex 0 maxIndex 0 relations 1 .*glyphs 1 .*candidateIdBefore 330 existingGlyph glyph:330 existingActive true existingStem 2365 ' "$rows" || \
        ! grep -q '^stemsheadclinkresult headOrder 81 allocatorBefore 2385 allocatorAfter 2385 .*addedVertices - addedEdges - addedSystemStems - .*linkerChanges \[h:99:LEFT:BOTTOM:true:false->true:true' "$rows"; then
    echo "v81 existing-stem reconciliation contract differs" >&2
    exit 1
fi
: <<'DISABLED_V27_ACTIVE'
...
v80_runner_sha=$(shasum -a 256 "$script_dir/run-stems-head-phase-prefix-v80.sh" | awk '{print $1}')
if [ "$v80_runner_sha" != "f2a107f92bcd9beb6005b2d098832ff1b71fc8b2f342ff069b0e980ed03b97b1" ]; then
    echo "v81 base v80 runner drifted" >&2
    exit 1
fi
v80_fixture="$repo_root/rust/oracle/stems-head-phase-prefix-chula-system1-v80.txt"
v80_fixture_sha=$(shasum -a 256 "$v80_fixture" | awk '{print $1}')
if [ "$v80_fixture_sha" != "71e7a89bd72b1f4df666a348f02a9ba9180b8189493a1ec5b6f82aa3ff5158a3" ]; then
    echo "v81 base v80 fixture drifted" >&2
    exit 1
fi
...
out="/private/tmp/stems-head-phase-prefix-chula-system1-v81-audit.txt"
{
    echo '# Java Audiveris 5.11 (Temurin JDK 25.0.3+9 LTS) post-STUMPS head phase v81.'
    echo '# schema: stems-head-phase-prefix-v81'
    echo '# Snapshot-minimized order-81 derivative: orders 1-80 mutate without snapshots; order 81 emits the existing-stem reconciliation.'
    echo '# This bounded scope is intentional for deterministic replay under the full-snapshot heap limit.'
    cat "$rows"
    printf '%s\n' \
        "stemsheadphaseprefix summary schema stems-head-phase-prefix-v81 page chula.png#1 system 1 rows $row_count baseProbeSourceSha256 $base_probe_sha baseV80RunnerSourceSha256 $v80_runner_sha baseV80FixtureSha256 $v80_fixture_sha fragmentSourceSha256 $fragment_sha probeSourceSha256 $probe_sha runnerSourceSha256 $runner_sha emittedBodySha256 $body_sha semanticPassSha256 $semantic_sha completeStumpsFixtureSha256 $stumps_sha freshRuns 2 freshRunsByteIdentical true nativeScope BoundedSnapshotMinimizedOrder81ExistingStemReconciliation javaEvidence ReturnedBeforeEightySecondHead"
} > "$out"
echo "wrote $out"
