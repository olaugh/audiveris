...
# Freeze Java Bach system-2 phase-two queue 7's no-link retry.
set -eu
...
tmp_dir=$(mktemp -d /private/tmp/stems-head-phase-two-bach-s2-q7.XXXXXX)
...
grep -E '^(stemsheadphase2baseline .* system 2 |stemsheadphase2retry .* system 2 queueIndex 7 )' \
    "$tmp_dir/pass1" > "$tmp_dir/rows1"
grep -E '^(stemsheadphase2baseline .* system 2 |stemsheadphase2retry .* system 2 queueIndex 7 )' \
    "$tmp_dir/pass2" > "$tmp_dir/rows2"
if ! cmp -s "$tmp_dir/rows1" "$tmp_dir/rows2"; then
    echo "fresh Bach system-2 phase-two queue-7 Java passes are not byte-identical" >&2
...
out="$repo_root/rust/oracle/stems-head-phase-two-bach-system2-order7.txt"
{
    echo '# Java Audiveris 5.11 (Temurin JDK 25.0.3+9 LTS) Bach system-2 phase-two queue 7.'
    echo '# schema: stems-head-phase-two-bach-system2-order7-v1'
    cat "$tmp_dir/rows1"
    printf '%s\n' \
