...
# Read-only snapshot-minimized order67 continuation from v66; prior orders mutate without snapshots.
set -eu
...
tmp_base=/private/tmp/stems-head-phase-prefix-v67
...
awk -v fragment="$fragment18" '
{
    ...
    if (capture_after && index($0, "heads, allLinkers);") != 0) {
        print
        print "            if (headOrder == 7 || headOrder == 22 || headOrder == 27 || headOrder == 34 || headOrder == 36 || headOrder == 37 || headOrder == 38 || headOrder == 39 || headOrder == 40 || headOrder == 41 || headOrder == 42 || headOrder == 43 || headOrder == 44 || headOrder == 45 || headOrder == 46 || headOrder == 47 || headOrder == 48 || headOrder == 49 || headOrder == 50 || headOrder == 51 || headOrder == 52 || headOrder == 53 || headOrder == 54 || headOrder == 55 || headOrder == 56 || headOrder == 57 || headOrder == 58 || headOrder == 59 || headOrder == 60 || headOrder == 61 || headOrder == 62 || headOrder == 63 || headOrder == 64 || headOrder == 65 || headOrder == 66 || headOrder == 67) emitHeadCLinkResult(before, after, headOrder);"
        capture_after = 0
        in_linker = 0
        next
    }
...
if [ "$(grep -c '^stemsheadphasecontinue ' "$rows")" -ne 1 ] || \
        ! grep -q 'headOrder 67 headX 73 headSig 18 headInterId 1321 .*decisions \[LEFT:top=false:bottom=true:branch=BottomOnly,RIGHT:top=false:bottom=false:branch=Neither\].*incident - returned true .*undefs \[\].*closureWrites - closedValueChanges 0 .*sigEdgesBefore 697 sigEdgesAfter 700 .*nextHeadOrder 68 nextHeadX 0 nextHeadSig 51 nextHeadInterId 1390 ' "$rows" || \
        ! grep -q '^stemsheadclinkfrontier .*headOrder 67 headX 73 headSig 18 .*cAlias h:73:LEFT:BOTTOM .*lastIndex 3 maxIndex 3 relations 3 .*glyphs 2 .*candidateIdBefore 0 existingGlyph glyph:332 existingActive true existingStem 2382 ' "$rows" || \
        ! grep -q '^stemsheadclinkresult headOrder 67 allocatorBefore 2385 allocatorAfter 2385 .*addedVertices - addedEdges \[system1:sourceId1321:targetId2382:org.audiveris.omr.sig.relation.HeadStemRelation.*sourceId1377:targetId2382.*sourceId1387:targetId2382.*addedSystemStems - linkerChanges \[h:70:LEFT:BOTTOM:false:false->true:true' "$rows"; then
    echo "v67 multi-head reuse C-link contract differs" >&2
    exit 1
fi
: <<'DISABLED_V27_ACTIVE'
...
v66_runner_sha=$(shasum -a 256 "$script_dir/run-stems-head-phase-prefix-v66.sh" | awk '{print $1}')
if [ "$v66_runner_sha" != "a72d2a1657bdb32f4e179d9bc633a2830e98b2c4e672a7f0faed4562f9955e04" ]; then
    echo "v67 base v66 runner drifted" >&2
    exit 1
fi
v66_fixture="$repo_root/rust/oracle/stems-head-phase-prefix-chula-system1-v66.txt"
v66_fixture_sha=$(shasum -a 256 "$v66_fixture" | awk '{print $1}')
if [ "$v66_fixture_sha" != "2f1778c60a1beb687eddc90b6d5cc340f9d863ff4610d4664a1f7d6211079eb5" ]; then
    echo "v67 base v66 fixture drifted" >&2
    exit 1
fi
...
out="/private/tmp/stems-head-phase-prefix-chula-system1-v67-audit.txt"
{
    echo '# Java Audiveris 5.11 (Temurin JDK 25.0.3+9 LTS) post-STUMPS head phase v67.'
    echo '# schema: stems-head-phase-prefix-v67'
    echo '# Snapshot-minimized order-67 derivative: orders 1-66 mutate without snapshots; order 67 emits the multi-head existing-stem C-link.'
    echo '# This bounded scope is intentional for deterministic replay under the full-snapshot heap limit.'
    cat "$rows"
    printf '%s\n' \
        "stemsheadphaseprefix summary schema stems-head-phase-prefix-v67 page chula.png#1 system 1 rows $row_count baseProbeSourceSha256 $base_probe_sha baseV66RunnerSourceSha256 $v66_runner_sha baseV66FixtureSha256 $v66_fixture_sha fragmentSourceSha256 $fragment_sha probeSourceSha256 $probe_sha runnerSourceSha256 $runner_sha emittedBodySha256 $body_sha semanticPassSha256 $semantic_sha completeStumpsFixtureSha256 $stumps_sha freshRuns 2 freshRunsByteIdentical true nativeScope BoundedSnapshotMinimizedOrder67MultiHeadReuseCLink javaEvidence ReturnedBeforeSixtyEighthHead"
} > "$out"
echo "wrote $out"
