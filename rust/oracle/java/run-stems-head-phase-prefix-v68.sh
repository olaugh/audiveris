...
# Read-only snapshot-minimized order68 continuation from v67; prior orders mutate without snapshots.
set -eu
...
tmp_base=/private/tmp/stems-head-phase-prefix-v68
...
awk -v fragment="$fragment18" '
{
    ...
    if (capture_after && index($0, "heads, allLinkers);") != 0) {
        print
        print "            if (headOrder == 7 || headOrder == 22 || headOrder == 27 || headOrder == 34 || headOrder == 36 || headOrder == 37 || headOrder == 38 || headOrder == 39 || headOrder == 40 || headOrder == 41 || headOrder == 42 || headOrder == 43 || headOrder == 44 || headOrder == 45 || headOrder == 46 || headOrder == 47 || headOrder == 48 || headOrder == 49 || headOrder == 50 || headOrder == 51 || headOrder == 52 || headOrder == 53 || headOrder == 54 || headOrder == 55 || headOrder == 56 || headOrder == 57 || headOrder == 58 || headOrder == 59 || headOrder == 60 || headOrder == 61 || headOrder == 62 || headOrder == 63 || headOrder == 64 || headOrder == 65 || headOrder == 66 || headOrder == 67 || headOrder == 68) emitHeadCLinkResult(before, after, headOrder);"
        capture_after = 0
        in_linker = 0
        next
    }
...
if [ "$(grep -c '^stemsheadphasecontinue ' "$rows")" -ne 1 ] || \
        ! grep -q 'headOrder 68 headX 0 headSig 51 headInterId 1390 .*decisions \[LEFT:top=true:bottom=true:branch=Both,RIGHT:top=false:bottom=false:branch=Neither\].*incident - returned false .*undefs \[LEFT\].*closedValueChanges 0 .*sigEdgesBefore 700 sigEdgesAfter 700 .*nextHeadOrder 69 nextHeadX 87 nextHeadSig 83 nextHeadInterId 1455 ' "$rows" || \
        ! grep -q '^stemsheadclinkfrontier .*headOrder 68 headX 0 headSig 51 .*cAlias h:0:LEFT:BOTTOM .*lastIndex -1 maxIndex 1 relations 1 .*glyphs 2 .*candidateIdBefore 0 existingGlyph glyph:322 existingActive true existingStem 2384 ' "$rows" || \
        ! grep -q '^stemsheadclinkresult headOrder 68 allocatorBefore 2385 allocatorAfter 2385 .*addedVertices - addedEdges - addedSystemStems - linkerChanges - .*terminal ReturnedHeadCLinkTransaction' "$rows"; then
    echo "v68 open-frontier contract differs" >&2
    exit 1
fi
: <<'DISABLED_V27_ACTIVE'
...
v67_runner_sha=$(shasum -a 256 "$script_dir/run-stems-head-phase-prefix-v67.sh" | awk '{print $1}')
if [ "$v67_runner_sha" != "0e7656dc064aedfb9cbbab018630198294c83efec005a6451f0d11938e213a5e" ]; then
    echo "v68 base v67 runner drifted" >&2
    exit 1
fi
v67_fixture="$repo_root/rust/oracle/stems-head-phase-prefix-chula-system1-v67.txt"
v67_fixture_sha=$(shasum -a 256 "$v67_fixture" | awk '{print $1}')
if [ "$v67_fixture_sha" != "0f7c0e68fbba2ff4bae8fd5f69218c829e4969892966a8baab03eec4aff03d9f" ]; then
    echo "v68 base v67 fixture drifted" >&2
    exit 1
fi
...
out="/private/tmp/stems-head-phase-prefix-chula-system1-v68-audit.txt"
{
    echo '# Java Audiveris 5.11 (Temurin JDK 25.0.3+9 LTS) post-STUMPS head phase v68.'
    echo '# schema: stems-head-phase-prefix-v68'
    echo '# Snapshot-minimized order-68 derivative: orders 1-67 mutate without snapshots; order 68 emits the open-frontier transaction.'
    echo '# This bounded scope is intentional for deterministic replay under the full-snapshot heap limit.'
    cat "$rows"
    printf '%s\n' \
        "stemsheadphaseprefix summary schema stems-head-phase-prefix-v68 page chula.png#1 system 1 rows $row_count baseProbeSourceSha256 $base_probe_sha baseV67RunnerSourceSha256 $v67_runner_sha baseV67FixtureSha256 $v67_fixture_sha fragmentSourceSha256 $fragment_sha probeSourceSha256 $probe_sha runnerSourceSha256 $runner_sha emittedBodySha256 $body_sha semanticPassSha256 $semantic_sha completeStumpsFixtureSha256 $stumps_sha freshRuns 2 freshRunsByteIdentical true nativeScope BoundedSnapshotMinimizedOrder68OpenFrontier javaEvidence ReturnedBeforeSixtyNinthHead"
} > "$out"
echo "wrote $out"
