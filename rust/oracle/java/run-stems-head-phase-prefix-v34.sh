...
# Freeze a snapshot-minimized order34 both-open C-link continuation from v33; prior orders mutate without snapshots.
set -eu
...
tmp_base=/private/tmp/stems-head-phase-prefix-v34
...
awk -v fragment="$fragment18" '
{
    ...
    if (index($0, "final boolean returned = linker.linkSides(") != 0) {
        print "            if (headOrder == 7 || headOrder == 22 || headOrder == 27 || headOrder == 34) emitHeadCLinkEnvelope(head, linker, before, headOrder);"
        print
        in_linker = 1
        next
    }
    ...
    if (capture_after && index($0, "heads, allLinkers);") != 0) {
        print
        print "            if (headOrder == 7 || headOrder == 22 || headOrder == 27 || headOrder == 34) emitHeadCLinkResult(before, after, headOrder);"
        capture_after = 0
        in_linker = 0
        next
    }
...
if [ "$(grep -c '^stemsheadphasecontinue ' "$rows")" -ne 1 ] || \
        ! grep -q 'headOrder 34 headX 2 headSig 36 headInterId 1357 .*decisions \[LEFT:top=false:bottom=true:branch=BottomOnly,RIGHT:top=false:bottom=false:branch=Neither\].*sigVerticesBefore 683 sigVerticesAfter 684 sigEdgesBefore 694 sigEdgesAfter 695 .*nextHeadOrder 35 nextHeadX 50 nextHeadSig 72 nextHeadInterId 1433 ' "$rows" || \
        ! grep -q '^stemsheadclinkfrontier .*headOrder 34 headX 2 headSig 36 .*cAlias h:2:LEFT:BOTTOM .*lastIndex 1 maxIndex 1 relations 1 .*glyphs 2 .*candidateIdBefore 0 existingGlyph glyph:322 existingActive true existingStem - ' "$rows" || \
        ! grep -q '^stemsheadclinkresult headOrder 34 allocatorBefore 2383 allocatorAfter 2384 .*addedVertices \[id2384:.*addedEdges \[system1:sourceId1357:targetId2384:.*addedSystemStems \[g:393:393:3:86:.*:stemId2384\]' "$rows"; then
    echo "v34 both-open C-link contract differs" >&2
    exit 1
fi
: <<'DISABLED_V27_ACTIVE'
...
v33_runner_sha=$(shasum -a 256 "$script_dir/run-stems-head-phase-prefix-v33.sh" | awk '{print $1}')
if [ "$v33_runner_sha" != "472e88ea561df7db9280c5ec79a2ea8a5204783d3ffec8894455adcf5b342692" ]; then
    echo "v34 base v33 runner drifted" >&2
    exit 1
fi
v33_fixture="$repo_root/rust/oracle/stems-head-phase-prefix-chula-system1-v33.txt"
v33_fixture_sha=$(shasum -a 256 "$v33_fixture" | awk '{print $1}')
if [ "$v33_fixture_sha" != "a058341d3f661be4a677206c7a067f39a0785ae5adeed96be7d7073541fe2982" ]; then
    echo "v34 base v33 fixture drifted" >&2
    exit 1
fi
...
out="$repo_root/rust/oracle/stems-head-phase-prefix-chula-system1-v34.txt"
{
    echo '# Java Audiveris 5.11 (Temurin JDK 25.0.3+9 LTS) post-STUMPS head phase v34.'
    echo '# schema: stems-head-phase-prefix-v34'
    echo '# Snapshot-minimized order-34 derivative: orders 1-33 mutate without snapshots; order 34 emits the both-open C-link row.'
    echo '# This minimized scope is intentional for deterministic replay under the full-snapshot heap limit.'
    cat "$rows"
    printf '%s\n' \
        "stemsheadphaseprefix summary schema stems-head-phase-prefix-v34 page chula.png#1 system 1 rows $row_count baseProbeSourceSha256 $base_probe_sha baseV33RunnerSourceSha256 $v33_runner_sha baseV33FixtureSha256 $v33_fixture_sha fragmentSourceSha256 $fragment_sha probeSourceSha256 $probe_sha runnerSourceSha256 $runner_sha emittedBodySha256 $body_sha semanticPassSha256 $semantic_sha completeStumpsFixtureSha256 $stumps_sha freshRuns 2 freshRunsByteIdentical true nativeScope BoundedSnapshotMinimizedOrder34CLink javaEvidence ReturnedBeforeThirtyFifthHead"
} > "$out"
echo "wrote $out"
