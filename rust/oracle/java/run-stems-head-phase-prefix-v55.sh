...
# Read-only snapshot-minimized order55 continuation from v54; prior orders mutate without snapshots.
set -eu
...
tmp_base=/private/tmp/stems-head-phase-prefix-v55
...
awk -v fragment="$fragment18" '
{
    ...
    if (index($0, "final boolean returned = linker.linkSides(") != 0) {
        print "            if (headOrder == 7 || headOrder == 22 || headOrder == 27 || headOrder == 34 || headOrder == 36 || headOrder == 37 || headOrder == 38 || headOrder == 39 || headOrder == 40 || headOrder == 41 || headOrder == 42 || headOrder == 43 || headOrder == 44 || headOrder == 45 || headOrder == 46 || headOrder == 47 || headOrder == 48 || headOrder == 49 || headOrder == 50 || headOrder == 51 || headOrder == 52 || headOrder == 53 || headOrder == 54 || headOrder == 55) emitHeadCLinkEnvelope(head, linker, before, headOrder);"
        print
        in_linker = 1
        next
    }
    ...
    if (capture_after && index($0, "heads, allLinkers);") != 0) {
        print
        print "            if (headOrder == 7 || headOrder == 22 || headOrder == 27 || headOrder == 34 || headOrder == 36 || headOrder == 37 || headOrder == 38 || headOrder == 39 || headOrder == 40 || headOrder == 41 || headOrder == 42 || headOrder == 43 || headOrder == 44 || headOrder == 45 || headOrder == 46 || headOrder == 47 || headOrder == 48 || headOrder == 49 || headOrder == 50 || headOrder == 51 || headOrder == 52 || headOrder == 53 || headOrder == 54 || headOrder == 55) emitHeadCLinkResult(before, after, headOrder);"
        capture_after = 0
        in_linker = 0
        next
    }
...
if [ "$(grep -c '^stemsheadphasecontinue ' "$rows")" -ne 1 ] || \
        ! grep -q 'headOrder 55 headX 68 headSig 75 headInterId 1439 .*decisions \[LEFT:SkipAlreadyLinked,RIGHT:SkipClosed\].*incident \[stem2347:headSideLEFT:heads\[x68:sig75:id1439:sideLEFT,x69:sig76:id1441:sideLEFT\].*closureWrites \[x69:sig76:LEFT:false->true,x69:sig76:RIGHT:false->true\].*closedValueChanges 2 .*nextHeadOrder 56 nextHeadX 21 nextHeadSig 11 nextHeadInterId 1307 ' "$rows" || \
        ! grep -q '^stemsheadclinkfrontier .*headOrder 55 headX 68 headSig 75 .*cAlias h:68:LEFT:BOTTOM .*lastIndex 1 maxIndex 1 relations 1 .*glyphs 2 .*candidateIdBefore 0 existingGlyph glyph:331 existingActive true existingStem 2347 ' "$rows" || \
        ! grep -q '^stemsheadclinkresult headOrder 55 allocatorBefore 2385 allocatorAfter 2385 .*addedVertices - addedEdges - addedSystemStems - .*linkerChanges \[h:69:LEFT:BOTTOM:true:false->true:true' "$rows"; then
    echo "v55 existing-stem reconciliation contract differs" >&2
    exit 1
fi
: <<'DISABLED_V27_ACTIVE'
...
v54_runner_sha=$(shasum -a 256 "$script_dir/run-stems-head-phase-prefix-v54.sh" | awk '{print $1}')
if [ "$v54_runner_sha" != "1fec8bbf7c5561150d9d69079275fccc1879f5d61756f8c525957c0ef90b16ca" ]; then
    echo "v55 base v54 runner drifted" >&2
    exit 1
fi
v54_fixture="$repo_root/rust/oracle/stems-head-phase-prefix-chula-system1-v54.txt"
v54_fixture_sha=$(shasum -a 256 "$v54_fixture" | awk '{print $1}')
if [ "$v54_fixture_sha" != "c8b6567ddac7269d126846e036fc7e4fbb8a9c430a7b110006ff8e980ad85305" ]; then
    echo "v55 base v54 fixture drifted" >&2
    exit 1
fi
...
out="/private/tmp/stems-head-phase-prefix-chula-system1-v55-audit.txt"
{
    echo '# Java Audiveris 5.11 (Temurin JDK 25.0.3+9 LTS) post-STUMPS head phase v55.'
    echo '# schema: stems-head-phase-prefix-v55'
    echo '# Snapshot-minimized order-55 derivative: orders 1-54 mutate without snapshots; order 55 emits existing-stem reconciliation.'
    echo '# This bounded scope is intentional for deterministic replay under the full-snapshot heap limit.'
    cat "$rows"
    printf '%s\n' \
        "stemsheadphaseprefix summary schema stems-head-phase-prefix-v55 page chula.png#1 system 1 rows $row_count baseProbeSourceSha256 $base_probe_sha baseV54RunnerSourceSha256 $v54_runner_sha baseV54FixtureSha256 $v54_fixture_sha fragmentSourceSha256 $fragment_sha probeSourceSha256 $probe_sha runnerSourceSha256 $runner_sha emittedBodySha256 $body_sha semanticPassSha256 $semantic_sha completeStumpsFixtureSha256 $stumps_sha freshRuns 2 freshRunsByteIdentical true nativeScope BoundedSnapshotMinimizedOrder55ExistingStemReconciliation javaEvidence ReturnedBeforeFiftySixthHead"
} > "$out"
echo "wrote $out"
