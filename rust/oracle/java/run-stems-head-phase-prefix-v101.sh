...
# Read-only snapshot-minimized order101 continuation from v100; prior orders mutate without snapshots.
set -eu
...
tmp_base=/private/tmp/stems-head-phase-prefix-v101
...
if [ "$(grep -c '^stemsheadphasecontinue ' "$rows")" -ne 1 ] || \
        ! grep -q 'headOrder 101 headX 5 headSig 88 headInterId 1465 .*decisions \[LEFT:SkipAlreadyLinked,RIGHT:SkipClosed\].*incident \[stem2348:headSideLEFT:heads\[x5:sig88:id1465:sideLEFT,x6:sig89:id1467:sideLEFT\].*closureWrites \[x6:sig89:LEFT:false->true,x6:sig89:RIGHT:false->true\].*closedValueChanges 2 .*nextHeadOrder - nextHeadX - nextHeadSig - nextHeadInterId - nextGrade - nextSides - terminal ReturnedAfterFinalHead' "$rows" || \
        ! grep -q '^stemsheadclinkfrontier .*headOrder 101 headX 5 headSig 88 .*cAlias h:5:LEFT:BOTTOM .*lastIndex 2 maxIndex 2 relations 3 .*glyphs 1 .*candidateIdBefore 290 existingGlyph glyph:290 existingActive true existingStem 2348 ' "$rows" || \
        ! grep -q '^stemsheadclinkresult headOrder 101 allocatorBefore 2385 allocatorAfter 2385 .*addedVertices - addedEdges - addedSystemStems - .*linkerChanges \[h:6:LEFT:BOTTOM:true:false->true:true' "$rows"; then
    echo "v101 final-head existing-stem reconciliation contract differs" >&2
    exit 1
fi
: <<'DISABLED_V27_ACTIVE'
...
v100_runner_sha=$(shasum -a 256 "$script_dir/run-stems-head-phase-prefix-v100.sh" | awk '{print $1}')
if [ "$v100_runner_sha" != "8c665c06c8b49b34c0d9b1b5375619946b75f95d4803c8ab960c2cb73858fb37" ]; then
    echo "v101 base v100 runner drifted" >&2
    exit 1
fi
v100_fixture="$repo_root/rust/oracle/stems-head-phase-prefix-chula-system1-v100.txt"
v100_fixture_sha=$(shasum -a 256 "$v100_fixture" | awk '{print $1}')
if [ "$v100_fixture_sha" != "aea95a084cac25e2984c848b9de386e047ab3dc9c4fdc3f1fb503d04fa76bc92" ]; then
    echo "v101 base v100 fixture drifted" >&2
    exit 1
fi
...
out="/private/tmp/stems-head-phase-prefix-chula-system1-v101-audit.txt"
{
    echo '# Java Audiveris 5.11 (Temurin JDK 25.0.3+9 LTS) post-STUMPS head phase v101.'
    echo '# schema: stems-head-phase-prefix-v101'
    ...
    cat "$rows"
    printf '%s\n' \
        "stemsheadphaseprefix summary schema stems-head-phase-prefix-v101 page chula.png#1 system 1 rows $row_count baseProbeSourceSha256 $base_probe_sha baseV100RunnerSourceSha256 $v100_runner_sha baseV100FixtureSha256 $v100_fixture_sha fragmentSourceSha256 $fragment_sha probeSourceSha256 $probe_sha runnerSourceSha256 $runner_sha emittedBodySha256 $body_sha semanticPassSha256 $semantic_sha completeStumpsFixtureSha256 $stumps_sha freshRuns 2 freshRunsByteIdentical true nativeScope BoundedSnapshotMinimizedOrder101FinalHeadExistingStemReconciliation javaEvidence ReturnedAfterFinalPhaseOneHead"
} > "$out"
echo "wrote $out"
