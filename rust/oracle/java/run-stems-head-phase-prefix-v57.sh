...
# Read-only snapshot-minimized order57 continuation from v56; prior orders mutate without snapshots.
set -eu
...
tmp_base=/private/tmp/stems-head-phase-prefix-v57
...
awk -v fragment="$fragment18" '
{
    ...
    if (capture_after && index($0, "heads, allLinkers);") != 0) {
        print
        print "            if (headOrder == 7 || headOrder == 22 || headOrder == 27 || headOrder == 34 || headOrder == 36 || headOrder == 37 || headOrder == 38 || headOrder == 39 || headOrder == 40 || headOrder == 41 || headOrder == 42 || headOrder == 43 || headOrder == 44 || headOrder == 45 || headOrder == 46 || headOrder == 47 || headOrder == 48 || headOrder == 49 || headOrder == 50 || headOrder == 51 || headOrder == 52 || headOrder == 53 || headOrder == 54 || headOrder == 55 || headOrder == 56 || headOrder == 57) emitHeadCLinkResult(before, after, headOrder);"
        capture_after = 0
        in_linker = 0
        next
    }
...
if [ "$(grep -c '^stemsheadphasecontinue ' "$rows")" -ne 1 ] || \
        ! grep -q 'headOrder 57 headX 62 headSig 16 headInterId 1317 .*decisions \[LEFT:top=false:bottom=true:branch=BottomOnly,RIGHT:top=false:bottom=false:branch=Neither\].*incident - returned true .*undefs \[\].*closureWrites - closedValueChanges 0 .*sigEdgesBefore 696 sigEdgesAfter 697 .*nextHeadOrder 58 nextHeadX 92 nextHeadSig 24 nextHeadInterId 1333 ' "$rows" || \
        ! grep -q '^stemsheadclinkfrontier .*headOrder 57 headX 62 headSig 16 .*cAlias h:62:LEFT:BOTTOM .*lastIndex 1 maxIndex 1 relations 1 .*glyphs 2 .*candidateIdBefore 0 existingGlyph glyph:328 existingActive true existingStem 2381 ' "$rows" || \
        ! grep -q '^stemsheadclinkresult headOrder 57 allocatorBefore 2385 allocatorAfter 2385 .*addedVertices - addedEdges \[system1:sourceId1317:targetId2381:org.audiveris.omr.sig.relation.HeadStemRelation.*addedSystemStems - linkerChanges \[h:62:LEFT:BOTTOM:false:false->true:false' "$rows"; then
    echo "v57 existing-stem C-link contract differs" >&2
    exit 1
fi
: <<'DISABLED_V27_ACTIVE'
...
v56_runner_sha=$(shasum -a 256 "$script_dir/run-stems-head-phase-prefix-v56.sh" | awk '{print $1}')
if [ "$v56_runner_sha" != "b6a0b6cd9a618052e02da16022a44fe3b218626100852bb6f702ceedc09f3387" ]; then
    echo "v57 base v56 runner drifted" >&2
    exit 1
fi
v56_fixture="$repo_root/rust/oracle/stems-head-phase-prefix-chula-system1-v56.txt"
v56_fixture_sha=$(shasum -a 256 "$v56_fixture" | awk '{print $1}')
if [ "$v56_fixture_sha" != "e463b46a707b8c534a1896bf47d7668c580f38c27b2947ccdfb12fb7984e2cc5" ]; then
    echo "v57 base v56 fixture drifted" >&2
    exit 1
fi
...
out="/private/tmp/stems-head-phase-prefix-chula-system1-v57-audit.txt"
{
    echo '# Java Audiveris 5.11 (Temurin JDK 25.0.3+9 LTS) post-STUMPS head phase v57.'
    echo '# schema: stems-head-phase-prefix-v57'
    echo '# Snapshot-minimized order-57 derivative: orders 1-56 mutate without snapshots; order 57 emits the existing-stem C-link.'
    echo '# This bounded scope is intentional for deterministic replay under the full-snapshot heap limit.'
    cat "$rows"
    printf '%s\n' \
        "stemsheadphaseprefix summary schema stems-head-phase-prefix-v57 page chula.png#1 system 1 rows $row_count baseProbeSourceSha256 $base_probe_sha baseV56RunnerSourceSha256 $v56_runner_sha baseV56FixtureSha256 $v56_fixture_sha fragmentSourceSha256 $fragment_sha probeSourceSha256 $probe_sha runnerSourceSha256 $runner_sha emittedBodySha256 $body_sha semanticPassSha256 $semantic_sha completeStumpsFixtureSha256 $stumps_sha freshRuns 2 freshRunsByteIdentical true nativeScope BoundedSnapshotMinimizedOrder57ExistingStemCLink javaEvidence ReturnedBeforeFiftyEighthHead"
} > "$out"
echo "wrote $out"
