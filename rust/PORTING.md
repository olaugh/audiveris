...
| Rust workspace | The workspace now contains two hundred and seventy-three exact STEMS boundaries. Boundaries 1-267 retain the detailed construction, scheduler, mutation, head-linking, phase-two, finalization, and Bach completion evidence below. Boundary 268 completes Allegretto and proves Rust-only schema-1 STEMS completion across all eight frozen parity pages. Boundary 269 generalizes rejected SIDES carriage, initial retries/no-link, C-link expansion and reuse, JDK25 TimSort, and phase-two append/no-link behavior; D039 completes with 4 systems, 255 stems, 947 checked heads, 725 HeadStem relations, and 223 abnormal heads. Boundary 270 adds both extracted Dichterliebe PDF pages, bringing the exact Rust-only completion census to eleven real pages. Boundary 271 removes 1,102 lines of production page/queue-specific head dispatch; Boundary 272 removes the final Chula-shaped production authentication branch from generic `finalizeStems`; Boundary 273 carries Java's mutable per-corner C-link theoretical lines across failed profiles and phase two, removing production dependence on coordinate ULP and Bach gap exceptions. The same eleven-page and measured transaction gates pass through only the generic engines. CLI, report, all eleven real-page completion gates, the 35-case transaction suite, strict workspace Clippy, formatting, and diff checks are green. `425d58e82` remains the exact fully green remote CI baseline pending a newer terminal Rust workflow. CI repeats formatting, strict Clippy, and workspace tests on Ubuntu and macOS |
...
| Later recognition stages | the remaining dependency-light lifecycles are native for `STEMS`, `REDUCTION`, `CUE_BEAMS`, `TEXTS`, `MEASURES`, `CHORDS`, `CURVES`, `SYMBOLS`, `LINKS`, `RHYTHMS`, and `PAGE`; STEMS has two hundred and seventy-one exact semantic components. Chula, Allegretto, Batuque, Zizi, Carmen, Cucaracha, Hove, and Bach all complete transactional recognition, generic `finalizeStems`, and schema-1 publication. The production generic path also completes D039 and both extracted Dichterliebe pages beyond the frozen parity corpus. Production no longer dispatches by authenticated page/system/queue/head coordinates; those measured wrappers remain test-only oracle evidence. Remaining STEMS work is broader out-of-corpus robustness and parity scanning, not a known missing transaction, finalize, or publication phase. |
