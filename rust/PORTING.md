...
| GRID staff limits | **known divergence.** Java refines `staff.getAbscissa(side)` before `completeLines`: `BarsRetriever` sets LEFT from the start column's peak stop, and `StaffProjector.refineStaffRightEnd` sets RIGHT from the trailing blank or the end peak's midpoint. The port still feeds `completeLines` the unrefined cluster-candidate limits, so on chula every line ending sits 1-3 px inside Java's (staff 6 right, 13 px) and `defineEndPoints` extrapolates by `dx * slope` from there, leaving a <= 0.11 px ordinate residual. `completed_staff_lines_match_the_java_ordinates_on_chula` pins the Java endpoints with exactly that bound. The pieces are already ported but unwired: `LinesRootUpdate`, `RightEndUpdate` from `process_bars_right_ends_and_c_clefs`, and `BarsCoordinatorResult::start_column_index`. Wiring them and re-tightening that test to equality is the next GRID slice |
