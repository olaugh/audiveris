...
| Rust workspace | 569 tests green at checkpoint 219: core 38, image 403, OMR 113, testkit 6, CLI 4, xtask 5 |
...
| GRID coordinator | exact `GridStep.doit` order through builder, real filament-to-registered-glyph persistent-line conversion, staff-line cleanup, and score regrouping; concrete raster lag creation/short-section insertion and sheet handoff; production-backed prepared and live-raw primary cluster retrieval, bar-system processing, and completion state with explicit remaining geometry collaborators; a concrete raw-raster executor installs the retrieved staff, measured sheet skew, retained slope rejects, and failure prefixes into sheet state; StaffLineCleaner lifecycle and no-staff horizontal-lag rebuild; source-order system population, curved areas/slicing, section ownership, indentation, physical-page/PageRef allocation, report maxima, and `SystemInfo.buildRef` soft-reference identity/ordering/defaults; raw projector/bar/completion construction, use of the stored skew by downstream geometry, secondary lines, and some exceptional prefixes remain incomplete |
