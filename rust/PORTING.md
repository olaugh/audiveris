...
| Later recognition stages | the remaining dependency-light lifecycles are native for `STEMS`, `REDUCTION`, `CUE_BEAMS`, `TEXTS`, `MEASURES`, `CHORDS`, `CURVES`, `SYMBOLS`, `LINKS`, `RHYTHMS`, and `PAGE`; STEMS has two hundred and seventy exact semantic components. Chula, Allegretto, Batuque, Zizi, Carmen, Cucaracha, Hove, and Bach all complete transactional recognition, generic `finalizeStems`, and schema-1 publication. The production generic path also completes D039 and both extracted Dichterliebe pages beyond the frozen parity corpus. These pages are completion regressions, not yet Java terminal-parity oracles. Remaining STEMS work is broader out-of-corpus robustness scanning and retiring authenticated legacy dispatch wrappers, not a known missing finalize or publication phase. |
