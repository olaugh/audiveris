...
| Later recognition stages | the remaining dependency-light lifecycles are native for `STEMS`, `REDUCTION`, `CUE_BEAMS`, `TEXTS`, `MEASURES`, `CHORDS`, `CURVES`, `SYMBOLS`, `LINKS`, `RHYTHMS`, and `PAGE`; STEMS has one hundred and ninety-one exact semantic components. Chula completes all three systems through phase 1, phase 2, generic `finalizeStems`, and transactional `recognize_native_stems`. Allegretto completes all three SIDES/STUMPS systems, system 1 through queue 79, system 2 through queue 111, and system 3 through all 118 phase-1 heads, the corrected five-entry phase-2 queue, and exact generic `finalizeStems`; its system-3 finalizer checks 118 heads and preserves the complete carrier. Batuque completes all three SIDES/STUMPS, both HEADS phases, generic `finalizeStems`, and schema-1 publication with one shared page identity stream. Zizi completes transactional recognition and schema-1 publication. Carmen completes all five systems through phase 1, phase 2, generic `finalizeStems`, and transactional `recognize_native_stems`: its shared-stump dual corners, show-stopping Gap no-link, relation-only crossed-head expansion, recursive close-head/gap feasibility, final-relation rejection, candidate-stem reuse, and ordered append-mode reuse of an earlier crossed stem all run through production code. Next is the first fail-closed frontier on Cucaracha, Hove, or BachInvention5 |
