...
| Native recognition entry | `audiveris_omr::recognize` runs LOAD-BINARY-SCALE and the GRID staff-line slice (run partition, horizontal lag, measure-then-cluster primary passes, staff candidates) on a raster with production scale-derived parameters; `audiveris-cli -batch -step SCALE|GRID` prints reports; chula locked against the live Java oracle (slope 0.00792, 6 standard staves in 3 systems, system-1 indentation) |
