...
| Rust workspace | The workspace now contains one hundred and seventy-nine exact STEMS boundaries. Boundaries 1-134 retain the detailed scheduler, mutation, head-linking, phase-2, and generic `finalizeStems` evidence. Boundaries 135-154 compose and drive Batuque's page-wide predecessor/SIDES/STUMPS state; Boundaries 155-159 complete all three phase-1 head queues; Boundary 160 consumes all four page-wide phase-2 retries; Boundary 161 atomically finalizes all three systems; Boundary 162 exposes fail-closed `recognize_native_stems`; Boundary 163 publishes the complete schema-1 ordinary/stream result; Boundaries 164-166 complete Chula through transactional recognition; Boundary 167 integrates generic competing-hook removal into the page carrier and completes Allegretto's three SIDES/STUMPS systems; Boundaries 168-169 carry Allegretto system 1 through queue-65 existing-stem reuse and queue-79 crossed-side checked-stem creation; Boundaries 170-171 carry system 2 through queue-89's generic beam-bearing checked-stem creation and queue-111's multi-head hard-tail rejection; Boundaries 172-173 carry system 3 through queue 29's two-head checked stem and queue 61's two-chunk beam-bearing checked stem; Boundary 174 fixes the generic two-horizontal-side transaction, carries queue 53's same-glyph multi-head reuse, and proves queue 115 is an ordinary returned-false no-link; Boundaries 175-176 carry queue 116 and exhaust phase 1 through queue 117's idempotent Stem2368 closure; Boundary 177 reproduces Java's full-page x0 rejected-chunk early stop, creates the missing checked stem, and corrects the phase-2 retry queue from six heads to five; Boundary 178 consumes retry zero x112; Boundary 179 executes x14's first real phase-2 append mutation by reusing Stem3148 and adding one HeadStem edge. Wider HEADS and corpus branches remain explicit. CLI, report, focused Batuque/Chula/Allegretto, and full sibling gates plus strict workspace Clippy, formatting, and diff checks are green. `e99e93a92` is the exact remote CI baseline: Build & Test run 32528147579 and Rust port run 32528147610 both succeeded, with all 12 Rust shards green. CI repeats formatting, strict Clippy, and workspace tests on Ubuntu and macOS |
...
| Later recognition stages | the remaining dependency-light lifecycles are native for `STEMS`, `REDUCTION`, `CUE_BEAMS`, `TEXTS`, `MEASURES`, `CHORDS`, `CURVES`, `SYMBOLS`, `LINKS`, `RHYTHMS`, and `PAGE`; STEMS has one hundred and seventy-nine exact semantic components. Chula completes all three systems through phase 1, phase 2, generic `finalizeStems`, and transactional `recognize_native_stems`. Allegretto completes all three SIDES/STUMPS systems, system 1 through queue 79, system 2 through queue 111, and system 3 through all 118 phase-1 heads after carrying queue 53's two-side same-glyph reuse and the full-page x0 rejected-chunk checked-stem creation; its corrected five-entry phase-2 queue has consumed x112/SIG68 and x14/SIG50 and resumes at x13/SIG0. Batuque completes all three SIDES/STUMPS, both HEADS phases, generic `finalizeStems`, and schema-1 publication with one shared page identity stream. Next is Allegretto system 3 phase-2 retry index 2 and wider-corpus completion |
