...
| Live Java/Rust vectors | 61 canonical fixtures, including composed StaffProjector, recursive clusters, bar-column construction/start selection, production `LagManager.dispatchRuns`, and `Book.updateScores` regrouping |
...
| Rust workspace | 488 tests green at checkpoint 190: core 38, image 341, OMR 94, testkit 6, CLI 4, xtask 5 |
...
| GRID LinesRetriever | exact outer `GridBuilder` and inner `completeLines` stage/exception lifecycles; source-preserving run dispatch, long/short partitions, initial vertical/horizontal lag policies, append-only short-section registration, sticker classification/acceptance/assignment, StaffFilament hole insertion/interpolation, full endpoint retrieval, discarded-filament traversal, curvature polishing, and crossing-chunk inspection; transactional main and optional small-interline cluster passes, Java-ordered secondary filament input, buildStaves rejection/layout/right-indent sequence, typed staff candidates, and staff-line simplification lifecycle; concrete `toStaffLine` glyph conversion remains queued |
...
| GRID coordinator | exact `GridStep.doit` order through builder, staff-line cleanup, and score regrouping; Java-compatible outer exception semantics plus a separate transactional LinesRetriever-before-BarsRetriever join; StaffLineCleaner simplification/removal/rebuild/population lifecycle; no-staff horizontal-lag rebuild; sequential staff validation; source-order system population, section ownership, indentation, physical-page/PageRef allocation, and report maxima; concrete area construction, `toStaffLine`, `SystemInfo.buildRef`, glyph ownership, and production SIG mutation remain queued |
