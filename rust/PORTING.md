...
| Rust workspace | 663 tests green at checkpoint 257: core 38, image 460, OMR 150, testkit 6, CLI 4, xtask 5 |
...
| GRID peaks and peak graph | neutral `StaffPeak`, `PartGroup`, and stable-ID graph storage; incident/connection queries, alignment purge, connection median geometry, brace checks, stable glyph/inter identities, peak backlinks, connector relations, freezing, and bar-group relations; raw registry peaks materialize in exact order, acquire VLAG/HLAG bar sticks and curvature marks, run Java's skew-aware alignments and pixel-backed connections, execute fixed-point merged-group splitting, build ordered systems/columns, detect and replace brace portions, construct/register brace compounds and SIG inters, purge peaks left of braces, and correct lines roots; bracket-end detection is the next raw bar boundary |
...
| GRID coordinator | exact `GridStep.doit` order through builder, real filament-to-registered-glyph persistent-line conversion, staff-line cleanup, and score regrouping; concrete raster lag creation/short-section insertion and sheet handoff; production-backed live-raw primary/secondary clusters, projectors, peak graphs, systems, bars, and full line completion; retained raw metadata survives the `ProcessBars` extraction; measured sheet skew/slope, discarded candidates, ordered system ownership, and failure prefixes remain explicit; StaffLineCleaner lifecycle and no-staff horizontal-lag rebuild plus source-order system population, curved areas/slicing, section ownership, indentation, physical-page/PageRef allocation, report maxima, and `SystemInfo.buildRef` identity/ordering/defaults are concrete; automatic transfer of exact raw bar-system ownership into line completion is the remaining composition seam |
...
| HEADERS recognition stage | headless `StaffHeader`, step lifecycle, and injected `HeaderBuilder` shell implemented with exact header-start/dummy-bar ownership, recognizer call order, stop propagation, bar purge, freeze, and failure prefixes; clef/key/time visual recognition remains queued |
| Later recognition stages | queued in pipeline order after HEADERS |
