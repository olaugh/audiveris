...
| Rust workspace | The workspace now contains one hundred and thirty-five exact STEMS boundaries. Boundaries 1-134 retain the detailed exact scheduler, mutation, head-linking, phase-2, and generic `finalizeStems` evidence below. Boundary 135 adds `native_stems::prepare_native_stems`, the owned production composition of every live pre-mutation STEMS component plus native SIG assembly; Chula's exact downstream carrier now consumes that entry instead of a test-local orchestration chain. The disclosed first-STEMS snapshot, sparse selected-base Java identity bridge, reconstructed Allegretto predecessor, and wider mutation/corpus branches remain explicit. Focused 1/1, full sibling 14/14, strict workspace Clippy, formatting, and diff checks are green. `664185a6b` remains the current remote CI baseline until this boundary's exact workflows finish. CI repeats formatting, strict Clippy, and workspace tests on Ubuntu and macOS |
...
| Later recognition stages | the remaining dependency-light lifecycles are native for `STEMS`, `REDUCTION`, `CUE_BEAMS`, `TEXTS`, `MEASURES`, `CHORDS`, `CURVES`, `SYMBOLS`, `LINKS`, `RHYTHMS`, and `PAGE`; STEMS has one hundred and thirty-five exact semantic components. Chula system 1 runs the carried SIDES, STUMPS, head-linking, phase-2, and generic `finalizeStems` path described below. Boundary 135 now composes the complete live read-only STEMS predecessor and native SIG through a production API. Next is replacement of the disclosed first-STEMS snapshot and sparse 16-row selected-base Java identity authority, native Allegretto predecessor carriage, wider linked-S/hook/STUMPS coverage, successful append `reuseStem`, actually-unlinked/no-link and generic retry, broader C-link shapes, and finally the transactional `recognize_native_stems` entry point |
...
first-STEMS persistent snapshot, sparse selected-base identity bridge, and wider
mutation/corpus branches remain the next authorities to remove.
