...
| Live Java/Rust vectors | 67 canonical fixtures, including composed StaffProjector, recursive clusters, bar-column construction/start selection, production `LagManager.dispatchRuns`, `Book.updateScores` regrouping, live `SystemInfo.buildRef` ownership, a composed GRID output boundary, production SIG contextual grading, exact sheet-skew transforms, raw-raster `retrieveLines`, and raster-fitted mutable line endpoints |
...
| Rust workspace | 598 tests green at checkpoint 233: core 38, image 427, OMR 118, testkit 6, CLI 4, xtask 5 |
...
| GRID peaks and peak graph | neutral `StaffPeak`, `PartGroup`, and stable-ID graph storage; incident/connection queries, alignment purge, connection median geometry, brace checks, stable glyph/inter identities, peak backlinks, connector relations, freezing, and bar-group relations; raw registry peaks now materialize in exact order, acquire VLAG/HLAG bar sticks and curvature marks, and run Java's unpurged skew-aware `findAllAlignments` traversal |
...
| GRID coordinator | exact `GridStep.doit` order through builder, real filament-to-registered-glyph persistent-line conversion, staff-line cleanup, and score regrouping; concrete raster lag creation/short-section insertion and sheet handoff; production-backed prepared and live-raw primary/secondary cluster retrieval, bar-system processing, and completion state with explicit remaining geometry collaborators; a concrete raw-raster executor installs retrieved staffs, measured sheet skew, retained slope rejects, and failure prefixes into sheet state; the exact final cluster-reject-plus-slope fallback population reaches concrete completion; raw staff projectors are constructed from the live raster with Java-rounded geometry and deskew centers, then materialized into peak graphs, bar sticks, curvature marks, and unpurged alignments; StaffLineCleaner lifecycle and no-staff horizontal-lag rebuild; source-order system population, curved areas/slicing, section ownership, indentation, physical-page/PageRef allocation, report maxima, and `SystemInfo.buildRef` soft-reference identity/ordering/defaults; raw connection discovery/system construction and later concrete line-completion stages remain incomplete |
