...
| Live Java/Rust vectors | 64 canonical fixtures, including composed StaffProjector, recursive clusters, bar-column construction/start selection, production `LagManager.dispatchRuns`, `Book.updateScores` regrouping, live `SystemInfo.buildRef` ownership, a composed GRID output boundary, and production SIG contextual grading |
...
| Rust workspace | 565 tests green at checkpoint 217: core 38, image 400, OMR 112, testkit 6, CLI 4, xtask 5 |
...
| GRID filament factory | core and local-fatness filtering, stable reverse-length traversal, real-gap/overlap merge gates, leftover expansion, and final merge; exact merge/rejection plus bounded real-page vectors; raw adapter IDs are stable but do not yet reproduce Java `FilamentIndex` gaps from temporary/merged candidates; glyph/index and vertical behavior otherwise queued |
...
| GRID coordinator | exact `GridStep.doit` order through builder, real filament-to-registered-glyph persistent-line conversion, staff-line cleanup, and score regrouping; concrete raster lag creation/short-section insertion and sheet handoff; production-backed prepared and live-raw primary cluster retrieval, bar-system processing, and completion state with explicit remaining geometry collaborators; a concrete raw-raster executor installs the retrieved staff and failure prefixes into sheet state; StaffLineCleaner lifecycle and no-staff horizontal-lag rebuild; source-order system population, curved areas/slicing, section ownership, indentation, physical-page/PageRef allocation, report maxima, and `SystemInfo.buildRef` soft-reference identity/ordering/defaults; raw projector/bar/completion construction, computed-skew propagation, secondary lines, and some exceptional prefixes remain incomplete |
