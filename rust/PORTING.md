...
| Later recognition stages | `STEMS` is complete as a generic headless recognition stage: construction, SIDES/STUMPS, both head phases, `finalizeStems`, `finalizeBeams`, SIG contextualization, and schema-1 publication all run from native products with no page/system/queue identity dispatch. Chula, Allegretto, Batuque, Zizi, Carmen, Cucaracha, Hove, Bach, D039, and both extracted Dichterliebe pages complete the Rust-only path. `REDUCTION` is in progress: terminal native STEMS SIGs now execute the complete first Java foundations epoch from production lossless overlap discovery through the consistency fixed point, remaining-exclusion reduction, repeated chord analysis, late exclusion reduction, and late contextual purge. Stable tombstones, multi-staff ledger ownership, exact geometry, per-pass mutation traces, and Java's distinct outer-local versus adapter-owned removal sets are retained. The next exact frontier is repetition of outer foundations epochs to convergence. Frozen measured wrappers remain test-only oracle evidence. Additional score scans are continuing robustness evidence rather than an open STEMS implementation phase. |
