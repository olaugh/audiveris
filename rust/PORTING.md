...
| Rust workspace | The workspace now contains one hundred and fifty-seven exact STEMS boundaries. Boundaries 1-134 retain the detailed scheduler, mutation, head-linking, phase-2, and generic `finalizeStems` evidence. Boundaries 135-147 compose and internalize the live native predecessor; Boundaries 148-154 drive Batuque's complete three-system SIDES/STUMPS page; Boundary 155 closes each native prelinked head prefix; Boundary 156 executes two first C-link creations and one rejected-link/no-link closure; Boundary 157 carries all three systems to their next C-link frontiers while preserving system 2's phase-2 retry queue. Wider head mutation/retry and corpus branches remain explicit. Focused Batuque and full sibling 15/15, strict workspace Clippy, formatting, and diff checks are green. `664185a6b` remains the documented remote CI baseline pending newer terminal evidence. CI repeats formatting, strict Clippy, and workspace tests on Ubuntu and macOS |
...
| Later recognition stages | the remaining dependency-light lifecycles are native for `STEMS`, `REDUCTION`, `CUE_BEAMS`, `TEXTS`, `MEASURES`, `CHORDS`, `CURVES`, `SYMBOLS`, `LINKS`, `RHYTHMS`, and `PAGE`; STEMS has one hundred and fifty-seven exact semantic components. Chula system 1 runs carried SIDES, STUMPS, head linking, phase 2, and generic `finalizeStems`; Allegretto system 1 carries SIDES transactions 1-28 through linked-S reuse and competing-hook removal; Batuque now drives all three systems through SIDES/STUMPS, mixed first HEADS outcomes, and their next actionable frontiers with one shared page identity stream. Boundaries 135-157 compose that page-wide production carriage. Next are the remaining head/retry queues, then transactional `recognize_native_stems` and schema-1 publication |
