...
| Live Java/Rust vectors | 60 canonical fixtures, including composed StaffProjector, recursive clusters, bar-column construction/start selection, and production `LagManager.dispatchRuns` |
...
| Rust workspace | 442 tests green at checkpoint 178: core 38, image 298, OMR 91, testkit 6, CLI 4, xtask 5 |
...
| GRID LinesRetriever | exact outer `GridBuilder` and inner `completeLines` stage/exception lifecycles; source-preserving run dispatch, long/short partitions, initial vertical/horizontal lag policies, append-only short-section registration, sticker classification/acceptance/assignment, and StaffFilament hole-position/two-sided interpolation planning; transactional main and optional small-interline cluster passes, Java-ordered secondary filament input, buildStaves rejection/layout/right-indent sequence, typed standard/one-line/tablature staff candidates, and small/short flags; concrete endpoint retrieval, discarded-filament traversal, curvature polishing, crossing-chunk inspection, and sheet ownership queued |
...
| GRID coordinator | both Java-compatible outer lifecycle semantics (ordinary throwable logged/swallowed with partial mutation retained; `StepException` propagated) and a separate transactional LinesRetriever-before-BarsRetriever join; sequential staff identity/kind validation, Java-compatible omitted one-line staffs, source-order system processing, and whole-grid rollback; concrete line completion and production sheet/SIG mutation queued |
