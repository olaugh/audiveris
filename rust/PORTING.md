...
| Rust workspace | The workspace now contains one hundred and thirty-seven exact STEMS boundaries. Boundaries 1-134 retain the detailed scheduler, mutation, head-linking, phase-2, and generic `finalizeStems` evidence. Boundary 135 composes all live pre-mutation products; Boundary 136 replaces the page-wide first-STEMS snapshot from transaction 3 onward; Boundary 137 extends native modeled-glyph identity to transaction 2. Transaction 1's compact fixture state, the sparse selected-base Java InterIndex bridge, reconstructed Allegretto predecessor, and wider mutation/corpus branches remain explicit. Focused 1/1, full sibling 14/14, strict workspace Clippy, formatting, and diff checks are green. `664185a6b` remains the current remote CI baseline pending this boundary's workflows. CI repeats formatting, strict Clippy, and workspace tests on Ubuntu and macOS |
...
| Later recognition stages | the remaining dependency-light lifecycles are native for `STEMS`, `REDUCTION`, `CUE_BEAMS`, `TEXTS`, `MEASURES`, `CHORDS`, `CURVES`, `SYMBOLS`, `LINKS`, `RHYTHMS`, and `PAGE`; STEMS has one hundred and thirty-seven exact semantic components. Chula system 1 runs carried SIDES, STUMPS, head linking, phase 2, and generic `finalizeStems`. Boundary 135 composes the live predecessor and native SIG; Boundaries 136-137 supply native glyph identity from transaction 2 through terminal HEADS. Next are transaction 1 and sparse selected-base identity replacement, native Allegretto predecessor carriage, wider linked-S/hook/STUMPS and head-branch coverage, then transactional `recognize_native_stems` and schema-1 publication |
