...
| Rust workspace | The workspace now contains one hundred and fifty-one exact STEMS boundaries. Boundaries 1-134 retain the detailed scheduler, mutation, head-linking, phase-2, and generic `finalizeStems` evidence. Boundary 135 composes all live pre-mutation products; Boundaries 136-138 replace the page-wide first-STEMS snapshot and compact B12/B13 glyph bootstrap; Boundary 139 removes the sparse selected-base Java identity bridge; Boundary 140 derives the first B14 compact graph/index state; Boundary 141 replaces Java's allocator seed with a native modeled-glyph/StemInter identity domain; Boundary 142 derives each system-visible modeled prefix from production head-builder chronology; Boundary 143 atomically constructs the first B12-B19 SIDES carrier; Boundary 144 carries Allegretto transactions 1-28 natively through linked-S reuse and competing-hook removal; Boundary 145 derives BeamStem relation parameters from native products; Boundary 146 internalizes the established STEMS-entry edit state; Boundary 147 makes the production-prepared page own its StemChecker context and initialize a third page's first-system SIDES transaction without test hydration; Boundary 148 moves that page's complete system-1 SIDES drive into production; Boundary 149 carries the exact page registry and shared persistent allocator through Batuque's system-2 constructor chronology; Boundary 150 commits Batuque system 2's first serial SIDES transaction; Boundary 151 drives all 40 system-2 SIDES transactions to true exhaustion. Wider system chronology and mutation/corpus branches remain explicit. Focused Batuque and full sibling 15/15, strict workspace Clippy, formatting, and diff checks are green. `664185a6b` remains the current remote CI baseline pending this boundary's workflows. CI repeats formatting, strict Clippy, and workspace tests on Ubuntu and macOS |
...
| Later recognition stages | the remaining dependency-light lifecycles are native for `STEMS`, `REDUCTION`, `CUE_BEAMS`, `TEXTS`, `MEASURES`, `CHORDS`, `CURVES`, `SYMBOLS`, `LINKS`, `RHYTHMS`, and `PAGE`; STEMS has one hundred and fifty-one exact semantic components. Chula system 1 runs carried SIDES, STUMPS, head linking, phase 2, and generic `finalizeStems`; Allegretto system 1 now carries SIDES transactions 1-28 through linked-S reuse and competing-hook removal; Batuque system 1 drives all 33 SIDES transactions from production-prepared state to true `SidesExhausted`, carries the exact registry and allocator through system 2's constructors, and drives all 40 system-2 SIDES transactions to true exhaustion. Boundary 135 composes the live predecessor and native SIG; Boundaries 136-138 supply native glyph identity; Boundary 139 derives selected-beam identity; Boundary 140 derives initial B14 state; Boundary 141 supplies native persistent IDs; Boundary 142 derives the visible modeled registry prefix from head-builder chronology; Boundary 143 atomically initializes the first SIDES carrier; Boundary 144 removes the reconstructed Allegretto checkpoint; Boundary 145 removes caller-supplied BeamStem relation parameters; Boundary 146 internalizes the STEMS-entry edit state; Boundary 147 internalizes the page-wide checker and first-system join; Boundary 148 moves the bounded system-1 transaction loop into production; Boundary 149 carries cross-system registry/allocator chronology fail-closed; Boundary 150 constructs the first `SharedSheetSerial` carrier; Boundary 151 completes its system-2 SIDES worklist. Next are later-system registry handoff, STUMPS, and wider head-branch coverage, then transactional `recognize_native_stems` and schema-1 publication |
