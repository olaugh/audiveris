...
| Later recognition stages | the remaining dependency-light lifecycles are native for `STEMS`, `REDUCTION`, `CUE_BEAMS`, `TEXTS`, `MEASURES`, `CHORDS`, `CURVES`, `SYMBOLS`, `LINKS`, `RHYTHMS`, and `PAGE`; STEMS has one hundred and thirty-nine exact semantic components. Chula system 1 runs carried SIDES, STUMPS, head linking, phase 2, and generic `finalizeStems`. Boundary 135 composes the live predecessor and native SIG; Boundaries 136-138 supply native glyph identity from transaction 1 through terminal HEADS; Boundary 139 derives selected-beam identity from native SIG bindings. Next are first-B14 persistent-ID/InterIndex replacement, native Allegretto predecessor carriage, wider linked-S/hook/STUMPS and head-branch coverage, then transactional `recognize_native_stems` and schema-1 publication |
