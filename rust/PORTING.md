...
| Rust workspace | The workspace now contains one hundred and fifty-six exact STEMS boundaries. Boundaries 1-134 retain the detailed scheduler, mutation, head-linking, phase-2, and generic `finalizeStems` evidence. Boundaries 135-147 compose and internalize the live native predecessor; Boundaries 148-152 carry Batuque's complete three-system SIDES page; Boundaries 153-154 drive its full page-wide STUMPS pass; Boundary 155 closes each native prelinked head prefix; Boundary 156 executes two first C-link creations and one rejected-link/no-link closure with prepared-state-owned free-glyph authority. Wider head mutation/retry and corpus branches remain explicit. Focused Batuque and full sibling 15/15, strict workspace Clippy, formatting, and diff checks are green. `664185a6b` remains the documented remote CI baseline pending newer terminal evidence. CI repeats formatting, strict Clippy, and workspace tests on Ubuntu and macOS |
...
| Later recognition stages | the remaining dependency-light lifecycles are native for `STEMS`, `REDUCTION`, `CUE_BEAMS`, `TEXTS`, `MEASURES`, `CHORDS`, `CURVES`, `SYMBOLS`, `LINKS`, `RHYTHMS`, and `PAGE`; STEMS has one hundred and fifty-six exact semantic components. Chula system 1 runs carried SIDES, STUMPS, head linking, phase 2, and generic `finalizeStems`; Allegretto system 1 carries SIDES transactions 1-28 through linked-S reuse and competing-hook removal; Batuque now drives all three systems through SIDES/STUMPS and their first mixed HEADS outcomes with one shared page identity stream. Boundaries 135-147 compose and internalize the production predecessor; Boundaries 148-152 drive the three-system SIDES page; Boundaries 153-156 drive page-wide STUMPS, prelinked head prefixes, first C-link mutations, and a rejected-link/no-link closure. Next are the remaining head/retry queues, then transactional `recognize_native_stems` and schema-1 publication |
