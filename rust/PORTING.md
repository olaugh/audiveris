...
| Rust workspace | The workspace now contains one hundred and sixty-six exact STEMS boundaries. Boundaries 1-134 retain the detailed scheduler, mutation, head-linking, phase-2, and generic `finalizeStems` evidence. Boundaries 135-154 compose and drive Batuque's page-wide predecessor/SIDES/STUMPS state; Boundaries 155-159 complete all three phase-1 head queues; Boundary 160 consumes all four page-wide phase-2 retries; Boundary 161 atomically finalizes all three systems; Boundary 162 exposes fail-closed `recognize_native_stems`; Boundary 163 publishes the complete schema-1 ordinary/stream result; Boundaries 164-166 carry Chula through its wider existing-stem frontiers and complete all three systems through both HEADS phases, generic `finalizeStems`, and transactional recognition. Wider corpus branches remain explicit. CLI, report, focused Batuque/Chula, and full sibling gates plus strict workspace Clippy, formatting, and diff checks are green. `664185a6b` remains the documented remote CI baseline pending newer terminal evidence. CI repeats formatting, strict Clippy, and workspace tests on Ubuntu and macOS |
...
| Later recognition stages | the remaining dependency-light lifecycles are native for `STEMS`, `REDUCTION`, `CUE_BEAMS`, `TEXTS`, `MEASURES`, `CHORDS`, `CURVES`, `SYMBOLS`, `LINKS`, `RHYTHMS`, and `PAGE`; STEMS has one hundred and sixty-six exact semantic components. Chula now completes all three systems through phase 1, phase 2, generic `finalizeStems`, and transactional `recognize_native_stems`, including system 1 orders 67/70/73, system 2 order 54, and system 3's failed LEFT/TOP then successful RIGHT/BOTTOM order-109 reuse; Allegretto system 1 carries SIDES transactions 1-28 through linked-S reuse and competing-hook removal; Batuque completes all three SIDES/STUMPS, both HEADS phases, generic `finalizeStems`, and schema-1 publication with one shared page identity stream. Next is the next wider-corpus fail-closed frontier and exact remote CI |
