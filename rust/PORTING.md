...
| Later recognition stages | the remaining dependency-light lifecycles are native for `STEMS`, `REDUCTION`, `CUE_BEAMS`, `TEXTS`, `MEASURES`, `CHORDS`, `CURVES`, `SYMBOLS`, `LINKS`, `RHYTHMS`, and `PAGE`; STEMS has one hundred and fifty-two exact semantic components. Chula system 1 runs carried SIDES, STUMPS, head linking, phase 2, and generic `finalizeStems`; Allegretto system 1 carries SIDES transactions 1-28 through linked-S reuse and competing-hook removal; Batuque now drives all three SIDES systems to true exhaustion with one shared page identity stream. Boundaries 135-147 compose and internalize the production predecessor; Boundaries 148-152 initialize, carry, and drive the three-system Batuque SIDES page, including accepted head-relation subsets and native compound registration. Next are page-wide STUMPS and wider head-branch coverage, then transactional `recognize_native_stems` and schema-1 publication |
