...
| HEADS active small-head catalog | Boundary 298 passes the exact qualified `ProcessingSwitches.smallHeads` value into production HEADS and selects Java's eight normal/small scanner and factory orders while leaving the default four-shape wrapper unchanged. A fresh Temurin-25 corpus oracle freezes five point sizes, 64 page templates, 384 anchors, and 41,492 keyed pixels (SHA-256 `0f1c86ff8a8318520faeeb43e3375b5c76301eb163ba0cd4442b8ccb1180f832`). Boundary 308 adds measured point sizes 52/53/54 from real Chopin inputs (24 templates, 144 anchors, 5,854 pixels; oracle `0005d65f3b0ee4dd66a92600efd2ec3e3af7643c9eba56a6d278c2c7a206d6c2`, asset `82397efc76d2c51da8aff124d5a428ab24c63be1d665f873991a4ea82338d001`). Batuque's live result remains exact, while the Chopin crop reaches a non-empty connected CUE_BEAMS result. |
...
| STEMS beam-origin `StemBuilder` construction | `materialize_native_stems_beam_builders` is the seventh production semantic STEMS boundary. It drives each of the 2,417 beam-origin VLinkers through the actual source-ordered `inspect(maxProfile)`, complete `StemBuilder` constructor return, and V `sb` assignment. The constructor recomputes its direction from its theoretical line rather than reusing the V direction: the only divergence is Carmen system 2 / builder 56, yielding 1,390 TOP and 1,027 BOTTOM builders. It removes 215 of 2,169 seeds, retaining 1,954; reduces 6,676 targets to 6,670 (1,617 B and 5,053 C); creates 1,442 chunk glyph registrations (799 new, 643 canonical reuse); removes 175 chunks; and retains 9,417 final items with 12,085 length rows. The bounded registry records zero external members and zero unmodeled reuse, which does not claim global glyph novelty. The PR-984 context-independent ordinate comparator reports zero cycles and zero equivalence inconsistencies across all 4,657 sort audits; observed maxima remain 11 target and 14 final items. All 35,419 builder checks report zero SIG, system-stem, linker, C-builder, and unexpected-builder mutations. The emitted body is 91,209 lines / 29,164,943 bytes and the complete fixture 91,210 lines / 29,167,131 bytes. Probe, runner, emitted-body, and complete-fixture SHA-256 are `acf21ca496ec309138cb530c05827a4e5b639763d7f4cc5cc0bff4d2b8657646`, `54bad840185412fa504a6f093f464e904ae16f0b8e8c2e4e2fac84a294681b8e`, `be00a4a2c5ee05b92b3fe70b157cbb246ab066ddd9fe89168db63657438672e4`, and `638dd815d5d110dd67bab202c31ee966c4fd229d6998107cdc3f9483045ffcf1` |
...
| STEMS head-origin `StemBuilder` construction | `materialize_native_stems_head_builders` is the ninth exact production semantic STEMS boundary. It resumes the real page-persistent chronology after the eighth boundary and constructs all 14,084 C-origin builders in system/head/corner order, after the already graded 2,417 beam builders. The exact eight-page / 30-system gate replays 8,939 stump registrations (5,581 new / 3,358 reuse), 1,442 beam registrations (796 new / 646 reuse), and 19,295 head-chunk registrations (4,619 new / 14,676 reuse), including eight stump chronology action changes and three head-to-later-beam reuse/action changes. Its builders scan 15,953,076 vertical and 14,436,784 horizontal sections, accept 34,526 and 23,787, retain 45,938 filament members, produce 29,130 final items with 166 inserted gaps, and reproduce all 70,420 profile-0-through-4 lengths. All 42,252 corrected-comparator audits report zero cycles and zero equivalence findings; frozen maxima are 2 / 7 / 13. The corpus uses inspect profile 1 with no divergence; production rejects an inspect/system-profile mismatch. It contains no VIP heads, but the native branch deliberately preserves Java's VIP-only removal bug: all 6,087 low-remain non-VIP chunks stay. The shared `VerticalStickFactory` also preserves Java's processed-without-compound rule, so a thickening side can still be reused later as an isolated sticker. The bounded registry contains only structurally projectable live glyphs after MultipleRest replacement and does not claim page-global Java `GlyphIndex` identity. The gate proves zero SIG, system-stem, link, or unexpected-builder mutations; the seam stops before expansion/linking. The split fixtures total 593,767 lines / 171,930,398 bytes. Manifest, probe, and runner SHA-256 are `31db6d63abc6c7e38152a9aac4a73f690717bfb814bf974825f2189a5a383480`, `ab657f96502869a4b710bcc98e507cb7539f1492635f9d097cff012b9629a9bb`, and `8542d682241d7be645f3bfc1e474a6acbc430f15a6e4c5ee92f25221a20d751b`; the eight full fixture hashes are `cb80aa2601596fd956144cd086e5500f24e7f10ad21adf1bfe282716e0af44c0`, `2f6b98785ff2b6f867a62fcf918108fbcfbd732d8027c648ce963d3bbba79874`, `551dafd731b7dd1c1360bb7545cba3e0728c067e4a4780e3b2043b93d901d793`, `7dffdfdc897b0a31f6675b4d10ee629d8baa63de48a6f85c575d22d9984c0709`, `e467d41e46fa0fcb5d9c7e1a0cf5a6504f91bc1135f12e84484cb21a146f3f69`, `7af707f95a8f687abf93cd7f2f4df45d057bbd018a1f90070d51459e34ab1a69`, `ad35a832adc2f10e331dca442da4f7a2d01b9e0376fedfd5e44ab30b0a6b2289`, and `497bd42a8baeaa39d0aadb1e260aee9c3d0c6f960f46bd92cf9494a05c282d58`. The corrected Java/Rust beam and head builder differential gates pass; strict integration-test Clippy remains required in the final verification |
...
| Later recognition stages | `STEMS` is complete as a generic headless recognition stage: construction, SIDES/STUMPS, both head phases, `finalizeStems`, `finalizeBeams`, SIG contextualization, and schema-1 publication all run from native products with no page/system/queue identity dispatch. Chula, Allegretto, Batuque, Zizi, Carmen, Cucaracha, Hove, Bach, D039, and both extracted Dichterliebe pages complete the Rust-only path. `REDUCTION` is in progress: terminal native STEMS SIGs execute Java's complete `reduceFoundations()` outer fixed point, enabled stem head-end refinement, beam-group consistency, sheet-wide free-stem measurement, and the final keep/remove sweep over the complete native-owned modeled glyph registry. Stable tombstones, exact head anchors, median/skew line selection, deskewed beam ordering, concrete-stem filtering, exact free-length anchors, and per-pass traces are retained. Remaining work is to own the Java-only opaque GlyphIndex entries, then publish and corpus-grade REDUCTION. Frozen measured wrappers remain test-only oracle evidence. Additional score scans are continuing robustness evidence rather than an open STEMS implementation phase. |
...
SIG15/b2 items, then a trailing support glyph. Corrected Java stops at the
second beam (`lastIndex=2`, `maxIndex=3`) and excludes that trailing glyph. The candidate raster
...
advances to queue197 x30/SIG95/Inter3796. Pre-expansion HeadStem grade/dx bits
are `3feffffffffffebf` / `3d0e1e1e1e1e1e1e`.

Upstream PR #977 is applied in Java and Rust: the later-item scan reads `j`,
skips non-beam items, continues only to a later beam in the same live group,
and otherwise stops at the current beam. The oracle publishes the stable
active candidate glyph 532 rather than a fresh-JVM auxiliary glyph number.
Fixture/runner/transform/init/probe/body
hashes are
...
look-ahead unit and corrected Bach lifecycle gate pass, as do formatting and
strict Clippy. The sibling suite is 28/35, matching the untouched baseline's
same seven pre-existing comparator-oracle failures. Continue at queue197.
...
the Java result directly. Its ordered LEFT/RIGHT closure attempts are
idempotent, `closed_value_changes` is zero, and only `phase_two_index` advances
from 0 to 1. The complete phase-one carrier and all 15 queue identities remain
intact; the next retry is x159/SIG164.
...
tail before `reuseStem`, returns false, and preserves both closed local side
states. No undefined side, linker write, SIG or stem mutation, or allocator
change occurs.

The generic native phase-two append retry reproduces this without a
corpus-specific production wrapper. The local LEFT/RIGHT closure attempts are
idempotent, zero cells change, and only `phase_two_index` advances from 1 to 2.
The 15-entry worklist and completed phase-one carrier remain unchanged; the
next retry is x194/SIG78.
...
in the carried undefined-side set, and performs no local closure, linker write,
SIG/stem mutation, or allocation.

The generic native phase-two append retry matches this branch directly. It
emits no closure cells, preserves the complete undefined-side authority and
all 15 worklist identities, and advances only `phase_two_index` from 2 to 3.
The next retry is x163/SIG170.
...
The generic native append retry follows the linked-side short circuit and
performs the ordered incident-stem closure: x161/SIG212 LEFT then RIGHT, both
idempotent. Zero cells change, all carried state and all 15 worklist identities
remain intact, and only `phase_two_index` advances from 3 to 4. The next retry
is x160/SIG169.
...
The generic native append retry matches the branch directly. It emits no
closure cells, preserves the complete carried state and all 15 queue identities,
and advances only `phase_two_index` from 4 to 5. The next retry is x162/SIG168.
...
The generic native append retry matches the rejection directly. Its ordered
local closure attempts are idempotent, zero cells change, and only
`phase_two_index` advances from 5 to 6. The next retry is x158/SIG88.
...
The generic native append retry matches the rejection directly. Its ordered
local closure attempts are idempotent, zero cells change, and only
`phase_two_index` advances from 6 to 7. The next retry is x152/SIG90.
...
The generic native append retry matches the no-link result directly. Its
ordered local closure attempts are idempotent, zero cells change, and only
`phase_two_index` advances from 7 to 8. The next retry is x123/SIG14, the first
measured phase-two graph mutation.
...
`BottomOnly`. Java reuses glyph 488 / StemInter 6750 and adds only x123's
missing RIGHT HeadStem edge. SIG counts move 394/600→394/601 while 77 system
stems and allocator 6815 remain unchanged; `phase_two_index` advances 8→9.

The native transaction authenticates carried glyph 149, stem identity 11, SIG
vertex 328, and x125/SIG25's existing edge 304, then appends native edge 598.
The new relation has exact grade/dx bits `3fe452a9b8a231bc` /
`bfce8c8a19648d2d` and consistency `3ff6db6db6db6db7`. A correction bounded to
this x123/SIG14 RIGHT/BOTTOM frontier reproduces Java's two-representable-step
working-line interpolation difference without altering the reused stem. The
next retry is x149/SIG18; six entries remain.
...
The strict Boundary-249 predecessors remain pinned; warmup plus two fresh JVM
runs are byte-identical. Focused 1/1, all 29 sibling tests, formatting, strict
all-target/all-feature workspace Clippy, and diff checks pass. The remote CI
baseline remains `425d58e82` until newer exact workflows become terminal.

## Boundary 251: second Bach system-2 phase-two reused-stem append

Queue index 9 x149/SIG18/Inter3641 selects only RIGHT/BOTTOM. Java reuses glyph
497 / StemInter 6786 and adds x149's missing RIGHT HeadStem edge; SIG counts
move 394/601→394/602, system stems remain 77, allocator 6815 is unchanged, and
`phase_two_index` advances 9→10.

Native authenticates glyph 158, stem identity 47, SIG vertex 364, and the
existing x150/SIG29 edge 449 before appending edge 599. Its exact
grade/dx/consistency bits are `3fe3c8a4915237cf` / `bfcfa150d80c0969` /
`3ff62f53e62f53e7`, with no rounding correction. The next retry is
x190/SIG214; five entries remain.
...
The strict Boundary-250 predecessors remain pinned; warmup plus two fresh JVM
runs are byte-identical. Focused 1/1, all 29 sibling tests, formatting, strict
all-target/all-feature workspace Clippy, and diff checks pass. The remote CI
baseline remains `425d58e82` until newer exact workflows become terminal.
...
Queue index 10 x190/SIG214/Inter4036 leaves both sides `Neither`. Java returns
false with no undefined side or mutation; SIG 394/602, 77 system stems, and
allocator 6815 remain fixed. The generic native append retry performs two
idempotent local closure attempts, changes zero cells, and advances only
`phase_two_index` 10→11. Next is x98/SIG136 with four entries remaining.
...
The strict Boundary-251 predecessors remain pinned; warmup plus two fresh JVM
runs are byte-identical. Focused 1/1, all 29 sibling tests, formatting, strict
all-target/all-feature workspace Clippy, and diff checks pass. The remote CI
baseline remains `425d58e82` until newer exact workflows become terminal.
...
`Neither`. Java rejects the append expansion before mutation, returns false,
and leaves SIG 394/602, 77 stems, allocator 6815, and undefined sides unchanged.
The generic native append retry performs two idempotent local closure attempts,
changes zero cells, and advances only `phase_two_index` 11→12. Next is
...
The strict Boundary-252 predecessors remain pinned; warmup plus two fresh JVM
runs are byte-identical. Focused 1/1, all 29 sibling tests, formatting, strict
all-target/all-feature workspace Clippy, and diff checks pass. The remote CI
baseline remains `425d58e82` until newer exact workflows become terminal.
...
fixed. Generic native retry changes no closure cells and advances only
`phase_two_index` 12→13. Next is x118/SIG211 with two entries remaining.
...
The strict Boundary-253 predecessors remain pinned; warmup plus two fresh JVM
runs are byte-identical. Focused 1/1, all 29 sibling tests, formatting, strict
all-target/all-feature workspace Clippy, and diff checks pass. The remote CI
baseline remains `425d58e82` until newer exact workflows become terminal.
...
only `phase_two_index` 13→14. Next is x54/SIG59 with one entry remaining.
...
The strict Boundary-254 predecessors remain pinned; warmup plus two fresh JVM
runs are byte-identical. Focused 1/1, all 29 sibling tests, formatting, strict
all-target/all-feature workspace Clippy, and diff checks pass. The remote CI
baseline remains `425d58e82` until newer exact workflows become terminal.
...
closure attempts, advances `phase_two_index` 14→15, and exhausts the queue.
...
The strict Boundary-255 predecessors remain pinned; warmup plus two fresh JVM
runs are byte-identical. Focused 1/1, all 29 sibling tests, formatting, strict
all-target/all-feature workspace Clippy, and diff checks pass. The remote CI
baseline remains `425d58e82` until newer exact workflows become terminal.
...
linkable corner; RIGHT/BOTTOM selects an existing stem. Java reuses StemInter
7385 through Java Inter4399's LEFT relation, preserves the allocator and
77 system stems, and adds one HeadStem edge (537→538). The native transaction
maps this to glyph 249, stem identity 46, SIG vertex 338, and the carried
x97/SIG176 edge 392, then appends native edge 535 with grade/dx bits
`3fe613e185913e1e` / `bfcad42f4a207c3c`. The strict JDK25 runner and fixture
are byte-identical across two fresh runs. The focused regression and full
30-test sibling suite pass, along with strict workspace Clippy, formatting,
and diff checks. The full Bach CLI now reaches system-3 queue 5 x146/SIG56.
...
bits `3fe4e04a170fd2a1` / `bfcd68cbbb961a5a`. The JDK25 runner and fixture
...
## Boundary 268: Allegretto terminal phase two and eight-page publication

All three Allegretto systems now exhaust phase two and pass generic
`finalizeStems`. The last deterministic JDK25 oracle freezes system-1 terminal
create/reuse appends and system-2 reused-stem append; fixture, runner,
transform, and semantic-body SHA-256 are
...
and `046d73ae65ab0829a5d0826ca0953750cd488ce9039fa0ce62c29fc96922c554`.
The Rust-only CLI now completes and publishes STEMS for every page in the
eight-page frozen parity corpus.

## Boundary 269: generic transaction coverage and D039 completion

Rejected SIDES transactions, remembered unattached stems, initial head profile
retries/no-link, SeedGlyph and soft-tail C-link expansion, mixed head/beam
expansion, rejected candidate chronology, multi-relation closure, reused-stem
attachment, and phase-two append/no-link are now generic production paths.
Builder ordering shares a stable JDK25 TimSort implementation, and header
ordinate lookup uses Java-compatible endpoint extrapolation outside a sampled
spline. `D0392410-1.256.png` is the first additional real page to complete the
entire Rust-only stage: 4 systems, 255 stems, 947 checked heads, 725 HeadStem
relations, and 223 abnormal heads. This pins successful completion, not Java
terminal parity for D039. The nine-page CLI gate, 35/35 sibling suite, TimSort
units, strict workspace Clippy, formatting, and diff checks pass. Continue with
broader out-of-corpus scans and only then retire remaining legacy dispatch
wrappers.

## Boundary 270: two extracted Dichterliebe pages complete generic STEMS

The two raster pages extracted from `Dichterliebe01.pdf` complete the same
Rust-only generic path without page-specific dispatch. Exact
`(systems,stems,checked,relations,abnormal)` summaries are
`(3,177,449,413,66)` and `(4,227,590,544,81)`. The completion census now covers
eleven real pages. Direct PDF inputs still stop before recognition on unrelated
unsupported content-stream operators, and the rotated Schubert test image stops
during SCALE; neither is classified as a STEMS gap. The expanded CLI regression
passes in 115.99s.

## Boundary 271: remove production page-specific head dispatch

Production no longer routes phase-one or phase-two head work through measured
page/system/queue/x/SIG matches. The 1,102-line dispatch chain and its 40
production imports are removed; the authenticated wrappers stay test-only as
oracle evidence. All production pages now use only the generic continuation,
C-link/no-link, and phase-two append/no-link engines. The eleven-page
ordinary/stream gate passes in 118.27s, the 35/35 sibling suite passes in
274.89s, and strict workspace Clippy is clean.

## Boundary 272: remove the Chula-shaped finalizer branch

The production `finalize_native_stems` terminal is now wholly generic. It no
longer recognizes one Chula state by system/head/queue/SIG/stem counts and
runs a redundant 184-line page-specific authenticator. Its generic carrier,
SIG/binding, HeadStem partition, relation-removal, canonical-share, and
abnormal-head invariants remain authoritative for every system. The measured
Chula/Java evidence remains test-side. The focused native-carrier gate and all
eleven real-page STEMS completions pass; strict workspace Clippy, formatting,
and diff checks are clean. The next production-specific debt is mutable Java
C-link theoretical-line history, not finalization.

## Boundary 273: generic state replaces C-link coordinate shims

Production now carries Java's mutable downward `CLinker.theoLine` per corner
through all failed profiles and from phase one into phase two, while upward
expansion continues to use a fresh reversed line. The stateful path also uses
Java's general `intersectionAtY` operation order. This removes production
dependence on six coordinate-specific ULP adjustments and two Bach-system-6
gap exceptions; measured wrapper arithmetic remains isolated as frozen oracle
evidence. The eleven-page ordinary/stream suite passes 2/2, all 35 sibling
...
## Boundary 275: full sheet epilog and generic STEMS completion

`recognize_native_stems` now continues after generic per-system
`finalizeStems` through Java's `StemsStep.doEpilog`. The native
`finalizeBeams` pass removes vertically neighboring cross-system orphan beams
and their empty groups, creates every good-beam-side BeamHead support, and then
contextualizes every live SIG vertex. Support ratios, exclusion partitions,
head/stem concurrency, and BeamGroup mean-member best grades follow the Java
implementations rather than a page-shaped summary.

The two-fresh-pass JDK25 Chula oracle records 12 orphan beams, 12 empty groups,
342 BeamHead relations, and 766 contextualized vertices. Per system the counts
are `(4,4,123,259)`, `(3,3,109,233)`, and `(5,5,110,274)`. BeamHead grade
SHA-256 matches exactly in all systems. System 1 contextual grades are wholly
bit-identical at FNV `ba83426ee73b2b10`; the only remaining per-vertex
differences are four inherited HEADERS inputs in system 2 and two inherited
LEDGERS ULPs in system 3, so Rust reports `125acaf46320d86e` and
`21bda55a2a32d2c4` without contaminating STEMS with upstream corrections.

Fixture/probe/runner/init/body SHA-256 are
...
Four focused units, the full carrier, eleven-page CLI completion and stream
identity, 35/35 measured sibling cases, strict workspace Clippy, formatting,
and diff checks are green.

The final reachability audit confirms the production page driver calls only
generic transaction engines, all authenticated order/system wrappers are
test-side evidence, and remaining `Unsupported*` terminals reject malformed
or upstream-impossible inputs rather than valid music. This closes generic
headless STEMS; the port now advances to downstream REDUCTION and SYMBOLS
prerequisites. Additional score rasters extend robustness evidence but are not
an unimplemented STEMS phase.

## Boundary 276: first production REDUCTION semantic primitive

`native_reduction` now bridges terminal `NativeStemsRecognition` state into
real REDUCTION mutation. Its existing-exclusion solver follows
`SIGraph.reduceExclusions()` exactly: select the exclusion with the greatest
strong-endpoint best grade, retain insertion order when those contributions
tie, delete the weaker endpoint and the target on an endpoint-grade tie,
remove a dying sole-member BeamGroup, preserve graph tombstones, refresh
contextual grades, and continue. The transaction records decision order,
Java-style removal order, and every contextualization without claiming that
the surrounding REDUCTION epochs are complete.

Three focused units pass, including support-driven grade refresh and ensemble
death. The full library gate passes 707/707 (two ignored), and strict
...
The lossless resolver is now bound to the real terminal native SIG. Each
system binding retains exact overlap evidence for GRID, BEAMS, and HEADS, and
the resolver joins the terminal STEMS identities before scanning. Barline and
bracket glyph run tables are frozen immediately after `buildBarSticks`, before
later GRID cleanup can consume their source sections. Barline, connector,
beam, and stem areas use their exact median ribbons; bracket areas add the
Bravura upper/lower serif rectangles with Java integer-area bounds. Heads use
their registered run tables, staff identity, and rounded staff pitch.

`native_stems_lossless_overlap_resolver` rejects any active scanned vertex
whose interpretation is absent, and
`detect_native_stems_reduction_overlaps` now runs the production scheduler
against the owned terminal SIG with no rectangle reconstruction. A complete
Batuque page regression constructs and executes this epoch for every system.
The focused REDUCTION suite passes 21/21, the bracket-area regression and all
14 music-font tests pass, the Batuque production carrier passes, and the full
library passes 726 tests with two ignored. Formatting, strict Clippy, and diff
checks are clean. The next REDUCTION slice is Java-order composition around
this overlap transaction and the already ported exclusion/purge/foundation
primitives, followed by the remaining head, ledger, stem, and group checks.

## Boundary 285: Java-order foundations prefix and chord analysis

Native REDUCTION now crosses the first real multi-epoch composition boundary.
`reduce_native_stems_foundation_prefix` runs, in Java order, exact overlap
discovery, the foundations adapter's no-op frozen check, whole-SIG
contextualization, `analyzeChords`, the initial contextual weak purge, the
adapter's empty slur check, `checkStemEndingHeads`, and its following weak
purge. It stops deliberately before `checkHeads()` rather than jumping over an
incomplete branch.

The newly native chord prolog visits good stems by stable decreasing intrinsic
grade. It uses OpenJDK `Line2D.intersects(Rectangle)` clipping and the exact
0.02 stem/head IOU gate, excludes unlinked intersected heads, groups linked
heads by intrinsic duration, adds Java `HeadHeadRelation` support at its 0.75
context coefficient, and inserts duration, head-size, beam-size, and
head/beam-size incompatibilities in source order. Missing medians or unknown
head shapes fail typed instead of being guessed.

Every Batuque system executes the composed production prefix. Focused
REDUCTION coverage passes 25/25; the frozen native SIG baseline passes all
10 active gates (five diagnostics ignored); and the full library passes 730
tests with two ignored. Formatting, strict Clippy, and diff checks are clean.
The next exact frontier is the remaining `checkHeadSide()` portion of
`checkHeads()`, after which composition can continue to hooks and beams without
reordering Java.

## Boundary 286: Java-exact head, hook, and beam foundations segment

The composed production prefix now continues beyond stem-ending cleanup
through full `checkHeads()` and its purge, hook validation and its purge, and
standard-beam validation and its purge. It stops immediately before
`checkLedgers()`.

`checkHeads()` uses Java's stem-head shape set and relation traversal. Stem
direction is selected from HeadStem/BeamStem sources by stable decreasing
best grade and exact extended-line portion rules, including percussion heads.
Wrong-side lookup uses exact staff/pitch identity and the native binding's
retained merged-grand-staff pairs for Java's gutter remap. Missing identity or
median evidence fails typed. Bad links are removed and invading ones create
overlap exclusions. Directionless stem removal also reproduces
`StemInter.remove()` cleanup of unsupported HeadHead relations. Focused
REDUCTION tests pass 28/28, the Batuque production carrier crosses the new
frontier for every system, and the frozen native SIG baseline remains 10
active passes with five diagnostics ignored. Next is `checkLedgers()`, then
the remaining stem and fixed-point/late REDUCTION epochs. The complete library
passes 733 tests with two ignored.

## Boundary 287: Java-exact ledger repair and iterative support pruning

The composed foundations prefix now continues through complete
`checkLedgers()` and its contextual weak purge, stopping immediately before
`checkStems()`. `NativeSigSystemBindings` retains live ledger-to-vertex
identity, ordered staff maps (including one identity temporarily owned by two
staffs), the sheet/staff interlines, exact first/last staff splines, and full
ledger-line paths. The retained geometry is used directly; REDUCTION does not
reconstruct staff or ledger ordinates from midpoint samples.

The shared-ledger prepass follows Java's adjacent-staff, +1/-1/most-negative
map rules. Its exact 0.33-interline column box selects live ledgers by rectangle
intersection and heads by center containment. Stable reverse-best-grade head
selection chooses the farther owner staff, updates every column head's staff
and curve-aware integer pitch, and purges the other ownership map; a headless
column is purged from both maps without deleting the SIG ledger. The orphan
loop then accepts only next-outward positive x overlap or an intersecting head
at the ledger/next pitch, removes unsupported ledgers with stable tombstones,
and repeats to a fixed point. Its typed transaction records shared repairs,
pass-numbered removals, and the exact Java modification count.

The focused native REDUCTION suite is 32/32, with explicit two-pass cascade,
pitch-support, headed-shared, and headless-shared cases. The production Batuque
carrier runs the enlarged prefix on every system, the frozen SIG baseline is
10 active passes plus five ignored diagnostics, and the library is 737 passed
plus two ignored. Formatting, strict Clippy, and diff checks are clean. The
next frontier is `checkStems()`, then the outer consistency fixed point and
late consistency/refinement/group epochs.

## Boundary 288: complete `checkStems` and its contextual purge

The native foundations transaction now completes one entire Java
`AdapterForFoundations.checkConsistencies()` pass. After ledger validation it
snapshots live stems in SIG insertion order, removes stems with no live
`HeadStemRelation`, performs Java's direction-aware `stemHasSingleHeadEnd`,
and runs the immediately following contextual weak purge.

Direction and stem portions reuse the exact retained medians, extended-line
geometry, stable decreasing best-grade connection order, head-side rules, and
percussion handling already exercised by the complete head check. Direction
zero remains undecidable and unchanged. For a directed stem, the forbidden
tail is bottom for direction +1 and top for direction -1. Every forbidden
relation is collected against one line snapshot and removed as a batch; a
relation with `dy <= 0` and `dx <= 0.05` then creates Java's normalized overlap
exclusion. The typed transaction retains the stem snapshot, orphan and
ensemble removals, each modified stem's direction, ordered edge removals and
new exclusions, and Java's one-modification-per-stem count.

Focused REDUCTION coverage passes 33/33, including orphan, invading tail,
stable relation order, and direction-zero preservation. Every Batuque system
crosses the complete pass; all ten active frozen-SIG gates pass with five
diagnostics ignored; the full library passes 738 tests with two ignored.
Formatting, strict all-target/all-feature workspace Clippy, and diff checks
are clean. The next frontier is the outer foundations consistency fixed point,
then late chord/exclusion consistency and the remaining refinement/group
epochs.

## Boundary 289: foundations consistency fixed point

Production REDUCTION now preserves Java's exact split between one-time setup
and repeated foundation consistency. Overlap discovery, frozen no-op,
whole-SIG contextualization, chord prolog, initial weak purge, and empty slur
check run once. The six checks from stem-ending through `checkStems()`, each
with its following contextual purge, then repeat while their summed Java
modification count is positive.

Each typed pass records every checker and purge transaction plus the exact
sum: modified stem endings, head mutations, removed hooks, removed standard
beams, ledger modifications, and stem modifications. Weak-purge deletions are
retained but deliberately do not enter that sum. The transaction includes the
final zero-modification invocation that terminates Java's condition. Boundary
288 names remain compatibility entry points but now return the converged
fixed-point transaction.

A focused cascade proves the necessary revisit: pass one removes a forbidden
tail relation, pass two removes the newly orphaned head, and pass three is the
zero terminator (`[1, 1, 0]`). Focused REDUCTION coverage passes 34/34; every
Batuque system converges; all ten active frozen-SIG gates pass with five
diagnostics ignored; and the full library passes 739 tests with two ignored.
Strict all-target/all-feature Clippy, formatting, and diff checks are green.
Next are remaining-exclusion reduction and the foundations late-consistency
epoch, followed by refinement and grouping.

## Boundary 290: remaining exclusions and foundations late consistency

Production REDUCTION now continues past the foundations consistency fixed
point through the rest of Java's first outer epoch. It reduces every active
exclusion left by the converged checks, invokes
`AdapterForFoundations.checkLateConsistencies()` once, repeats chord analysis,
reduces any exclusions freshly inserted there, contextualizes, and purges.
The late checker returns zero because Java's stem-length check remains
commented out, so its inner `while` always performs exactly one invocation.

The typed epoch preserves Java's otherwise easy-to-miss set shadowing. The
outer reducer's local `deleted` set contains the epoch-opening purge and its
local `reduced` set contains the pre-late remaining-exclusion result. Purges
and exclusions inside adapter consistency methods mutate the adapter's
separate inherited sets; they do not independently request another outer
epoch. `requires_outer_repeat` is therefore derived only from the two local
sets, exactly like Java, rather than from every graph mutation observed.

Focused tests reduce a surviving header exclusion and prove a late chord pass
can create and immediately resolve a fresh standard/small-beam exclusion while
still returning zero. Coverage is 36/36. Every Batuque system completes the
first epoch, late creates no extra Batuque decisions, and at least one real
system raises the next outer-repeat signal. The frozen SIG baseline passes ten
active gates with five diagnostics ignored; the full library passes 741 tests
with two ignored; strict Clippy, formatting, and diff checks pass. Next repeat
outer foundations epochs to Java convergence.

## Boundary 291: complete foundations outer convergence

Production REDUCTION now executes all of Java `SigReducer.reduceFoundations()`
through its terminal `do/while` epoch. One-time overlap/context/prolog work is
not repeated. Every continuation starts with the outer-local contextual weak
purge, reaches the full six-check consistency fixed point, reduces remaining
exclusions, performs the single zero-returning late-consistency invocation,
and repeats only when the opening-purge or pre-late-reduction local set is
nonempty.

Typed first and continuation epoch records preserve the final empty-local-set
epoch. The complete transaction also retains Java `allRemoved` insertion
order, adding only outer-local opening-purge vertices followed by pre-late
exclusion vertices. Adapter-owned purges and reductions remain visible in
their own transactions and mutate the graph, but Java's field/local shadowing
correctly prevents them from controlling the outer loop or entering that
ledger.

Two focused cases independently force repetition via opening purge and via
remaining exclusion, then prove terminal convergence and ordered
`allRemoved`; focused coverage is 38/38. Every Batuque system converges and at
least one takes a continuation epoch. The frozen SIG baseline passes ten
active gates with five diagnostics ignored, the full library passes 743 tests
with two ignored, and strict Clippy, formatting, and diff checks are green.
The next exact REDUCTION frontier is Java's enabled stem head-end refinement
in `ReductionStep`; tail-end refinement is disabled by default.

## Boundary 292: Java-exact stem head-end refinement

Production REDUCTION now executes the enabled `StemInter.refineHeadEnd()` loop
immediately after the complete foundations outer fixed point. Active stems
retain SIG insertion order. Heads on each stem are stably sorted by their
integer bounds-center ordinate, Java's existing stem-direction calculation
selects the leading head, and the exact retained head corner supplies the
horizontal-side/vertical-side reference ordinate.

Long stems project that ordinate onto their median. Stems shorter than one
main interline instead use Java's skewed vertical through the integer bounds
center. Both branches use the existing Java-determinant intersection kernel,
replace only the direction-selected median endpoint, and recompute the same
vertical-ribbon integer bounds. Main interline and sheet skew are now retained
on the terminal STEMS carrier; changed medians and bounds are synchronized to
both the owned SIG and canonical known-stem registry. The dependency-light
transaction is atomic and explicitly records no-head stems, reliable-line
selection, anchors, and before/after geometry.

Focused REDUCTION coverage passes 40/40, including both reliable-line branches
and missing-anchor rollback. Every Batuque system crosses the production
refinement after foundations convergence. The frozen SIG baseline passes ten
active gates with five diagnostics ignored, the full library passes 745 tests
with two ignored, and strict all-target/all-feature Clippy, formatting, and
diff checks are green. Tail refinement remains disabled exactly as in Java.
The next frontier is `BeamGroupInter.checkBeamGroups(system)` in the epilog,
then free-stem length measurement and glyph cleanup.

## Boundary 293: split inconsistent beam groups in the epilog

Native REDUCTION now ports `BeamGroupInter.checkBeamGroups(system)`. The outer
group list and each group's member loop retain Java's snapshot timing. For
each beam, the native transaction applies the complete Java deskew transform,
uses strict deskewed abscissa overlap, and stably sorts siblings by ordinate at
the inspected beam's deskewed midpoint. Common stems are filtered through the
exact quarter-interline concrete-gap calculation against beam borders.

When adjacent siblings share no concrete stem, the atomic splitter appends a
new grade-one BeamGroup, moves the lower sibling suffix in relation insertion
order, preserves `insertSupport` exclusion/duplicate behavior, removes both
cross-group BeamStem relations and BeamBeam supports, and recomputes group
bounds without renumbering tombstones. Java's apparent non-sibling dispatch is
also preserved literally: its stream ends in `peek` without a terminal
operation and therefore performs no mutation.

The focused split case covers a three-beam group with a valid upper pair and a
separated lower suffix, exact relation tombstones, and rollback on missing
geometry. Focused REDUCTION coverage passes 41/41. Every Batuque system runs
the production check and requires zero splits. All 35 sibling transactions,
the frozen SIG baseline's ten active gates plus five ignored diagnostics, and
the full library at 746 passed plus two ignored are green. Strict
all-target/all-feature Clippy, formatting, and diff checks pass. Resume with
sheet-wide `StemInter.getFreeLength()` collection/median logging, then glyph
cleanup.

## Boundary 294: measure free stem tails sheet-wide

Production REDUCTION now computes every non-null `StemInter.getFreeLength()`
before glyph cleanup. Systems and stems retain Java sheet/SIG order. A live
BeamStem relation returns null immediately; a stem with no live heads returns
null next. Remaining heads are deduplicated in relation insertion order and
stably sorted by integer center ordinate before the existing exact direction
kernel selects Java's last head.

The reference corner is deliberately opposite the endpoint-refinement corner:
up stems use the selected head's BOTTOM corner and median P1, while down or
unknown stems use TOP and median P2. Absolute ordinate distance uses
`Math.rint` ties-to-even conversion. Sheet collection sorts the pixel lengths,
selects Java's upper-middle `size / 2` item, and divides by the main interline
without changing the SIG.

The focused direction case covers both corner sides plus beam-attached and
headless null returns. Batuque has 147 live post-REDUCTION stems: five measured
and 142 skipped, with median 83 pixels / `83 / 21` interlines. Focused
REDUCTION is 42/42; all 35 sibling transactions, ten active frozen-SIG gates
plus five ignored diagnostics, and the full library at 747 passed plus two
ignored are green. Strict all-target/all-feature Clippy, formatting, and diff
checks pass. Next port Java's final glyph-index keep/remove cleanup, then wire
and grade REDUCTION publication.

## Boundary 295: reduction glyph-index cleanup over the owned registry

Native REDUCTION now executes the final `ReductionStep.doEpilog()` algorithm
over the complete glyph registry domain it owns. The page-global registry is
materialized from the last system's carried prefix plus every strongly retained
STEMS registration. Java's `LinkedHashSet` order is reproduced system by
system: persistent staff-line glyphs first, then live SIG vertices in insertion
order. Dynamic stems resolve through their canonical glyph IDs; other modeled
inters resolve by full bounds, weight, and `RunTable` equality.

The sweep preserves allocator watermarks and the originals union and changes
only active-index membership, in entity order. It is idempotent and rejects an
unknown keep identity before mutation. Batuque begins with 1,820 active modeled
entities, retains 406, and removes 1,414. Keep, retained, and removal ID-stream
SHA-256 values are `04be33649fdcfbe7ea3b152d7bd4817c596543a852ac256bb5d2cf2d949cc8f4`,
...
The boundary deliberately does not fabricate Java glyph identities: 59 unique
live Inter glyph contents on Batuque are retained as an explicit opaque count
outside the modeled registry. Focused REDUCTION remains 42/42, the cleanup
kernel passes, all 35 sibling transactions pass, frozen SIG is ten active plus
five ignored, and the full library is 748 passed plus two ignored. Strict
...
## Boundary 298: exact active small-head HEADS prerequisite

An explicit qualified `ProcessingSwitches.smallHeads=true` request now changes
the production HEADS scan, not merely the later CUE_BEAMS gate. The scanner and
template factory use Java's exact eight normal/small shape orders at all five
real point sizes. The fresh-JVM corpus fixture contains 64 templates, 384
anchors, and 41,492 keyed pixels and is pinned at
`0f1c86ff8a8318520faeeb43e3375b5c76301eb163ba0cd4442b8ccb1180f832`.
Its generated 160,857-byte native asset has 40 unique templates and SHA-256
`30b60d5d27f6d7bc8ee71413539b7ca159844e29e6f0abcd06e00e1a4304ddea`.
Probe, runner, and generator are pinned at
...
The oracle also freezes final live Java normal/small shape counts for all eight
pages. Batuque is exact at 155 normal black, 170 normal void, 221 small black,
and 150 small void heads. Rust preserves Java's normal-black-only hole
conversion and carries all 696 stemmed heads into 2,784 STEMS corners. The
...
Cucaracha reports 115 stems while the CLI test expects 114. Boundary 308 below
closes the non-empty Chopin and connected-sidecar publication work.

## Boundary 308: real Chopin cue beam and connected sidecar

The active stage is now pinned on a lossless one-system Chopin page-23 crop.
Java and Rust agree exactly on the retained cue beam's `587,80,42,12` bounds,
median, thickness, and grade bits. The Rust terminal result contains two cue
aggregates, seven spots, one `BEAM_SMALL`, one cue group, and four accepted
BeamStem mutations in LEFT/CENTER/CENTER/RIGHT order. Java's two fresh oracle
runs are byte-identical and retain the same one beam, one group, and four
relations.

The schema-1 `cue_beams.systems` payload is now connected rather than census
only. It assigns deterministic system-local IDs to heads, stems, beams, groups,
aggregates, and relations; emits aggregate and group memberships; and preserves
geometry, grades, portions, extensions, abnormal flags, and ordinary/cue
provenance. The black-box CLI test runs the page twice and verifies byte
identity plus endpoint resolution for six HeadStem, four BeamStem, and one
Containment edge.

The fixture required three bounded prerequisite corrections that follow Java's
mutation order: removed tremolo beams no longer invalidate pre-removal stump
diagnostics; downstream head reachability sees only surviving BeamLinkers; and
reusing a stem skips an already present HeadStem edge. Cue grading now uses
`1 - meanDistance/maxDistanceToBorder`, exactly as
`BeamsBuilder.createSmallBeamInters`, instead of ordinary beam jitter.

Exact SHA-256 pins: image
`40d14bf054640e925dccebe2f765525cbdc35748a16067bb06bba0d6c475a98e`;
Java fixture/probe/runner
...
## Boundary 310: fix non-transitive StemBuilder sorting

PR 984's context-independent ordinate key is now shared by Java and both Rust
`StemBuilder` mirrors. Half linkers use reference-point y; all other items use
the directional line endpoint. The 40-item regressions cross TimSort's merge
threshold and prove both directions, while regenerated Java beam/head corpora
...
## Boundary 312: retain complete registry authority in HEADS C-links

The production modeled glyph registry now promotes transaction-created entries
before a HEADS C-link asks for an exhaustive equality scan, matching the
already-authenticated beam-origin path. This matters after `StemChecker`
rejects a newly registered compound: Java may retain that original only through
weak indexes, while Rust owns its exact content in the carried transaction.
That owned entry can therefore participate in a later complete lookup without
turning an unrelated candidate into `AwaitingCompleteGlyphRegistry`. Snapshot
authorities still fail closed because they do not own the omitted contents.

A focused deterministic regression starts with a rejected weak compound,
performs an unrelated absent lookup, and proves both the complete two-entry
scan and the retained canonical identity. The exact StageAligner diagnostic
generated by resizing Chopin page 11 to `157.902439%` (`2416x3126`, SHA-256
`c050aac4ac8799b710ba8c2664a548b0e96c87ed98c44bd4a290e33666ea7893`)
now reaches terminal release-mode STEMS JSON instead of failing at system 4,
