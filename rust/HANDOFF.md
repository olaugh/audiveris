...
The Chula carrier switches to this registry after the two legacy bootstrap
transactions. Transactions 3-32, all seven STUMPS transactions, every measured
...
This does not yet replace transactions 1-2's fixture-backed bootstrap, the
...
1. Starting from Boundary 136's native glyph registry, replace transactions
   1-2's fixture bootstrap and the sparse 16-row selected-base Java InterIndex
   identity authority. Transactions 3 through the terminal are now independent
   of the disclosed first-STEMS page snapshot, but the whole mutating pass is
   not yet native-products-only.
...
3. Continue after Boundary 134, which completes generic `finalizeStems`, with the rather-good profile escalation and `reuseStem` on a system where an append retry actually links:
   carry the
