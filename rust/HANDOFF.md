...
first serial SIDES carrier remains next.

## Next implementation slices

Commit each slice separately after the full verification block above.

1. Use Boundary 149's shared allocator and exact registry handoff to construct
   system 2's own SIG, bindings, B/S cells, empty `systemStems`, and first
   `SharedSheetSerial` frontier. Do not weaken the
   `SharedSheetFirstFrontier` guard or reconstruct a later system in isolation.
   Then generalize beyond the Chula, Allegretto, and Batuque
