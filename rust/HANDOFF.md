...
| STEMS self-driving chain: original blocker survey (partly superseded) | This dated survey established that registry completeness, rather than per-transaction algebra, was the core problem. The SIG-bootstrap portion is now resolved by production `NativeSigSystem` assembly and native B14-B17 carriage. The final native builder-registry replay now publishes exact canonical content plus stable modeled ordinals, and link plans retain those ordinals, but the persistent-ID/allocator/union baseline is still inherited from fixture state, with disclosed page evidence supplying exhaustive equality. `AUDIVERIS_GLYPH_REGISTRY_OUT` records the 1,650-entry first-transaction baseline rather than the 1,253-entry page-load state, because linker construction registers hundreds more in between. The weak-index rejected-compound lifetime still needs resolution before claiming native-products-only execution |
| STEMS chain self-drive: full chula SIDES carried | `advance_native_stems_beam_sides_transaction` owns each already-awaited B12-B19 transaction as an atomic shadow/swap over scheduler, latest B14/transaction state, SIG/bindings, and persistent B/S cells. Repeated calls execute all 32 chula-system-1 SIDES transactions, carry all 29 B16 sibling-write lists, and reach `Completed` at the native 253-vertex / 331-edge terminal with 61 linked B and 68 linked S cells. Exact plan/B order and sibling aliases match Java only after return. Native modeled ordinals now preserve the selected-object identity needed by a future persistent bridge, but current execution still inherits the persistent-ID/allocator/union baseline and page evidence. A late missing sibling cell proves provisional work cannot escape. Wider linked-S reuse and removal of the remaining fixture-backed registries remain |
| What finishing STEMS needed, measured 2026-08-12 (diagnosis now implemented) | This dated diagnosis correctly identified the absence of a production SIG as the cross-stage blocker. `NativeSigSystem` now assembles the exact GRID-through-HEADS graph with native identities and the production carrier runs chula system 1's full SIDES pass, so “the port owns no SIG,” “transaction 2 remains,” and “an iterative driver remains” are no longer current. There is still no `recognize_native_stems`: linked-S reuse, replacement of the disclosed GlyphIndex/beam-identity bootstraps, SIDES-to-STUMPS continuation, general dirty state, and wider BEAMS-group coverage remain |
...
`stems-beam-inter-index-chula-system1.txt` authority contains all 48 live system-1 beams
(SHA-256 `fde4daebadc5c7158fa8e83dcbd4ac0ca6381c614876b6fe48408ec2e245064e`,
52 lines / 6,259 bytes); production imports only persistent Inter ID, sorted InterIndex
ordinal, and VIP while resolving beam 22's native vertex, group, removal, abnormal state,
and every query from stump/SIG products. Plan 618's two selected glyphs are materialized
natively, and the complete 1,650-entry page GlyphIndex resolves their compound candidate
to existing canonical glyph 298 before B12 creates a new checked stem. B14 appends Stem
...
the rest of chula system 1's SIDES pass from that same authority. The native terminal is
...
frozen Java pass exactly, and the scheduler reaches `Completed` with identical retained
and final STUMPS worklists. The expected pass is opened only after the native terminal
exists. Missing beam identity authority fails before mutation; removing a later sibling
cell rejects B16 after provisional B12-B15 work while leaving the complete carrier
unchanged. Both disclosed page registries, wider linked-S reuse, general dirty state, and
the SIDES-to-STUMPS transition remain outside this bounded chula-system-1 claim.

The native registry replay now retains identity evidence that the later carrier previously
discarded. `NativeStemsHeadBuilderRecognition::modeled_canonical_glyphs` publishes final
canonical content with dense native ordinals, and each selected link-plan glyph carries
its ordinal. The link-plan gate proves ordinal/content identity directly. These ordinals
are not Java IDs and are not yet a page GlyphIndex authority: the first-STEMS persistent
IDs, allocator, union, and exhaustive page evidence still come from frozen state, and the
beam InterIndex authority remains disclosed. A positive carrier bridge must additionally
model the registry's system-visible boundary and its opaque page-global entries; this
commit deliberately does not expose an entry point that could confuse the final modeled
registry with that earlier persistent state.
...
1. Replace the disclosed page GlyphIndex and beam InterIndex bootstrap authorities; the
   full native SIDES execution has no per-transaction Java inputs, but it is not yet a
   native-products-only entry point.
2. Extend B13's graph-derived path to linked-S HeadStem reuse for wider systems, even
   though chula system 1's 68 executed head-side references are unique.
3. Carry the exhausted SIDES scheduler, graph, and shared cells through STUMPS,
   competing-hook removal, and the
