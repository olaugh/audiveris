...
This completes `finalizeStems` for the authenticated Chula system only; a
wider system with competing head stems is still required to exercise
`HeadStemsCleaner` mutation.
...
3. Continue after Boundary 133, which completes `finalizeStems` for chula system 1, with the rather-good profile escalation, `reuseStem` on a system where an append retry actually links, and a wider `HeadStemsCleaner` mutation case:
   carry the
