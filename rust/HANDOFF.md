...
SIG12/b2 and SIG15/b2, then a trailing support glyph. Corrected Java stops at
the second beam, so `lastIndex=2` while `maxIndex=3` and the trailing glyph is
not selected.
...
LEFT then RIGHT, and advances to queue197 x30/SIG95/Inter3796. The exact
pre-expansion HeadStem grade/dx bits are `3feffffffffffebf` and
`3d0e1e1e1e1e1e1e`.

This boundary now carries upstream PR #977 in both implementations. Java reads
`sb.get(j)` in the later-item scan; Rust likewise skips intervening non-beam
items, continues only when the next beam belongs to the same live BeamGroup,
and stops when no later same-group beam exists. At queue196 the first beam sees
the second sibling and continues, while the second beam stops before the
support glyph. The candidate is the stable active glyph 532; no transient
auxiliary glyph ID or evolved trailing-glyph relation is authoritative.
...
byte-identical. The focused Rust look-ahead unit and corrected Bach lifecycle
gate pass; strict formatting and workspace Clippy pass. The sibling suite is
28/35, with exactly the same seven comparator-oracle failures reproduced by
the untouched pre-PR-977 baseline. Continue at queue197 x30/SIG95.
...
Bach system 6 queue 25 x72/SIG27/Inter5389 resolves RIGHT/BOTTOM and reuses
Java StemInter9367 / glyph846 through Inter5419's LEFT relation. Java adds the
single missing HeadStem edge (558→559) while preserving allocator 9413 and 74
system stems. Native maps the same transaction to glyph 507 / stem identity 29
/ vertex 341 plus crossed x73/SIG39 edge 370; the main relation matches Java at
grade/dx bits `3fe5453e7e3d92c5` / `bfcc940d06edf788`.

This is the final mutating retry in Bach system 6. The generic driver consumes
the following no-link retry, exhausts every phase-two queue on all six systems,
and the same production preparation now passes page-wide generic
`finalizeStems`. The deterministic JDK25 fixture is byte-identical across two
fresh passes. Exact fixture/runner/transform SHA-256 values are
...
workspace Clippy, and diff checks are green. The next work is the corpus-wide
...
reuse: Java StemInter 7366 maps to native glyph 248, stem identity 27, vertex
319, and carried x29/SIG66 edge 325. It appends edge 537 (Java 539→540) without
