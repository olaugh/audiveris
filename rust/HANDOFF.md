...
chronology is still the next boundary.

## Next implementation slices

Commit each slice separately after the full verification block above.

1. Carry the shared allocator, modeled registry, SIG, bindings, and mutable
   linker cells across a completed first system into system 2. Do not weaken
   the `SharedSheetFirstFrontier` guard or reconstruct a later system in
   isolation. Then generalize beyond the Chula, Allegretto, and Batuque
