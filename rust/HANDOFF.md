...
3. Continue from Boundary 132, which completes phase 2 for chula system 1, with `finalizeStems` - the untouched third call in `StemsRetriever.process` - plus the rather-good profile escalation, and `reuseStem` on a system where an append retry actually links:
   carry the
