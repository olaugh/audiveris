...
//! Production scanner-band -> accepted HEADS competitor-slice boundary.
...
struct ExpectedScan {
    ...
    competitor_indices: Vec<usize>,
}

#[test]
fn native_competitor_slices_match_java_on_every_corpus_scanner() {
    ...
    for (page, expected_page) in expected {
        ...
        let actual = materialize_native_head_scanner_competitor_slices(&pools, &competitors)
        ...
        for actual_system in &actual.systems {
            ...
            for actual_staff in &actual_system.staffs {
                ...
                for (actual_scan, expected_scan) in actual_staff.scans.iter().zip(expected_scans) {
                    ...
                    assert_eq!(
                        actual_scan.competitor_indices, expected_scan.competitor_indices,
                        "{scan_label} competitor slice"
                    );
                    scan_count += 1;
                    nonempty_slice_count += usize::from(!actual_scan.competitor_indices.is_empty());
                    reference_count += actual_scan.competitor_indices.len();
                }
                staff_count += 1;
            }
            system_count += 1;
    ...
    assert_eq!(nonempty_slice_count, 408);
    assert_eq!(reference_count, 1_944);
}
...
fn parse_oracle() -> BTreeMap<String, ExpectedPage> {
    ...
    for (line_number, line) in ORACLE.lines().enumerate() {
        ...
        let competitors = fields
            .iter()
            .position(|field| *field == "competitors")
            .unwrap_or_else(|| panic!("line {} has no competitors field", line_number + 1));
        assert!(
            competitors + 1 < fields.len(),
            "line {} tail",
            line_number + 1
        );
        let competitor_indices = parse_indices(fields[competitors + 1]);
        let scans = pages
...
fn parse_indices(text: &str) -> Vec<usize> {
    if text == "-" {
        ...
        text.split(',')
            .map(|value| value.parse().expect("competitor index"))
            .collect()
    }
}
